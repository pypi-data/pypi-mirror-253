<script lang="ts">
  ...
  import type { I18nFormatter, LikeData, SelectData } from '@gradio/utils';
  import { copy } from '@gradio/utils';
  import {
    type CustomComponents,
    type CustomData,
    FileView,
  } from '@modelscope-gradio/compiled';
  ...
  import type { AvatarImage, FlushData, MultimodalMessage } from './utils';
  import {
    copy_to_clipboard,
  ...
  export let value:
    | [MultimodalMessage | null, MultimodalMessage | null][]
    | null;
  ...
  export let avatar_images: [AvatarImage, AvatarImage] = [null, null];
  ...
  const dispatch = createEventDispatcher<{
    ...
    like: LikeData;
    flushed: FlushData;
    custom: CustomData & { index: [number, number] };
  }>();
  ...
  function handle_custom(i: number, j: number, custom_data: CustomData): void {
    dispatch('custom', {
      index: [i, j],
      ...custom_data,
    });
  ...
  function handle_flushed(i: number, j: number, message: string): void {
    dispatch('flushed', {
      index: [i, j],
      value: message,
    });
  ...
  function get_message_avatar(message: MultimodalMessage, index: number) {
    const avatar_image = avatar_images[index];
    return get_fetchable_url_or_file(
      typeof message.avatar === 'object' && message.avatar !== null
        ? message.avatar.path
  ...
  function get_message_name(message: MultimodalMessage, index: number) {
    const avatar_image = avatar_images[index];
    return (
      message.name ||
        ...
        {#each message_pair as message, j}
          {#if message !== null}
            <div class="message-row {layout} {j == 0 ? 'user-row' : 'bot-row'}">
              {#if message.avatar !== null && (message.avatar || avatar_images[j] !== null)}
                {@const message_name = get_message_name(message, j)}
                <div class="avatar-container">
                  <img
                    class="avatar-image"
                    src={get_message_avatar(message, j)}
              ...
              >
                <button
                  ...
                  aria-label={(j == 0 ? 'user' : 'bot') +
                    ...
                    message}
                >
                  <!-- 手动指定 flushing 为 false-->
                  <FlushingMarkdown
                    {custom_components}
                    flushing={flushing &&
                      i === value.length - 1 &&
                      should_flushing(j === 0 ? 'user' : 'chatbot', message)}
                    ...
                    on:update={scroll}
                    on:load={scroll}
                    on:custom={(e) => handle_custom(i, j, e.detail)}
                    on:flushed={(e) => handle_flushed(i, j, e.detail)}
                  />

                  {#each message.files || [] as file}
                    {#if message !== null && file.file}
                      <FileView
                        {theme}
