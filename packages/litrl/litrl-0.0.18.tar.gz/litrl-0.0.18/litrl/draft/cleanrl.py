...
class SoftQNetwork(nn.Module):
    def __init__(self, envs):
        ...
        ### TEMP
        c = layer_init(nn.Conv2d(obs_shape[0], 32, kernel_size=8, stride=4))
        with torch.inference_mode():
            dummy_input = torch.zeros(1, *obs_shape)
            output_dim = c(dummy_input).shape[1]
            print(output_dim)
            raise ValueError
        ###
...
class Actor(nn.Module):
    def __init__(self, envs):
        ...
        ### TEMP
        c = layer_init(nn.Conv2d(obs_shape[1], 32, kernel_size=8, stride=4))
        with torch.inference_mode():
            dummy_input = torch.zeros(1, *obs_shape)
        ...
        with torch.inference_mode():
            dummy_input = torch.zeros(1, *obs_shape)
            output_dim = self.conv(dummy_input).shape[1]
