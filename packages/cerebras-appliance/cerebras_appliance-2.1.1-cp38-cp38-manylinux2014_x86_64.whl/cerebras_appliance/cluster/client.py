...
from cerebras_appliance.cluster import cluster_logger
from cerebras_appliance.cluster.config import (
    CSCONFIG_PATH,
    get_cs_cluster_config,
)
from cerebras_appliance.cluster.job_timer import JobTimer
from cerebras_appliance.cluster.surrogate_job import get_surrogate_job
...
from cerebras_appliance.pb.workflow.appliance.cluster_mgmt.csctl import api_pb2
from cerebras_appliance.pb.workflow.appliance.cluster_mgmt.csctl.api_pb2_grpc import (
    CsCtlV1Stub,
)
from cerebras_appliance.pb.workflow.appliance.cluster_mgmt.csctl.v1.resources_pb2 import (
    VolumeList,
)
...
class AuthClientInterceptor(grpc.UnaryUnaryClientInterceptor):
    ...
    def intercept_unary_unary(self, continuation, client_call_details, request):
        ...
        if is_user_auth_enabled and not os.path.exists(
        ...
        if is_user_auth_enabled:
            try:
                ...
                if not output.startswith("Token="):
                    raise RuntimeError(
                        f"Unexpected token from {get_cerebras_token_binary}"
                    )
...
class ClusterManagementClient:
    ...
    def __init__(
        ...
        self.cs_cluster_config = get_cs_cluster_config()
        if namespace:
            ...
            if not self.cs_cluster_config.namespace_exists(self.namespace):
                ...
                self._crt_bytes = self.cs_cluster_config.get_namespace_certificate_authority(
                    self.namespace
            ...
            if len(self.cs_cluster_config.namespaces) == 1:
                ...
                raise RuntimeError(
                    f"Usernode config has {CSCONFIG_PATH} access to multiple namespaces. "
        ...
        if self.workdir.parent.parent.name == "cerebras_logs":
            self.run_meta_file = self.workdir.parent.parent / "run_meta.json"
        else:
        ...
        crt_data_message = "EMPTY" if not self._crt_bytes else "OMITTED"
        client_message = (
            f"ClusterClient: server={self.server}, authority={self.authority}, cert={crt_data_message}"
    ...
    def _get_curr_time(self):
        return datetime.datetime.now(timezone.utc).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        )
    ...
    def init_execute_job(
        ...
        return self.get_job_handle(
            init_response.job_id, num_cs2_str, job_props, skip_ingress_creation
        )
    ...
    def init_sdk_execute_job(self, job_mode, skip_ingress_creation=False):
        ...
        request = cluster_pb2.SdkExecuteJobRequest(job_mode=job_mode,)
        logger.info(
    ...
    def init_image_build_job(
        ...
        workdir_hostpath_override: Optional[str] = None,
    ...
    def get_job_handle(
        self, job_id, num_cs2_suffix, job_props, skip_ingress_creation
    ) -> Dict[str, Any]:
    ...
    def get_job_meta(self, job_id):
        ...
        cluster_details_file = path.join(
            self.workdir, f"{job_id}-cluster-details.json"
        )
    ...
    def _log_image_reference(self, job_id, image_ref, image_ready):
        prop_dict = {
            "image": {"reference": image_ref, "ready": image_ready,},
        }
        if image_ready:
    ...
    def get_job(self, job_id):
        """Get Job Request"""
        request = cluster_pb2.GetJobRequest(job_id=job_id,)
    ...
    def validate_connectivity(self):
        # skip for test cases
        if "localhost" or "127.0.0.1" in self.server:
            return
        try:
            subprocess.check_output(
                shlex.split(f"ping -c 1 -w 3 {self.server.split(':')[0]}")
            )
        except subprocess.CalledProcessError as e:
            logger.error(
                f"Failed to ping cluster mgmt svc: {self.server}, please check network, err: {e}"
            )
            raise e
