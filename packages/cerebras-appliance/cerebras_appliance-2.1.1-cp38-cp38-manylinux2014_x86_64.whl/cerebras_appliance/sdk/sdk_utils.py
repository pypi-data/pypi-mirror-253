...
# WARNING: we cannot use cerebras.numeric which is
#   py_root/cerebras/numeric.cpython-38-x86_64-linux-gnu.so
#
# The issue is the appliance client which is supported by a
# python wheel. The python wheel may be broken if we prepare
# py_root/cerebras_appliance/sdk/numeric.cpython-38-x86_64-linux-gnu.so
#
# Secondly cn_double_to_fp16(el) is simply a bit-cast from fp16 to u16
# because there is no parameter to choose which f16 format, f16 or bfloat.
# We can replace np.uint32(cn_double_to_fp16(el)) by cast_uint32(el).
#
# To sum up, we disable cerebras.numeric and cn_double_to_fp16() here.
#
#import cerebras.numeric as cnum
#
#def cn_double_to_fp16(el: float) -> int:
#    """Uses Cerebras numeric library to convert a Python ``float`` type to its
#    ``int`` representation of a Cerebras half-precision floating point (called
#    ``fp16`` in the numeric library and elsewhere in Cerebras).
#    """
#    return cnum.double_to_fp16(el)
...
def input_array_to_u32(
    ...
    for (i,), el in np.ndenumerate(np_arr):
        ...
        if floating:
            # np.uint32(cn_double_to_fp16(el)) is equivalent to cast_uint32(el)
            #csel = cn_double_to_fp16(el)
            #el32 = np.uint32(csel)
            el32 = cast_uint32(el)
