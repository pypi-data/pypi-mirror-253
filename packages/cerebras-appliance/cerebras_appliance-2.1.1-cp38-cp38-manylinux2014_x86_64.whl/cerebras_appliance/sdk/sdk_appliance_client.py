...
from cerebras_appliance.pb.sdk import sdk_appliance_pb2, sdk_common_pb2
from cerebras_appliance.pb.sdk.sdk_appliance_pb2_grpc import sdk_applianceStub
from cerebras_appliance.sdk import sdk_utils
from cerebras_appliance.sdk.sdk_appliance_utils import (
...
from cerebras_appliance.cluster.config import get_cs_cluster_config
...
class SdkContext:
    ...
    def _init_mgmt_client(self, **kwargs):
        ...
        # server = kwargs.get("server", get_cs_cluster_config().mgmt_address)
        # authority = kwargs.get(
        #    "authority", get_cs_cluster_config().authority)
        # credentials_path = kwargs.get(
        #    "credentials_path", get_cs_cluster_config().credentials_path)
        ...
        self._cs_config = CSConfig(
            mgmt_address=get_cs_cluster_config().mgmt_address,
            num_csx=num_csx,
            max_wgt_servers=0,
        ...
        self._mgmt_client_args = (
            self._cs_config.mgmt_address,
            # crt_path, usually empty and will be loaded automatically by the cluster client
            self._cs_config.credentials_path,
            self._cs_config.mgmt_namespace,
            self._cs_config.job_timer,
        )

        # TODO instantiate an appliance compiler client and request a compile job

        # defaults to 0 which is no op currently (unlimited memory/cpu)
        # once restrictions are known, update the value
        compile_memory_bytes = resource_mem  # current default ram memory for coord
        ...
        if job == JobMode.Job.SDK_COMPILE:
            ...
            response = self._mgmt_client.init_sdk_execute_job(job_mode,)
    ...
    def _init_appliance_stub(self):
        ...
        if self._credentials:
            self._channel = grpc.secure_channel(
                wrk_address, self._credentials, options=channel_options,
            )
        else:
            self._channel = grpc.insecure_channel(
                wrk_address, options=channel_options,
            )
...
class SdkClient:
    ...
    def upload_artifact(self, artifact_path: str, destination_rel_path: Optional[str] = None):
        ...
        return self._upload_files(app_path, compile_hash, files, destination_rel_path)

    def _upload_files(self, app_path: str, compile_hash: str,
        files: List[str], destination_rel_path: Optional[str] = None):
        if destination_rel_path is None:
...
class SdkRuntime(SdkClient):
    ...
    def __init__(self, artifact_path: str, **kwargs,) -> None:
        ...
        self._check_sdk_response(response,
            "SdkRuntime creation (upload phase)")
        ...
        response = self._stub.sdk_runtime_create(
            sdk_appliance_pb2.SdkCreateArgs(app_hash=artifact_id,)
        )
        self._check_sdk_response(response,
            "SdkRuntime creation (runtime creation phase)")
...
class SdkCompiler(SdkClient):
    ...
    def compile(self, app_path: str, csl_main: str,
        options: str, out_path: str) -> str:
        ...
        if response.code != sdk_common_pb2.StatusCode.SDK_RT_ALREADY_COMPILED:
            ...
            response = self._stub.sdk_compile(
                sdk_appliance_pb2.SdkCompilerArgs(
                    compile_hash=compile_hash, options=option_list,
                )
            )
