...
class AbstractClient:
    ...
    def get_jwt_for_instance_access(self, instance_uuid: str):
        """
        Return a JWT allowed to connect remotely to the instance (for stream or ADB)
        """
        raise NotImplementedError()
...
class ApiTokenAuthClient(AbstractClient):
    ...
    def get_jwt_for_instance_access(self, instance_uuid: str):
        ...
        Return a JWT allowed to connect remotely to the instance (for stream or ADB)
        """
        LOGGER.info("Requesting JWT for instance access")
        response = _http_call("post", get_instances_adb_access_url(instance_uuid), json={}, headers=self._get_headers())
        if response.status_code == 200:
            try:
                return response.json()["adb_connect_token"]
            except Exception as exception:
