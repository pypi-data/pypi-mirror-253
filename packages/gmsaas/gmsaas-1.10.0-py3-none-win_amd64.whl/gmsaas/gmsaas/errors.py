...
class ApiTokenNotSupportedError(GmsaasError):
    ...
    def __init__(self, extra_message=None):
        super().__init__(
            ...
            f"Error: command not supported when using API Token." + (" " + extra_message if extra_message else ""),
        )
...
class CredentialsNotSupportedError(GmsaasError):
    ...
    def __init__(self, extra_message=None):
        super().__init__(
            ExitCode.NOT_SUPPORTED_ERROR.value,
            f"Error: command not supported when using credentials." + (" " + extra_message if extra_message else ""),
        )
...
class PackageError(GmsaasError):
    ...
    def __init__(self, adbtunnel_path):
        super().__init__(
            ExitCode.PACKAGE_ERROR.value,
            "Error: 'gmsaas' seems corrupted, try to re-install it.",
            "Unable to execute `{}` binary embedded in package".format(adbtunnel_path),
        )


class AppleM1ChipError(GmsaasError):
    """
    AppleM1ChipError Error
    """

    def __init__(self):
        super().__init__(
            ExitCode.APPLE_M1_CHIP_ERROR.value,
            "Error: 'gmsaas' needs Rosetta 2 on Apple M1 machines, please install it first (hint: 'softwareupdate --install-rosetta').",
        )
