...
class SQLDatabase(object):
    ...
    def update_or_insert_data(self,table_name:str,data_dict:dict,deduplicate_mode = "primary") ->bool:
        ...
        try:
            ...
            self.session.execute(stmt)
            # print(f"Updated existing record with id {condition_dict}")
            self.session.commit()
            return True
        except SQLAlchemyError as e:
            self.session.rollback()
            print("Error occurred during record update_or_insert_data!")
            traceback.print_exc()
            # print(str(e))
            return False
        else:
...
class RedisCookies(object):
    ...
    def read_cookies2hash(self, hname, kname):
        ...
        print(f'hname :{hname}, kname :{kname}, kvalue:{kvalue}')
        return kvalue
...
class RedisPriorityQueue(object):
    ...
    def push(self, item_primary_key:str,item:[str,dict], priority):
        if isinstance(item, dict):
            ...
            print("The input is neither a dictionary nor a string.")
            return False
        ...
        if not self.redis.zscore(self.name, item_primary_key):
            ...
            print(f"键名：{self.name}, zadd {item_str}, priority:{priority} successful")
            return True
        else:
            print(f"键名：{self.name}, zadd {item_str}, priority:{priority} is exist item_primary_key:{item_primary_key}")
            return True
    def pop(self) -> str:
        """
        Description:
