...
def teams_alert(title, facts, html, max_chars):
    ...
    facts : list
        The facts part (summary in top) of the message
    html : str
        The html table of records
    max_chars : int
    ...
    if len(payload) > max_chars:
        ...
        payload = ms_teams_json.get_json_output(title, facts, text)
...
def slack_alert(title, report, max_chars):
    ...
    if len(report) > max_chars:
        warning_msg = f"The {title} length is above the character limit count of {max_chars}"
        logging.warning(warning_msg)
        summary = re.split("^-.*-$", report, flags=re.MULTILINE)[-2]
        title = f"WARNING! {warning_msg}"
    ...
    if teams_output:
        ...
        if report_if_no_html or html:
            if stdout_only:
                ...
                success = teams_alert(title, facts, html, teams_max_chars)
    else:
