...
class GenAIDocExtract:
  ...
  def textBisonFreePrompt(data = None, promptTemplate = None, prompt = "",
    ...
    if DevoteamLib.GenAIStatus('textBisonNER'):
      ...
      for cc in (' '.join(context_con)).split(' '):
        ...
        if len(cc) > 1:
          ...
          single_text.append({"text":cc_data[0],"ocr_confidence":float(cc_data[1])})

      new_answer['ocr_text_result'] = {
          'full_text': ' '.join(context),
          'ocr_confidence': np.mean(total_confidence),
