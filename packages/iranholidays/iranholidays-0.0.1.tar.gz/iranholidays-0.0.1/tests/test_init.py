...
def test_holiday_occasion_gregorian_calendar():
    assert holiday_occasion(2023, 4, 21, 'G') == 'Friday'
...
def test_holiday_occasion_from_jdate():
    # In reality, this was NOT a holiday in Iran (30st of Ramazan)
    assert holiday_occasion_from_jdate(jdate(1402, 2, 1)) == 'Eid al-Fitr'
    assert (
        holiday_occasion_from_jdate(jdatetime(1402, 2, 2, 23, 59))
        == 'Eid al-Fitr'
    )
    # In reality, this WAS a holiday in Iran (2nd of Shawal)
    assert holiday_occasion_from_jdate(jdatetime(1402, 2, 3, 0)) is None
    assert holiday_occasion_from_jdate(jdate(1402, 2, 4)) is None
    assert (
        holiday_occasion_from_jdate(jdatetime(1402, 1, 13)) == 'Sizdah Be-dar'
    )


def test_holiday_occasion_from_date():
    assert holiday_occasion_from_date(date(2023, 4, 21)) == 'Friday'
    assert holiday_occasion_from_date(datetime(2023, 4, 22)) == 'Eid al-Fitr'
    # In reality, this WAS a holiday in Iran (2nd of Shawal)
    assert holiday_occasion_from_date(date(2023, 4, 23)) is None
    assert holiday_occasion_from_date(datetime(2023, 4, 24, 10)) is None

    assert holiday_occasion_from_date(date(2023, 4, 2)) == 'Sizdah Be-dar'


def test_holiday_occasion_from_hijri():
    ...
    assert holiday_occasion_from_hijri(r29) is None
    assert holiday_occasion_from_hijri(Hijri(1444, 10, 1)) == 'Friday'
    assert holiday_occasion_from_hijri(Hijri(1444, 10, 2)) == 'Eid al-Fitr'
    assert holiday_occasion_from_hijri(Hijri(1444, 10, 3)) is None

    assert holiday_occasion_from_hijri(Hijri(1444, 9, 11)) == 'Sizdah Be-dar'
