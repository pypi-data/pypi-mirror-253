...
urlpatterns = [
    ...
    path("api/explore", views.explore_global),
    ...
    path("api/collections/<collection_name>/explore", views.explore_collection),
    ...
    path("utils/parse_for_edit", views.parse_for_edit),
...
if getattr(settings, "TORQUE_COLLECTIONS_ALIAS", False) and getattr(
    ...
    urlpatterns.extend(
        [
            path(
                "api/%s.<fmt>" % settings.TORQUE_COLLECTIONS_ALIAS,
                views.get_collections,
            ),
            path(
            ...
            path(
                "api/%s/<collection_name>/search.<fmt>"
                % (settings.TORQUE_COLLECTIONS_ALIAS),
                views.search_collection,
            ),
            path(
                "api/%s/<collection_name>/explore"
                % (settings.TORQUE_COLLECTIONS_ALIAS),
                views.explore_collection,
            ),
            path(
