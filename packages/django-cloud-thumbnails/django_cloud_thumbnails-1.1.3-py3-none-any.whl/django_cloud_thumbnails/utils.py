...
def find_or_create_thumbnail(source: Source, width: int, height: int, box: str) -> Thumbnail:
    thumbnail = Thumbnail.objects.filter(source=source, width=width, height=height, box=box).first()

    if not thumbnail:
        name = generate_thumbnail(source, width, height, box)
        thumbnail = Thumbnail.objects.create(source=source, name=name, width=width, height=height, box=box)

    return thumbnail


def generate_thumbnail(source: Source, width: int, height: int, box: str) -> str:
    ...
    if format == 'JPG':
        image.save(image_bytes, format=format, quality=80)
    elif format == 'PNG':
        image.save(image_bytes, format=format, optimize=True, compress_level=9)
    elif format == 'WEBP':
        image.save(image_bytes, format=format, quality=80, method=6)
    else:
