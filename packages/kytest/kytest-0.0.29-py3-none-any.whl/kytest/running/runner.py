    ...
    if is_all is True:
        if serial:
            ...
            for result in result_list:
                ...
                content["name"] = content["name"] + "-" + serial
                # 获取historyId，在后面带上-{serial}的后缀
                content["historyId"] = content["historyId"] + "-" + serial
                # content重新写入回result中
        ...
        if udid:
            ...
            _allure = AllureData(result_path=report_path)
            result_list = _allure.get_result_list()
            for result in result_list:
                content = _allure.get_file_content(result)
                # 获取name，给name带上-{serial}的后缀
                content["name"] = content["name"] + "-" + udid
                # 获取historyId，在后面带上-{serial}的后缀
                content["historyId"] = content["historyId"] + "-" + udid
                # content重新写入回result中
                with open(result, "w") as f:
                    f.write(json.dumps(content))
...
class TestMain(object):
    ...
    def __init__(
        ...
        # 想想serial或udid为list但长度为1或者0的情况怎么处理？？？
