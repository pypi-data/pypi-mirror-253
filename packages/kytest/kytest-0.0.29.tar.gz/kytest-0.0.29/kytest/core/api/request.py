...
class HttpReq(object):
    ...
    def assertEq(path: str, value):
        ...
        if isinstance(search_value, list):
            if search_value:
                ...
                assert search_value == value, f"断言: {search_value} 不等于 {value}"
        else:
            logger.info(f"断言: {search_value} 等于 {value}")
            assert search_value == value, f"断言: {search_value} 不等于 {value}"
        ...
        if isinstance(search_value, list):
            if search_value:
                for i, _search_value in enumerate(search_value):
                    ...
                    assert _search_value != value, f'匹配到的第{i + 1}个结果: {_search_value} 等于 {value}'
            else:
        ...
        if search_value:
            for _search_value in search_value:
                if not isinstance(_search_value, list):
                    ...
                    assert len(search_value) > value, f'{len(search_value)} 小于等于 {value}'
                    break
                else:
                    logger.info(f"断言: {len(_search_value)} 大于 {value}")
                    assert len(_search_value) > value, f'{len(_search_value)} 小于等于 {value}'
        else:
            logger.info(f"断言: {len(search_value)} 大于 {value}")
            assert len(search_value) > value, f'{len(search_value)} 小于 {value}'
        ...
        if search_value:
            for _search_value in search_value:
                if not isinstance(_search_value, list):
                    ...
                    assert len(search_value) < value, f'{len(search_value)} 大于等于 {value}'
                    break
                else:
                    logger.info(f"断言: {len(_search_value)} 小于 {value}")
                    assert len(_search_value) < value, f'{len(_search_value)} 大于等于 {value}'
        else:
            logger.info(f"断言: {len(search_value)} 小于 {value}")
            assert len(search_value) < value, f'{len(search_value)} 大于等于 {value}'
        ...
        if isinstance(search_value, list):
            if search_value:
                for i, _search_value in enumerate(search_value):
                    ...
                    assert float(_search_value) > value, f'匹配到的第{i + 1}个结果: {_search_value} 小于等于 {value}'
            else:
                assert False, '匹配结果为空，断言失败'
        else:
            logger.info(f"断言: {search_value} 大于 {value}")
            assert float(search_value) > value, f"断言: {search_value} 小于等于 {value}"
        ...
        if isinstance(search_value, list):
            if search_value:
                for i, _search_value in enumerate(search_value):
                    ...
                    assert float(_search_value) < value, f'匹配到的第{i + 1}个结果: {_search_value} 大于等于 {value}'
            else:
                assert False, '匹配结果为空，断言失败'
        else:
            logger.info(f"断言: {search_value} 小于 {value}")
            assert float(search_value) < value, f"断言: {search_value} 大于等于 {value}"
        ...
        if isinstance(search_value, list):
            if search_value:
                for i, _search_value in enumerate(search_value):
                    ...
                    assert _search_value in value, f'匹配到的第{i + 1}个结果: {_search_value} 不被 {value} 包含'
            else:
                logger.info(f"断言: {search_value} 被 {value} 包含")
                assert search_value in value, f"{search_value} 不被 {value} 包含"
        else:
            logger.info(f"断言: {search_value} 被 {value} 包含")
            assert search_value in value, f"{search_value} 不被 {value} 包含"
        ...
        if isinstance(search_value, list):
            if search_value:
                for i, _search_value in enumerate(search_value):
                    ...
                    assert _search_value not in value, f'匹配到的第{i + 1}个结果: {_search_value} 被 {value} 包含'
            else:
                logger.info(f"断言: {search_value} 不被 {value} 包含")
                assert search_value not in value, f"{search_value} 被 {value} 包含"
        else:
            logger.info(f"断言: {search_value} 不被 {value} 包含")
            assert search_value not in value, f"{search_value} 被 {value} 包含"
        ...
        if isinstance(search_value, list):
            if search_value:
                for i, _search_value in enumerate(search_value):
                    ...
                    assert value in _search_value, f'匹配到的第{i + 1}个结果: {_search_value} 不包含 {value}'
            else:
        ...
        if isinstance(search_value, list):
            if search_value:
                for i, _search_value in enumerate(search_value):
                    ...
                    assert value not in _search_value, f'匹配到的第{i + 1}个结果: {_search_value} 包含 {value}'
            else:
    ...
    # @staticmethod
    # def assertType(path: str, value_type):
    #     """
    #     字段类型
    #     """
    ...
    #
    # @staticmethod
    # def assertStart(path: str, value):
    #     """
    #     以什么开始
    #     """
    ...
    #
    # @staticmethod
    # def assertRegex(path: str, value):
    #     """正则匹配"""
    #     search_value = jmespath.search(path, ResponseResult.response)
