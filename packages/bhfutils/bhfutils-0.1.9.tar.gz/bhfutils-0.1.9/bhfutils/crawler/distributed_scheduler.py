...
from .redis_domain_max_page_filter import RFDomainMaxPageFilter
from .redis_global_page_per_domain_filter import RFGlobalPagePerDomainFilter

from bhfutils.scutils.log_factory import LogFactory
from bhfutils.scutils.redis_queue import RedisPriorityQueue
from bhfutils.scutils.redis_throttled_queue import RedisThrottledQueue
...
class DistributedScheduler(object):
    ...
    def __init__(self, server, persist, update_int, timeout, retries, logger,
                 signals, hits, window, mod, ip_refresh, ip_proxy, add_type, add_ip,
                 ip_regex, backlog_blacklist, queue_timeout, global_page_per_domain_limit,
        ...
        self.ip_request_proxy = ip_proxy
    ...
    def create_queues(self):
        ...
        for key in self.queue_keys:
            ...
            if self.add_ip and self.my_ip is not None:
    ...
    def update_ip_address(self):
        ...
        try:
            req = urllib.request.Request(url=settings.get('PUBLIC_IP_URL',
                                                          'http://ip.42.pl/raw'))
            if self.ip_request_proxy is not None:
                req.set_proxy(self.ip_request_proxy, 'http')
            ...
            if self.old_ip is not None and self.old_ip != self.my_ip:
                self.logger.info("Changed Public IP: {old} -> {new}".format(
                    old=self.old_ip, new=self.my_ip))
                if self.my_ip != '52.28.30.162':
                    self.signals.send_catch_log_deferred(signal=bhf_signals.ip_changed)
        except IOError:
    ...
    @classmethod
    def from_settings(cls, signals, settings):
        ...
        ip_proxy = settings.get('SCHEDULER_IP_PROXY')
        ...
        return cls(server, persist, up_int, timeout, retries, logger, signals,
                   hits, window, mod, ip_refresh, ip_proxy, add_type, add_ip, ip_regex,
                   backlog_blacklist, queue_timeout, global_page_per_domain_limit,
                   global_page_per_domain_limit_timeout, domain_max_page_timeout)

    @classmethod
    def from_crawler(cls, crawler):
        return cls.from_settings(crawler.signals, crawler.settings)
    ...
    def enqueue_request(self, request: Request):
        ...
        req_dict = request.to_dict(spider=self.spider)
            ...
            if (self.backlog_blacklist or
                ...
                if key in self.queue_keys:
                    ...
                    self.redis_conn.zadd(key,
                                         {ujson.dumps(req_dict, reject_bytes=False): -req_dict['meta']['priority']})
