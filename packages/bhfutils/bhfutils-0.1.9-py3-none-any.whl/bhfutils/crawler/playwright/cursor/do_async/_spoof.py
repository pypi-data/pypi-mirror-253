...
class GhostCursor:
    ...
    async def get_random_page_point(self) -> Vector:
    ...
    async def move(
        ...
        if isinstance(selector, str):
            ...
            if elem is None:
                raise Exception(
                    'Could not find element with selector "${}", make sure you\'re waiting for the elements with "wait_for_selector"'.format(
                        selector
                    )
                )
        else:  # ElementHandle
