...
def model_deployment(project_id,model_id,token):
    try:
        ...
        model_select = f"{base_url}training/model-selector"
        access_token = 'Bearer '+ token
        headers = {"Authorization": access_token}
        # Send the POST request
        ...
        #updating ds file folder with model id for running visualizer directly 
        requests.post(model_select, data=data2, headers=headers)
        return response.json()
    except:
