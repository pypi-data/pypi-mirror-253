...
class Ilea(Lea):
    ...
    def _gen_true_p(cond_leas):
        ...
        if len(cond_leas) == 0:
            ...
            (head_cond_lea, *tail_cond_leas) = cond_leas
            for (cv0, p0) in head_cond_lea.gen_vp():
                if cv0:
    ...
    def _gen_one_random_true_cond(self, cond_leas, with_exception):
        if len(cond_leas) == 0:
            ...
            (head_cond_lea, *tail_cond_leas) = cond_leas
            for cv in head_cond_lea.gen_one_random_mc():
                if cv:
                    for v in self._gen_one_random_true_cond(tail_cond_leas, with_exception):
                        yield v
                else:
                    if with_exception:
