...
class ImageDataGenerator(Sequence):
    ...
    def __init__(self, dataframe, batch_size, target_size_hw=None, target_size_wh=None, shuffle=True, image_path_col='image_path', target_col='target', weight_col=None, process_image=None, mult_input_proc=None, file_cache=None, rgb=True, scale_colors=True, zoom=1, mode=None, n_jobs=-1, parallel_one_epoch_only=True):
        """
        :param dataframe:
        :param batch_size:
        ...
        print(f'- INFO: generator mode={mode}, shuffle={"on" if self.shuffle else "off"}, parallel={"off" if self.n_jobs == 1 else "on"}, file_cache={"on" if self.file_cache else "off"}')
    ...
    def on_epoch_end(self):
        ...
        if self.n_jobs != 1 and self.file_cache and self.parallel_one_epoch_only:
    ...
    def _load_and_preprocess_image_do(self, image_path):
        ...
        assert img_array.shape[:2] == self.target_size_hw, img_array.shape
