...
def remove_output(args, subcommand=None):
    try:
        ...
        for k, v in dict(json_payload).items():
            ...
            if str(k) == "conf" and subcommand in ['create-conf', 'edit-conf']:
...
def process_cluster_arguments(json_payload):
    try:
        for k, v in dict(json_payload).items():
            ...
            if str(k) in ['conf', 'conf_secret', 'env_var_secret', 'env_var', 'labels']:
                json_payload[k] = set_labels_or_conf_or_env_var(
                    argument=k,
                    conf=v,
                    delimiter='=' if (
            ...
            if str(k) == 'bootstrap_shell_script_file_path':
                bootstrap_shell_script_file_path = checkFilePathExists(
                    file_path=v,
                    argument='bootstrap_shell_script_file_path'
                )
                json_payload["bootstrap_shell_script"] = readFileContent(
...
def process_cluster_spark_config_arguments(json_payload):
    try:
        spark_config_keys = ['conf', 'packages', 'repositories', 'jars',
                             'archives', 'env_var', 'conf_secret', 'env_var_secret']
...
def confirm_output(response_json, status_code, json_data):
    ...
    if status_code in [200, 201, 400, 401, 404, 409, 422, 440]:
...
def response_validator(response):
    try:
        if response.status_code in [200, 201, 400, 401, 403, 404, 440, 409, 422, 500]:
