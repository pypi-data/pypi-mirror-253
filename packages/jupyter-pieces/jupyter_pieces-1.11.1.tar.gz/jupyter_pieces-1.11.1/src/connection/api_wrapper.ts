...
import { AccessEnum, Asset, Assets } from '@pieces.app/client';
import { Format } from '@pieces.app/client';
import { mergeAssetsWithTransferables } from '../transferables';
import { ClassificationSpecificEnum } from '@pieces.app/client';
...
export const loadPieces = async (): Promise<returnedSnippet[]> => {
  ...
  if (!config.context) {
    ...
    if (!config.context) {
      notifications.error({
        message:
          'Failed to connect to Pieces OS. Please check that Pieces OS is installed.',
      });
...
const fetchSnapshot = async ({
  ...
  try {
    ...
    if (!retry) {
      ...
      notifications.error({
        message:
          'Failed to load snippets. Please restart Pieces OS, ensure that it is up-to-date, and try again. If the problem persists please reach out to support.',
        actions: [
          {
