...
import { AssetsSearchWithFiltersRequest } from '@pieces.app/client';
  ...
  async runFilters() {
    ...
    const assets = await config.assetsApi
      ...
      .catch(() => {
        Notifications.getInstance().error({
          message:
            'Something went wrong with filtering your snippets. Please make sure that Pieces OS is installed, updated, and running. If the issue persists please contact support',
          actions: [
            {
