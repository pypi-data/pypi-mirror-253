...
import { Annotation, AnnotationTypeEnum } from '@pieces.app/client';
...
import { NotificationActionTypeEnum } from 'copilot/dist/types/NotificationParameters';
    ...
    return config.annotationApi
      ...
      .catch(() => {
        Notifications.getInstance().error({
          message:
            'Something went wrong updating that annotation! If the issue persists, please contact support. Make sure that Pieces OS is installed, up to date, and running!',
          actions: [
            {
    ...
    if (!isNew) {
      config.annotationsApi
        ...
        .catch((e) => {
          Notifications.getInstance().error({
            message:
              'Error deleting that annotation! If the issue persists, please contact support. Make sure that Pieces OS is installed, up to date, and running.',
            actions: [
              {
                title: 'Contact Support',
                type: NotificationActionTypeEnum.OPEN_LINK,
    ...
    if (isNew) {
      ...
      return config.annotationsApi
        ...
        .catch(() => {
          Notifications.getInstance().error({
            message:
              'Something went wrong creating that annotation! If the issue persists, please contact support. Make sure that Pieces OS is installed, up to date, and running!',
            actions: [
              {
                title: 'Contact Support',
                type: NotificationActionTypeEnum.OPEN_LINK,
