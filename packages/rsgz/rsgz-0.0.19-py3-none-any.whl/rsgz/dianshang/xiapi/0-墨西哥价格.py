...
def start(重量列表, 成本, sku, 利润, point, 尺寸列表):
    ...
    def xiapi(重量列表, 尺寸列表, 成本, 利润, point):
        ...
        for i in range(len(重量列表)):
            ...
            藏价=(20+7*5+(重量-100)/10*4)*(1/huilv)  # 人民币
            全球商品价格 = 成本+国内运费+利润
            ...
            客户商品价格= (全球商品价格+藏价)/point  # 人民币
            kehuduan_list.append(format(客户商品价格*huilv, '.2f'))  # 墨西哥币

            # 一倍芒果价 = 客户商品价格 - 藏价-(1-point)*客户商品价格  # 客户商品价格 - 藏价- 0.1*客户商品价格
            # 一倍 = "{}-{}-(1-{})*{}=".format(format(客户商品价格, '.2f'), format(藏价, '.2f'), point, format(客户商品价格, '.2f'))

            藏价2 = (20+7*5+((float(重量列表[-1])*1000+10)-100)/10*4)*(1/huilv)  # 人民币
            藏价差 = 藏价2-藏价
            ...
            if i==0:
                print("尺寸:", 尺寸列表[i], 重量列表[i], "2倍芒果价格:", format(芒果价格-40, '.2f'), "元", "客户商品价格:", format(客户商品价格, '.2f'), "元=---->", format((客户商品价格 *huilv), '.2f'), '墨西哥币', "藏价:", format(藏价, '.2f'), "元", "实际利润:", format(实际利润, '.2f'), '={}'.format(利), "一倍芒果价={}".format(一倍), format(一倍芒果价, '.2f'), 尺寸列表[i])
            else:
                print("尺寸:", 尺寸列表[i], 重量列表[i], "2倍芒果价格:", format(芒果价格, '.2f'),"元","客户商品价格:", format(客户商品价格, '.2f'), "元=---->", format((客户商品价格*huilv), '.2f'), '墨西哥币', "藏价:", format(藏价, '.2f'), "元", "实际利润:", format(实际利润, '.2f'),'={}'.format(利), "一倍芒果价={}".format(一倍), format(一倍芒果价, '.2f'), 尺寸列表[i])
            一倍_list.append(format(一倍芒果价, '.2f'))

        price_arr = kehuduan_list
        shou_price = format(float(kehuduan_list[-1])/2, '.2f')
        print("墨西哥币价格列表:")
...
# point = 0.90 # 剩余点数  没免运
point = 0.82 # 剩余点数82  有免运  墨西哥扣了18个点
尺寸列表 = ["XS","S","M","L","XL","2XL","3XL","4XL","5XL","6XL"]
