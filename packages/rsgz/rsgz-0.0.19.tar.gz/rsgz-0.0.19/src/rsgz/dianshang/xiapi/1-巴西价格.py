...
def start(重量列表, 成本, sku, 利润, point, 尺寸列表):
    ...
    def xiapi(重量列表, 尺寸列表, 成本, 利润, point):
        ...
        for i in range(len(重量列表)):
            ...
            藏价=(5+7*1.4+(重量-100)/10*0.9)/huilv
            全球商品价格 = 成本+国内运费+利润
            ...
            if i==0:
                print("尺寸:", 尺寸列表[i], 重量列表[i], "2倍芒果价格:", format(芒果价格-40, '.2f'), "元", "客户商品价格:", format(客户商品价格, '.2f'), "元=---->", format((客户商品价格 * huilv), '.2f'), '巴西币', "藏价:", format(藏价, '.2f'), "元", "实际利润:", format(实际利润, '.2f'), '={}'.format(利), "一倍芒果价={}".format(一倍), format(一倍芒果价-20, '.2f'), 尺寸列表[i])
            else:
                print("尺寸:", 尺寸列表[i], 重量列表[i], "2倍芒果价格:", format(芒果价格, '.2f'),"元","客户商品价格:", format(客户商品价格, '.2f'), "元=---->", format((客户商品价格*huilv), '.2f'), '巴西币', "藏价:", format(藏价, '.2f'), "元", "实际利润:", format(实际利润, '.2f'),'={}'.format(利), "一倍芒果价={}".format(一倍), format(一倍芒果价, '.2f'), 尺寸列表[i])
            一倍_list.append(format(一倍芒果价, '.2f'))

        price_arr = kehuduan_list
        shou_price = format(float(kehuduan_list[-1])/2+random.random(), '.2f')
        print("巴西币价格列表:")
