...
class F04(BasePage):
    def __init__(self, parent, controller):
        ...
        explain = tk.Label(master=self.headframe, text = 'This will produce the tables of instantaneous values for each node, supply points, branch and transformers.', font = controller.text_font)
        explain.pack()
...
class F06(BasePage):
    def __init__(self, parent, controller):
        ...
        # add widgets here
        head = tk.Label(master=self.headframe, text = 'Page 5: Train Low Voltage Summary',font = controller.sub_title_font)
        head.pack()
...
class F07(BasePage):
    def __init__(self, parent, controller):
        ...
        explain = tk.Message(master=self.headframe, text = 'Averages of MW and MVA demand are calculated over succesive intervals of time for the requested supply points, metering points, static VAR compensators and motor alternators requested. Averages are NOT rolling average.', aspect = 1200, font = controller.text_font)
        explain.pack()
...
class F12(BasePage):
    def __init__(self, parent, controller):
        ...
        explain = tk.Message(master=self.headframe, text = 'This will produce a table of maximum rolling branch currents (average and RMS) for the specified branch(es), together with the time-period over twhich the maximum occurred is produced.', aspect = 1200, font = controller.text_font)
        explain.pack()
        ...
        choice2 = tk.Radiobutton(master=self.optionframe, text = 'Option 2: Multiple Feeder Currents Output (A text file named as ""BranchNodeList.txt"" is required"',value="2", variable=option1)
        choice2.grid(row = 1, column = 0, sticky = "w", padx=5, pady=5)
        # Option 3 to be considered if needed in the future
        # choice3 = tk.Radiobutton(master=self.optionframe, text = 'Option 3: Extract the List in the Train List ', value="3", variable=Local_Shared.option_select)
        # choice3.grid(row = 2, column = 0, sticky = "w", padx=5, pady=5)
        ...
        label3 = tk.Label(master=self.inputframe, text = 'Branch Name (Format: XXXX/X):',font = controller.text_font)
        label3.grid(row = 1, column = 0, sticky = "w", padx=2, pady=2)
