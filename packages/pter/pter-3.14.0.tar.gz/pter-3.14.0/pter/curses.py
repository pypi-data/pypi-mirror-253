...
from pter.timetracking import Tracker
...
class CursesApplication(Application):
    def __init__(self, sources, conf, initial_search):
        ...
        if tracker is not None:
            self.tracker = Tracker(self, tracker)
    ...
    def do_toggle_tracking(self):
        ...
        if self.tracker is not None:
            result = self.tracker.start(self.tasks.selected_item.task)
            logging.debug(f"Started time tracker with {result.args}")
            if result.returncode == 0:
                self.info(tr("Tracking started"))
            else:
                ...
                logging.warning(f"Tracker call '{result.args}' returned {result.returncode}: {details}")
                self.error(tr("Tracker returned {}: {}")
                             .format(result.returncode, details))
            self.paint(result.returncode != 0)
            return
