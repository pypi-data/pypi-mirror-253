...
def convert_polars_lazyframe(df: pl.DataFrame | pl.LazyFrame) -> pl.DataFrame:
    """
...
def filter_dataframe(options: LoadOptions, df: pl.DataFrame) -> pl.DataFrame:
    ...
    if options.relation_filter:
        ...
        if isinstance(options.relation_filter, FilterGroup):
            group: FilterGroup = options.relation_filter
            ...
            df = df.filter(options.relation_filter.visit(visitor))
...
def aggregate_dataframe(options: LoadOptions, df: pl.DataFrame) -> pl.DataFrame:
    ...
    if len(groups) == 0:
        ...
        for prop, load_prop in groups:
            group_list.append(pl.col(prop.alias))
...
def convert_dataframe_to_result(options: LoadOptions, df: pl.DataFrame) -> str:
    ...
    if options.max_rows:
        df = convert_polars_lazyframe(df).head(options.max_rows)
    ...
    return convert_polars_lazyframe(df).write_json()
