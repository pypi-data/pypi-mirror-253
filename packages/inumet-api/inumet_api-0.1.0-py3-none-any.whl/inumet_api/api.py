...
class INUMET:
    def __init__(self, station:str="", zone:str="") -> None:
        self._endpoints()
        if station != "":
            self.station = [x['id'] for x in INUMET().estaciones()['estaciones'] if x['NombreEstacion'] == station][0]
        if zone != "":
            self.zone = [x['idInt'] for x in INUMET().zonas()["zonas"] if x['nombre'] == zone][0]
        pass
