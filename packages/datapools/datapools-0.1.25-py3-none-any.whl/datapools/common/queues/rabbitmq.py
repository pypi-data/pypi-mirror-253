        ...
        while True:
            ...
            if self.internal_queue.empty():
                ...
                if self.role == QueueRole.Receiver:
                    ...
                    # logger.info(queue)
                    if "messages" in queue and 'message_stats' in queue:
                        if time.time() - last_log > 5:
                            last_log = time.time()
                            logger.info(
                                f'=================== receiver queue size {self.queue_name} {queue["messages"]} {queue["messages_unacknowledged"]=} {queue["message_stats"]["deliver_get"]=}'
                            )
                        if (
                            queue["messages"] == 0
