...
def generate_video(img, interrogators, interrogators_data, name, nx, dt, c=None, verbose=False):
    """
    ...
    for i in tqdm(range(len(img))):
        ...
        im = ax[0].imshow(img[i], cmap=get_black_cmap(), vmin=-np.max(np.abs(img[i:])), vmax=np.max(np.abs(img[i:])))
        plt.colorbar(im, shrink=0.75, ax=ax[0])
        for interrogator in interrogators:
            ax[0].scatter(interrogator[1]+(nx//2), interrogator[0]+(nx//2), marker="1", color=colors[interrogator])
        for interrogator in interrogators:
