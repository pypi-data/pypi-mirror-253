...
class AcousticWaveDataset(torch.utils.data.Dataset):
    ...
    def generate_data(self):
        ...
        for i in tqdm(range(self.size)):
            self.data.append(solve_pde(self.grid, self.nx, self.ndt, self.ddt, self.epicenters[i], self.velocity_model)[::int(self.ndt/self.nt), ::int(1/self.sx), ::int(1/self.sx)])
    ...
    def __getitem__(self, idx):
        return self.epicenters[idx], self.data[idx]
