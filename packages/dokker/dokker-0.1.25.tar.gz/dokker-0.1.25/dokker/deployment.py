...
class Deployment(KoiledModel):
    ...
    async def await_for_healthz(self, services: Optional[List[str]] = None):
        if services is None:
            services = [
    ...
    def wait_for_healthz(self, services: Optional[List[str]] = None):
        return unkoil(self.await_for_healthz, services=services)
