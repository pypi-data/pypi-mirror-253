...
class MessageLocal(Item, CompoundMessage, GenericCRUD):
    """Message Local Class"""

    def __init__(self, *, to_recipients: List[Recipient], api_type_id: int, campaign_id: int = None,
                 ...
                 sender_profile_id: int = DEFAULT_SENDER_PROFILE_ID, **kwargs) -> None:
        ...
        logger.start()
        CompoundMessage.__init__(self, message_id=message_id, campaign_id=campaign_id, body=original_body,
                                 subject=original_subject, to_recipients=to_recipients)
    ...
    def get_body_after_text_template(self, recipient: Recipient = None,
        ...
        body = ""
        for profile_block in self.get_compound_message_dict(message_channel)[
            message_channel.name]["bodyBlocks"]["profileBlocks"]:
