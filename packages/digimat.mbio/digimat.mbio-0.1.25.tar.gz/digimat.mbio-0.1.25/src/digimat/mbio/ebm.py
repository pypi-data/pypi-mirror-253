...
class MBIODeviceEBM(MBIODevice):
    def onInit(self):
        ...
        self.value('dcv', 'V', resolution=1.0)
    ...
    def onLoad(self, xml: XMLConfig):
        ...
        if item:
            ...
            if self.config.speedsource=='bus':
                self.value('sp', unit='%', writable=True, resolution=0.1)
            ...
            if subitem:
                ...
                if self.config.watchdogsource=='bus':
                    self.config.set('watchdogdelay', subitem.get('delay'))
        ...
        if self.config.watchdogsource:
            ...
            if self.config.watchdogsource=='bus':
                ...
                self.writeRegistersIfChanged(0xd15f,
                    self.config.getFloat('min', default=0, vmin=0)/10.0*65535.0)
    ...
    def poweroff(self):
        value=self.myvalues.enable
        if value is not None:
            value.off()
        value=self.myvalues.sp
        if value is not None:
            value.set(0)
    ...
    def refresh(self):
        if self.config.maxrpm is not None:
            ...
            if r:
                try:
                    self.myvalues.rpm.updateValue(r[0]/64000.0*self.config.maxrpm)
                except:
                    pass
        ...
        if r:
            self.myvalues.status.updateValue(r[0])
            self.myvalues.warning.updateValue(r[1])
            self.myvalues.dcv.updateValue(r[2]/255.0*self.config.vref)
            self.myvalues.dci.updateValue(r[3]/255.0*self.config.iref)
            self.myvalues.t1.updateValue(r[4])
            self.myvalues.t2.updateValue(r[5])
            self.myvalues.t3.updateValue(r[6])
            self.myvalues.speed.updateValue(r[8]/65535.0*100.0)

        if self.isTimeout(self._timeoutRefreshSlow):
            r=self.readInputRegisters(0xd023, 8)
            if r:
                if self.myvalues.ai1 is not None:
                    self.myvalues.ai1.updateValue(r[0]/65535.0*10.0)
                if self.myvalues.ai2 is not None:
                    self.myvalues.ai2.updateValue(r[0]/65535.0*10.0)
                decoder=self.decoderFromRegisters(r[6:])
                self.myvalues.ene.updateValue(decoder.dword())

            r=self.readInputRegisters(0xd021, 1)
            if r:
                v=r[0]/65535.0*self.config.vref*self.config.iref
                self.myvalues.pow.updateValue(v)

            r=self.readHoldingRegisters(0xd009, 1)
            if r:
                self.myvalues.hcnt.updateValue(r[0])
