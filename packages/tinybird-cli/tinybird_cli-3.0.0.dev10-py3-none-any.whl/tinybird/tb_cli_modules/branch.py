...
async def release_rm(semver: str, oldest_rollback: bool, force: bool, yes: bool, dry_run: bool) -> None:
    ...
    if dry_run or yes or click.confirm(FeedbackManager.warning_confirm_delete_release(semver=semver)):
        try:
            ...
            result = await client.release_rm(config["id"], semver, confirmation=config["name"], dry_run=dry_run)
            if dry_run:
                click.echo(FeedbackManager.success_release_delete_dry_run(semver=result.get("semver")))
                await print_release_summary(config, semver)
            else:
