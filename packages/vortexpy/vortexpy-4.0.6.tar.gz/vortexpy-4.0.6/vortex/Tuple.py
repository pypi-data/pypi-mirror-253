...
TUPLE_TYPES: list[Type["Tuple"]] = []
TUPLE_TYPES_BY_NAME: Dict[str, Type["Tuple"]] = {}
TUPLE_TYPES_BY_SHORT_NAME: Dict[str, Type["Tuple"]] = {}
    ...
    for name, value in objDict.items():
        ...
        if shortName != JSON_EXCLUDE:
            if shortName in shortFieldNames:
                raise Exception(
                    "TupleField %s short name %s is already registered"
                    % (name, shortName)
                )
    ...
    def restfulJsonDictToTupleWithValidation(
        ...
        def convert(fieldName: str, value):
            ...
            if (
                ...
                and datetime is typeHint
                and re.match(ISO8601_REGEXP, value)
            ):
                return datetime.strptime(value, ISO8601)
            ...
            if isinstance(value, dict):
                ...
                if hasattr(valueTypeHint, "__tupleType__"):
                    ...
                    logger.warning(
                        f"typing is not checked for key 'key' in value '{value}'"
                    )
                    for key, item in value.items():
                        # raiseIfNotType(
                        ...
                        # this should be in SerialiseUtil.py
                        if isinstance(item, str) and re.match(
                            ISO8601_REGEXP, item
                        ):
                            item = datetime.strptime(item, ISO8601)
...
class Tuple(Jsonable, _TupleToPlainJsonMixin, _TupleToSqlaJsonMixin):
    ...
    * Datetimes and ints are serialised and deserialsied
    ...
    def __init__(self, **kwargs):
        ...
        if getattr(self.__class__, "__slots__", None):
