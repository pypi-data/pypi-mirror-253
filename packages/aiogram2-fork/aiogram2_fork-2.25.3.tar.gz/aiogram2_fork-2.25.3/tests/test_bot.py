...
async def test_set_my_description(bot: Bot):
    """ setMyDescription method test """
    from .types.dataset import BOT_DESCRIPTION

    async with FakeTelegram(message_data=True):
        description = BOT_DESCRIPTION['description']
        result = await bot.set_my_description(description)
        assert isinstance(result, bool)
        assert result is True
