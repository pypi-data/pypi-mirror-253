...
def prepare_stickers(value):

    files_and_stickers = {'files': list(), 'stickers': list()}
    for item in value:
        if isinstance(item.sticker, types.InputFile):
            files_and_stickers['files'].append({
                "sticker": item.sticker,
                "emoji_list": item.emoji_list,
            })
        else:
            files_and_stickers['stickers'].append(item)

    return files_and_stickers
