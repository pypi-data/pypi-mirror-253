...
def download_video(link, extension):
    ...
    for line in iter(process_video.stdout.readline, ''):
        if os.path.exists(output_video):
            ...
            print(f"\rTamanho do arquivo baixado: {downloaded_size:.2f} MB", end='')
        time.sleep(1)
    video_download_output = ""
    video_progress_shown = False
    for line in iter(process_video.stdout.readline, ''):
        ...
        if match:
            ...
            video_progress_shown = True
        elif not video_progress_shown and os.path.exists(output_video):
            clear_output(wait=True)
            video_size = os.path.getsize(output_video) / (1024 ** 2)
            print(f"Download do vídeo: {video_size:.2f} MB", end='\r')
