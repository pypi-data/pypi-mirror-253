...
def baixar_e_converter_video(url, extension):
    """
    Baixa e converte um vídeo do YouTube.

    Args:
    - url (str): URL do vídeo do YouTube a ser baixado.
    - extension (str): Extensão do arquivo de saída (por exemplo, '.mp4', '.mkv', '.mov').
    """
    ...
    folder_path, final_output, output_converted = preparar_diretorios(video_title, extension)

    # Download do vídeo e do áudio
    output_video, output_audio = download_video(url, folder_path)
    if not output_video or not output_audio:
        print("Erro no download do vídeo ou áudio.")
        return

    # Mesclando áudio e vídeo
    if not mesclar_audio_video(output_video, output_audio, final_output):
        print("Erro ao mesclar áudio e vídeo.")
        return

    # Convertendo o vídeo, se necessário
    final_output_converted = converter_video_se_necessario(final_output, output_converted, extension)
    if not final_output_converted:
        print("Erro na conversão do vídeo.")
        return

    print(f"Vídeo baixado e convertido com sucesso. Localização: {final_output_converted}")
