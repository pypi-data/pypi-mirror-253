...
class Collection(dict):
    ...
    def find(self, fn, self_arg = None) -> str|int|list|dict|None:
        ...
        for key in self:
            val = self.get(key)
            if bool(self.__run(fn, val, key)): return val
        return None
    ...
    def find_key(self, fn, self_arg = None) -> str|int|list|dict|None:
        if self_arg: fn = partial(fn, self_arg)
        for key in self:
            if bool(self.__run(fn, self[key], key)): return key
        return None
    ...
    def sweep(self, fn, self_arg = None) -> int:
        ...
        results = self.clone(True)
        for key in self:
            val = self.get(key)
            if not bool(self.__run(fn, val, key)):
                results.set(key, val)
        for key in results: self.pop(key)
    ...
    def filter(self, fn, self_arg = None) -> Self:
        ...
        for key in self:
            val = self.get(key)
            if bool(self.__run(fn, val, key)): results.set(key, val)
        return results
    ...
    def map(self, fn, self_arg = None) -> list[str|int|list|dict]:
        ...
        result = []
        for key in iterable:
            result.append(self.__run(fn, iterable[key], key))
        return result
    ...
    def map_values(self, fn, self_arg = None) -> Self:
        ...
        coll = self.clone()
        for key in self:
            coll.set(key, self.__run(fn, self[key], key))
    ...
    def some(self, fn, self_arg = None) -> bool:
        if self_arg: partial(fn, self_arg)
        for key in self:
            if bool(self.__run(fn, self[key], key)): return True
        return False
    ...
    def every(self, fn, self_arg = None) -> bool:
        if self_arg: partial(fn, self_arg)
        for key in self:
            if not bool(self.__run(fn, self[key], key)): return False
        return True
    ...
    def sorted(self, compare_function):
        return self.clone().sort(compare_function)
