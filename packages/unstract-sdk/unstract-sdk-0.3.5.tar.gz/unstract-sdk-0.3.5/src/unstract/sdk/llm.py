...
from unstract.sdk.adapters import UnstractToolAdapter
...
class UnstractToolLLM:
    ...
    def __init__(self, tool: UnstractAbstractTool, llm_id: str = None):
        """
    ...
    @staticmethod
    def run_completion(
        llm: LLM, prompt: str, retries: int = 3, **kwargs
    ) -> Optional[dict]:
    ...
    def get_llm(self, adapter_instance_id: str = None) -> Optional[LLM]:
        """Returns the LLM object for the tool.
        ...
        if adapter_instance_id is not None:
            try:
                llm_config_data = UnstractToolAdapter.get_adapter_config(
                ...
                if llm_adapter_id in self.llm_adapters:
                    ...
                    llm_adapter_class = llm_adapter(llm_metadata)
                    return llm_adapter_class.get_llm_instance()
                else:
                    return None
            except Exception as e:
                logger.error(f"Unable to get llm instance: {e}")
                return None
        else:
            # Support for legacy get_llm.
            # Will be removed once all tools have ported to the new get_llm
            return self.__get_llm()
