...
class UnstractPromptTool:
    ...
    def __init__(
        ...
        self.bearer_token = tool.get_env_or_die(
            PlatformServiceKeys.PLATFORM_API_KEY
        )
    ...
    def get_exported_tool(
        tool: UnstractAbstractTool, prompt_registry_id: str
        ...
        """
        platform_host = tool.get_env_or_die(PlatformServiceKeys.PLATFORM_HOST)
        platform_port = tool.get_env_or_die(PlatformServiceKeys.PLATFORM_PORT)
        ...
        bearer_token = tool.get_env_or_die(PlatformServiceKeys.PLATFORM_API_KEY)
        ...
        if response.status_code == 200:
            ...
            tool.stream_log(
                "Successfully retrieved metadata for exported tool "
                f"for : {prompt_registry_id}"
            )
            ...
            tool.stream_error_and_exit(
                f"Exported tool not found for: for custom_tool_instance"
                f"{prompt_registry_id}",
                level=LogLevel.ERROR,
            )
            return None
