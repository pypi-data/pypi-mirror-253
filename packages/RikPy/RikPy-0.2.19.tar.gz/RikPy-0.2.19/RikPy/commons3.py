...
def s3_environment():
    ...
    S3_ACCESS_KEY_ID = os.getenv("S3_ACCESS_KEY_ID", 'AKIAVRUVVBONFIN2H27T')
    S3_SECRET_ACCESS_KEY = os.getenv("S3_SECRET_ACCESS_KEY", '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')
    S3_URL = os.getenv("S3_URL", 'https://eu-central-1.s3.amazonaws.com/getaiir')
    S3_CUBE_NAME = os.getenv("S3_CUBE_NAME", 'getaiir')
    S3_CUBE_PUBLIC = os.getenv("S3_CUBE_PUBLIC", 'getaiir/public/')
    S3_BUCKET_NAME = os.getenv("S3_BUCKET_NAME", 'eu-central-1')
...
def s3_upload_local_file(file_name="", folder_name=None, s3_config_dict=None):
    '''
...
def s3_upload_file_from_url(file_url="", folder_name="", s3_config_dict=None, bnewname=False):
    ...
    if bnewname:
        ...
        object_key = s3_upload_local_file(file_name=new_file_path, folder_name=folder_name, s3_config_dict=s3_config_dict)
