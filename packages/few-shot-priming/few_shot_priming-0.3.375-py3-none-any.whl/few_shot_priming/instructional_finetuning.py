...
def prepare_dataset(dataset, prompter, label_mapper, tokenizer, logger, cutoff_len, few_shot_size=None, fine_tune=False, diverse=False):
    ...
    if not fine_tune:
        ...
        examples = format_prompt(dataset["training"], tokenizer, label_mapper,  2048, few_shot_size, logger, record=None, alpaca_examples=True)
    ...
    for split in dataset:
        if fine_tune:
            ...
            for i in range(10):
                log_message(logger, "label output " + huggingface_dataset["training"]["output"][i])
...
def run_instructional_fine_tuning(config, params, experiment, validate, logger, debug, topic_count=None, sampling_strategy=None, similarity=None, fine_tune=True, analyze_primes=False):
    ...
    if sampling_strategy =="similar":
        ...
        train_data, val_data, test_data = prepare_dataset(dataset, prompter, mapping_dictionary, tokenizer, logger, cutoff_len, few_shot_size=config["few-shot-size"], fine_tune=fine_tune, diverse=diverse)
