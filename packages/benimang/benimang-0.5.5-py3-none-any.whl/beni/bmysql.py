...
class MysqlDbCursor:
    ...
    async def execute(self, query: str, args: Sequence[Any] = ()):
        return await self.cursor.execute(query, args)

    async def getOne(self, modelClass: Type[_BModel], sql: str, args: Sequence[Any] = ()) -> _BModel | None:
        await self.cursor.execute(_sqlFormat(sql), args)
    ...
    async def getList(self, modelClass: Type[_BModel], sql: str, args: Sequence[Any] = ()) -> list[_BModel]:
        await self.cursor.execute(_sqlFormat(sql), args)
    ...
    async def getValue(self, valueClass: Type[_T], sql: str, args: Sequence[Any] = ()) -> _T | None:
    ...
    async def getValueList(self, valueClass: Type[_T], sql: str, args: Sequence[Any] = ()) -> list[_T] | None:
        await self.cursor.execute(sql, args)
        result: Sequence[Sequence[Any]] = await self.cursor.fetchall()
        if not result:
            return None
...
class MysqlDb:
    ...
    @asynccontextmanager
    async def getCursor(self) -> AsyncGenerator[MysqlDbCursor, None]:
    ...
    async def getOne(self, modelClass: Type[_BModel], sql: str, args: Sequence[Any] = ()) -> _BModel | None:
        async with self.getCursor() as cursor:
            return await cursor.getOne(modelClass, sql, args)

    async def getList(self, modelClass: Type[_BModel], sql: str, args: Sequence[Any] = ()) -> list[_BModel]:
        async with self.getCursor() as cursor:
            return await cursor.getList(modelClass, sql, args)
    ...
    async def saveOne(self, model: BModel, *, tableName: str = ''):
        ...
        async with self.getCursor() as cursor:
            result = await cursor.execute(sql, [values])
            if result:
                await cursor.commit()
                return result
    ...
    async def execute(self, sql: str, args: Sequence[Any] = ()):
        async with self.getCursor() as cursor:
    ...
    async def getValue(self, valueClass: Type[_T], sql: str, args: Sequence[Any] = ()) -> _T | None:
        async with self.getCursor() as cursor:
            return await cursor.getValue(valueClass, sql, args)

    async def getValueList(self, valueClass: Type[_T], sql: str, args: Sequence[Any] = ()) -> list[_T] | None:
        async with self.getCursor() as cursor:
            return await cursor.getValueList(valueClass, sql, args)
