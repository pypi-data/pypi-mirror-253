...
class Context(object):
    ...
    def _add_bar2bar_data_cache(self, k, bar):
        ...
        if kk in self.bar_data_set:
            ...
            context._add_data_to_cache(bar["symol"], bar["frequency"], bar)
    ...
    def _has_cache(self, symbol, freq):
        self._cache.has_cache(symbol, freq)
...
class _Cache:
    ...
    def get_data(self, symbol, freq, count, fields):
        ...
        if miss_count != 0:
            ...
            # 15:15:01 后才会更新日线bar, 如果时间在这之前通过 history_n 补的数据应该往前一天
            if freq == "1d" and context.now.time() < datetime.time(15, 15, 1):
                query_data_end_time -= datetime.timedelta(days=1)
            adjust_end_time = (
            ...
            for item in data[-2::-1]: # 去除最后一个值并逆序遍历
