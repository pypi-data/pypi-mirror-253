...
class WolfMapViewer(wx.Frame):
    ...
    def upobj(self):
        ...
        if myobj is not None:
            ...
            # mouvement dans les dictionnaires pour garder l'ordre identique à l'arbre
            for curdict in self.added.values():
                keys = list(curdict.keys())
                if id.lower() in keys:

                    # recherche de l'index de l'élément à déplacer
                    idx = keys.index(id.lower())

                    toappend=[]
                    if idx>0:
                        # on n'est pas sur le premier élément
                        # on retient l'élément précédent
                        toappend.append(curdict[keys[idx-1]])
                        # on retient tous les éléments suivants
                        for i in range(idx+1,len(keys)):
                            toappend.append(curdict[keys[i]])

                        # on supprime tous les éléments à déplacer
                        for locdict in toappend:
                            curdict.pop(locdict.idx)

                        # on les réinsère dans l'ordre
                        for locdict in toappend:
                            curdict[locdict.idx]=locdict
            ...
            for curdict in self.added:
                ...
                if id.lower() in keys:
                    ...
                    if idx<len(keys)-1:
                        ...
                        for locdict in toappend:
                            self.added[curdict].pop(locdict.idx)

                        for locdict in toappend:
                            self.added[curdict][locdict.idx]=locdict
    ...
    def print_shortcuts(self, inframe:bool = None):
        ...
        shortcuts = {'F1': _('Results : read the last step'),
                     ...
                     'CTRL+C': _('Drawing : copy canvas to Clipboard wo axes'),
                     'ALT+C': _('Drawing : copy Matplotlib image to Clipboard with axes'),
    ...
    def OntreeRight(self, e: wx.MouseEvent):
        ...
        if self.selected_object is not None:
            if isinstance(self.selected_object, WolfArray):
                ...
                if _('Boundary conditions') in self.popupmenu.GetMenuItems():
                    self.popupmenu.RemoveItem(self.popupmenu.FindItem(_('Boundary conditions')))
