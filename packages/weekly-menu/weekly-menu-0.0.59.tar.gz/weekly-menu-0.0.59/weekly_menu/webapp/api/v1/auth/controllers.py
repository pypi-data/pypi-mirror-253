...
auth_blueprint = Blueprint("auth", __name__, url_prefix=BASE_PATH + "/auth")
...
def get_token(user: PostUserTokenSchema):
    ...
    return (
        jsonify(
            ...
            expires_in=config.access_expires.seconds,
        ),
        200,
    )
...
def register_user(user_meta: PostRegisterUserSchema):
    ...
    user_prefs.owner = user.id
    user_prefs.language = user_meta.get("language")
    user_prefs.save()
