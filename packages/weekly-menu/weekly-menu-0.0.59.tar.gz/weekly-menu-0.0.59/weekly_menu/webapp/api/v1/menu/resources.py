...
class MenuInstance(Resource):
    ...
    def get(self, user_info: User, menu_id=''):
        menu = Menu.objects(Q(owner=str(user_info.id)) &
                            Q(_id=menu_id)).get_or_404()
    ...
    def delete(self, user_info: User, menu_id=''):
        Menu.objects(Q(owner=str(user_info.id)) & Q(
            _id=menu_id)).get_or_404().delete()
        return "", 204
    ...
    def put(self, new_menu: Ingredient, user_info: User, menu_id=''):
        old_menu = Menu.objects(Q(_id=menu_id) & Q(
        ...
        if (result.modified_count != 1):
            raise BadRequest(
                description='no matching menu with id: {}'.format(menu_id))
...
class MenuRecipesList(Resource):
    ...
    def post(self, menu_id, menu_recipe, user_info: User):
        menu = _retrieve_base_menu(menu_id, str(user_info.id))
        recipe = Recipe.objects(Q(owner=str(user_info.id)) & Q(
            _id=menu_recipe['recipe_id'])).get_or_404()
...
class MenuRecipesInstance(Resource):
    ...
    def delete(self, user_info: User, menu_id='', ingredient_id=''):
        ...
        menu.recipes = [
            menu_menu for menu_menu in menu.recipes if menu_menu.ingredient.id != ingredient_id]
