...
from .schemas import (
    ...
    RecipeIngredientWithoutRequiredIngredientSchema,
)
from ...models import Recipe, User, RecipeIngredient, ExternalRecipe
...
def _dereference_ingredients(recipe: Recipe):
    if recipe.ingredients != None:
        recipe_ingredients = [ing.ingredient.to_mongo() for ing in recipe.ingredients]
...
class RecipeList(Resource):
    ...
    def get(self, query_args, page_args, user_info: User):
        return search_on_model(
            Recipe, Q(owner=str(user_info.id)), query_args, page_args
        )
...
class RecipeInstance(Resource):
    ...
    def get(self, user_info: User, recipe_id=""):
        recipe = Recipe.objects(
            Q(_id=recipe_id) & Q(owner=str(user_info.id))
        ).get_or_404()
    ...
    def put(self, new_recipe: Recipe, user_info: User, recipe_id=""):
        ...
        if result.modified_count != 1:
            raise BadRequest(
                description="no matching recipe with id: {}".format(recipe_id)
            )
...
class RecipeIngredientsList(Resource):
    ...
    def get(self, user_info: User, recipe_id=""):
        ...
        return (
            [recipe_ingredient.to_mongo() for recipe_ingredient in recipe.ingredients]
            if recipe.ingredients != None
            else []
        )
    ...
    def post(self, recipe_id, recipe_ingredient: RecipeIngredient, user_info: User):
        ...
        current_ingredients_in_recipe = [
            str(ing.ingredient.id) for ing in recipe.ingredients
        ]
...
class RecipeIngredientInstance(Resource):
    ...
    def get(self, user_info: User, recipe_id: str, ingredient_id: str):
        recipe = Recipe.objects(
            Q(_id=recipe_id)
            & Q(owner=str(user_info.id))
            & Q(ingredients__ingredient=ingredient_id)
        ).get_or_404()
        ...
        raise NotFound(
            description="can't find ingredient with id {} in recipe {}".format(
    ...
    @load_user_info
    @validate_payload(
        RecipeIngredientWithoutRequiredIngredientSchema(), "recipe_ingredient"
    )
    def patch(
        self,
        user_info: User,
        ...
        for ing_doc in recipe.ingredients:
            if str(ing_doc.ingredient.id) == ingredient_id:
                recipe_ingredient = ing_doc = patch_embedded_document(
                    recipe_ingredient, ing_doc
                )
                break
    ...
    def put(
        ...
        Recipe.objects(
            ...
            & Q(owner=str(user_info.id))
            & Q(ingredients__ingredient=ingredient_id)
        ).update(set__ingredients__S=recipe_ingredient)

        return recipe_ingredient, 200


class RecipeExploreList(Resource):
    @jwt_required
    @parse_query_args
    @paginated
    @load_user_info
    def get(self, query_args, page_args, user_info: User):
        return search_on_model(ExternalRecipe, Q(scraped=True), query_args, page_args)
