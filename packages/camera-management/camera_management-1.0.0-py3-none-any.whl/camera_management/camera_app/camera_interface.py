...
from camera_management.tools import types
from camera_management.tools.camera_dataclasses import CameraDescription, PreprocessingSettings
from camera_management.tools.config_tools import check_preprocessing, write_configuration_file
from camera_management.tools.image_processor import ImageProcessor
...
class CameraApplication(threading.Thread):
    def __init__(self, port: int):
        ...
        self.preprocessing_settings = PreprocessingSettings(bw=False, undistort=False, rotate=0)
        ...
        self._last_frame = types.ImageProcessorData(image=np.zeros((1080, 720)))
        ...
        def img_processor():
            ...
            data: types.ImageProcessorData = self._preprocessing_thread.data
            if data.image is None:
                data = self._last_frame
            else:
                self._last_frame = data

            # print(f"MaskRequest for \"{img_id}\", type: {type(img_id)}")
        ...
        #     INSERT USEFUL DESCRIPTION WHEN SEEING THIS
        #     """
        #     pd: types.ProcessedData = _interface.data.processed_data
        #     return jsonify(pd.pack())
        ...
        def set_pre_processing_settings():
            ...
            if request.method == "POST":
                self.preprocessing_settings = check_preprocessing(request.json, self.preprocessing_settings)
                ...
                return "Settings JSON was not accepted.", 400
        ...
        def camera_settings():
            ...
            if request.method == "POST":
                ...
                raise ValueError(f"Only GET and POST requests are allowed. Tried to use {request.method}")

        def gen_frames():
            ret, buffer = cv.imencode(".jpg", self._preprocessing_thread.data.image)
        ...
        #
        @self._flask.route("/kill", methods=["GET", "POST"])
        def die_post():
            """
            INSERT USEFUL DESCRIPTION WHEN SEEING THIS
            """
            if request.method == "GET":
                return Response("Kill GET received")

            cmd = request.get_data(as_text=True)
            print(f"{self.description.device.product} running on port {self._port} is shutting down. {cmd}")
            if "now" == cmd:
                exit(0)
            return Response("Please only send 'now'\n")
