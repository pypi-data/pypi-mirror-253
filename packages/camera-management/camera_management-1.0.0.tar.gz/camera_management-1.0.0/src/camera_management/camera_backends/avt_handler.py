...
class AVTDevice(cv.VideoCapture):
    ...
    def _start_streaming_thread(self, handler):
        print(f"Trying to start Thread and connect so {self.camera_id}!")
        with vmbpy.VmbSystem.get_instance() as vmb:
            print("Got to the Vimba instance!")
            cam = vmb.get_camera_by_id(self.camera_id)
            try:
                with cam:
                    print(f"Found Camera {cam.get_name()}")
                    # self._setup_camera(cam)
                    cam.start_streaming(handler=handler)
                    print(f"Found the stream {cam.get_streams()}")
        ...
        print(f"{vmb.get_version()} terminated.")
