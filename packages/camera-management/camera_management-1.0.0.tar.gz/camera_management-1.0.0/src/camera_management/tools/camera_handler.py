...
from camera_management.camera_backends.basler_handler import BaslerDevice_a2A5328_15ucBAS, BaslerDevice_acA5472_17uc

try:
    from camera_management.camera_backends.avt_handler import AVTDevice

    avt_import = True
except Exception:
    avt_import = False
    print("camera_handler: (Ignore if you do not want to use AVT Cameras) Could not import AVTDevice. See the readme for more information.")
...
def configure_stream(cap: cv.VideoCapture, resolution: ImageResolution, codec: str) -> dict:
    """
    ...
    return {"codec": current_codec, "resolution": actual_resolution}
...
def setup_camera(device: VideoDevice, resolution: ImageResolution) -> tuple[cv.VideoCapture, ImageResolution]:
    ...
    results = configure_stream(cap, resolution, codec="MJPG")
