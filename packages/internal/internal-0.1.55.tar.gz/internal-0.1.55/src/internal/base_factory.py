...
class BaseFactory(metaclass=ABCMeta):
    ...
    async def process_exception(self, app, request: Request, exc):
        if issubclass(type(exc), InternalBaseException):
            detail = exc.detail
            await async_response(data=detail.get("data"), code=detail.get("code"), message=detail.get("message"),
                                 status_code=exc.status_code)
        elif issubclass(type(exc), RequestValidationError):
