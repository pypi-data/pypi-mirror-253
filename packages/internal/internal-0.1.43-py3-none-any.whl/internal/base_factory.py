...
class BaseFactory(metaclass=ABCMeta):
    ...
    async def process_exception(self, app, request, exc):
        if isinstance(exc, InternalBaseException):
            ...
            print("GGGGGG")
            return await async_response(data=exc, code="error_unprocessable_entity", message="Validation failed",
                                        status_code=exc.status_code)
        else:
            print("CCCCC")
            app.state.logger.warn(f"Exception, request:{request.__dict__}, exc:{exc}")
            return await async_response(data=exc, code="error_internal_server", message="Internal server error",
