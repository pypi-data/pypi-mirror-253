...
class BaseFactory(metaclass=ABCMeta):
    ...
    def create_app(self, title=DEFAULT_APP_NAME) -> FastAPI:
        ...
        async def http_exception_handler(request: Request, exc: InternalBaseException):
            detail = exc.detail
            await async_response(data=detail.data, code=detail.code, message=detail.message,
                                 status_code=exc.status_code)

        @app.exception_handler(RequestValidationError)
        async def validation_exception_handler(request, exc):
            detail = exc.detail
            await async_response(data=detail, code="error_unprocessable_entity", message="Validation failed",
                                 status_code=exc.status_code)
