...
class LinkAIClient:
    def __init__(self, api_key: str, host: str = "", client_type: str = "", heartbeat_interval=30, reconnect_times=3, reconnect_interval=10):
    ...
    def _connect(self):
        if not self.need_connect:
            return
        attempts = 0
        while attempts < self.reconnect_times and not self.connected:
            try:
    ...
    def _send_heartbeat(self):
        while True:
            ...
            if not self.connected:
                ...
                self._connect()
            if self.connect_failed_times > 240:
                break
    ...
    def _receive_messages(self):
        while True:
            ...
            if not self.connected:
                self._connect()
                logger.debug("Receive handler try to reconnect...")
