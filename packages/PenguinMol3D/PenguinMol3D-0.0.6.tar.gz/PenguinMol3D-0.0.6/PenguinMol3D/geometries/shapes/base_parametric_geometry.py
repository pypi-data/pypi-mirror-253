...
from PenguinMol3D.geometries.shapes.base_shape_geometry import BaseShapeGeometry


class BaseParametricGeometry(BaseShapeGeometry):
    def __init__(self,
                 u_start: int|float,
                 u_end: int|float,
        ...
        BaseShapeGeometry.__init__(self)
        ...
        for x_idx in range(u_resolution):
            for y_idx in range(v_resolution):
                ...
                """uv0 = uvs[x_idx + 0][y_idx + 0]
                uv1 = uvs[x_idx + 1][y_idx + 0]
                uv2 = uvs[x_idx + 0][y_idx + 1]
                uv3 = uvs[x_idx + 1][y_idx + 1]

                uv_data += [uv0, uv1, uv3, uv0, uv3, uv2]"""
        ...
        """self.add_attribute(vertex_positions, "vec2", "uv_data")"""
