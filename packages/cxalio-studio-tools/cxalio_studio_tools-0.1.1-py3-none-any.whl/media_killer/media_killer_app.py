...
class MediaKillerApp(AbstractApp):
    def __init__(self):
        ...
        parser.add_argument('profile', help='指定配置文件路径')
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is None:
            ...
            result = True
        elif issubclass(exc_type, ArgumentTypeError):
    ...
    def run(self):
        ...
        env.info(f'整理结束，已添加 {len(planner)} 项计划')

        env.print('已添加 {0} 项计划'.format(len(planner)))

        env.debug('\n'.join([str(x) for x in planner]))
