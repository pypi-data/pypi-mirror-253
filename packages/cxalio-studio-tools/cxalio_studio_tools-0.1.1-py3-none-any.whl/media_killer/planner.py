...
class Planner:
    ...
    def load_source(self, filename):
        ...
        env.debug('文件{0}的扩展名为 {1}'.format(source, suffix))
        if suffix in adapters:
            env.debug('已识别 {0} 为列表文件'.format(source))
            try:
            ...
            self._loaded_file.add(source)
            env.debug('文件 {0} 无法识别为列表文件，将直接添加进入计划'.format(source.absolute()))
    ...
    def arrange(self, profile=None):
        ...
        for k, v in env.progress.track(groupby(new_files), task_id=task):
            kk = Path(k)
            if not kk.exists():
                env.warning('文件 {0} 不存在，将从任务列表中去除'.format(kk))
                continue
            arranged_files.append(kk)
            env.debug('重新添加 {0}'.format(kk))
            count = len(list(v))
            if count > 1:
                env.debug('发现 {0} 个重复的 {1} ，已经去除'.format(count, k))
        env.progress.remove_task(task)
