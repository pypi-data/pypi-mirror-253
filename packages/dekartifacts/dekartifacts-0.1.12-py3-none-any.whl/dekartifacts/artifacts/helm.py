...
from dektools.url import get_url_auth
...
class HelmGitArtifact(ArtifactHelm):
    ...
    def auth_from_local(self, registry):
        if os.path.exists(self.path_auth):
            ...
            shell_wrapper(f'git config user.name "{username}"')
            shell_wrapper(f'git config user.email "{email}"')
            shell_wrapper(f'git config user.password "{password}"')
    ...
    def clone_repo(self, url):
        ...
        if auth:
            _, username, password = auth
            repo = get_url_auth(repo, username, password)
