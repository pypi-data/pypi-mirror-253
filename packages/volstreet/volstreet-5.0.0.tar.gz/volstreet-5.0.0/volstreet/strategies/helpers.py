...
def sleep_until_next_action(interval_minutes: int, exit_time: tuple[int, int]) -> None:
    time_now = current_time()

    if isinstance(interval_minutes, int):
        next_timestamp = time_now.replace(
            minute=(time_now.minute + interval_minutes) % 60
    ...
    seconds_left = (next_timestamp - time_now).total_seconds()
    sleep_time = max(seconds_left, 0)
    sleep(sleep_time)
