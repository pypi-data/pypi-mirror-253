...
class DeltaBackTest(IntradayBackTest):
    ...
    def calculate_strikes_and_ratios(
        ...
        def process_option_type(options, delta, option_type):
            ...
            if best_option is not None:
                return best_option[f"{option_type}_delta"], [
                    *zip([best_option["strike"], 1] * 2)
                ]  # Check if this works
            ...
            trade_options = [*zip(*[*filter(lambda x: x[1] != 0, trade_options)])]
    ...
    @staticmethod
    def streamline_positions(position_data_frame: pd.DataFrame):
        position_data_frame = position_data_frame.copy()

        # Splitting into call and put dataframes
        call_df = position_data_frame.filter(regex="call|timestamp")
        put_df = position_data_frame.filter(regex="put|timestamp")

        call_df = call_df.drop_duplicates(subset=["timestamp", "call_strike"])
        put_df = put_df.drop_duplicates(subset=["timestamp", "put_strike"])

        # Combining the two dataframes
        call_df = call_df.set_index("timestamp")
        put_df = put_df.set_index("timestamp")

        merged_df = pd.concat([call_df, put_df], axis=1)
    ...
    def prepare_positions(
        ...
        position_df = position_df.explode(["call_strike", "call_ratio"])
        position_df = position_df.explode(["put_strike", "put_ratio"])
        ...
        streamlined_positions = self.streamline_positions(position_with_greeks)
        logger.info(f"{self.underlying.name} delta backtest: streamlined positions")
        return streamlined_positions
    ...
    def calculate_hedge_delta(
        ...
        for option_type in hedges.keys():
            logger.debug(f"Calculating hedge delta for {option_type}")
            prefix = "call" if option_type == OptionType.CALL else "put"
            for strike in hedges[option_type].keys():
                logger.debug(f"Calculating hedge delta for {strike}")
                price = (
    ...
    def process_segment(
        ...
        logger.debug(f"total_premium_received: {total_premium_received}")
        ...
        for i, row in prepared_segment.iterrows():
            ...
            mtm = np.nan
            logger.debug(f"Processing {i}, segment length {len(prepared_segment)}")
            ...
            if i.time() >= time(*exit_time):
                ...
                logger.debug(f"Breaking at {i} because exit time reached")
                break
            ...
            logger.debug(f"main_net_delta: {main_net_delta}")
            hedge_delta = self.calculate_hedge_delta(i, row, hedges)
            logger.debug(f"hedge_delta: {hedge_delta}")
