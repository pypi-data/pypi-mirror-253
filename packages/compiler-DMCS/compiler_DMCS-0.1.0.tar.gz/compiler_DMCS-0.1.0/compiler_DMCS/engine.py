 ...
 try: 
  ...
  for i in strings :
    if len(i) > 0:
     ...
     c += 1
  q = str(s)     
  s = s + "\n" + "\n" + """Is that a receipt,Invoice,bill  or not ?
  if yes reply 1
  if not reply 0"""
 except Exception as er :
  print(f"Error : {er}")   
...
def total(strings):
 try: 
  x = []
  y = None
  e = r'\d+\.\d{3}\b|\d+\,\d{3}\b|\d+\ \d{3}\b|\d+\.*?000$|\d+\.*?000d$|\d+\*?000$'
  df = pd.read_csv('/work/key/key.csv')
  pa = ""
  for j in list(df["filter_total"]):
      pa += f"""{j}(.*)|"""
...
def endpoint(img:str) -> str:
 try: 
  ...
  ocr = PaddleOCR(lang="en",use_gpu = False) # Specify language (optional)
  results = ocr.ocr(img)
  delete_img(t)
  response = {
          "header":header(relocate(results),mapping.header_map(mapping.nrelocate(results))[1]),
          "header_mapping":mapping.header_map(mapping.nrelocate(results))[0],
          "data_items":item_api_ouput(extract_item(relocate(results))),
          "Total":total(relocate(results)),
          "date":str(date(results)),
          "location":str(address(results)),
          "extra":extra(relocate(results)),
          "bill_checking":odetect(rtext(results)[0]),
          "len_text":len(rtext(results)[2]),
          "raw":rtext(results)[1]
          }
  for i in response:
