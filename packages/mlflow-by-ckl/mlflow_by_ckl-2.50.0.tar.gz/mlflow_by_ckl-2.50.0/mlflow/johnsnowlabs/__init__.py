...
def infer_jsl_signature(spark_model):
    ...
    signature = None
    if hasattr(spark_model,'has_span_classifiers') and spark_model.has_span_classifiers:
        signature = infer_signature("Init|||this", spark_model.predict("Init|||this"))
    elif hasattr(spark_model,'has_table_qa_models') and spark_model.has_table_qa_models:
        signature = infer_signature("Init|||this", spark_model.predict("Init|||this"))
    else :
        signature = infer_signature("Init|||this", spark_model.predict("Init|||this"))
    return signature
