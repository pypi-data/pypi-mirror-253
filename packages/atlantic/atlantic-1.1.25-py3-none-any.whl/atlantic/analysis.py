...
class Analysis:
    ...
    def target_type(self, X: pd.DataFrame):
        ...
        if target_dtype not in ['int','int32', 'int64','float','float32', 'float64']:
    ...
    def num_cols(self, X: pd.DataFrame):
        ...
        return [col for col in X.select_dtypes(include=['int','int32', 'int64','float','float32', 'float64']).columns if col != self.target]
