...
class ModelPhotonCollectionOpticalPsfArguments(Mapping[str, Any]):
    ...
    apply_jitter: bool = field(
        ...
        metadata=schema(
            title='apply_jitter'
            ,description='Defines whether jitter should be applied. Default = False.'
        )
    )
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('wavelength', 'fov_arcsec', 'optical_system', 'apply_jitter', 'jitter_sigma'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
#
# Model: Photon Collection / Optical Psf Multi Wavelength
#
@schema(title='Parameters')
@dataclass
class ModelPhotonCollectionOpticalPsfMultiWavelengthArguments(Mapping[str, Any]):
    wavelengths: tuple[float, float] = field(
        metadata=schema(
            title='wavelengths'
            ,description='Wavelengths of incoming light in meters.'
        )
    )
    fov_arcsec: float = field(
        metadata=schema(
    ...
    def __iter__(self) -> Iterator[str]:
        return iter(('wavelengths', 'fov_arcsec', 'optical_system', 'apply_jitter', 'jitter_sigma'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
@schema(
    title="Model 'optical_psf_multi_wavelength'"
    ,description='Model function for poppy optics model: convolve photon array with psf.'
)
@dataclass
class ModelPhotonCollectionOpticalPsfMultiWavelength:
    name: str
    arguments: ModelPhotonCollectionOpticalPsfMultiWavelengthArguments
    func: Literal['pyxel.models.photon_collection.optical_psf_multi_wavelength'] = 'pyxel.models.photon_collection.optical_psf_multi_wavelength'
    enabled: bool = True
...
#
# Model: Photon Collection / Simple Aperture
#
@schema(title='Parameters')
@dataclass
class ModelPhotonCollectionSimpleApertureArguments(Mapping[str, Any]):
    ...
    wavelength_band: tuple[float, float] = field(
        metadata=schema(
            title='wavelength_band'
            ,description='Wavelength band. Unit: nm.'
        )
    )
    def __iter__(self) -> Iterator[str]:
        return iter(('aperture', 'wavelength_band'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
@schema(
    title="Model 'simple_aperture'"
    ,description='Convert scene(photon/s/cm2) to photon.'
)
@dataclass
class ModelPhotonCollectionSimpleAperture:
    name: str
    arguments: ModelPhotonCollectionSimpleApertureArguments
    func: Literal['pyxel.models.photon_collection.simple_aperture'] = 'pyxel.models.photon_collection.simple_aperture'
    enabled: bool = True
...
class ModelPhotonCollectionSimpleCollectionArguments(Mapping[str, Any]):
    ...
    filter_band: tuple[float, float] = field(
        metadata=schema(
            title='filter_band'
            ,description='Wavelength range of selected filter band. Unit: nm.'
        )
    )
    integrate_wavelength: bool = field(
        ...
        metadata=schema(
            ...
            ,description='If true, integrates along the wavelength else multiwavelength.'
        )
    )
    def __iter__(self) -> Iterator[str]:
        return iter(('aperture', 'filter_band', 'integrate_wavelength'))
    def __getitem__(self, item: Any) -> Any:
        if item in tuple(self):
            return getattr(self, item)
        else:
...
@schema(
    ...
    ,description='Convert scene in ph/(cm2 nm s) to 3D photon in ph/nm s.'
)
@dataclass
...
class DetailedDetectionPipeline(DetectionPipeline):
    ...
    photon_collection: Optional[
        Sequence[Union[ModelPhotonCollectionIllumination, ModelPhotonCollectionLoadImage, ModelPhotonCollectionLoadPsf, ModelPhotonCollectionLoadWavelengthPsf, ModelPhotonCollectionOpticalPsf, ModelPhotonCollectionOpticalPsfMultiWavelength, ModelPhotonCollectionShotNoise, ModelPhotonCollectionSimpleAperture, ModelPhotonCollectionSimpleCollection, ModelPhotonCollectionStripePattern, ModelPhotonCollectionWavelengthDependenceAirs, ModelLoadDetector, ModelSaveDetector, ModelFunction]]
