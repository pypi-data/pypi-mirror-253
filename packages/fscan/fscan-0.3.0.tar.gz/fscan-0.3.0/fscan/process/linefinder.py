...
def running(x, window):
    ...
    x: 1d np.ndarray, dtype = float
        The input array for which to calculating running statistics
...
def peaks(val, far, window):
    ''' This identifies peaks in an array of data.
    It is intended specifically to identify narrow
    lines in high-resolution spectral data.
    ...
    _, _, med0 = running(val0, int(window))
