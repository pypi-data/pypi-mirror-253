...
def tokenize(input_string):
    ...
    out_lines = []
    url_hash = {}
    acrfile = os.path.dirname(__file__) + "\\tam.acr"
    ...
    if(not string_exists):
        ...
        acr_file_exists = os.path.exists(acrfile)
        if(acr_file_exists):
        ...
        for l in inlines:
            ...
            urls = re.findall(r'(?:(?:https?:\/\/|ftp:\/\/|:file:\/\/|www)+[a-zA-Z0-9.\-]+\.[a-zA-Z]+\/?[a-zA-Z0-9_.\-]*)', l)
            #print(urls)
            for u in urls:
    ...
    for key in all_hash:
        ...
        for o in out_lines:
            ...
            log.logging.debug("Before substitution with actual values, current key=|%s|, value=|%s|, line=|%s|" %(key, inv_hash[key], o))
            o = re.sub(r'' + key , r'' + inv_hash[key], o)
            log.logging.debug("After substitution with actual values, current key=|%s|, value=|%s|, line=|%s|" %(key, inv_hash[key], o))
