...
import aiohttp
from _markets.finished.db_manager import DbManager
...
db = PolygonDatabase(host='localhost', user='chuck', database='markets', password='fud',port=5432)
...
async def conditional_embed(contract: dict, hook_url, title, description):
    hook = AsyncDiscordWebhook(hook_url)
    volume = contract.get('volume')
    underlying_symbol = contract.get('underlying_symbol')
    sym = contract.get('option_symbol')
...
async def poly_data_embed(title, description, color, hook, data_dict: OptionData):


    webhook = AsyncDiscordWebhook(hook)
    embed = DiscordEmbed(title=f"{title}", description=f"```py\n{description}```", color=color)
...
async def sector_dark_pool_embed(logo, ticker, webhook_url, sector, dollar_cost, conditions, exchange, size, price, volume_indicator, trade_timestamp):

    hook = AsyncDiscordWebhook(webhook_url, content=f"<@375862240601047070>")
    embed = DiscordEmbed(title=f"SECTOR DARK POOLS - {sector} | {ticker}", description=f"```py\n> Size: **{size}**\n> Price: **${price}**\n> Dollar Cost: **${round(float(dollar_cost),2)}**```\n> **These feeds are SECTOR DARK POOLS - Or Trades occuring out of the FINRA ADF/TRF Facility. Trades with dollar value of $1,000,000 or more will post here out of these exchanges.", color=market.colors.get('gold'))
