...
def post_contact_avatar(self, contact_id: int, image_url: str) -> dict:
    ...
    """
    filename = get_file_url(image_url)  # Ensure this extracts the filename correctly
    files = [("file1", (filename, requests.get(image_url).content, "image/jpeg"))]

    # Temporarily remove 'Content-Type' from headers
    ...
    # Revert headers back to original
