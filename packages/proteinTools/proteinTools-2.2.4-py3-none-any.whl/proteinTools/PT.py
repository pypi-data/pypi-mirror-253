...
class ligand:
    ...
    def download(self, directory = os.getcwd(), convert_pdb = False, file_path = None):
        ...
        if file_path != None:
            if '.sdf' in file_path:
                os.system('obabel -isdf "' + file_path + '" -O "' + file_path[:-4] + '.pdb" --gen3d')
...
class Protein:
    ...
    def download(self, destination = os.getcwd()):
        ...
        if self.ID_type == 'PDB':
            ...
            try:
                out = mg.querymany(self.protein, fields = 'uniprot', species = self.species)
            except:
                out = mg.querymany(self.protein, fields = 'uniprot', species = 'all')
            try:
                url = 'https://alphafold.ebi.ac.uk/files/'
        ...
        if self.cif == False:
            ...
            with open(self.protein + '.pdb', 'r') as f:
                data = f.readlines()
            self.residue_list, curr_residue, currnum = [], -.5, 0
            if self.ID_type == 'PDB':
            ...
            if self.ID_type == 'PDB':
                ...
                for count, line in enumerate(data):
                    if line[0:6] == 'HETATM':
                        ...
                        if lignum != currnum or abs(atom_number - int(line[6:12])) > 100:
                            currnum = lignum
                            lig = ligand(ligands)
    ...
    def strip_primary(self, destination = os.getcwd()):
        """
        Strips primary ligand from pdb file linedata
        
        This method generates a .pdb file without the primary ligand molecules in the specified directory.
        ...
        """
        data = []
        primary_ligand = self.ligands['Primary Ligand'].iloc[1][0]
        for line in self.data:
            if line[0:6] != 'HETATM':
                data.append(line)
            else:
                if primary_ligand.ID not in line:
                    print(line)
                    data.append(line)
    ...
    def PDB(self):
        ...
        if self.ID_type == 'PDB': 
            ...
            try:
                try:
                    out = mg.querymany(self.protein, fields = 'pdb', species = self.species)
                except:
                    out = mg.querymany(self.protein, fields = 'pdb', species = 'all')
                pdb = out[0]['pdb']
    ...
    def Gene(self):
        ...
        if self.ID_type == 'Gene':
            ...
            try:
                out = mg.querymany(self.protein, fields = 'symbol', species = self.species)
            except:
                out = mg.querymany(self.protein, fields = 'symbol', species = 'all')
    ...
    def ligands(self):
        ...
        for ligand in self.ligand_list:
            if ligand.ID not in ligcache:
                ...
                if len(ligand.ID) < 3 or len(ligand.atoms) < 2:
                    ions.append(ligand.ID)
                elif ligand.ID[2] == '4' and len(ligand.atoms) < 7:
                     ions.append(ligand.ID)
                elif ligand.ID != 'HEM' and len(ligand.atoms) > 4:
                    ligs.append(ligand.ID)
                else:
                    supp.append(ligand.ID)
