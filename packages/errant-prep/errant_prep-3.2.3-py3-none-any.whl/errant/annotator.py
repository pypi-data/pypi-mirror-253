...
class Annotator:
    ...
    def annotate_raw_text(self, original_text: str, corrected_text: str, lev=False, merging="rules"):
        ...
        doc_lower_orig, normalized_orig_text = self.preprocess(original_text, lowercase=True)

        doc_corr, _ = self.preprocess(corrected_text)

        _, normalized_corr_text = self.preprocess(corrected_text, lowercase=True)
    ...
    def _revise_edits(
        ...
        Returns:
            revised edits
        """
        def count_double_space(text: str) -> int:
            """
            Count double space in text from end_indice to the left.
            Args:
                text: text to count
            Returns:
                number of double space
            """
            if not text:
                return 0
            import re
            ...
            text = re.sub("\t", " ", text)
            text = text.split(" ")
            count = sum([1 for i in text if not i])
            return count
        ...
        for edit in edits:
            ...
            count_ds = count_double_space(orig[: edit.o_toks.start_char].strip())
            edit.o_toks.start_char = edit.o_toks.start_char + count_ds
            edit.o_toks.end_char = edit.o_toks.end_char + count_ds
        return edits
