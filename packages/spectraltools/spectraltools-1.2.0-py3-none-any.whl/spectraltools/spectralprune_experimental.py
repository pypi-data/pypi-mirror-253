...
def eigenvalue_cutoff_experimental(model: Model, perc):
    """
    ...
    for ind, lay in enumerate(layers):
        inbound, outbound = near_layer_experimental(lay)
...
def compare_eigenvalues_experimental(previous_layer: Spectral,
                        next_layer: Spectral,
                        cut_off: float,
                        mode: str):
...
def near_layer_experimental(current_layer: layer_type):
...
def dense_weights_distiller_experimental(to_prune: Dense, cut_off: float):
    inbound, outbound = near_layer_experimental(current_layer=to_prune)
...
def spectral_weights_distiller_experimental(to_prune: Spectral, cut_off: float, link_mode: str = 'and'):
    inbound, outbound = near_layer_experimental(current_layer=to_prune)
    ...
    if type(inbound).__name__ == "Spectral":
        in_cond = compare_eigenvalues_experimental(previous_layer=inbound,
                                      next_layer=to_prune,
                                      cut_off=cut_off,
                                      mode=link_mode)

    if type(outbound).__name__ == "Spectral":
        out_cond = compare_eigenvalues_experimental(previous_layer=to_prune,
                                       next_layer=outbound,
                                       cut_off=cut_off,
                                       mode=link_mode)
...
def spectral_pruning_experimental(model: Model, percentile: int):
    cut_off, index_list = eigenvalue_cutoff_experimental(model, percentile)
    new_weights = []
    layers = layers_unwrap(model)
    ...
    for lay in index_list:
        if lay["layer"] == 'Spectral':
            new_assigner = spectral_weights_distiller_experimental(layers[lay['index']], cut_off)
        elif lay["layer"] == 'Dense':
            new_assigner = dense_weights_distiller_experimental(layers[lay['index']], cut_off)
        else:
            continue
...
def spectral_pretrain_experimental(model,
                      fit_dictionary,
                      eval_dictionary,
                      max_delta,
                      compare_with='acc'):
    ...
    while abs(new_indic - indicator) / indicator < (np.clip(max_delta, 1, 99) / 100):
        new_model = spectral_pruning_experimental(spec_only, percentile=p)
...
def spectral_distiller_experimental(model,
                       fit_dictionary,
                       percentile):
    """
    ...
    spec_only.compile(**model._get_compile_args())
    spec_only.fit(**fit_dictionary)
    pruned_model = spectral_pruning_experimental(spec_only, percentile=percentile)
