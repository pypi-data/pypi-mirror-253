...
async def item_to_static_sticker(bot, chat_id, input_file, PACK_TYPE, PACK_KIND, is_circle=False, is_rbg=False):
...
async def item_to_dynamic_sticker(bot, chat_id, input_file, MEDIA_D, PACK_TYPE, PACK_KIND, is_circle=False, is_rbg=False):
    result = result_upl = None
    file_jpg = input_file[:input_file.rfind('.')] + '_.jpg'
    file_png = input_file[:input_file.rfind('.')] + '_.png'
    ...
    try:
        ...
        if input_file.lower().endswith(('.jpg', '.jpeg', '.png', '.webp')):
            ...
            frames_to_save = int(fps * 3) - 10
            # total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
            # success, image = cap.read()
            i = 0
            while i < frames_to_save and cap.isOpened():
                try:
                    ret, image = cap.read()
                    if not ret: break
                    it_png = f"{spec_dir}/frame_{i:04d}.png"
                    cv2.imwrite(it_png, image, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                    i += 1
                    ...
                    #     if os.path.getsize(file_jpg) < size_kb: break
                    #     quality = 1 if quality == 0 else quality - 10
                    # if os.path.getsize(file_jpg) > size_kb: continue
                    # # endregion
        ...
        # await bot.send_sticker(chat_id, result)
