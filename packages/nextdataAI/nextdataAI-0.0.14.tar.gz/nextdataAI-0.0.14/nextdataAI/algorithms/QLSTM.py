...
class QLSTM(Algorithm):
    ...
    def __call__(self, seed: int):
        ...
        local_state_copy = copy.deepcopy(local_state)
        ...
        target_reached, explored_positions = train(self.agent, local_env, local_game_map, start, target,
                                                   self.batch_size, self.past_states_seq_len, max_epoch=self.max_epoch, seed=seed)
        ...
        if self.animate:
            Animator(size=self.size, game_map=local_state_copy, path=explored_positions, visited=explored_positions, file_path=f'{self.name}.gif', disable_line=True)()
