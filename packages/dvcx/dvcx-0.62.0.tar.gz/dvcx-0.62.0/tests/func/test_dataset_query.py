...
@pytest.mark.parametrize("batch", [False, True])
def test_udf_distributed(cloud_test_catalog_tmpfile, batch):
    ...
    q = (
        ...
        .filter(C.size < 13)
        .filter(C.parent.glob("cats*") | (C.size < 4))
        .add_signals(udf_func, parallel=2, workers=2)
        .select(C.name, C.name_len)
    )
    result = q.results()
...
def test_udf_distributed_exec_error(cloud_test_catalog_tmpfile):
    if not os.environ.get("DVCX_DISTRIBUTED"):
        pytest.skip(
            "Set the DVCX_DISTRIBUTED environment variable to test distributed UDFs"
        )
    ...
    q = (
        ...
        .filter(C.size < 13)
        .filter(C.parent.glob("cats*") | (C.size < 4))
        .add_signals(name_len_error, parallel=2, workers=2)
    )
