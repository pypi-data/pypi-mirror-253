...
for probe_name in all_probes:
    ...
    if probe_name == "SexualContent":
        detector_names = [probe_instance.primary_detector]
    else:
        detector_names = probe_instance.recommended_detector
    probe_detectors = []
    ...
    setattr(
        ...
        type(
            ...
            (Test,),
            {
                ...
                "tags": ['vijil:Ethics', 'source:garak'] + probe_instance.tags,
                "goal": probe_instance.goal,
            },
        ),
    )

# manually change one detector
