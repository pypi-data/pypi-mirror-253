...
def main(arguments=[]) -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Vijil Auto Red Team Suite",
    ...
    # model is specified, we're doing something
        ...
        if generator_module_name in ['replicate','octo','anyscale']:
            generator_mod = importlib.import_module("autoredteam.agents." + generator_module_name)
        else:
            generator_mod = importlib.import_module("garak.generators." + generator_module_name)
        # generator_mod = importlib.import_module(
        #     "autoredteam.agents." + generator_module_name
        # )
        ...
        import autoredteam.command
        if detector_names == []:
            autoredteam.command.probewise_run(generator, probe_names, evaluator, buffs)
        else:
