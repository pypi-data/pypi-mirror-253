...
class Test:
    ...
    def generate(self, agent: Generator, logged: bool = False):
        """
    ...
    def evaluate(self, logged: bool = False):
        ...
        for d in self.detectors:
            ...
            attempt_iterator = tqdm(self.attempt_results, leave=False)
    ...
    def summarize(self, threshold: float = 0.5, logged: bool = False):
        ...
        for detectorname, detector_results in self.eval_results.items():
            ...
            eval_summary[detectorname]["pass_rate"] = (
                round(eval_summary[detectorname]["passed"] / eval_summary[detectorname]["total"]*100, 1)
            )
        ...
        for k, v in self.eval_summary.items():
            ...
            print(
                f"Test {self.name:<50} Detector {k:>50}: {v['passed']:>4}/{v['total']:>4} ({v['pass_rate']:>6}%) passed"
            )
                                ...
                                json.dumps(
                                    {
                                        ...
                                        "test": self.name,
                                        "detector": detname,
                                        "generations_per_prompt": len(attempt.detector_results[detname]),
                                    }
                                )
                                + "\n"
                            )
    ...
    def run(self, agent: Generator, detection_threshold: float = 0.5, logged: bool = False):
        """
        Runs the test
    ...
    def export(self, path=None, subset=None, threshold=0.5, mode="w"):
        """
    ...
    def evaluate(self, logged: bool = False):
        ...
        for d in self.detectors:
            ...
            for attempt1 in first_half:
                attempt1.detector_results[detname] = d.detect(attempt1, self.attempt_results[attempt1.seq + self.step])
