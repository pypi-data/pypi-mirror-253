...
def create_child_resource_group(
    ...
    @click.option(
        ...
        is_flag=True,
        default=False,
        help="Display columns by category",
    )
    @click.pass_obj
    def list(
        obj: AppContext,
        ...
        **kwargs,
        ...
        resources = endpoint.list(
            obj.resources.last(), from_time=start, to_time=end, categories=categories
        )
...
def slot_info_for_time(slot, t, with_start_slots):
    ...
    desc = (
        click.style(
            color_title(
                trim_or_pad(" " + slot.replacement.name + " ", size=width, pad=True)
            )
            if slot.replacement
            else color_title(trim_or_pad(" Ad Break ", size=width, pad=True)),
        )
        + "\n"
        + color_line("│")
        + trim_or_pad(
