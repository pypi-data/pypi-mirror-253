...
def populate_virtual_channel_slots_with_prompts(context: AppContext, vc_id, categories):
    ...
    gaps = _select_gaps(slot_sources, adbreaks_allowed=supports_ads)
    plan = []
    ...
    if repeat != 1:
        gaps2 = _select_gaps(slot_sources, adbreaks_allowed=supports_ads, previous=gaps)
        plan.extend(gaps2)
    ...
    _clear_conflicting_slots(api, final_schedule, vc_id, category_id)

    # Now create the slots
    slots = _create_slots_via_api(api, vc_id, category_id, final_schedule)
...
def _select_gaps(slot_sources, adbreaks_allowed=False, previous=None):
    inters = []
    ...
    while True:
        ...
        msg = (
            "Do you want to insert a regular break between those slots?"
            if len(inters) == 0
            else "Do you want to insert something else in the break?"
        )
        ...
        if previous:
            ...
            msg = "Do you want to insert breaks between repetitions?"
...
def _get_duration_from_asset(url):
    handler: ContentHandler = factory.create_handler(
        url, user_agent=ConfigProvider().get_user_agent()
    )
