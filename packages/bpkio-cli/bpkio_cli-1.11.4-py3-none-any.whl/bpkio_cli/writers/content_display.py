...
import click
from bpkio_cli.commands.live_monitor import LiveMonitor, detect_hls_signals, sound_alert
            ...
            if not displayed:
                ...
                match handler:
                    ...
                    case handlers.HLSHandler():
                        ...
                        content = formatter.format(
                            "raw" if raw else "standard", top=top, tail=tail, trim=trim
                        )
                        if handler.is_live():
                            changes = detect_hls_signals(
                                handler, stamp, monitor=monitor
                            )
