...
def monitor(
    ...
    **kwargs,
    ...
    if isinstance(handler, HLSHandler) and handler.has_children():
        ...
        monitor_hls(handler, max, interval, silent, with_schedule=table)
...
def play(sub: int, player: str):
    ...
    if player == "CONFIG":
        player = ConfigProvider().get("default-player")
...
@click.pass_obj
def profile(obj: AppContext, with_table: bool, save: bool):
    handler = get_handler()
...
url.add_section(cloup.Section("Other Commands", [store]))
