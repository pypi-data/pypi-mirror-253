...
from menrva.plugins.worker import DaskWorker
...
class MenrvaClient(distributed.Client):
    ...
    @staticmethod
    def instantiate_module(plugin: str, plugin_file: str, *args: Tuple[Any], **kwargs: Dict[str, Any])->Callable:
        """
        ...
        Args:
            ...
            *args (tuple(Any)): This is any *arg that needs to be passed to the pluging module.
            **kwargs (dict[str, Any]): This is any **kwarg default values that need to be passed to the pluging module.
    ...
    def load_plugin(
        ...
        if pathlib.Path(directory).joinpath(plugin_file).exists():
            plugin = MenrvaClient.instantiate_module(
                plugin=plugin,
            ...
            if sys.version_info.major == 3:
                if sys.version_info.minor > 8:
                    ...
                    self.register_worker_plugin(plugin, name=name)
            else:
...
@auror.parameter.validate(
    logger=logger.get_logger(logger_name="menrva"),
