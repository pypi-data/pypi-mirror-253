...
class NdBoundsSW(BoundFormatHandler):
    """ Handler containing: start, width """
    ...
    def _require_se(self):
        ...
        return self._pack_sew(start=self._start, end=end, width=self._width)

    def _require_cw(self):
        center, width = conversion._convert_sw_to_cw(self._start, self._width, inplace=False)
        return self._pack_scw(start=self._start, center=center, width=width)
    ...
    def _require_secw(self):
        ...
        end = np.add(self._start, self._width)
        return self._pack_secw(start=self._start, end=end, center=center, width=self._width)
    ...
    def _copy(self):
        return self._pack_sw(start=np.copy(self._start), width=np.copy(self._width))
