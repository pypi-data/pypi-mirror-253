...
def fuzzspecs(specdir=inspecdir):
    ...
    for spec in sfs:
        ...
        with open(specfname, encoding='utf8') as f:
            if specfname.endswith(".json"):
                indata = json.load(f)
                # indata1 = urllib.parse.unquote(indata)
                # indata = json.load(indata1.replace("'", '"'))
            elif specfname.endswith(".yaml") or specfname.endswith(".yml"):
        ...
        ImpactCore("", specfname1, "", runconfig.dfuzzusrname, "", runconfig.dcvconfig, "n", "", True)
