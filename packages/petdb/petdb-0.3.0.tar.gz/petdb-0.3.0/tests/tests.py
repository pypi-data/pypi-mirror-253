...
def updating():
	...
	@test(init=[{"a": 5}], expect=QueryException("Invalid set query: only lists supports integer keys"))
	def append_queryexception1(col: PetCollection):
		col.update({"$append": {1: "item"}})

	@test(init=[[0, [1, 2, 3]], [0, [4, 5, 6]]], expect=QueryException("Invalid set query: index out of range"))
	def append_queryexception2(col: PetCollection):
		col.update({"$append": {2: 222}})

	@test(init=[[0, {}], [0, {}]], expect=QueryException("Invalid set query: it's impossible to append not to a list"))
	def append_queryexception3(col: PetCollection):
		col.update({"$append": {1: 222}})

	@test(init=[[0, "123"], [0, "123"]],
		expect=QueryException("Invalid set query: it's impossible to append not to a list"))
	def append_queryexception4(col: PetCollection):
		col.update({"$append": {1: 222}})

	@test(init=[{"a": {}}], expect=QueryException("Invalid set query: path a.b doesn't exist"))
	def append_queryexception5(col: PetCollection):
		col.update({"$append": {"a.b": 5}})

	@test(init=[{}], expect=QueryException("Invalid set query: path a.b doesn't exist"))
	def append_queryexception6(col: PetCollection):
		col.update({"$append": {"a.b": 5}})

	@test(init=[{"a": {"b": {}}}],
		expect=QueryException(f"Invalid set query: it's impossible to append not to a list"))
