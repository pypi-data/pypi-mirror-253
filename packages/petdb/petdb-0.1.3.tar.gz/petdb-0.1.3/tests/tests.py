...
def operations():
	...
	def type2(col: PetCollection):
		return col.findall({"b": {"$type": NoneType}})
...
def sorting():
	...
	def sort5(col: PetCollection):
		return col.map(lambda doc: [doc.get("a", None)]).sort(0).list()
	...
	@test(init=[{"a": 5}, {"a": 4}, {"a": 8}, {"b": 0}, {"a": 10}, {"a": 2}, {"a": 1}],
		expect=[[[0, 1]], [[0, 2]], [[0, 4]], [[0, 5]], [[0, 8]], [[0, 10]], [[0, None]]])
	...
	def sort8(col: PetCollection):
		return col.map(lambda doc: doc.get("a", None)).sort().list()
