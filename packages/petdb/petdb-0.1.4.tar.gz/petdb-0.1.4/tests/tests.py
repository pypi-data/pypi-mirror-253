...
from petdb.putils import NON_EXISTENT, NonExistent
		...
		for i, test in enumerate(cls.tests, 1):
			try:
				...
				if result == test["expect"]:
					...
					message = f"{RED}Test {i} ({test["name"]}){COLOREND}: failed (expected '{test["expect"]}', got '{result}')"
					failed.append(message)
					print(message)
			except Exception as e:
				if isinstance(test["expect"], Exception) and test["expect"].__class__ == e.__class__:
					print(f"Test {i}: passed")
				else:
					message = f"{RED}Exception raised during Test {i} ({test["name"]}):{COLOREND}\n"
...
def selection():
	...
	@test(expect={"a": 1}, init_expectation=False)
	def find_by_id3(col: PetCollection):
		doc_id = col.insert({"a": 1})["_id"]
...
if __name__ == '__main__':
	...
	print()
	if len(failed) == 0:
		print(f"{GREEN}All tests passed{COLOREND}")
	else:
		print(f"{RED}Tests failed:{COLOREND}\n\n{"\n".join(failed)}")
