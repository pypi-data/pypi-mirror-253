...
class MarkdownFormatter:
    ...
    def _valid_link(self, path: str) -> str:
        """
        ...
        if self._is_url(path):
            ...
            if abs_path.is_file():
                # true file
                return self.replace_spaces_in_path(abs_path)[1]
        # no exist
        else:
            ...
            for new_abs_path in self.root_path.rglob("*"):
                if (
                    ...
                    return self.replace_spaces_in_path(new_abs_path)[1]
    ...
    def convert_links(self, content: str) -> str:
        ...
        def replace_markdown(match):
            ...
            valid_link = self._valid_link(link)
...
class MarkdownHandler(MarkdownFormatter):
    ...
    def _create_links(self, path: Path, level: int = 0) -> list:
        ...
        for item in path.iterdir():
            if item.is_dir() and not self.omited_file(item) and "assets" not in item.as_posix():
                links.append(f"{'  ' * level}- **{item.name}/:**")
                links.extend(self._create_links(item, level + 1))
            elif item.is_file() and not self.omited_file(item) and "assets" not in item.as_posix() and level > 0:
