...
class Alg_Type_Enum(Enum):
    ...
    mmdetv3_pano_seg = "mmdetv3_pano_seg"
    mmsegv1_seman_seg = "mmsegv1_seman_seg"
...
class Alg_Model_Enum(Enum):
    ...
    class MmdetV3:
        ...
        class Panseg:
            panoptic_fpn = "mmdet_v3-panseg-panoptic_fpn"

    class MmSegV1:
        class Semseg:
            segformer = "mmseg_v1-semseg-segformer"
            deeplab = "mmseg_v1-semseg-deeplab"
        ...
        def labelme2Citycapes(labelme_json_dir_path, label_suffix, citycapes_dir_path, alg_type, category_name):
            ...
            for i in range(len(all_lableme_list)):
                ...
                citycapes_json_file_path = os.path.join(citycapes_dir_path,
                                                        "{}_polygons.json".format(labelme_file_name))
                print("{}/{} 转换标签：{}".format(i + 1, len(all_lableme_list), labelme_file_path))
                # 生成_polygons.json
                with open(labelme_file_path, 'r', encoding='utf-8') as fp:
                    json_data = json.load(fp)
                    ...
                    for shape_obj in shapes_array:
                        ...
                        if label_name == "oiltank":
                            label_name = "storagetank"
                        points_array = shape_obj.get("points")
                        if alg_type == Alg_Type_Enum.mmsegv1_seman_seg.value:
                ...
                # 将_polygons.json转为_labelTrainIds.png'
                citycapes_lable_file_path = citycapes_json_file_path.replace('_polygons.json', '_labelTrainIds.png')
                ...
                labelImg = FileFormatConverter.CityScapesHelper.createLabelImage(annotation, "trainIds", name2label)
    ...
    class OpenmmLabHelper:
        ...
        def write_config_file_of_mm_train(mmdet_config_file_path, category_name,
            ...
            with open(base_config_py, "r") as f1:
                ...
                elif alg_type == Alg_Type_Enum.mmsegv1_seman_seg.value:
    ...
    class CityScapesHelper:
        ...
        def build_name2label(category_name):
            ...
            if category_name == "building":
                labels = [
                    #       name                     id    trainId   category            catId     hasInstances   ignoreInEval   color
                    Label('backgroud', 0, 255, 'void', 0, False, True, (0, 0, 0)),
                    Label('building', 1, 0, 'building', 1, False, True, (0, 0, 0)),
                ]
                # --------------------------------------------------------------------------------
...
class SampleHandler:
    ...
    def split_train_val_data_of_mm(sample_pic_dir_path, label_json_dir_path, mm_json_dir_path,
                                   ...
                                   category_name,
                                   train_radio, valid_radio, test_radio, alg_type, alg_model):
        ...
        if alg_type == Alg_Type_Enum.mmdetv3_obj_det.value or alg_type == Alg_Type_Enum.mmdetv3_ins_seg.value:
            # 转换labelme为coco
            FileFormatConverter.LablemeHelper.labelme2Coco2017(mm_train_dir, "json", mm_train_dir, alg_type)
            FileFormatConverter.LablemeHelper.labelme2Coco2017(mm_val_dir, "json", mm_val_dir, alg_type)
            FileFormatConverter.LablemeHelper.labelme2Coco2017(mm_test_dir, "json", mm_test_dir, alg_type)
        elif alg_type == Alg_Type_Enum.mmsegv1_seman_seg.value:
            # 图片名称增加后缀_leftImg8bit
            FileHelper.add_file_suffix_and_change_file_extension(mm_train_dir, "_leftImg8bit", "png", "png")
            FileHelper.add_file_suffix_and_change_file_extension(mm_val_dir, "_leftImg8bit", "png", "png")
            FileHelper.add_file_suffix_and_change_file_extension(mm_test_dir, "_leftImg8bit", "png", "png")
            # labelme转为cityscapes
            FileFormatConverter.LablemeHelper.labelme2Citycapes(mm_train_dir, "json", mm_train_dir, alg_type,
                                                                category_name)
            FileFormatConverter.LablemeHelper.labelme2Citycapes(mm_val_dir, "json", mm_val_dir, alg_type, category_name)
            FileFormatConverter.LablemeHelper.labelme2Citycapes(mm_test_dir, "json", mm_test_dir, alg_type,
                                                                category_name)
        # 删除拷贝的labelme json文件
        FileHelper.delete_files_by_condition(mm_train_dir, "json", "annotation_coco.json")
        FileHelper.delete_files_by_condition(mm_val_dir, "json", "annotation_coco.json")
        FileHelper.delete_files_by_condition(mm_test_dir, "json", "annotation_coco.json")
