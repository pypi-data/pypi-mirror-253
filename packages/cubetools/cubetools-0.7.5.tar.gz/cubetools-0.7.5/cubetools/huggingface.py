...
DATA_CACHE_DIR = os.path.join(os.path.expanduser('~'), '.huggingface_datasets_cache')
...
def download_model(model_id, hf_url='https://hf-mirror.com/', threads=8, parallels=1, exclude=None):
    if model_id.find('/') < 1:
        print('model_id格式不对！')
        return None
...
def download_model_cmd():
    ...
    download_model(model_id=args.model_id, hf_url=args.hf_url, threads=args.threads, parallels=args.parallels, exclude=exclude)


def download_dataset(model_id, hf_url='https://hf-mirror.com/', threads=8, parallels=1, exclude=None):
    if model_id.find('/') < 1:
        print('model_id格式不对！')
        return None

    model_dir = os.path.join(DATA_CACHE_DIR, model_id)
    if os.path.exists(model_dir):
        print('数据集 {} 已存在！'.format(model_id))
        return model_dir
    ...
    cwd = os.getcwd()
    model_owner, model_name = model_id.split('/')
    owner_dir = os.path.join(DATA_CACHE_DIR, model_owner)
    if not os.path.exists(owner_dir):
        os.mkdir(owner_dir)
    os.chdir(owner_dir)

    # Clone模型代码（不下载长文件）
    url = hf_url + 'datasets/' + model_id
    os.environ['GIT_LFS_SKIP_SMUDGE'] = '1'
    status, _ = subprocess.getstatusoutput('git clone ' + url)
    if status != 0:
        print('克隆 {} 失败！'.format(url))
    ...
    # 下载所有长文件
    for file in ls_files:
        url = hf_url + 'datasets/' + model_id + '/resolve/main/' + file
        file_dir = subprocess.getoutput('dirname ' + file)
        file_name = subprocess.getoutput('basename ' + file)
    ...
    os.chdir(cwd)
    return os.path.join(DATA_CACHE_DIR, model_id)
...
def download_dataset_cmd():
    ...
    exclude = None if args.exclude is None else args.exclude.split(',')
    download_dataset(model_id=args.model_id, hf_url=args.hf_url, threads=args.threads, parallels=args.parallels, exclude=exclude)


if __name__ == '__main__':
    download_model('vinvino02/glpn-kitti')
    download_dataset('keremberke/plane-detection')
