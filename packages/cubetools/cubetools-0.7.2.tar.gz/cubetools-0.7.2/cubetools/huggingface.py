...
def download_file_parallel(url, file_dir, file_name, parallels=8):
    filepath = os.path.join(file_dir, file_name)
    if os.path.exists(filepath):
        logging.critical(f'File {filepath} already exist!')
        return
...
def download_model(model_id, hf_url='https://hf-mirror.com/', threads=8, exclude=None):
    if model_id.find('/') < 1:
        print('model_id格式不对！')
        return None
    ...
    for file in ls_files:
        ...
        if status != 0:
            os.system('rm ' + file)
            download_file_parallel(url, file_dir, file_name)
...
def download_dataset(model_id, hf_url='https://hf-mirror.com/', threads=8, exclude=None):
    if model_id.find('/') < 1:
        print('model_id格式不对！')
        return None
...
def download_dataset_cmd():
    ...
    exclude = None if args.exclude is None else args.exclude.split(',')
    download_dataset(model_id=args.model_id, hf_url=args.hf_url, threads=args.threads, exclude=exclude)
