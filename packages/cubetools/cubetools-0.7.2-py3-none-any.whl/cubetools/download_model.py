...
def download_file_parallel(url, file_name, parallels=8):
    file_dir = LOCAL_DIR
    filepath = os.path.join(file_dir, file_name)
    if os.path.exists(filepath):
        logging.critical(f'File {filepath} already exist!')
...
def download_model_cmd():
    ...
    download_file(url=args.url, file_name=args.file_name)
...
def download_model_parallel_cmd():
    ...
    download_file_parallel(url=args.url, file_name=args.file_name)
