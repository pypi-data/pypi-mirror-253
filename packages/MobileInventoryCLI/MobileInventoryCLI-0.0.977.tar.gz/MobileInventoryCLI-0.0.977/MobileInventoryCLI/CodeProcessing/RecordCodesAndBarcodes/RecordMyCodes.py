 ...
 {Fore.red}b{Style.reset} -> back
				...
				if len(args) == 2:
					...
					with Session(self.engine) as session:
						...
						setattr(result,"user_updated",True)
						session.commit()
						session.flush()
				...
				with Session(self.engine) as session:
					...
					setattr(item,"user_updated",True)
					session.commit()
					session.flush()
			...
			print("-"*10)
			with Session(self.engine) as session:
					result=session.query(Entry).filter(Entry.InList==True).update({'InList':False,"user_updated":False})
					session.commit()
					session.flush()
					print(result)
			print("-"*10)
									...
									if num < 0:
										...
										if self.listMode:
											while True:
												...
												setattr(results[num],"user_updated",True)
