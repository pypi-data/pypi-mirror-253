...
class Initials:
    ...
    def make_ap_df(self):
        ...
        if self.mydict2.get(x) is not None else None)
        self.AP_df['FS Sub Line name'] = self.AP_df['Corp Account'].apply(lambda x: self.mydict2.get(x).get("FS Sub Line name")
        if self.mydict2.get(x) is not None else None)
    ...
    def make_ar_df(self):
        ...
        if self.mydict2.get(x) is not None else None)
        self.AR_df['FS Sub Line name'] = self.AR_df['Corp Account'].apply(lambda x: self.mydict2.get(x).get("FS Sub Line name")
        if self.mydict2.get(x) is not None else None)
