...
class ParseAccruals:
    ...
    def further_process(self):
        ...
        self.dataframe['Balance_or_PnL_Dr'] = self.dataframe['Corp Account_Dr'].apply(
            lambda x: self.mydict2.get(x).get("Balance or PnL") if self.mydict2.get(x) else x
        )
        ...
        self.dataframe['Balance_or_PnL_Dr'] = np.select(conds, choices, default=self.dataframe['Balance_or_PnL_Dr'])
        self.dataframe['Balance_or_PnL_Cr'] = np.select(conds_2, choices, default=self.dataframe['Balance_or_PnL_Cr'])

        return self.dataframe

    def make_bs_data(self, ag_dr='BS', ag_cr='BS'):
        df = self.further_process().copy()
        df = df[df.apply(lambda row: True if row['Balance_or_PnL_Dr'] == ag_dr or row['Balance_or_PnL_Cr'] == ag_cr else False,
                         axis=1)]
    ...
    def make_pnl_data(self, ag_dr='PL', ag_cr='PL'):
        ...
        conds = [
            (df['Balance_or_PnL_Cr'] == 'PL') & (df['Balance_or_PnL_Dr'] == 'PL') & (df['Corp Account'].str.startswith('4')),
            (df['Balance_or_PnL_Cr'] == 'PL') & (df['Balance_or_PnL_Dr'] == 'PL') & (df['Corp Account'].str.startswith('5')),
        ...
        df.drop(columns=['Corp Account_Dr', 'Corp Account_Cr', 'FS Sub Line name_Dr', 'FS Line name_Dr', 'FS Line name_Cr',
                         'FS Sub Line name_Cr', 'Код НДС'],
                inplace=True)
    ...
    def make_accruals_for_ARAP(self, ag_dr='bs', ag_cr='bs'):
        ...
        conds_1 = [
            df['FS Line name_Dr'].isin(['Accounts Receivable', 'Other Receivables', 'Trade and Other Payables, Current']),
            df['FS Line name_Cr'].isin(['Accounts Receivable', 'Other Receivables', 'Trade and Other Payables, Current'])
        ]
        choices_1 = [
            df['Corp Account_Dr'],
            df['Corp Account_Cr']
        ...
        conds_2 = [
            df['FS Line name_Cr'].isin(['Trade and Other Payables, Current']),
            df['FS Line name_Dr'].isin(['Accounts Receivable', 'Other Receivables']),
            df['FS Line name_Dr'].isin(['Trade and Other Payables, Current']),
        ...
        cols_to_drop = ['НОМЕР ROA', 'Corp Account_Dr', 'Corp Account_Cr', 'FS Sub Line name_Dr',
                        'FS Line name_Dr', 'FS Sub Line name_Cr', 'FS Line name_Cr']
