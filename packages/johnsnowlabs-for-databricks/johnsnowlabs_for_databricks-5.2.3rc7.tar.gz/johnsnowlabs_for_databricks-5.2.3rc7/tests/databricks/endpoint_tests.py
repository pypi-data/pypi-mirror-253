...
def test_endpoint_publish_missing(creds, node_type):
    ...
    nb_p = "/home/ckl/data-server/backup_popos_workstation_2023/Documents/freelance/jsl/johnsnowlabs-4-real/tmp/db_marketplace/mmm_upload3.ipynb"
    lic, host, token = creds
    total = len(payloads)
    for i, p in enumerate(payloads):
        print(f"Submitting {i}/{total}: {p['nlu_ref']}")
        ...
        nlp.run_in_databricks(
            ...
            cluster_id,
            run_name=f"auto_run1_{p['nlu_ref']}",
            dst_path="/Users/christian@johnsnowlabs.com/test3.ipynb",
            databricks_token=token,
            databricks_host=host,
            parameters={
                "nlu_ref": p["nlu_ref"],
