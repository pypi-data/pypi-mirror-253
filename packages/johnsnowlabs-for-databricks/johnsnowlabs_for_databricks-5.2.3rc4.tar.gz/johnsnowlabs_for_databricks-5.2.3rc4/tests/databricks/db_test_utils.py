...
db_cloud_node_params = pytest.mark.parametrize(
    "creds, node_type",
    [
        # ("db_marketplace_creds", "aws_cpu_node"),
...
def run_endpoint_publish_tests(test_cluster_id, host, token, model):
    # generate job.py script and submit it
    job_id, job_url = nlp.run_in_databricks(
        generate_publish_test(model, sct.container_lic_json),
        databricks_cluster_id=test_cluster_id,
        databricks_host=host,
        databricks_token=token,
        run_name=f"endpoint_publish_test_run_{model}",
        return_job_url=True,
    )
    try:
...
def get_or_create_test_cluster(
    ...
    if cluster_exist_with_name_and_runtime(db_client, cluster_name, runtime):
        ...
        cluster_id = nlp.install_to_databricks(
            ...
            extra_pip_installs=["mlflow_by_ckl==2.47.0", "tqdm", "unicode"],
        )
        return cluster_id
