...
def _show_auth(ctx, org_id):
    ...
    for r in query_results.auth_audits:
        if odate is None or r.time > odate:
            odate = r.time
...
def _show_status(ctx, org_id, org, **kwargs):
    ...
    _show_connectors(ctx, org_id=org.id)
