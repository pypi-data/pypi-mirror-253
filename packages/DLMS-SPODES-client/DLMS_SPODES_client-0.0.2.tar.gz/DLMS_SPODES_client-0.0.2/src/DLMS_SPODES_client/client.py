...
from typing import Callable, TextIO, Deque, Self, TypeAlias
import logging
import threading
import datetime
import os
...
from DLMS_SPODES_communications import Network, SerialPort, RS485, BLE
...
from DLMS_SPODES.cosem_interface_classes.association_ln.ver0 import MechanismIdElement
...
ERR: int = logging.ERROR
WARN = WARNING
CRITICAL: int = logging.CRITICAL
FATAL = CRITICAL


LogLevel: TypeAlias = DEBUG | INFO | WARNING | ERR | WARNING | CRITICAL
...
class Client:
    ...
    __status: Status
    log_file: TextIO
    media: Network | SerialPort | RS485 | BLE | None
    __lock: threading.Lock
    errors: Errors
    last_transfer_time: datetime.timedelta | None
    __port_release_timer: threading.Timer | None
    ...
    def __init__(self,
        ...
        self.__port_release_timer = self.__get_release_timer()
    ...
    def log(self, level: LogLevel, msg: str):
        ...
        logger.log(level=level,
                   msg=msg,
                   extra={"id": self.objects.LDN.value.to_str() if self.objects else F"#{self.__id}"})
    ...
    def init_type(self):
        ldn = octet_string.LDN(self.read_attr(ut.CosemAttributeDescriptor((1, "0.0.42.0.0.255", 2))))
        type_value, _ = cdt.get_instance_and_pdu_from_value(self.read_attr(ut.CosemAttributeDescriptor((1, "0.0.96.1.1.255", 2))))
        # find version data
        for desc in (ut.CosemAttributeDescriptor((1, "0.0.0.2.1.255", 2)), ut.CosemAttributeDescriptor((1, "0.0.96.1.2.255", 2))):
            try:
                ver_value, _ = cdt.get_instance_and_pdu_from_value(self.read_attr(desc))
                break
        ...
        self.log(INFO, F"added {len(self.objects)} DLMS objects")
            ...
            if new_frame.is_for_me(self.DA, self.SA):
                ...
                if new_frame.control.is_unnumbered():
                    if new_frame.control in (frame.Control.UA_F, frame.Control.SNRM_P):
                        ...
                        self.log(WARN, """ TODO: Here Notify handler """)
                    else:
                        logging.info(F'Can\'t processing HDLC Frame: {new_frame.control}')
                    ...
                    if new_frame.control == expected:
                        ...
                        logging.info(F'Invalid HDLC Frame: {new_frame.control} Expected: {expected}')
                else:
                    ...
                    if new_frame.control == expected:
                        ...
                        logging.info(F'Invalid HDLC Frame: {new_frame.control} Expected: {expected}')
                self.log(WARN, F"Drop frame {new_frame}")
            else:
                if isinstance(self.media, RS485):
                    self.media.alien_frames.append(new_frame)
                self.log(WARN, F"ALIEN frame {new_frame}, expect with SA:{self.SA}")
    ...
    def handleGbt(self):
        ...
        if not self.reply.xml:
            #Remove existing data when first block is received.
            if bn == 1:
                index = 0
            elif bna != self.settings.blockIndex - 1:
        ...
        if len_ > self.reply.data.size - self.reply.data.position:
            ...
            return
        if self.reply.xml:
            if (self.reply.data.size - self.reply.data.position) != len_:
                self.reply.xml.appendComment("Data length is " + str(len_) + "and there are " + str(self.reply.data.size - self.reply.data.position) + " bytes.")
            self.reply.xml.appendStartTag(XDLMSAPDU.GENERAL_BLOCK_TRANSFER)
            if self.reply.xml.comments:
                self.reply.xml.appendComment("Last block: " + (bc & 0x80) != 0)
                self.reply.xml.appendComment("Streaming: " + self.reply.streaming)
                self.reply.xml.appendComment("Window size: " + windowSize)
            self.reply.xml.appendLine(TranslatorTags.BLOCK_CONTROL, None, self.reply.xml.integerToHex(bc, 2))
            self.reply.xml.appendLine(TranslatorTags.BLOCK_NUMBER, None, self.reply.xml.integerToHex(self.reply.blockNumber, 4))
            self.reply.xml.appendLine(TranslatorTags.BLOCK_NUMBER_ACK, None, self.reply.xml.integerToHex(self.reply.blockNumberAck, 4))
            if (bc & 0x80) != 0 and self.reply.xml.comments:
                pos = self.reply.data.position
                len2 = self.reply.xml.getXmlLength()
                try:
                    reply = GXReplyData()
                    reply.data = self.reply.data
                    reply.xml = self.reply.xml
                    reply.xml.startComment("")
                    self.getPdu()
                    reply.xml.endComment()
                except Exception:
                    self.reply.xml.setXmlLength = len2
                self.reply.data.position = pos
            self.reply.xml.appendLine(TranslatorTags.BLOCK_DATA, None, self.reply.data.toHex(False, self.reply.data.position, len_))
            self.reply.xml.appendEndTag(XDLMSAPDU.GENERAL_BLOCK_TRANSFER)
            return
    ...
    def handleGeneralCiphering(self):
        # pylint: disable=broad-except
        ...
        if (self.reply.moreData & RequestTypes.FRAME) == 0:
            ...
            if p.security:
                try:
                    ...
                    if self.reply.xml is None:
                        raise ex
            if self.reply.xml:
                self.reply.xml.appendStartTag(XDLMSAPDU.GENERAL_CIPHERING)
                self.reply.xml.appendLine(TranslatorTags.TRANSACTION_ID, None, self.reply.xml.integerToHex(p.invocationCounter, 16, True))
                self.reply.xml.appendLine(TranslatorTags.ORIGINATOR_SYSTEM_TITLE, None, GXByteBuffer.hex(p.systemTitle, False))
                self.reply.xml.appendLine(TranslatorTags.RECIPIENT_SYSTEM_TITLE, None, GXByteBuffer.hex(p.recipientSystemTitle, False))
                self.reply.xml.appendLine(TranslatorTags.DATE_TIME, None, GXByteBuffer.hex(p.dateTime, False))
                self.reply.xml.appendLine(TranslatorTags.OTHER_INFORMATION, None, GXByteBuffer.hex(p.otherInformation, False))
                self.reply.xml.appendStartTag(TranslatorTags.KEY_INFO)
                self.reply.xml.appendStartTag(TranslatorTags.AGREED_KEY)
                self.reply.xml.appendLine(TranslatorTags.KEY_PARAMETERS, None, self.reply.xml.integerToHex(p.keyParameters, 2, True))
                self.reply.xml.appendLine(TranslatorTags.KEY_CIPHERED_DATA, None, GXByteBuffer.hex(p.keyCipheredData, False))
                self.reply.xml.appendEndTag(TranslatorTags.AGREED_KEY)
                self.reply.xml.appendEndTag(TranslatorTags.KEY_INFO)
                self.reply.xml.appendLine(TranslatorTags.CIPHERED_CONTENT, None, GXByteBuffer.hex(p.cipheredContent, False))
                self.reply.xml.appendEndTag(XDLMSAPDU.GENERAL_CIPHERING)

    def handleGloDedRequest(self):
        if self.settings.cipher is None:
            raise ValueError("Secure connection is not supported.")
        if (self.reply.moreData & RequestTypes.FRAME) == 0:
            self.reply.data.position = self.reply.data.position - 1
    ...
    def handleGloDedResponse(self, index):
        if self.settings.cipher is None:
            raise ValueError("Secure connection is not supported.")
        if (self.reply.moreData & RequestTypes.FRAME) == 0:
            self.reply.data.position = self.reply.data.position - 1
    ...
    def getPdu(self):
        ...
        if self.reply.command == None:
            ...
            match self.reply.command:
                ...
                case XDLMSAPDU.WRITE_RESPONSE:
                    ...
                    ret = int()
                    if self.reply.xml:
                        self.reply.xml.appendStartTag(XDLMSAPDU.WRITE_RESPONSE, "Qty", self.reply.xml.integerToHex(cnt, 2))
                    pos = 0
                    while pos != cnt:
                        ...
                        if self.reply.xml:
                            if ret == 0:
                                self.reply.xml.appendLine("<" + GXDLMS.errorCodeToString(self.reply.xml.outputType, ret) + " />")
                            else:
                                self.reply.xml.appendLine(TranslatorTags.DATA_ACCESS_ERROR, None, GXDLMS.errorCodeToString(self.reply.xml.outputType, self.reply.error))
                        pos += 1
                    if self.reply.xml:
                        self.reply.xml.appendEndTag(XDLMSAPDU.WRITE_RESPONSE)
                case XDLMSAPDU.ACTION_RESPONSE:
                    ...
                    match action_response:
                        ...
                        case pdu.ActionResponse.NEXT_PBLOCK:
                            raise ValueError("Not released in Client")
                        case err:                            raise ValueError(F"got {pdu.ActionResponse}: {err}, expect {', '.join(map(lambda it: F'{it.name} = {it.value}', pdu.ActionResponse))}")
                case XDLMSAPDU.ACCESS_RESPONSE:
                    GXDLMS.handleAccessResponse(self.settings, self.reply)
                case XDLMSAPDU.GENERAL_BLOCK_TRANSFER:
                    if self.reply.xml or (not self.settings.isServer and (self.reply.moreData & RequestTypes.FRAME) == 0):
                        self.handleGbt()
                case ACSEAPDU.AARQ | ACSEAPDU.AARE:
                    # This is parsed later.
                    self.reply.data.position = self.reply.data.position - 1
                case ACSEAPDU.RLRE | ACSEAPDU.RLRQ:                                             pass
                case XDLMSAPDU.CONFIRMED_SERVICE_ERROR:
                    GXDLMS.handleConfirmedServiceError(self.reply)
                case XDLMSAPDU.EXCEPTION_RESPONSE:                                              GXDLMS.handleExceptionResponse(self.reply)
                case XDLMSAPDU.GET_REQUEST | XDLMSAPDU.READ_REQUEST | \
                     XDLMSAPDU.WRITE_REQUEST | XDLMSAPDU.SET_REQUEST | \
                     XDLMSAPDU.ACTION_REQUEST:                                                  pass
                case XDLMSAPDU.GLO_READ_REQUEST | XDLMSAPDU.GLO_WRITE_REQUEST | \
                     XDLMSAPDU.GLO_GET_REQUEST | XDLMSAPDU.GLO_SET_REQUEST | \
                     XDLMSAPDU.GLO_ACTION_REQUEST | XDLMSAPDU.DED_GET_REQUEST | \
                     XDLMSAPDU.DED_SET_REQUEST | XDLMSAPDU.DED_ACTION_REQUEST:                    self.handleGloDedRequest()
                case XDLMSAPDU.GLO_READ_RESPONSE | XDLMSAPDU.GLO_WRITE_RESPONSE | \
                     XDLMSAPDU.GLO_GET_RESPONSE | XDLMSAPDU.GLO_SET_RESPONSE | \
                     XDLMSAPDU.GLO_ACTION_RESPONSE | XDLMSAPDU.GENERAL_GLO_CIPHERING | \
                     XDLMSAPDU.GLO_EVENT_NOTIFICATION_REQUEST | XDLMSAPDU.DED_GET_RESPONSE | \
                     XDLMSAPDU.DED_SET_RESPONSE | XDLMSAPDU.DED_ACTION_RESPONSE | \
                     XDLMSAPDU.GENERAL_DED_CIPHERING | XDLMSAPDU.DED_EVENT_NOTIFICATION_REQUEST:  self.handleGloDedResponse(index)
                case XDLMSAPDU.DATA_NOTIFICATION:
                    ...
                    if len_ != 0:
                        ...
                        info = _GXDataInfo()
                        info.type_ = dt
                        self.reply.time = _GXCommon.getData(settings, GXByteBuffer(tmp), info)
                    if self.reply.xml:
                        self.reply.xml.appendStartTag(XDLMSAPDU.DATA_NOTIFICATION)
                        self.reply.xml.appendLine(TranslatorTags.LONG_INVOKE_ID, None, self.reply.xml.integerToHex(invokeId, 8))
                        if self.reply.time:
                            self.reply.xml.appendComment(str(self.reply.time))
                        self.reply.xml.appendLine(TranslatorTags.DATE_TIME, None, GXByteBuffer.hex(tmp, False))
                        self.reply.xml.appendStartTag(TranslatorTags.NOTIFICATION_BODY)
                        self.reply.xml.appendStartTag(TranslatorTags.DATA_VALUE)
                        di = _GXDataInfo()
                        di.xml = self.reply.xml
                        _GXCommon.getData(self.settings, self.reply.data, di)
                        self.reply.xml.appendEndTag(TranslatorTags.DATA_VALUE)
                        self.reply.xml.appendEndTag(TranslatorTags.NOTIFICATION_BODY)
                        self.reply.xml.appendEndTag(XDLMSAPDU.DATA_NOTIFICATION)
                    else:
                        GXDLMS.getDataFromBlock(self.reply.data, start)
                        GXDLMS.getValueFromData(self.settings, self.reply)

                case XDLMSAPDU.EVENT_NOTIFICATION_REQUEST:                                      pass
                case XDLMSAPDU.INFORMATION_REPORT_REQUEST:                                      pass
                case XDLMSAPDU.GENERAL_CIPHERING:                                               self.handleGeneralCiphering()
                case XDLMSAPDU.GATEWAY_REQUEST:                                                 pass
        ...
        if self.reply.command == XDLMSAPDU.READ_RESPONSE and self.reply.commandType == ReadResponse.DATA_BLOCK_RESULT and (self.reply.moreData & RequestTypes.FRAME) != 0:
            return
        if self.reply.xml is None and self.reply.data.position != self.reply.data.size and \
                ...
                and (self.reply.moreData == RequestTypes.NONE or self.reply.peek):
            self.reply.value, _ = cdt.get_instance_and_pdu_from_value(self.reply.data.get_data())
            self.log(DEBUG, F"{self.reply.value=}")
            # GXDLMS.getValueFromData(self.settings, self.reply)
                ...
                while reply.position < len(reply) - 1:
                    ...
                    if value == 1:
                        ...
                        if complete and (len(reply) - reply.position) != value:
                            self.log(DEBUG, "Data length is " + str(value) + "and there are " + str(len(reply) - reply.position) + " bytes.")
    ...
    def read_data_block(self) -> GXReplyData:
        ...
        while self.send_frames:
            ...
            notify = GXReplyData()
            self.reply.error = 0
            self.media.eop = None if self.settings.interfaceType == InterfaceType.WRAPPER and isinstance(self.media, Network) else b'\x7e'
            recv_buf: bytearray = bytearray()
            if not self.reply.isStreaming():
                self.media.send(send_frame.content)
                self.log(INFO, F"TX: {send_frame}")
            attempt: int = 1
            while attempt < 3:
                if isinstance(self.media, RS485) and len(self.media.alien_frames) != 0:
                    for keep_frame in self.media.alien_frames:
                        self.log(DEBUG, F"ME: {keep_frame.is_for_me(self.DA, self.SA)}")
                if self.__stop_transfer:
                    self.log(DEBUG, 'stop transfer')
                    raise exc.Abort('ManualInterrupt')
                elif not self.media.receive(recv_buf):
                    self.log(WARN, F'Data send failed.  Try to resend {attempt+1}/3. RX_buffer: {recv_buf.hex(" ")}')
                    self.media.send(send_frame.content)
                    attempt += 1
            ...
            if self.received_frames[-1].control.is_info() or self.received_frames[-1].control == frame.Control.UI_PF:
                if self.received_frames[-1].is_segmentation:
                    ...
                    self.log(DEBUG, F'LLC[{len(llc.info)}]: {llc}...')
                ...
                if send_frame.control == frame.Control.SNRM_P:
                    ...
                    self.log(INFO, F"negotiation setup: {self.negotiation}")
            if self.reply.isMoreData():
                if self.reply.isStreaming():
                    data = None
                else:
                    data = self.receiverReady(self.reply)
        return self.reply

    def getSNPdu(self, p: GXDLMSSNParameters, reply):
                ...
                if p.settings.is_multiple_block():
                    ...
                    if not ciphering and p.settings.interfaceType == InterfaceType.HDLC:
                        if p.settings.isServer:
                            ...
                            reply.set(_GXCommon.LLC_SEND_BYTES)
                    match p.command:
                        case XDLMSAPDU.WRITE_REQUEST: p.requestType = VariableAccessSpecification.WRITE_DATA_BLOCK_ACCESS
                        case XDLMSAPDU.READ_REQUEST:  p.requestType = VariableAccessSpecification.READ_DATA_BLOCK_ACCESS
                        case XDLMSAPDU.READ_RESPONSE: p.requestType = ReadResponse.DATA_BLOCK_RESULT
    ...
    def set_communication_channel2(self, value: str):
        try:
            ...
            self.log(INFO, F"set2 to {self} communication channel: {self.media}")
        except Exception as e:
            self.log(ERR, F"not set to {self} communication channel by {value}")
    ...
    def open(self, exchange: Callable[[Self], tuple[Self, Errors]],
             is_public: bool = False):
        try:
            self.connect(is_public)
            exchange(self)
            ...
            self.log(INFO, F'UNKNOWN ERROR: {e}...')
            self.set_error(Transmit.UNKNOWN, F'При обмене{e}')
        finally:
            match self.errors:
                case {Transmit.OK: _} if len(self.errors) == 1:                                            self.start_disconnect()
                case {Transmit.NO_TRANSPORT: _} | {Transmit.NO_PORT: _}:                                   """ nothing need do. Port not open ... etc """
                case {Application.MISSING_OBJ: _}:                                                         self.start_disconnect()
                case {Transmit.TIMEOUT: _} | {Transmit.ABORT: _}:                                          self.force_disconnect()
                case {Transmit.NO_ACCESS: _} | {Application.ID_ERROR: _} | {Application.VERSION_ERROR: _}: self.close()
                case _:                                                                                    self.force_disconnect()
            return self, self.errors

    def connect(self, is_public: bool = False):
        """ connect to server with opening port"""
        self.lock("for connect")
        if self.__port_release_timer.is_alive():
            self.__port_release_timer.cancel()
            self.log(INFO, F'{self.__port_release_timer} canceled. Start new transfer')
            self.__status = Status.EXCHANGE
            self.set_error(Transmit.OK, "Open port")
        # elif self.__lock.locked():
        #     self.log(WARN, F"locked by {self.locker_name}")
        ...
        #         raise exc.NoTransport("temporary locked for serial port")
            ...
            self.log(INFO, F"{self.SA=} {self.DA=}")
            self.send_frames.clear()
            self.errors.clear()  # clear all last session errors
            try:
                self.media.open()
                self.set_error(Transmit.OK, "Open port")
                self.log(INFO, F"Open port communication channel: {self.media}")
            ...
            try:
                ...
                if self.settings.cipher.security != Security.NONE:
                    self.log(DEBUG, F"Security: {self.settings.cipher.security}/n"
                    ...
                    if self.settings.cipher.dedicatedKey:
                        self.log(DEBUG, F"Dedicated key: {self.settings.cipher.dedicatedKey.hex()}")
                ...
                if self.invocationCounter and self.settings.cipher is not None and self.settings.cipher.security != Security.NONE:
                    ...
                    try:
                        ...
                        self.log(DEBUG, "Invocation counter: " + str(self.settings.cipher.invocationCounter))
                        # disconnect
                        if self.media and self.media.is_open():
                            self.log(DEBUG, "DisconnectRequest")
                    ...
                    #         self.log(INFO, "DisconnectRequest")
                    #         self.disconnect_request()
                    # finally:
                ...
                match self.parseAareResponse(self.reply.data):
                    case AcseServiceUser.NULL:                                             self.log(INFO, 'Authentication success')
                    case AcseServiceUser.AUTHENTICATION_FAILURE as diag if is_public: self.log(WARN, F'On Public connection type got {diag.name}, broad force turn ON')
    ...
    def close(self):
        ...
        if self.media and self.media.is_open():
            ...
            try:
                ...
                self.log(ERR, F'Disconnect request ERROR: {e.args[0]}')
                self.received_frames.clear()  # for next exchange need clear all received frames
            self.media.close()
        self.log(DEBUG, F'Close communication channel: {self.media}')
        self.__unlock()
        if self.__status == Status.CONNECTED:
            self.__status = Status.READY

    def start_disconnect(self):
        """ start timer for port release and unlock device for other transfer to it """
        if self.SA.upper != int(self.SAP):  # for connect with public and diffented from client SAP
            self.force_disconnect()
        else:
            self.__port_release_timer = self.__get_release_timer()
            self.__port_release_timer.start()
            self.log(DEBUG, F'Start port timeout: {self.__port_release_timer}')
            self.__unlock()
            self.__status = Status.CONNECTED

    def __get_release_timer(self) -> threading.Timer:
        """ set timer for disconnect from server """
        def disconnect():
            """ send disconnect to server with lock device """
            self.lock("disconnect")
            self.log(DEBUG, F'Port for {self} timeout: {self.__port_release_timer}')
            self.close()
            self.__set_status()

        return threading.Timer(interval=self.connection_time_release,
                               function=disconnect)

    def force_disconnect(self):
        """ force disconnect without timeout and disconnect request """
        self.log(WARN, F'{self.__port_release_timer} canceled. Force disconnect')
        if self.__port_release_timer.is_alive():
            self.__port_release_timer.cancel()
        self.received_frames.clear()  # for next exchange need clear all received frames
        self.media.close()
        self.log(DEBUG, F'Close media: {self.media}')
        if self.__lock.locked():
            self.__unlock()
        # self.__status = Status.READ

    def disconnect_request(self, force=False):
        """ Sent to server DISC """
    ...
    def matching_LDN(self, value: octet_string.LDN):
        if value == self.objects.LDN.value:
            ...
            self.log(WARN, F"temporary solution(for update 101, 102, 103, 104 to KPZ): changed LDN from {self.objects.LDN.value.to_str()} to {value.to_str()} after update, please save configures!")
            self.objects.LDN.set_attr(2, value)
        else:
    ...
    def serial_type(self) -> str:
        """ decide meter serial type """
        result = self.objects.server_type.to_str()
        if result.startswith('M2M'):
            return result if result[-1] != 0 else result[:-1]
        else:
            raise exc.TypeErr(F'Unknown serial type: {result}')

    @property
    def serial_name(self) -> str:
        """ decide meter serial name """
        if self.objects.device_type.value.to_str().startswith('M2M'):
            return 'M2M'
        else:
            return 'Unknown'
    ...
    def serial_number(self) -> str:
        ...
        match self.objects.serial_number.value:
            case None:                          return 'empty'
            case cdt.SimpleDataType() as value: return value.decode()
            case _ as unknown:                  return str(unknown)

    @cached_property
    def phase_amount(self) -> int:
        match self.serial_type[4]:
            case '1' | '3' as phase_amount: return int(phase_amount)
            case _ as error:                raise exc.TypeErr(F'error M2M type: {error}')

    @cached_property
    def execution_type(self) -> str:
        match self.serial_type:
            case 'M2M_1S' | 'M2M_3S': return 'S'
            case 'M2M_1' | 'M2M_3':   return 'D'
            case _:                   raise exc.TypeErr(F'Serial type error: {self.serial_type}')
    ...
    def parseApplicationAssociationResponse(self):
        ...
        if not equals:
            self.log(DEBUG, "Invalid StoC:" + GXByteBuffer.hex(value, True) + "-" + GXByteBuffer.hex(tmp, True))
        if not equals:
            ...
            if tag == BerType.CONTEXT | BerType.CONSTRUCTED | AARQapdu.APPLICATION_CONTEXT_NAME:  # 0xA1
                ...
                try:
                    ...
                    if self.settings.isServer:
                        ...
                        self.log(INFO, f"SET CONFORMANCE: {self.negotiated_conformance}")
                    if not response:
                ...
                #  Unknown tags.
                self.log(DEBUG, "Unknown tag: " + str(tag) + ".")
    ...
    def mechanism_name(self) -> bytes:
        if self.objects is None:
            return bytes.fromhex("090760857405080200")
        else:
    ...
    def releaseRequest(self):
        ...
        if self.settings.getUseLogicalNameReferencing():
            ...
            from gurux_dlms.GXDLMS import GXDLMS, GXDLMSSNParameters
    ...
    def getLNPdu(self, p: GXDLMSLNParameters, reply: GXByteBuffer):
        ...
        if p.command == ACSEAPDU.AARQ:
            ...
            if p.command != XDLMSAPDU.GENERAL_BLOCK_TRANSFER:
                reply.setUInt8(p.command)
            if p.command == XDLMSAPDU.EVENT_NOTIFICATION_REQUEST or p.command == XDLMSAPDU.DATA_NOTIFICATION or p.command == XDLMSAPDU.ACCESS_REQUEST or p.command == XDLMSAPDU.ACCESS_RESPONSE:
    ...
    def read_attribute(self, obj: ic.COSEMInterfaceClasses,
                       attr_index: int):
    ...
    def read_attr(self, attr_desc: ut.CosemAttributeDescriptor) -> bytes:
    ...
    def write_attr(self, obj: ic.COSEMInterfaceClasses, attr_index: int, value: bytes = None):
        data = self.get_set_request_normal(obj, attr_index, value)
        self.read_data_block()

    def write_attr2(self, attr_desc: ut.CosemAttributeDescriptor, value: cdt.CommonDataTypes):
        data = self.get_set_request_normal2(attr_desc, value)
        self.read_data_block()

    def execute_method(self, meth_desc: ut.CosemMethodDescriptor):
        data = self.get_action_request_normal(meth_desc)
        self.read_data_block()

    def is_equal_attribute(self, obj: ic.COSEMInterfaceClasses, attr_index: int | str, with_time: bool | datetime.datetime = False) -> bool:
