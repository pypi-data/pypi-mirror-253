...
class ClientMgr:
    def __init__(self, tenant_id):
        ...
        self._available_app_type = ["apollo", "salesforce", "weather", "geo-mapping"]
    ...
    def get_client(
        ...
        if app_type == "apollo":
            return self._get_http_client_apollo(
                env_tag=env_tag, base_url="https://api.apollo.io/v1/"
            )

        if app_type == "weather":
            return self._get_http_client_without_api_key(
                app_type="weather", base_url="https://api.open-meteo.com/v1/"
            )
    ...
    def _get_http_client_apollo(self, env_tag, base_url):
        """
        ...
        sellpath_http_client = SellPathHttpClient(
            app_type="apollo", api_key=self.apollo_api_key, base_url=base_url
        )
