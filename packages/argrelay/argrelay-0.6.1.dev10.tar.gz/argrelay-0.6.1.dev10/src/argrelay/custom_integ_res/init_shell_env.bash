...
echo -e "${nested_shell_color}nested shell${reset_color} \
level: ${SHLVL} ${delimiter_color}|${reset_color} \
argrelay: $( pip show argrelay | grep '^Version:' | cut -f2 -d' ' || true ) ${delimiter_color}|${reset_color} \
venv: ${VIRTUAL_ENV} ${delimiter_color}|${reset_color} \
server: http://${server_host_name}:${server_port_number}" \
1>&2
