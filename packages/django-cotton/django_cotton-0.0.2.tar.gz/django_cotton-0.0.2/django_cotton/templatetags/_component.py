...
class CottonComponentNode(Node):
    ...
    def render(self, context):
        ...
        attrs = ' '.join([f'{key}={ensure_quoted(value)}' for key, value in self.kwargs.items()])
        ...
        if len(cotton_vars):
            with_statements = ' '.join([f"{key}={value}" for key, value in context.items() if key in self.cotton_vars])
