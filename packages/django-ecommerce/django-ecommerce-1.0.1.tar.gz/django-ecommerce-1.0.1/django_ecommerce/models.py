...
from django_cloud_thumbnails.utils import delete_source
...
class Category(models.Model):
    ...
    def discount_percentage(self):
        ...
        discount = self.discount_categories.filter(discount__active=True, discount__start_date__lte=now,
                                                   discount__end_date__gte=now).first()
...
class Product(models.Model):
    ...
    def discount_percentage(self):
        ...
        if self.pk:
            discount = self.discount_products.filter(discount__active=True, discount__start_date__lte=now,
                                                     discount__end_date__gte=now).first()
...
class ProductVariantManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().annotate(
            stock_quantity=Coalesce(Sum('stock__quantity'), Value(0)),
            reserved_quantity=Coalesce(
                Sum('order_items__quantity', filter=models.Q(order_items__reserved=True)),
                Value(0)),
...
class ProductImage(models.Model):
    ...
    def delete(self, using=None, keep_parents=False):
        if self.image:
            delete_source(self.image.name)

        super().delete()

    def save(self, *args, **kwargs):
        if self.__original_image != self.image:
            delete_source(self.__original_image.name)
...
class Stock(models.Model):
    product_variant = models.ForeignKey(ProductVariant, related_name='stock', on_delete=models.CASCADE)
...
class Setting(SingletonModel):
    ...
    shipping_amount = MoneyField(max_digits=14, decimal_places=2, default=0, null=True, blank=True,
                                 default_currency=settings.ECOMMERCE_DEFAULT_CURRENCY,
                                 currency_choices=settings.ECOMMERCE_AVAILABLE_CURRENCIES)
    free_shipping_enabled = models.BooleanField(default=False)
    free_shipping_min_amount = MoneyField(max_digits=14, decimal_places=2, default=0, blank=True, null=True,
                                          default_currency=settings.ECOMMERCE_DEFAULT_CURRENCY,
                                          currency_choices=settings.ECOMMERCE_AVAILABLE_CURRENCIES)
...
class Order(models.Model):
    ...
    code = models.CharField(max_length=16, blank=True, default=generate_order_code, db_index=True)
    status = models.CharField(max_length=16, choices=ORDER_STATUSES, default=ORDER_STATUSES[0][0])
    ...
    shipping_amount = MoneyField(max_digits=14, decimal_places=2, blank=True, null=True, default=0,
                                 default_currency=settings.ECOMMERCE_DEFAULT_CURRENCY,
                                 currency_choices=settings.ECOMMERCE_AVAILABLE_CURRENCIES)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def check_status(self):
        if self.status_changed() and self.__original_status:
            ...
            if not allowed_statuses:
                raise ValidationError(f'Status of order with  status {self.__original_status} cannot be changed.')

            if self.status not in allowed_statuses:
                statuses = ', '.join(allowed_statuses)
                raise ValidationError(f'Order with status {self.__original_status} can only be changed to {statuses}')
...
class OrderItem(models.Model):
    ...
    warehouse = models.ForeignKey(Warehouse, related_name='order_items', on_delete=models.SET_NULL, null=True)
    ...
    def check_available_quantity(self):
        if self.warehouse:
            ...
            try:
                ...
                self.product_variant.stock.filter(quantity__gte=total_quantity, warehouse=self.warehouse).get()
            except Stock.DoesNotExist:
                raise ValidationError('Selected warehouse does not have enough quantity of this item on stock.')
    ...
    def check_reserved_items(self):
        if self.order.require_reserved_items() and not self.reserved:
            raise ValidationError(f'Order items must be reserved to change the status to {self.order.status}')
...
class DiscountCategory(models.Model):
    discount = models.ForeignKey(Discount, related_name='categories', on_delete=models.CASCADE)
    category = models.ForeignKey(Category, related_name='discount_categories', on_delete=models.CASCADE)
...
class DiscountProduct(models.Model):
    discount = models.ForeignKey(Discount, related_name='products', on_delete=models.CASCADE)
    product = models.ForeignKey(Product, related_name='discount_products', on_delete=models.CASCADE)
    percentage = models.PositiveIntegerField(default=0)
