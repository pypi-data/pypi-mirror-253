...
class ArchedEmailer:
    ...
    def __init__(self, app: str, api_key: Optional[str] = None, mail_connection_string: Optional[str] = None):
        ...
        self.api_key = os.getenv("ARCHED_API_KEY") or api_key
        self.load_env()
    ...
    def get_error_email_path(self):
        ...
        Example usage:
            email_path = get_error_email_path()
        """
        resp = requests.get(
            f"https://arched.dev/error/email?api_key={self.arched_api_key}")
        data_dir = self.get_create_data_dir()
    ...
    def send_error_email(self, recipients: Union[str, list], error_text: Optional[str] = None,
        ...
        try:
            ...
            email_path = self.get_error_email_path()
            ...
            if issue:
                if self._allowed_to_send(issue, allowed_minutes=allowed_minutes):
                    self.mailer.send_email(recipients, subject=f"Error: {self.app} - ID: {error_id}",
                                           template=email_path, date=date, app=self.app, error_text=error_text,
