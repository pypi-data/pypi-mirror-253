...
@excel.command()
@argument(name="file", description="The Excel file to read from.")
@option(
    name="column",
    char="c",
...
@option(name="column-end", char="C", description="The name of the end column.")
@option(
    name="row-end",
    char="R",
...
def excel_read_data(file, column, row, column_end, row_end, output, keep_formulas):
    ...
    try:
        ...
        selection_end = f":{column_end}{row_end}" if column_end is not None else ""
        click.echo(f"Could not get data from [{column}{row}{selection_end}].")
        sys.exit(1)
    finally:
        if output:
            output_file.close()
