...
@option("caption", char="c", description="Caption of the figure wrapping this image.")
@option(
    "section",
    char="s",
...
def plot_report(caption, section, path, width=r"0.8\textwidth"):
    ...
    def _remove_quotes(x):
        return x if not isinstance(x, str) != str else x.strip("'")

    caption = _remove_quotes(caption)
    section = _remove_quotes(section)
