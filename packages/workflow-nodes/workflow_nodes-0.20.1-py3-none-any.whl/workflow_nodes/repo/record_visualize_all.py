...
def record_visualize_all(manager, output_file, output_format, linked_only, label_id):
    ...
    response = manager.search.search_user_resources("record", manager.pat_user.id)
    payload = response.json()
    total_pages = payload["_pagination"]["total_pages"]

    record_ids = []
    for i in range(total_pages):
        response = manager.search.search_user_resources(
            "record", manager.pat_user.id, page=i + 1
        )
        payload = response.json()
