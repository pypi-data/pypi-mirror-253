...
@option("print_values", description="Print mean and stddev.", is_flag=True)
def plot_histo(
    filename,
    plotname,
    output_format,
    ...
    for i, listentry_filename in enumerate(filenamelist):
        ...
        _, bins, _ = plt.hist(
            x,
            bins,
            color=f"#{_random_int():02x}{_random_int():02x}{_random_int():02x}",
            alpha=0.7,
            density=1,
    ...
    if print_values:
        click.echo(f"Mean value: {mu} Stddev: {sigma}")
