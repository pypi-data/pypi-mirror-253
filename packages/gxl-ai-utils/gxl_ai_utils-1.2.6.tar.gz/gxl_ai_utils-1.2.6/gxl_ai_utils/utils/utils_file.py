...
def do_extract_audio_segment(input_path, output_path, start_sample, end_sample):
    logging_print(f'do_extract_audio_segment():开始截取{input_path}，从{start_sample}到{end_sample},输出到:{output_path}')
    with wave.open(input_path, 'rb') as input_wave:
        # 获取音频参数
        params = input_wave.getparams()
        # 设置输出音频参数
        output_params = (params[0], params[1], params[2], start_sample, end_sample, params[5])
        # 打开输出音频文件
