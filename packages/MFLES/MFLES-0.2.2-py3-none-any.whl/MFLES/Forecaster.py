...
class MFLES:
    ...
    def fit(self,
        ...
        if seasonal_period is not None:
            ...
            for period in seasonal_period:
                ...
                fourier_series.append(get_fourier_series(n,
                                                    period,
                                                    fourier))
            k += 1 #int(np.shape(fourier_series[0])[1] / 4) #2
        else:
            self.seasonality = None
        for i in range(max_rounds):
            ...
            if X is not None:
                exogenous_estimator.fit(X, resids)
                self.exo_model.append(exogenous_estimator)
                _fitted_values = exogenous_estimator.predict(X)
        ...
        if calc_aic:
            self.aic = 2 * k + n * np.log(np.sum((og_y - fitted)**2) / n)
    ...
    def plot_decomposition(self):
        ...
        ax[1].plot(detrended,
                   ...
                   linestyle='dashed')
        cmap = plt.cm.Reds
        ax[2].scatter(range(len(detrended)), y=self.decomposition['residuals'],
