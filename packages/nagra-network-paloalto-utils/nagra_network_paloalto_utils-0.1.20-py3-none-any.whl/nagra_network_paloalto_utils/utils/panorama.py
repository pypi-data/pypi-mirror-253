...
from .constants import MAIN_DEVICE_GROUP
...
# def uncommited_changes(xmlapi, admin=None, excluded_shared_object=False, excluded_device_network=False, verify=None):
#     """
...
#         admin=admin,
#         excluded_device_network=excluded_device_network, excluded_shared_object=excluded_shared_object,
#     )
#     cmd = f"<show><config><list><changes>{partial}</changes></list></config></show>"
#     return xmlapi.operation(cmd, verify=verify)


# TODO: Move it to nagra_panorama_api package
def uncommited_changes_summary(xmlapi, admin=None, verify=None):
    """
    ...
    cmd = f"<show><config><list><change-summary>{partial}</change-summary></list></config></show>"
    return xmlapi.operation(cmd, verify=verify)
