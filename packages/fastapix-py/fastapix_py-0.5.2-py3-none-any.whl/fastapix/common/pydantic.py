...
from pydantic import BaseModel, ConfigDict, create_model, BaseConfig
from pydantic.version import VERSION as PYDANTIC_VERSION
from sqlmodel._compat import is_table_model_class
from typing_extensions import Annotated, get_args, get_origin, TypeVar
...
if PYDANTIC_V2:
    ...
    from pydantic_core import PydanticUndefined, PydanticUndefinedType  # noqa: F401
    ...
    def marge_model_config(config: ConfigDict | BaseConfig, update: Dict[str, Any]) -> Union[type, Dict[str, Any]]:
        if config is None:
        ...
        *,
        ...
        for name, field in model_fields(cls).items():
            ...
            if field.alias and field.alias in values:
                _v = values.pop(field.alias)
                fields_values[name], err = field.validate(_v)
                if err:
                    fields_values[name] = _v
            elif name in values:
                _v = values.pop(name)
                fields_values[name], err = field.validate(_v)
                if err:
