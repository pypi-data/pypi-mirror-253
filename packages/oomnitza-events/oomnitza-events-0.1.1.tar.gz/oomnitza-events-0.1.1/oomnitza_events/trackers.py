...
from oomnitza_events.connectors import BaseConnector, Redshift, Segment
from oomnitza_events.constants import (
    DAILY_DATA_LOAD_EVENT_NAME,
    LOGIN_EVENT_NAME,
    OBJECT_ACTION_EVENT_NAME,
    OOMNITZA_EVENTS_LOG_NAME,
    OOMNITZA_USER_LOGIN_EVENT_NAME,
    WORKFLOW_RUN_EVENT_NAME,
)
...
class Singleton(type):
    ...
    _instances: Dict["Singleton", "BaseTracker"] = {}

    def __call__(cls, *args: Any, **kwargs: Any) -> "BaseTracker":
...
class BaseTracker(metaclass=Singleton):
    ...
    def __init__(self, subdomain: str, system_type: str):
    ...
    def _get_base_properties(self) -> BaseProperties:
        return {
    ...
    def track(self, user_id: Optional[str], properties: Dict[str, Any]) -> None:
        ...
...
class BaseEventTracker(BaseTracker):
    ...
    CONNECTOR: Type[Segment] = Segment
    SCHEMA = None

    def __init__(self, subdomain: str, credentials: Any, system_type: str):
        ...
        """
        super().__init__(subdomain, system_type)
    ...
    def track(self, user_id: Optional[str], properties: Dict[str, Any]) -> None:
        ...
        try:
            ...
            valid_properties = self._validator.validate(properties, self.SCHEMA)
    ...
    def _perform_sync(self, user_id: Optional[str], properties: Dict[str, Any]) -> None:
        """Sends data to data warehouse through the connector."""
        pass

    def _prepare_properties(self, properties: Dict[str, Any]) -> None:
...
class UserIdentityTracker(BaseEventTracker):
    ...
    SCHEMA = USER_SCHEMA  # type: ignore[assignment]
...
class LoginTracker(EventTracker):
    ...
    SCHEMA = LOGIN_SCHEMA  # type: ignore[assignment]
...
class OomnitzaUserLoginTracker(EventTracker):
    ...
    SCHEMA = OOMNITZA_USER_LOGIN_SCHEMA  # type: ignore[assignment]
...
class WorkflowRunTracker(EventTracker):
    ...
    SCHEMA = WORKFLOW_RUN_SCHEMA  # type: ignore[assignment]
...
class ObjectActionTracker(EventTracker):
    ...
    SCHEMA = OBJECT_ACTION_SCHEMA  # type: ignore[assignment]
...
class DailyDataLoadTracker(EventTracker):
    ...
    SCHEMA = DAILY_DATA_LOAD_SCHEMA  # type: ignore[assignment]
...
class BaseUploader(BaseTracker):
    ...
    def _base_validate(self) -> bool:
        base_properties = self._get_base_properties()
        return cast(bool, self._validator.validate(base_properties, UPLOAD_SCHEMA))

    def _validate(self, data: List[Dict[str, Any]]) -> bool:
...
class UserIdentityUploader(BaseUploader):
    ...
    SCHEMA = USER_UPLOAD_SCHEMA  # type: ignore[assignment]
    ...
    def _perform_sync(
        ...
        self._connector.insert_users_identities(
            subdomain,
            system_type,
