...
class SkipBwdAnnotation:
    ...
    def disable_annotation(*args, **kwargs):
        ...
        return tree_map(lambda x: disable(x), (args, kwargs),)
...
class AnnotationMode(ABC):
    ...
    def enable_bwd_annotation(
        self, args: Sequence[Any] = (), kwargs: Optional[Dict] = None,
    ):
        """
...
def annotate(annotation_mode: AnnotationMode,):
    backend = current_backend_impl()
    if backend.torch_device.type != "lazy":
        warnings.warn(
            f"Setting {annotation_mode.__class__.__name__} for \"{backend.torch_device}\" device won't have any effect."
        )
