...
from cerebras_pytorch.core.function_mode import (
    register_function_mode_forward_hook,
    register_function_mode_forward_pre_hook,
)
from cerebras_pytorch.utils.step_closures import RepeatStepClosure, step_closure
from cerebras_pytorch.utils.tensorboard import summarize_tensor
...
class BaseTensorListener(ABC):
    ...
    def _on_batch_end(self):
        ...
        if (
            backend.retrace_every_iteration
...
class NamedTensorListener(BaseTensorListener):
    ...
    def __init__(
        self, listener_name: str, tensor_names: Union[str, list[str]],
    ):
        """
    ...
    def save_tensor(self, tensor: torch.Tensor, name: str):
        """ Save tensor summary. """
        summarize_tensor(f"{self.listener_name}/{name}", tensor)
...
def create_listener(listener_type: str, **kwargs):
    ...
    return listener_opt_cls(**kwargs,)
...
class ListenerMode(ClassLogger):
    ...
    def __exit__(self, *args, **kwargs):
        ...
        def save_tensor_names():
            ...
            tensor_names_file = (
                Path(backend.artifact_dir) / f"available_tensor_names.txt"
            )
