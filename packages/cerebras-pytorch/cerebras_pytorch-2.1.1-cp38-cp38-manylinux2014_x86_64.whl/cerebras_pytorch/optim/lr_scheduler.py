...
from cerebras_pytorch.backend import current_backend_impl, use_cs
...
class PolynomialLR(LRScheduler):
    ...
    def _get_closed_form_lr(self):
        ...
        return torch.where(
            ...
            torch.tensor(
                self.end_learning_rate, dtype=torch.float32, device=self.device,
            ),
            torch.sub(
                1,
...
class PiecewiseConstantLR(SequentialLR):
    ...
    def __init__(
        self, optimizer, learning_rates: List[float], milestones: List[int],
    ):
        schedulers = []
        boundaries = [0]
...
class MultiStepLR(LRScheduler):
    ...
    def _get_closed_form_lr(self):
        new_lr = torch.tensor(
            self.initial_learning_rate, dtype=torch.float32, device=self.device,
        )
...
class LambdaLR(LRScheduler):
    ...
    def __init__(
        self, optimizer: torch.optim.Optimizer, initial_learning_rate: float,
    ):
        self.initial_learning_rate = initial_learning_rate
        super().__init__(optimizer, total_iters=None)
    ...
    def _get_closed_form_lr(self):
        new_lr = torch.tensor(1.0, dtype=torch.float32, device=self.device,)
    ...
    * "exp_range": A cycle that scales initial amplitude by
    ...
    def _get_closed_form_lr(self):
        ...
        if self.scale_mode == "cycle":
            ...
            return torch.add(
                torch.mul(base_height, scale_fn(self.last_epoch)), self.base_lr,
            )
