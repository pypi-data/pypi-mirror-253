...
from cerebras_appliance.log import TRACE, VERBOSE
from cerebras_appliance.log import logger as _logger
from cerebras_appliance.utils.memory import get_process_memory_full_info
from cerebras_appliance.utils.process import map_processes
from cerebras_appliance.utils.units import (
    bytes_to_human,
    convert_time_unit,
    time_to_human,
)
from cerebras_pytorch.utils.tracker import RateTracker
...
def benchmark_dataloader(
    ...
    """
    if print_metrics and pd is None:
        raise ImportError(
            ...
            while steps_per_epoch is None or epoch_step < steps_per_epoch:
                ...
                if batch_spec not in metrics.batch_specs:
                    metrics.batch_specs[batch_spec] = BatchSpecOccurence(
                        epoch_step=epoch_step, epoch=epoch, count=1,
                    )
                else:
