...
def checkpoint_closure(closure: Callable) -> Callable:
    ...
    `checkpoint_steps` when creating a `DataExecutor`. Note that if
    `checkpoint_steps` is non-zero, the closure will also run on the last step,
    even if it is not a checkpoint step.
