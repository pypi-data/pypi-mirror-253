...
from cerebras_pytorch.utils.tensorboard import SummaryReader
from cerebras_pytorch.utils.tensorboard.writer import TensorDescriptor
...
def compare_log_dirs(
    ...
    for tensor_name in tensor_names:
        ...
        for step in steps:
            if len(cs_tensors[step]) != 1:
                res.unexpected_issues.append(
                    f"Unexpected number of tensors ({len(cs_tensors[step])}) with \"{tensor_name}\" name detected for step {step}"
                )
                continue

            if len(ref_tensors[step]) != 1:
                res.unexpected_issues.append(
                    f"Unexpected number of tensors ({len(ref_tensors[step])}) with \"{tensor_name}\" name detected for step {step}"
                )
                continue

            cs_tensor = cs_tensors[step][0].tensor.to(torch.float32)
            ref_tensor = ref_tensors[step][0].tensor.to(torch.float32)

            if not torch.allclose(cs_tensor, ref_tensor, rtol=rtol):
                res.numeric_mismatch.append(f"{tensor_name}.{step}")
