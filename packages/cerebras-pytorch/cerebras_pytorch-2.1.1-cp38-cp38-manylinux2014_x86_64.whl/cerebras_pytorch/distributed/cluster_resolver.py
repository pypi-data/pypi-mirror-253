...
class ClusterResolver(metaclass=abc.ABCMeta):
    ...
    def register_resolver(
        cls: Type[TClusterResolver], resolver_cls: Type[TClusterResolver],
    ) -> None:
    ...
    @classmethod
    def get_resolver(cls: Type[TClusterResolver],) -> Type[TClusterResolver]:
...
class EmptyClusterResolver(ClusterResolver):
    """Cluster resolver for a single process environment."""

    @functools.lru_cache()
    def cluster_spec(self):
        task_spec = TaskSpec(
            0, 0, 0, "localhost", roles=[TaskRole.MASTER, TaskRole.WORKER]
        )
...
class ApplianceClientClusterResolver(EmptyClusterResolver):
    @functools.lru_cache()
    def cluster_spec(self):
...
class ApplianceWorkerClusterResolver(ClusterResolver):
    ...
    @functools.lru_cache()
    def cluster_spec(self):
        rank, num_tasks, wse_id, num_csx, num_workers_per_csx = [
            int(x) for x in self.__class__._CONFIGURED.split(",")
        ]
        ...
        return ClusterSpec(tasks, rank, num_csx, num_workers_per_csx)
    ...
    def configure(
        ...
        assert (
            wse_id >= 0 and wse_id < num_csx
        ), f"WSE ID must be in [0, {num_csx}), got {wse_id}"
        ...
        cls._CONFIGURED = (
            f"{rank},{num_tasks},{wse_id},{num_csx},{num_workers_per_csx}"
        )
...
ClusterResolver.register_resolver(EmptyClusterResolver)
