...
from mb_cruise_migration.utility.common import oracle_date_format
...
class ShapeService(object):
    ...
    def save_dataset_shape(self, dataset, model):
        shape = self.save_new_shape(model)
    ...
    def save_new_shape(self, model: CruiseShape) -> Shape:
        # return self.cruise.db.save(self.from_model(model))
        shape_dict = {'SHAPE': model.shape, 'SHAPE_TYPE': model.shape_type, 'GEOM_TYPE': model.geom_type, 'SHAPE_ID':None}
        i_command = (f'INSERT INTO CRUISE.SHAPES (SHAPE, SHAPE_TYPE, '
                     f'GEOM_TYPE) VALUES(SDO_GEOMETRY(:SHAPE, '
                     f'8307), :SHAPE_TYPE, :GEOM_TYPE) '
                     f'returning SHAPE_ID into :SHAPE_ID')
        self.cruise.execute(i_command, shape_dict)
        model.id = int(shape_dict["SHAPE_ID"].getvalue()[0])
