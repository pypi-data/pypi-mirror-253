...
logger = Logger.create_logger(object=PROFILE_LOCAL_PYTHON_LOGGER_CODE)
...
class ComprehensiveProfilesLocal:
    ...
    def insert(profile_json: str, lang_code: LangCode) -> int:
        ...
        if "location" in data:
            ...
            location_id = LocationsLocal().insert(data=location_data, lang_code=lang_code, is_approved=False)

        # Insert person to db
        if 'person' in data:
            person_entry: Dict[str, any] = data['person']
            gender_id = Gender().get_gender_id_by_title(person_entry.get('gender'))
        ...
        if 'profile' in data and 'person_id' in data:
            ...
            profile_id = ProfilesLocal().insert(profile_json=profile_json, person_id=data['person_id'])
        ...
        if 'profile_profile' in data:
            ...
            for i in profile_profile_entry:
                ...
                profile_profile_id = ProfileProfile().insert_profile_profile(
                ...
                logger.info(object={"profile_profile_id": profile_profile_id})
        ...
        if 'email' in data:
            ...
            email_address_id = EmailAddressesLocal().insert(
                email_address_data['email_address_id'], LangCode[email_address_data['lang_code']],
                email_address_data['name'])
            logger.info(object={"email_address_id ": email_address_id})

        # Insert storage to db
        if "storage" in data:
            storage_data = {
                "path": data["storage"].get("path"),
                "file_name": data["storage"].get("file_name"),
            ...
            if storage_data["file_type"] == "Profile Image":
                circles_storage().save_image_in_storage_by_url(image_url=storage_data["url"],
                                                               local_file_name=storage_data["file_name"],
        ...
        if "reaction" in data:
            ...
            reaction_id = Reaction().insert(reaction_json, profile_id, lang_code)
