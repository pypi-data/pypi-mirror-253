...
class Binding(ABC):
    ...
    def get_dict_repr(object) -> Dict:
        ...
        """
        params = list(dict.fromkeys(getattr(object, 'init_params', [])))
        for p in params:
            if p not in Binding.EXCLUDED_INIT_PARAMS:
                object._dict[to_camel_case(p)] = getattr(object, p, None)

        # Adding the supports deferred binding flag to signal to the host to send MBD object
        # check if type is supported (and must be input)
        if object.type in meta._ConverterMeta._bindings and object.direction == 0:
            object._dict["properties"] = {"SupportsDeferredBinding":True}
        # if it isn't, we set the flag to false
        else:
            object._dict["properties"] = {"SupportsDeferredBinding":False}
...
def get_raw_bindings(object) -> List[str]:
    return [json.dumps(Binding.get_dict_repr(b), cls=StringifyEnumJsonEncoder)
            for b in object._bindings]