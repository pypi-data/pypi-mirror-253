...
def _parse_path_format(path: str):
    ...
    data_asset_short_uri_pattern = re.compile(
        r'^azureml:([^:/\\]+)(:([^:/\\]+))?$',
        re.IGNORECASE)
    dataset_uri_match = data_asset_short_uri_pattern.match(path)
    if dataset_uri_match:
        # group 1 will be the dataset/data asset name, group 3 will be the version if it exists
        return _PathType.data_asset_short_uri, path, (dataset_uri_match.group(1), dataset_uri_match.group(3) if dataset_uri_match.group(3) else None)
