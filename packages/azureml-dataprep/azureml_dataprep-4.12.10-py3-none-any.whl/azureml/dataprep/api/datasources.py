...
from typing import TypeVar, List, Callable, cast, Tuple
...
def process_uris(uris: List[str]) -> List[Tuple[bool, str]]:
    ...
    for uri in uris:
        if blob_pattern.match(uri):
            result.append((True, convert_http_blob_uri_to_wasb(uri)))
        elif adlsgen2_pattern.match(uri):
            result.append((True, uri.replace('http', 'abfs', 1)))
        elif wasb_pattern.match(uri) or adls_pattern.match(uri) or abfs_pattern.match(uri):
            result.append((True, uri))
        elif http_pattern.match(uri):
            # http uri is the only one that does not support listing
            result.append((False, uri))
        else:
            # local file
            file_uri = uri if uri.startswith('file://') else 'file://' + uri
            result.append((True, file_uri))
    return result
