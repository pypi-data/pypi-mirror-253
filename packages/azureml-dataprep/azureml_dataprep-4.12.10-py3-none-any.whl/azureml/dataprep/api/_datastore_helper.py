...
def file_datastores_to_uris(data_sources: List[Datastore]) -> List[str]:
    ...
    for source in data_sources:
        ...
        datastore_uris.append("azureml://subscriptions/{}/resourcegroups/{}/workspaces/{}/datastores/{}/paths/{}"
                              .format(datastore_value.subscription, datastore_value.resource_group, datastore_value.workspace_name, datastore_value.datastore_name, datastore_value.path))
...
def _set_auth_from_dict(auth_dict):
    ...
    authority = auth_dict.get('authority')
    if authority:
        _try_update_auth(lambda: authority, auth, 'authority')
