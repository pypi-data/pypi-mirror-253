...
def generate_dimers(path: str = None, source: str = None, subfolders: bool = None, 
...
def generate_MD(path:str, source:str=None, subfolders:bool=False, output_path:str=None, 
                    plot:str=None, reference:str=None, 
    ...
    if plot.upper() == 'COUNT':
        ...
        if isinstance(conteiner_index,int): 
            PT.containers[conteiner_index].AtomPositionManager.count_species(specie='WATER')
        else:
            for conteiner_index, conteiner in enumerate(PT.containers):
                conteiner.AtomPositionManager.count_species(specie='WATER')

    if plot.upper() == 'DISPLACEMENTS':
        PT.handleMDAnalysis( values= {'displacements':{'reference':reference, 'verbose':verbose} }  )

    if plot.upper() == 'RBF':
        PT.handleMDAnalysis( values= {'RBF':{'verbose':verbose} }  )
...
def main():
    ...
    parser_MD.add_argument('--plot', type=str, required=True, choices=['count', 'displacements', 'RBF'], help='')
    ...
    if args.command == 'vacancy':
        ...
        generate_dimers(path=args.path,     source=args.source, 
        ...
        generate_MD(path=args.path, source=args.source, subfolders=args.subfolders, conteiner_index=args.conteiner_index,
            verbose=args.verbose, output_path=args.output_path, plot=args.plot, reference=args.reference)
