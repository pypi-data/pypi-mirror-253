...
class AlphaDatabase(AlphaDatabaseCore):
        ...
        while retries <= MAX_RETRIES:
            try:
                ...
                if retries < MAX_RETRIES:
                    ...
                    # Vous pouvez ajouter ici du code pour réinitialiser l'engine si nécessaire
                else:
                    if log is not None:
                        log.error(
                            "Nombre maximal de tentatives atteint. Abandon.", ex=ex
                        )
    ...
    def get_model_from_parameters(self, model, parameters, update):
        ...
        if not all_keys and update:
            ...
            obj = self.select(
                model,
                filters=[model.__dict__[x] == parameters[x] for x in keys_to_update],
                first=True,
            )
            if obj is not None:
