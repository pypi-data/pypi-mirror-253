...
class ImageCrop:
    ...
    def crop(self, slot_config, img):
        ...
        self.img = img
        # if slot_config[DATA_SLOT_ID] == '0a874208-0779-43ac-a9a4-e5baaa4face1' and '38924' in img.filename:
        ...
        if interest_region_crop.get('need_crop'):  # 是否需要兴趣区域裁剪
            ...
            if expand_process_logic == 1 and self.edge_judgment_logic(left_edge, right_edge, top_edge, bottom_edge):
                # 1人体及物体贴边；不扩边
    ...
    def edge_judgment_logic(self, left, right, top, bottom):
        # 产品逻辑 允许ai识别结果有 1%的误差。
        min_edge = 0
        max_edge = 0.01

        if min_edge <= left <= max_edge:
            return True
        if min_edge <= top <= max_edge:
            return True
        if min_edge <= 1 - right <= max_edge:
            return True
        if min_edge <= 1 - bottom <= max_edge:
            return True
