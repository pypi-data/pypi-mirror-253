...
class Sara2secStructLists(CustomAttribute):
	...
	@property
	def sara2_struct_list(self)->None:
		return self.parent.sara2_struct_list_db

	@sara2_struct_list.setter
	def sara2_struct_list(self, value:List[['Sara2SecondaryStructure', 'DesignPerformanceData', 'CLASS']]):
		if isinstance(value, list) == False:
			raise ValueError("Invalid value assignment")
		if len(value) < 1:
			raise Exception("Empty lists not allowed")
		...
		for item in value:
			if isinstance(item, ['Sara2SecondaryStructure', 'DesignPerformanceData', 'CLASS']) == False:
				raise ValueError("Invalid value assignment")
