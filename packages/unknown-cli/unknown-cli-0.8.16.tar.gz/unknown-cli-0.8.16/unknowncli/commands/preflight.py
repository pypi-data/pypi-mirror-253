...
def get_script_path(script_file_name):
    main_root = "s:/sn2-main/BuildScripts"
    main_path = f"{main_root}/{script_file_name}"
    if os.path.isfile(main_path):
        return main_path
    if p4_client:
        p4_path = f"{get_client_root(p4_client)}/BuildScripts/{script_file_name}"
        if os.path.isfile(p4_path):
            return p4_path
    client_root = p4.run_info()[0]["clientRoot"]
    if client_root:
        p4_path = f"{client_root}/BuildScripts/{script_file_name}"
        if os.path.isfile(p4_path):
            return p4_path
...
def is_build_success(build_json):
    ...
    return all([s.type == BuildStatus.SUCCESS for s in parse_status_list(build_json)])
