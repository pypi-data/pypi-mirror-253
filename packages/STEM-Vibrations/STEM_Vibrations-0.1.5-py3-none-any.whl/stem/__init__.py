...
def install_external_dependency():
    ...
    if not check_package_version('gmsh_utils', '1.0'):
        subprocess.check_call(['pip', 'install', "git+https://github.com/StemVibrations/gmsh_utils@v1.0"])
    # check if RandFields is installed and it is version 1.0
    if not check_package_version('random_fields', '1.0'):
        subprocess.check_call(['pip', 'install', "git+https://github.com/StemVibrations/RandomFields@v1.0"])


def check_package_version(package_name: str, target_version: str):
    """
