...
from arena.config import AUTH_TOKEN_DIR, HOST_URL_FILE_PATH


@click.group(invoke_without_command=True)
@click.option(
    "-sh", "--set-host", help="Set the Host URL of the Arena Instance."
)
def host(set_host):
    """
        ...
        if not os.path.exists(HOST_URL_FILE_PATH):
            echo(
                style(
                    ...
                    "The CLI would be using https://arena.ai as the default url.",
                    bold=True,
                )
            )
        else:
            with open(HOST_URL_FILE_PATH, "r") as fr:
                try:
                    ...
                    echo(
                        style(
                            "{} is the Host URL of Arena.".format(data),
                            bold=True,
                        )
                    )
                except (OSError, IOError) as e:
