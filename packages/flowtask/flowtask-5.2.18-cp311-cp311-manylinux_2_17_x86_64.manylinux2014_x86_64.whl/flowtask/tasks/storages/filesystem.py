...
from flowtask.parsers import JSONParser, TOMLParser, YAMLParser
...
class FileTaskStorage(AbstractTaskStorage):
    ...
    def __init__(self, path: PurePath, *args, **kwargs):
        ...
        if not path:
            ## Default Task Path
            raise FlowTaskError("Required Task Path for Filesystem Task Storage")
        else:
            self.path = path
    ...
    async def open_task(
        self, taskname: str, program: str, **kwargs
    ) -> Union[dict, str]:
        """open_task.
        ...
        if not taskpath.exists():
            raise TaskNotFound(f"FlowTask: Task Path not found: {taskpath}")
        ext = None
        for f in (
            "json",
            "yaml",
