...
import imaplib
import aiofiles
from ..conf import IMAP_RETRY_SELECT
...
class DownloadFromIMAP(IMAPClient, DownloadFromBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        IMAPClient.__init__(self, **params)
    ...
    def start(self, **kwargs):  # pylint: disable=W0613
        if hasattr(self, "attachments"):
            ...
            try:
                ...
                self._logger.error(f"IMAP: Error creating directory: {err}")
                raise ComponentError(f"IMAP: Error creating directory: {err}") from err
            # masking elements:
    ...
    async def run(self):
        ...
        try:
            ...
            if result == "NO" or result == "BAD":
                ...
                raise ComponentError(message=f"Error on Search: {msgs[0]}", code=500)
