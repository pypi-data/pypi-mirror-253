...
print('Software de visualizacao de arquivos de STS Nanosurf v1.3.6. Arquivos tipo csv (x,y,z) ')
    ...
    if filenames!=None:
        ...
        if conca == False:
            for i in range(len(files_mult[0][0])):
                ...
                printProgressBar(i,len(files_mult[0][0])-1)
...
def to_table(path_files,files_df):
    ...
    df2 = df2.set_index('V')
    df3 = df3.set_index('V')
    df2.to_csv(path_files+'Dtaaframe_I_limeted_by_V.csv')
...
def Display_MetalandMolecule():
        #file = load_file(folder)
        file = files_data
        def plot_curve(file,curve = 0,smooth = 5,xlim=(0,1),resolution=0.010):
                ...
                global delta_glob
                global selec 
                curve_glob = curve; smooth_glob = smooth; 
                xmin,xmax = deltaSlider.value
                ...
                ax[0].plot(x,y,label = 'Integral: '+str(I))
                ax[0].plot(x_cut,x_cut*a+b,c='red',label = 'a: ' + str(a) + ' b: '+str(b))
                ax[0].set_xlabel('Sample bias (V)')
                ax[0].set_ylabel('Current (nA)')
                ax[0].legend()
                ...
                ax[1].plot(dx,dy,label = 'Int neg: '+str(I_neg)+' Int pos: '+str(I_pos))
                ax[1].plot(x_cut_neg,a_neg*x_cut_neg+b_neg,c = 'red',label = 'a_neg: ' + str(a_neg) + ' b_neg: '+str(b_neg))
                ax[1].plot(x_cut_pos,a_pos*x_cut_pos+b_pos,c = 'black',label = 'a_pos: ' + str(a_pos) + ' b_pos: '+str(b_pos))
        ...
        deltaSlider=widgets.FloatRangeSlider(
    ...
    readout_format='.1f',layout=widgets.Layout(width='400px')
        ...
        valores = deltaSlider.value       
        widgets.interact(plot_curve, file = widgets.fixed(file),curve=curve_slider, smooth = smoothingSlider,xlim =deltaSlider,resolution = resolutionSlider)
        #print(curve_glob,smooth_glob,delta_glob)


def select_sts(path_file, n = [],smooth = 5,delta = 10,resolution = 0.01,save_um_file = False,metal = False,molecula = False,xmin_hist=-1,xmax_hist=1):
            global hist_path
            global hist_path_metal
            global hist_path_molecula
            ...
            if metal:
                ...
                for i in range(len(file[0])):                        
                        ...
                        a_pos,b_pos = regression(x_cut_pos,y_cut_pos)
                        a_pos = round(a_pos,3); b_pos = round(b_pos,3)    
                        hist['a_neg'].append(a_neg)
                        hist['a_pos'].append(a_pos)
                df = DataFrame(hist)
            ...
            if molecula:
                ...
                hist_path_metal = paste+'/'+'histogram_molecule.csv'
            to_table(paste+'/',files_df)
...
def Save_data():
    ...
    def save_files(list_n,salvar = False,smooth = 5,delta = 5,resolution=0.01,save_um_file = False,metal= False, molecula = False,rang=(-1,1)):
    ...
    widgets.interact(save_files, list_n = list_n,salvar = save_buttom,smooth = smoothingSlider,delta = deltaSlider,
                     resolution = resolutionSlider,save_um_file = slidersave,metal = metal,molecula = molecula,rang = xlim)
