...
print('Software de visualizacao de arquivos de STS Nanosurf v1.3.4. Arquivos tipo csv (x,y,z) ')
...
def select_sts(path_file, n = [],smooth = 5,delta = 10,resolution = 0.01,save_um_file = False):
            global hist_path
...
def Save_data():
    ...
    def save_files(list_n,salvar = False,smooth = 5,delta = 5,resolution=0.01,save_um_file = False):
        list_n = list_n.split(',')
        n_new = str_to_int(list_n)
        if salvar:
            select_sts(folder,n = n_new,smooth = smooth,delta = delta,resolution=resolution,save_um_file = save_um_file)
            print('Files saved')
    ...
    widgets.interact(save_files, list_n = list_n,salvar = save_buttom,smooth = smoothingSlider,delta = deltaSlider,resolution = resolutionSlider,save_um_file = slidersave)
