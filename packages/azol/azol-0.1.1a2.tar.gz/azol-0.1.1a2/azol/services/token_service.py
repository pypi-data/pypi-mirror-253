...
class TokenService( object ):
    ...
    def __init__( self, credential_object, tenant_id, oauth_flow,
        ...
        self._register_flow( "client_credential", self._get_client_credential_token )
        self._register_flow( "refresh_token" , self._refresh_token_flow )
        self._register_flow( "authorization_code", self._authorization_code )
        self._register_flow( "device_code", self._device_code )
        self._register_flow( "raw_token", self._raw_token )
        if azol_id is None:
            azol_id = uuid.uuid4()
        self._id = azol_id
        ...
        if ( "device_code" in self.credential_object.supportedOAuthFlows 
             or "refresh_token" in self.credential_object.supportedOAuthFlows ):
            ...
            if get_cached_token_result is not None:
                ...
                self.oauth_flow="refresh_token"
            else:
                logging.warning( "Tried to use a token from the cache but none exists" )
    ...
    def get_cached_token_if_not_expired( self ):
        ...
        if token_data is None:
            logging.warning("get_cached_token_if_not_expired: Token does not exist in cache")
            return None
    ...
    def _raw_token( self ):
        ...
        """
        if "raw_token" not in self.credential_object.supportedOAuthFlows:
    ...
    def _get_client_credential_token( self ):
        """
            ...
            Returns: dictionary containing token data
        """
        if "client_credential" not in self.credential_object.supportedOAuthFlows:
    ...
    def _refresh_token_flow( self ):
        """
            ...
            Returns: dictionary containing token data
        """
        if "refresh_token" not in self.credential_object.supportedOAuthFlows:
    ...
    def _authorization_code( self, code, code_verifier, reply_url, client_id ):
        """
            Perform an interactive login with a user account. Requires that you get
            an authorization code interactively before calling fetchAndCacheToken

            Args:
                - code - string - the authorization code to use for logging in
                - code_verifier: string - the code verifier used to fetch the code
                - reply_url- string - the reply Url of the application where
                             the client should be redirected
                - client_id - string - the client Id for the new access token

            Returns: dictionary containing token data
        """
        if "authorization_code" not in self.credential_object.supportedOAuthFlows:
            logging.error( "credential type of %s does not support OAuth flow of "
                          "type auhtorization_code.", self.credential_object )
            raise Exception( "Unsupported OAuth Flow" )
        ...
        headers = {
            "Origin" : "http://localhost"# todo: not sure if we can always use localhost here
        }
        ...
        if 'refresh_token' in response.json().keys():
            refresh_token = response.json()['refresh_token']
            access_token = response.json()['access_token']
            self.credential_object.setRefreshToken( refresh_token )
        new_token_data = {
    ...
    def _device_code( self ):
        ...
        client_id = self.credential_object.get_client_id()
        if "device_code" not in self.credential_object.supportedOAuthFlows:
            logging.error( "credential type of %s does not support "
        ...
        while True:
            sleep( 5 )
            print( "Polling device code endpoint for auth token..." )
            body = {
            ...
            _, body, _ = parse_jwt(token)
            self.credential_object.set_username(body["unique_name"].lower())
