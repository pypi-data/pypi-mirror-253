...
from .type_def import FrozenKeys, NoneType, DefFactory, T, M, S, DD, LSQ, N, NT
...
class LiteralParser(AbstractParser):
    ...
    # noinspection PyDataclass
    def __post_init__(self, *_):
        self.value_to_type = {val: type(val) for val in get_args(self.base_type)}
    ...
    def __call__(self, o: Any):
        ...
        try:
            ...
            if type_does_not_match:
                expected_val = next(
                    v for v in self.value_to_type if v == o
                )  # pragma: no branch
                e = TypeError("Value did not match expected type for the Literal")
...
class OptionalParser(AbstractParser):
    ...
    def __post_init__(self, cls: Type, extras: Extras, get_parser: GetParserType):
...
class UnionParser(AbstractParser):
    ...
    def __post_init__(self, cls: Type, extras: Extras, get_parser: GetParserType):
        ...
        self.parsers = tuple(
            get_parser(t, cls, extras) for t in self.base_type if t is not NoneType
        )
        ...
        try:
            ...
            try:
                ...
                raise ParseError(
                    ...
                    o,
                    [p.base_type for p in self.parsers],
                    input_tag=tag,
    ...
    def __call__(self, o: Iterable) -> LSQ:
        ...
        # TODO
            ...
            if not isinstance(o, self.base_type):
                ...
                raise ParseError(e, o, self.base_type, desired_type=self.base_type)
            else:
                raise
...
class TupleParser(AbstractParser):
    ...
    __slots__ = ("hook", "elem_parsers", "total_count", "required_count")
    ...
    def __post_init__(self, cls: Type, extras: Extras, get_parser: GetParserType):
        ...
        self.elem_parsers = tuple(get_parser(t, cls, extras) for t in elem_types)
        ...
        self.required_count: int = len(
            tuple(p for p in self.elem_parsers if None not in p)
        )
    ...
    def __call__(self, o: M) -> M:
        ...
        if not self.required_count <= len(o) <= self.total_count:
            ...
            raise ParseError(
                e,
                o,
                [p.base_type for p in self.elem_parsers],
...
class VariadicTupleParser(TupleParser):
    ...
    def __post_init__(self, cls: Type, extras: Extras, get_parser: GetParserType):
        ...
        # Per PEP 484, ... can be use to define "arbitrary-length homogeneous
        # tuples," therefore the required count can always be 0
...
class NamedTupleParser(AbstractParser):
    __slots__ = ("hook", "field_to_parser", "field_parsers")

    base_type: Type[S]
    hook: Callable[[Any, Type[NT], Optional[FieldToParser], List[AbstractParser]], NT]
    get_parser: InitVar[GetParserType]

    def __post_init__(self, cls: Type, extras: Extras, get_parser: GetParserType):
        # Get the field annotations for the `NamedTuple` type
        type_anns: Dict[str, Type[T]] = get_named_tuple_field_types(self.base_type)

        self.field_to_parser: Optional[FieldToParser] = {
            f: get_parser(ftype, cls, extras) for f, ftype in type_anns.items()
        }
    ...
    def __call__(self, o: Any):
        ...
        """
        return self.hook(o, self.base_type, self.field_to_parser, self.field_parsers)


@dataclass
class NamedTupleUntypedParser(AbstractParser):
    __slots__ = ("hook", "dict_parser", "list_parser")
    ...
    def __call__(self, o: Any):
        ...
        un-annotated `namedtuple`)
        """
        return self.hook(o, self.base_type, self.dict_parser, self.list_parser)


@dataclass
class MappingParser(AbstractParser):
    __slots__ = ("hook", "key_parser", "val_parser")
...
class DefaultDictParser(MappingParser):
    ...
    base_type: Type[DD]
    hook: Callable[[Any, Type[DD], DefFactory, AbstractParser, AbstractParser], DD]
    ...
    def __call__(self, o: M) -> M:
        return self.hook(
            o, self.base_type, self.default_factory, self.key_parser, self.val_parser
        )


@dataclass
class TypedDictParser(AbstractParser):
    __slots__ = ("hook", "key_to_parser", "required_keys", "optional_keys")
    ...
    def __post_init__(self, cls: Type, extras: Extras, get_parser: GetParserType):
        ...
        self.required_keys, self.optional_keys = get_keys_for_typed_dict(self.base_type)
