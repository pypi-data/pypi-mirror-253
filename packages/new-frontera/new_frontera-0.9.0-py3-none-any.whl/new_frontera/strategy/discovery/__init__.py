...
from new_frontera.core.components import States, DomainMetadata
...
def is_domain_to_ignore(domain, max_pages):
    return (
        not is_accessible_domain(domain)
        or "banned" in domain
...
class Discovery(BaseCrawlingStrategy):
    def __init__(self, manager, args, mb_stream, states_context):
        ...
        try:
            ...
            self.logger.exception(
                "Please get the public suffix file from https://publicsuffix.org/"
            )
            raise
    ...
    def read_seeds(self, stream):
        ...
        for line in stream:
            ...
            try:
                request = self.create_request(
                    url, meta={b"home": True}, headers=DEFAULT_HEADERS
                )
                ...
                if len(requests) % 40000 == 0:
                    ...
                    self.logger.info(
                        "Processed %d, scheduled %d urls.", processed, scheduled
                    )
                    requests = []
            except Exception:
        ...
        if requests:
            ...
            processed += len(requests)
        self.logger.info(
            "Processed %d, and scheduled %d urls overall.", processed, scheduled
        )
    ...
    def process_seeds(self, seeds):
        ...
        for seed in seeds:
            ...
            meta = {b"netloc": parsed_url.netloc, b"seed": seed.url, b"robots": True}
            request = self.create_request(
                robots_url, meta=meta, headers=DEFAULT_HEADERS
            )
    ...
    def links_extracted(self, request, links):
        ...
        self.logger.debug(
            "LE %s (seed %s) %d extracted links",
            request.url,
            request.meta.get(b"seed"),
            len(links),
        )
    ...
    def request_error(self, request, error):
        ...
        self.logger.debug(
            "PE %s error: %s (seed: %s)", request.url, error, request.meta.get(b"seed")
        )
        self._log_redirects_if_defined(request)
    ...
    def _process_robots_txt(self, response, domain):
        ...
        try:
            body = response.body.decode(
                "utf-8"
            )  # TODO: use encoding from response.meta.get(b'encoding', 'utf-8')
        ...
        for line in robots_lines:
            if line.startswith("Sitemap:"):
                ...
                meta = {
                    b"seed": domain.get("seed"),
                    b"sitemap": True,
                    b"scrapy_meta": {b"download_maxsize": SITEMAP_DOWNLOAD_MAXSIZE},
                }
                requests.add(
                    self.create_request(sitemap_url, meta=meta, headers=DEFAULT_HEADERS)
                )
        self.refresh_states(requests)
        ...
        if not requests:
            self.logger.debug(
                "Sitemap in robots.txt wasn't found for url %s", response.url
            )
        update_domain_with_parser_data(
            domain, parser=parser, url=response.url, body=body
        )
    ...
    def _process_sitemap(self, netloc, body, domain):
                ...
                meta = (
                    {
                        b"seed": domain.get("seed"),
                        b"sitemap": True,
                        b"scrapy_meta": sitemap_scrapy_meta,
                    }
                    if sub_sitemap
                    else ({b"home": True} if is_home_page_url(url) else {})
                )
        ...
        if len(sitemaps) > MAX_SITEMAPS:
            ...
            self.logger.warning(
                "Amount of sub-sitemaps > %d for url %s", MAX_SITEMAPS, netloc
            )
    ...
    def _schedule_requests(self, requests, domain, score, count=True):
        ...
        for request in consume_randomly(requests):
            ...
            if count and domain["queued_pages"] >= self.max_pages:
                self.logger.debug(
                    "LIMIT REACHED pages (%d) for seed %s",
                    domain["queued_pages"],
                    domain["seed"],
                )
                break
    ...
    def _get_domain_after_redirects(self, request):
        ...
        if redirect_urls and (
            b"robots" in request.meta
