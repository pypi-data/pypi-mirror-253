...
from new_frontera.contrib.backends.sqlalchemy.models import DeclarativeBase
from new_frontera.core.components import DistributedBackend
...
class Distributed(DistributedBackend):
    def __init__(self, manager):
        ...
        self.models = dict(
            [(name, load_object(klass)) for name, klass in models.items()]
        )
    ...
    def _init_strategy_worker(self, manager):
        ...
        self.check_and_create_tables(
            drop_all_tables, clear_content, (model_states, model_dm)
        )
        self._states = States(
            self.session_cls, model_states, settings.get("STATE_CACHE_SIZE_LIMIT")
        )
    ...
    def _init_db_worker(self, manager):
        ...
        self._metadata = Metadata(
            self.session_cls, metadata_m, settings.get("SQLALCHEMYBACKEND_CACHE_SIZE")
        )
        self._queue = Queue(
            self.session_cls, queue_m, settings.get("SPIDER_FEED_PARTITIONS")
        )
