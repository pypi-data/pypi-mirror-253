...
from new_frontera import DistributedBackend
from new_frontera.core.components import Metadata, Queue, States
from new_frontera.core.models import Request
from new_frontera.contrib.backends.partitioners import Crc32NamePartitioner
from new_frontera.utils.misc import chunks, get_crc32, time_elapsed
from new_frontera.contrib.backends.remote.codecs.msgpack import Decoder, Encoder
from new_frontera.contrib.backends.hbase.domaincache import DomainCache
...
class HBaseQueue(Queue):
    ...
    def __init__(
        self, connection, partitions, table_name, drop=False, use_snappy=False
    ):
    ...
    def schedule(self, batch):
        ...
        for fprint, score, request, schedule in batch:
            if schedule:
                ...
                timestamp = (
                    request.meta[b"crawl_at"] if b"crawl_at" in request.meta else now
                )
    ...
    def _schedule(self, batch, timestamp):
        ...
        for request, score in batch:
            ...
            rk = "%d_%s_%d" % (
                partition_id,
                "%0.2f_%0.2f" % get_interval(score, 0.01),
                random_str,
            )
    ...
    def get_next_requests(self, max_n_requests, partition_id, **kwargs):
        ...
        while tries < self.GET_RETRIES:
            ...
            scan_gen = table.scan(
                limit=int(limit), batch_size=256, row_prefix=prefix, sorted_columns=True
            )
            try:
                    ...
                    for cq, buf in six.iteritems(data):
                        ...
                        for item in unpacker:
                            ...
                            if (
                                ...
                                and len(queue[key_crc32]) > max_requests_per_host
                            ):
                                continue
...
class HBaseState(States):
    def __init__(
        self, connection, table_name, cache_size_limit, write_log_size, drop_all_tables
    ):
        self.connection = connection
        self._table_name = to_bytes(table_name)
        self.logger = logging.getLogger("hbase.states")
        self._state_batch = self.connection.table(self._table_name).batch(
            batch_size=write_log_size
        )
        self._state_stats = defaultdict(int)
        self._state_cache = LRUCacheWithStats(
            maxsize=cache_size_limit, stats=self._state_stats
        )
    ...
    def update_cache(self, objs):
        ...
        for obj in objs:
            ...
            self._state_batch.put(
                unhexlify(fingerprint), prepare_hbase_object(state=state)
            )
    ...
    def set_states(self, objs):
        objs = objs if isinstance(objs, Iterable) else [objs]
        for obj in objs:
            obj.meta[b"state"] = self._state_cache.get(
                obj.meta[b"fingerprint"], States.DEFAULT
            )
    ...
    def fetch(self, fingerprints):
        ...
        self._update_cache_stats(
            hits=len(fingerprints) - len(to_fetch), misses=len(to_fetch)
        )
        if not to_fetch:
            return
        self.logger.debug(
            "Fetching %d/%d elements from HBase (cache size %d)",
            len(to_fetch),
            len(fingerprints),
...
class HBaseMetadata(Metadata):
    def __init__(
        ...
        if self._table_name not in tables:
            schema = {"m": {"max_versions": 1}, "c": {"max_versions": 1}}
            if use_snappy:
    ...
    def page_crawled(self, response):
        ...
        obj = (
            prepare_hbase_object(
                status_code=response.status_code, headers=headers, content=response.body
            )
            if self.store_content
            else prepare_hbase_object(status_code=response.status_code, headers=headers)
        )
    ...
    def links_extracted(self, request, links):
        ...
        for link_fingerprint, (link, link_url, link_domain) in six.iteritems(
            links_dict
        ):
            obj = prepare_hbase_object(
                url=link_url,
    ...
    def request_error(self, request, error):
        ...
        if b"redirect_urls" in request.meta:
            for url, fprint in zip(
                request.meta[b"redirect_urls"], request.meta[b"redirect_fingerprints"]
            ):
                obj = prepare_hbase_object(
                    url=url,
                    created_at=utcnow_timestamp(),
...
class HBaseBackend(DistributedBackend):
    ...
    def get_next_requests(self, max_next_requests, **kwargs):
        ...
        for partition_id in set(
            kwargs.pop("partitions", [i for i in range(self.queue_partitions)])
        ):
            ...
            self.logger.debug(
                "Got %d requests for partition id %d", len(requests), partition_id
            )
        return results
