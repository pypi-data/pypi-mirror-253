...
from new_frontera.utils.twisted_helpers import listen_tcp
...
class StatusResource(JsonResource):
    ...
    def render_GET(self, txrequest):
        ...
        disable_new_batches = (
            batches_disabled_event.is_set() if batches_disabled_event else None
        )
        return {
    ...
    def render_POST(self, txrequest):
        ...
        try:
            try:
                ...
                raise JsonRpcError(
                    500, "Error processing request: %s" % (str("").join(trace_lines))
                )
...
class JsonRpcService(server.Site):
    ...
    def start_listening(self):
        ...
        h = self.port.getHost()
        logger.info(
            "Web service listening on {host}:{port}".format(host=h.host, port=h.port)
        )
