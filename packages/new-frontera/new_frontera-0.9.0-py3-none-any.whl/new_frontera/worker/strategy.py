...
from new_frontera.core.manager import WorkerFrontierManager, MessageBusUpdateScoreStream
...
from new_frontera.utils.ossignal import install_shutdown_handlers
from new_frontera.worker.server import WorkerJsonRpcService
from new_frontera.worker.stats import StatsExportMixin
...
class BaseStrategyWorker(object):
    ...
    def add_seeds(self, seeds_url):
        ...
        if not seeds_url:
            ...
            if parsed.scheme == "s3":
                import boto3
                from new_frontera.utils.s3 import StreamingBodyIOBase
    ...
    def run(self, seeds_url):
        ...
        def run_flush_states_task():
            (
                self._flush_states_task.start(interval=self._flush_interval).addErrback(
                    errback_flush_states
                )
            )
        ...
        if self.add_seeds_mode:
            ...
            logger.info(
                "Starting flush-states task in %d seconds", flush_states_task_delay
            )
...
def setup_environment():
    parser = ArgumentParser(description="new_frontera strategy worker.")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
