...
from new_frontera.utils.misc import load_object, utc_timestamp

logger = getLogger("messagebus.stats")


class StatsExportMixin(object):
    """Extending new_frontera worker's logic by sending stats to message bus.
    ...
    def run(self, *args, **kwargs):
        def errback_export_stats(failure):
            ...
            self._export_stats_task.start(interval=self._stats_interval).addErrback(
                errback_export_stats
            )
    ...
    def get_stats(self):
        ...
        stats = {
            stats_key: self.stats[stats_key]
