...
class BatchGenerator(DBWorkerThreadComponent):
    ...
    def __init__(
        self, worker, settings, stop_event, no_batches=False, partitions=None, **kwargs
    ):
        ...
        self.domain_stats = dict(
            [(partition_id, defaultdict(int)) for partition_id in self.partitions]
        )
    ...
    def run(self):
        ...
        if not partitions:
            return True
        batch_count = sum(
            self._handle_partition(partition_id) for partition_id in partitions
        )
        if not batch_count:
            return True
        # let's count full batches in the same way as before
        self.update_stats(
            increments={"batches_after_start": 1},
            replacements={
    ...
    def _handle_partition(self, partition_id):
        ...
        count = 0
        for request in self.backend.get_next_requests(
            ...
            try:
                ...
                self.logger.error(
                    "Encoding error, %s, fingerprint: %s, url: %s"
            ...
            try:
                ...
                self.logger.exception(
                    "Sending message error fingerprint: %s, url: %s"
    ...
    def rotate_and_log_domain_stats(self):
        ...
        for partition_id, host_stats in sorted(
            self.domain_stats.items(), key=lambda x: x[0]
        ):
            self.logger.debug(
                "PID %d =================================================================",
                partition_id,
