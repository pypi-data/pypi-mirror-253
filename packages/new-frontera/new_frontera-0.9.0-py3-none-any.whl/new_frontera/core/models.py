...
class Request(FrontierObject):
    """
    A :class:`Request <new_frontera.core.models.Request>` object represents an HTTP request, which is generated for
    seeds, extracted page links and next pages to crawl. Each one should be associated to a
    :class:`Response <new_frontera.core.models.Response>` object when crawled.
    ...
    def meta(self):
        ...
        populated by different new_frontera components (middlewares, etc). So the data contained in this dict depends
    ...
    def __str__(self):
        return "<%s at 0x%0x %s meta=%s body=%s... cookies=%s, headers=%s>" % (
            type(self).__name__,
            id(self),
            self.url,
            str(self.meta),
            str(self.body[:20]) if self.body is not None else None,
            str(self.cookies),
            str(self.headers),
        )
...
class Response(FrontierObject):
    """
    A :class:`Response <new_frontera.core.models.Response>` object represents an HTTP response, which is usually
    ...
    def request(self):
        """
        The :class:`Request <new_frontera.core.models.Request>` object that generated this response.
        """
    ...
    def meta(self):
        """
        A shortcut to the :attr:`Request.meta <new_frontera.core.models.Request.meta>` attribute of the
        :attr:`Response.request <new_frontera.core.models.Response.request>` object (ie. self.request.meta).
        """
        try:
    ...
    def __str__(self):
        return "<%s at 0x%0x %s %s meta=%s body=%s... headers=%s>" % (
            type(self).__name__,
            id(self),
            self.status_code,
            self.url,
