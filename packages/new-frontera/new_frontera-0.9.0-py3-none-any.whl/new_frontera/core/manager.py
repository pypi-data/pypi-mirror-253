...
from new_frontera.core import models
...
from new_frontera.utils.misc import load_object
...
class BackendMixin(object):
    ...
    def _load_backend(self, backend, db_worker, strategy_worker):
        ...
        assert issubclass(cls, Backend), (
            "backend '%s' must subclass Backend" % cls.__name__
        )
    ...
    def backend(self):
        """
        The :class:`Backend <new_frontera.core.components.Backend>` object to be used by the frontier. \
...
class StrategyMixin(object):
    def __init__(self, strategy_class, strategy_args, scoring_stream):
        self._scoring_stream = (
            scoring_stream
            if scoring_stream
            else LocalUpdateScoreStream(self.backend.queue)
        )
        ...
        self._strategy = strategy_class.from_worker(
            self, strategy_args, self._scoring_stream, self._states_context
        )
...
class ComponentsPipelineMixin(BackendMixin):
    def __init__(
        ...
        if canonicalsolver:
            ...
            assert isinstance(self.canonicalsolver, CanonicalSolver), (
                "canonical solver '%s' must subclass CanonicalSolver"
                % self.canonicalsolver.__class__.__name__
            )
    ...
    def middlewares(self):
        """
        A list of :class:`Middleware <new_frontera.core.components.Middleware>` objects to be used by the frontier. \
        ...
        for mw_name in middleware_names or []:
            ...
            try:
                ...
                assert isinstance(mw, Middleware), (
                    "middleware '%s' must subclass Middleware" % mw.__class__.__name__
                )
                if mw:
                    mws.append(mw)
    ...
    def _process_components(
        self, method_name, obj=None, return_classes=None, components=None, **kwargs
    ):
        pipeline = (
            self._components_pipeline
            if components is None
            else [self._components_pipeline[c] for c in components]
        )
        return_obj = obj
    ...
    def _process_component(
        ...
        assert return_obj is None or isinstance(
            return_obj, return_classes
...
class StrategyComponentsPipelineMixin(ComponentsPipelineMixin, StrategyMixin):
    def __init__(
        self, backend, strategy_class, strategy_args, scoring_stream, **kwargs
    ):
...
class BaseContext(object):
    def __init__(self, request_model, response_model, settings=None):
        ...
        assert issubclass(self._request_model, models.Request), (
            "Request model '%s' must subclass 'Request'" % self._request_model.__name__
        )

        # Load response model
        self._response_model = load_object(response_model)
        assert issubclass(self._response_model, models.Response), (
            "Response model '%s' must subclass 'Response'"
    ...
    def request_model(self):
        """
        The :class:`Request <new_frontera.core.models.Request>` object to be used by the frontier. \
    ...
    def response_model(self):
        """
        The :class:`Response <new_frontera.core.models.Response>` object to be used by the frontier. \
    ...
    def settings(self):
        """
        The :class:`Settings <new_frontera.settings.Settings>` object used by the frontier.
        """
...
class BaseManager(object):
    def get_next_requests(self, max_next_requests=0, **kwargs):
        ...
        value is passed, \
        :attr:`FrontierManager.max_next_requests <new_frontera.core.manager.FrontierManager.max_next_requests>`
        ...
        :return: list of :class:`Request <new_frontera.core.models.Request>` objects.
        """

        # log (in)
        self._logger.debug(
            "GET_NEXT_REQUESTS(in) max_next_requests=%s", max_next_requests
        )
        ...
        # log (out)
        self._logger.debug(
            "GET_NEXT_REQUESTS(out) returned_requests=%s", len(next_requests)
        )
    ...
    def page_crawled(self, response):
        ...
        :param object response: The :class:`Response <new_frontera.core.models.Response>` object for the crawled page.
    ...
    def links_extracted(self, request, links):
        ...
        :param object request: The :class:`Request <new_frontera.core.models.Request>` object from which the links where crawled.
        :param list links: A list of :class:`Request <new_frontera.core.models.Request>` objects generated from the links \
...
class LocalFrontierManager(BaseContext, StrategyComponentsPipelineMixin, BaseManager):
    """
    The :class:`FrontierManager <new_frontera.core.manager.FrontierManager>` object encapsulates the whole frontier,
    ...
    def __init__(
        ...
        """
        :param object/string request_model: The :class:`Request <new_frontera.core.models.Request>` object to be \
            used by the frontier.

        :param object/string response_model: The :class:`Response <new_frontera.core.models.Response>` object to be \
            used by the frontier.

        :param object/string backend: The :class:`Backend <new_frontera.core.components.Backend>` object to be \
            used by the frontier.

        :param list middlewares: A list of :class:`Middleware <new_frontera.core.components.Middleware>` \
        ...
        :param object/string settings: The :class:`Settings <new_frontera.settings.Settings>` object used by \
            the frontier.

        :param object/string canonicalsolver: The :class:`CanonicalSolver <new_frontera.core.components.CanonicalSolver>`
        ...
        StrategyComponentsPipelineMixin.__init__(
            self,
            backend,
            strategy_class,
            strategy_args,
    ...
    def from_settings(cls, settings=None, db_worker=False, strategy_worker=False):
        """
        Returns a :class:`FrontierManager <new_frontera.core.manager.FrontierManager>`  instance initialized with \
    ...
    def get_next_requests(self, max_next_requests=0, **kwargs):
        ...
        # get next requests
        next_requests = super(LocalFrontierManager, self).get_next_requests(
            max_next_requests, **kwargs
        )
    ...
    def page_crawled(self, response):
        ...
        ), "Response object must subclass '%s', '%s' found" % (
            self.response_model.__name__,
            type(response).__name__,
        )
        assert (
            hasattr(response, "request") and response.request
        ), "Empty response request"
        assert isinstance(
            response.request, self.request_model
        ), "Response request object must subclass '%s', " "'%s' found" % (
    ...
    def links_extracted(self, request, links):
        ...
        ), "Request object must subclass '%s', '%s' found" % (
            self.request_model.__name__,
            type(request).__name__,
        )
        for link in links:
            assert isinstance(
                link, self._request_model
            ), "Link objects must subclass '%s', '%s' found" % (
    ...
    def request_error(self, request, error):
        ...
        :param object request: The crawled with error :class:`Request <new_frontera.core.models.Request>` object.
    ...
    def create_request(
        ...
        :param meta: dict
        :param body: bytes
        :return: :class:`Request <new_frontera.core.models.Request>` object
        """
        r = self.request_model(
...
class WorkerFrontierManager(BaseContext, StrategyComponentsPipelineMixin):
    """
    The :class:`WorkerFrontierManager <new_frontera.core.manager.WorkerFrontierManager>` class role is to
    ...
    def __init__(
        ...
        :param object/string canonicalsolver: The :class:`CanonicalSolver <new_frontera.core.components.CanonicalSolver>`
            object to be used by frontier.
        :param object scoring_stream: Instance of :class:`UpdateScoreStream <new_frontera.core.manager.UpdateScoreStream>`
        ...
        if db_worker:
            ComponentsPipelineMixin.__init__(
                self, backend, db_worker=db_worker, strategy_worker=strategy_worker
            )
    ...
    @classmethod
    def from_settings(
        cls, settings=None, db_worker=False, strategy_worker=False, scoring_stream=None
    ):
        manager_settings = Settings.object_from(settings)
        kwargs = {
            "request_model": manager_settings.REQUEST_MODEL,
...
class SpiderFrontierManager(BaseContext, ComponentsPipelineMixin, BaseManager):
    ...
    def get_next_requests(self, max_next_requests=0, **kwargs):
        return super(SpiderFrontierManager, self).get_next_requests(
            max_next_requests=max_next_requests or self.max_next_requests, **kwargs
        )
...
class MessageBusUpdateScoreStream(UpdateScoreStream):
    ...
    def send(self, request, score=1.0, dont_queue=False):
        encoded = self._encoder.encode_update_score(
            request=request, score=score, schedule=not dont_queue
        )
...
class LocalUpdateScoreStream(UpdateScoreStream):
    ...
    def send(self, request, score=1.0, dont_queue=False):
        self._queue.schedule(
            [(request.meta[b"fingerprint"], score, request, not dont_queue)]
        )
...
class StatesContext(object):
    ...
    def refresh_and_keep(self, requests):
        ...
        self._requests.extend(
            requests if isinstance(requests, Iterable) else [requests]
        )
