...
from new_frontera.utils.url import parse_domain_from_url_fast
...
from new_frontera.utils.misc import get_crc32, load_object
import functools
import logging
        ...
        while True:
            try:
                ...
                self._logger.exception(
                    "Connection to Redis failed when executing pipeline"
                )
                pause = timeout.next()
                if pause is None:
                    break
...
class RedisQueue(Queue):
    ...
    def _get_items(
        ...
        for data in self._redis.zrevrange(
            partition_id, start=start, end=max_n_requests + start
        ):
            start += 1
    ...
    def get_next_requests(self, max_n_requests, partition_id, **kwargs):
        ...
        self._logger.debug(
            "Finished: hosts {}, requests {}".format(len(queue.keys()), count)
        )
    ...
    def schedule(self, batch):
        ...
        for fprint, score, request, schedule in batch:
            if schedule:
                ...
                timestamp = (
                    request.meta[FIELD_CRAWL_AT]
                    if FIELD_CRAWL_AT in request.meta
                    else now
                )
    ...
    def _schedule(self, batch, timestamp):
        ...
        for request, score in batch:
            ...
            item = (
                ...
                fingerprint,
                host_crc32,
                self._encoder.encode_request(request),
                score,
            )
    ...
    def count(self):
        return sum(
            [self._redis.zcard(partition_id) for partition_id in self._partitions]
        )
...
class RedisState(States):
    ...
    def set_states(self, objs):
        ...
        def get(obj):
            ...
            obj.meta[FIELD_STATE] = (
                self._cache[fprint] if fprint in self._cache else States.DEFAULT
            )
    ...
    def flush(self, force_clear=False):
        if len(self._cache) > self._cache_size_limit:
            force_clear = True
        [
            self._redis_pipeline.hmset(fprint, {FIELD_STATE: state})
...
class RedisMetadata(Metadata):
    ...
    def add_seeds(self, seeds):
        [
            self._redis_pipeline.hmset(
                seed.meta[FIELD_FINGERPRINT], self._create_seed(seed)
            )
            for seed in seeds
        ]
        self._redis_pipeline.execute()
    ...
    def request_error(self, page, error):
        self._redis.hmset(
            page.meta[FIELD_FINGERPRINT], self._create_request_error(page, error)
        )

    @staticmethod
    def _create_crawl_info(response):
        return {FIELD_STATUS_CODE: response.status_code}

    def page_crawled(self, response):
        self._redis.hmset(
            response.meta[FIELD_FINGERPRINT], self._create_crawl_info(response)
        )
...
class RedisBackend(DistributedBackend):
    ...
    def __init__(self, manager):
        ...
        self._logger.info(
            "RedisBackend started with {} partitions".format(self.queue_partitions)
        )
    ...
    def _init(self, manager, typ="all"):
        ...
        if typ in ["strategy_worker", "all"]:
            self._states = RedisState(
                self.pool, settings.get("REDIS_STATE_CACHE_SIZE_LIMIT")
            )
        if typ in ["db_worker", "all"]:
            clear = settings.get("REDIS_DROP_ALL_TABLES")
            self._queue = RedisQueue(
                manager, self.pool, self.queue_partitions, delete_all_keys=clear
            )
            self._metadata = RedisMetadata(self.pool, clear)
