...
from new_frontera.contrib.backends.sqlalchemy.models import (
    DeclarativeBase,
    DomainMetadataModel as DomainMetadataKV,
)
from new_frontera.core.components import (
    Metadata as BaseMetadata,
    Queue as BaseQueue,
    DomainMetadata as BaseDomainMetadata,
)
from new_frontera.core.models import Request, Response
from new_frontera.utils.misc import get_crc32, chunks
...
class DomainMetadata(BaseDomainMetadata):
    ...
    def __getitem__(self, key):
        result = (
            self.session.query(DomainMetadataKV)
            .filter(DomainMetadataKV.key == key)
            .first()
        )
        if result is None:
            raise KeyError
    ...
    def __contains__(self, key):
        result = (
            self.session.query(DomainMetadataKV.key)
            .filter(DomainMetadataKV.key == key)
            .first()
        )
    ...
    def __delitem__(self, key):
        self.session.query(DomainMetadataKV).filter(DomainMetadataKV.key == key).delete(
