...
class Server(object):
    ...
    def __init__(self, address, base_port):
        ...
        self.logger = logging.getLogger(
            "distributed_new_frontera.messagebus" ".zeromq.broker.Server"
        )
        self.logger.info(
            "Using socket: {}:{}".format(socket_config.ip_addr, socket_config.base_port)
        )

    def start(self):
        self.logger.info("Distributed new_frontera ZeroMQ broker is started.")
        self.log_stats()
        try:
            self.loop.start()
