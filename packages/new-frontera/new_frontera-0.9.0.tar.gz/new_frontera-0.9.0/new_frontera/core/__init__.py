...
def get_slot_key(request, type):  # TODO: Probably use caching here
    ...
    statistics and in backend for distinguishing free and overloaded resources. This method used in all new_frontera
    backends.

    :param object request: is the instance of :class:`Request <new_frontera.core.models.Request>`.
...
class OverusedBuffer(object):
    """
    A buffering object for implementing the buffer of new_frontera requests for overused domains/ips. It can be used
    ...
    def _check_and_purge(self, key):
        ...
        if self._max_per_key is not None and len(pending) > self._max_per_key:
            self._log.warning(
                "Purging of key %s, of size %d has started", key, len(pending)
            )
            purged = 0
