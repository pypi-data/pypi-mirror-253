...
from new_frontera.core.manager import WorkerFrontierManager
...
class DBWorker(StatsExportMixin, BaseDBWorker):
    ...
    def get_stats_tags(self, settings, no_batches, no_incoming, no_scoring, **kwargs):
        if no_batches and no_scoring:
            ...
            logger.warning(
                "Can't identify DB worker type "
                "(no-scoring {}, no-batches {}, no-incoming {})".format(
                    no_scoring, no_batches, no_incoming
                )
            )
            db_worker_type = "none"
...
if __name__ == "__main__":
    parser = ArgumentParser(description="new_frontera DB worker.")
    parser.add_argument(
