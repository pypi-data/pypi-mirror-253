...
"""
        ...
        while i < n:
            ...
            for q in self.qlist:
                ...
                if not self.show_label:
                    label = ""
                latex_text += (
                    f'{label} {"".join(qcircuit[q][i:j])} & \qw \\\\\n'
                )
            latex_text += f'{"".join(["&"] * m)} \\\\\n'
        ...
        if self.split_gate_group:
            ...
            for a, b, c, d in self.split_gate_group:
                y_offset = (b // self.max_circuit_depth) * (
                    len(self.qlist) + 1
                ) + 1
...
def normalize(isq_ir):
    ...
    if ir_item[0] in single_gate:
        if ir_item[0] == "M":
            return Operation(
                gate=ir_item[0], targets=[qid1], isMeasurement=True
            )
...
class Drawer:
    ...
    def set_figure(self):
        self.fg = plt.figure(
            figsize=(self.figure_width / 2, self.figure_height / 2)
        )
    ...
    def add_continuous_gate(self, opt, seq_t):
        ...
        if len(opt.controls) > 0 and opt.gate == "X":
            ...
            box_length = (
                self.qmap[opt.targets[-1]]
                - self.qmap[opt.targets[0]]
                + self.box_length
            )
