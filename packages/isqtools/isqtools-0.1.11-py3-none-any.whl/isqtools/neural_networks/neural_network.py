...
class NeuralNetwork(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            input_gradients: Determines whether to compute gradients with
    ...
    def input_gradients(self) -> bool:
        """Returns whether gradients with respect to input data are computed by
        this neural network in the ``backward`` method or not. By default such
    ...
    def _validate_output_shape(self, output_shape):
        ...
        if not np.all([s > 0 for s in output_shape]):
            raise NeuralNetworkError(
                f"Invalid output shape, all components must be > 0, "
                f"but got: {output_shape}."
            )
        return output_shape
    ...
    def _preprocess_forward(
        ...
        """Pre-processing during forward pass of the network for the
        primitive-based networks.
        """
    ...
    def forward(
        ...
        Args:
            input_data: input data of the shape (num_inputs). In case of a
                single scalar input it is directly cast to and interpreted like
                a one-element array.
            weights: trainable weights of the shape (num_weights). In case of a
                single scalar weightit is directly cast to and interpreted like
                a one-element array.
    ...
    def backward(
        ...
        np.ndarray | SparseArray | None, np.ndarray | SparseArray | None
    ]:
        input_, shape = self._validate_input(input_data)
        weights_ = self._validate_weights(weights)
