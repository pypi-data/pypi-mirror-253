...
import { LegacyQiSession } from './qimessaging';
import { IQiSession } from './iqisession'
import { WebsocketQiSession } from './wsqimessaging'
...
export class NaoRobotModel extends DOMWidgetModel {
  ...
  async connect(ipAddress: string, requestID: number, authToken: string) {
    ...
    if (useLegacyQiSession) {
      ...
      this.qiSession = new LegacyQiSession(ipAddress);
    } else {
      this.qiSession = new WebsocketQiSession(ipAddress, authToken);
    }
  ...
  private async checkConnection(requestID: number) {
    ...
    if (!this.qiSession.isConnected()) {
      this.disconnect();
      await this.connect(this._ipAddress, requestID, "");
    }
    return true;
  }
