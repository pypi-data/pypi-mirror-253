...
def init_job(result_map: Dict[str, List], spider_ids: List[str], channel_code: str) -> (Dict[str, List[StreamTaskItem]], List[dict], str):
    for spider_id in spider_ids:
        ...
        if not spider_conf:
            LOGGER.warning(BizException(f'[初始化队列]{spider_id}爬虫配置表zcbot_stream_spider异常或不存在'))
            continue
        else:
            # 初始化队列
...
def _init_redis_batch(redis_key, rows: List[StreamTaskItem], batch_size):
    ...
    for row in rows:
        ...
        if len(batch_list) >= batch_size:
            ...
            if row.callback:
                batch_row_data["callback"] = row.callback
            if row.itemConfig:
                batch_row_data["itemConfig"] = row.itemConfig
            # 任务入队
