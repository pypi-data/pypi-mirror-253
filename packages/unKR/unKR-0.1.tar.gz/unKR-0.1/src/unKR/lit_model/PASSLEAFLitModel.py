...
class PASSLEAFLitModel(BaseLitModel):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # ranks_link_predict_high_filter = link_predict_filter_high_filter(batch, self.model, confidence, prediction=prediction)
        ...
        # results["hf_count_for_link"] = torch.numel(ranks_link_predict_high_filter)
        # results["hf_mrr"] = torch.sum(1.0 / ranks_link_predict_high_filter).item()
        # results["hf_mr"] = torch.sum(ranks_link_predict_high_filter).item()
        # for k in self.args.calc_hits:
        #     results['hf_hits@{}'.format(k)] = torch.numel(ranks_link_predict_high_filter[ranks_link_predict_high_filter <= k])
        ...
        """calculate nDCG"""
        # ndcg, exp_ndcg = mean_ndcg(batch["hr_map"], self.model, self.args.num_ent)
        # print("----valid-----ndcg, exp_ndcg:", ndcg, exp_ndcg)
        # results["ndcg"] = ndcg
        # results["exp_ndcg"] = exp_ndcg
    ...
    def test_step(self, batch, batch_idx):
        ...
        """calculate nDCG"""
        # ndcg, exp_ndcg = mean_ndcg(batch["hr_map"], self.model, self.args.num_ent)
        # print("----test-----ndcg, exp_ndcg:", ndcg, exp_ndcg)
        # results["ndcg"] = ndcg
        # results["exp_ndcg"] = exp_ndcg
