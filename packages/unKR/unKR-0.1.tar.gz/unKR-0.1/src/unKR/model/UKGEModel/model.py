...
class Model(nn.Module):
    ...
    def tri2emb(self, triples, negs=None, mode="single"):
        ...
        #     else:
        #         head_emb = self.ent_emb(negs[:, 0]).unsqueeze(1)  # [bs, 1, dim]
        #         relation_emb = self.rel_emb(negs[:, 1]).unsqueeze(1)  # [bs, 1, dim]
        #         tail_emb = self.ent_emb(negs[:, 2]).unsqueeze(1)  # [bs, 1, dim]
        #TODO (ZST：此处注释部分为我调试修改所使用)
