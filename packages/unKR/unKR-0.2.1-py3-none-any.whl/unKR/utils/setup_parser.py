...
def setup_parser():
    ...
    parser.add_argument('--confidence_filter', default=0, type=float, help='Set the confidence threshold in the link prediction, where only samples above  filter_confidence will participate in the link prediction task ')
    ...
    parser.add_argument('--emb_shape', default=20, type=int,
                        help='only on ConvE,The first dimension of the reshaped 2D embedding')
    ...
    parser.add_argument('--fet_drop', default=0.2, type=float,
                        help='only on ConvE,Dropout for the convolutional features')
    parser.add_argument('--hid_size_component', default=3648, type=int,
                        help='only on ConvE,The side of the hidden layer. The required size changes with the size of the embeddings.')
    parser.add_argument('--hid_size', default=9728, type=int,
                        help='only on ConvE,The side of the hidden layer. The required size changes with the size of the embeddings.')
    ...
    parser.add_argument("--passleaf_score_function", default='DistMult', type=str, help='only on PASSLEAF, choose the score function')
    parser.add_argument("--T_new_semi", default=15 , type=int, help='only on PASSLEAF, set the epoch to generate semi-supervised sample')
    parser.add_argument("--T_semi_train", default=20 , type=int, help='only on PASSLEAF, set the epoch to use semi-supervised sample to train')
    parser.add_argument("--alpha_PASSLEAF", default=0.02 , type=float, help='only on PASSLEAF, set the speed to use semi-supervised sample')
    parser.add_argument("--max_pool", default=10000000 , type=int, help='only on PASSLEAF, set the maximum number of triples in the pool')
