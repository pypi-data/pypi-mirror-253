...
class UKGEPSLLitModel(BaseLitModel):
    """Processing of training, evaluation and testing for UKGEPSL.
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss(pos_score, neg_score, PSL_score, pos_sample, neg_sample, PSL_sample) + 0.005 * regularizer
    ...
    def test_step(self, batch, batch_idx):
        ...
        prediction = "tail"
        confidence = self.args.confidence_filter
        ranks_link_predict = link_predict_filter(batch, self.model, confidence,
                                                 prediction=prediction)  # get the ranks of entities in link prediction
        ranks_link_predict_raw = link_predict_raw(batch, self.model, confidence,
                                                  prediction=prediction)  # get the raw rank (no filter when calculate the rank)
        results["count_for_link"] = torch.numel(ranks_link_predict)
        results["mrr"] = torch.sum(1.0 / ranks_link_predict).item()
        results["mr"] = torch.sum(ranks_link_predict).item()
        results["raw_count_for_link"] = torch.numel(ranks_link_predict_raw)
    ...
    def configure_optimizers(self):
        ...
        We use adam optimizer for UKGE, set β1 as 0.9, set β2 as 0.99.
        Returns:
