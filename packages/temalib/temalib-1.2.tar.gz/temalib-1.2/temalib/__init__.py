...
def get_file_path(caller, folder, filename, create_file=True):
    """
    returns "folder where caller is"/folder/filename"
    ...
    """
    folder_dir = get_folder_path(caller, folder, create_folders=create_file)
...
def isWord(word):
    ...
    """
    for every in word:
        if not any(every.lower()==_ for _ in "абвгдеёжзийклмнопрстуфхцчшщъыьэюяabcdefghijklmnopqrstuvwxyz"):
            return False
    return True
