...
main = (__file__=="__main__")
try:
    import disnake
    if main: print("temalib: disnake found")
    ...
    if main: print("temalib: disnake not found so next commands will be disabled:\nget_username()\nis_online()")

def get__file__():
    if not __name__ == "__main__":
        for frame_info in inspect.stack()[1:]:  # Iterate through stack frames
            frame = frame_info.frame
            if frame.f_globals["__name__"] != "<frozen importlib._bootstrap>":
...
def get_folder_path(*args, create_folders=True):
    ...
    """
    folder_dir = os.path.dirname(get__file__())[0]
    for f in args[1:]:
