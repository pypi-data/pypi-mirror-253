...
export default {
  ...
  methods: {
    ...
    updateData(endpoint) {
      ...
      axios.get(endpoint)
          ...
          if (!this.initialized) {
            ...
            if (data.children !== null) {
              ...
              for (const child of collection.children) {
                ...
                this.links.push({id: `${collection.id}-${child.id}`, source: collection.id, target: child.id});
              }
            }
          }
        ...
        .catch((error) => kadi.base.flashDanger($t('Error loading collection links.'), {request: error.request}))
        .finally(() => {
  ...
  mounted() {
    this.createContainers(this.$refs.svgContainer);
