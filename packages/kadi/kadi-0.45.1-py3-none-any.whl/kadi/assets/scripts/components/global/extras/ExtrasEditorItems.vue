    ...
    <div class="form-row align-items-center">
      <div class="col-md-3">
        <button type="button"
                ...
                v-if="editingMode || nestedType === 'list'">
...
export default {
  ...
  methods: {
    ...
    focusExtra(extra) {
      this.$nextTick(() => {
        // Try to focus the key input first and fallback to the respective value input.
        let input = document.getElementsByClassName(`key-input-${extra.id}`)[0];

        if (!input || input.readOnly) {
          input = document.getElementsByClassName(`value-input-${extra.id}`)[0];
        }
