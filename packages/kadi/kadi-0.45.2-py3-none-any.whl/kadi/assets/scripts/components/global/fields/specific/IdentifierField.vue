...
export default {
  ...
  methods: {
    ...
    checkIdentifier() {
      // This requires the message set via server-side validation to be the same.
      const errorMsg = $t('Identifier is already in use.');

      axios.get(this.checkIdentifierEndpoint, {params: {identifier: this.identifier, exclude: this.exclude}})
        .then((response) => {
  ...
  mounted() {
    // Listen to the custom event of the submission form component.
    window.addEventListener('submitted', this.onSubmitted);
  ...
  beforeDestroy() {
    window.removeEventListener('submitted', this.onSubmitted);
  },
};
</script>
