...
for cpu_flag in _cpu_flags_by_priority:
    if cpu_flag == "ref":
        _work = importlib.import_module("nano-lib-py._work_ref")
        break
    elif cpu_flag in _cpu_flags:
        _work = importlib.import_module(
            "nano-lib-py._work_{}".format(cpu_flag)
        )
        break
...
def parse_work(work):
    ...
    .. note:: This method only checks that the work's format is correct.
              ...
              use :func:`nano-lib-py.work.validate_work`
...
def get_work_value(block_hash, work, as_hex=False):
    ...
    from nano-lib-py.blocks import validate_block_hash
