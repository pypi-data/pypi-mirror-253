...
from unctl.config.config import get_config_instance, load_config, set_config_instance
...
def process(options):
    ...
    if not options.explain and not options.remediate:
        ...
        prompt_interactive(app=interactive_app)
...
def unctl(argv=None):
    ...
    results, reports, groups = process(options)
