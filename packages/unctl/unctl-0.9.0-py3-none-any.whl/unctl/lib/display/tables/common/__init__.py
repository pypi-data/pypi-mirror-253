...
class CommonChecksList(
    ...
    def get_check_title(self, check, context):
        return textwrap.fill(check.CheckTitle, width=30)

    @header_processor("Id")
    def get_check_id(self, check, context):
        return textwrap.fill(check.CheckID, width=20)

    @header_processor("Service")
    def get_check_service(self, check, context):
        return textwrap.fill(check.SubServiceName, width=20)

    @header_processor("Categories")
    def get_check_categories(self, check, context):
        return textwrap.fill(",".join(check.Categories), width=30)
    ...
    def get_check_description(self, check, context):
        ...
        if descr_width < 30:
            descr_width = 30
        return textwrap.fill(check.Description, width=descr_width)
