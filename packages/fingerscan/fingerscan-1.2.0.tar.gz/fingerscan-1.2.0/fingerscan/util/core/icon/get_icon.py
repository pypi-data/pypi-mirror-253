...
class GetIconHash(object):
    ...
    def _get_icon_hash(self, icon_url, retries: int = 3):
        """
        获取icon的hash值
        :param icon_url:url地址
        :param retries:重试次数
    ...
    def run(self):
        ...
        :return:
        """
        icon_hash = self._get_icon_hash(icon_url=self.icon_url, retries=self.retries)
        if icon_hash is None:
            ...
            if default_icon_url:
                ...
                icon_hash = self._get_icon_hash(icon_url=default_icon_url, retries=self.retries)
        return icon_hash
