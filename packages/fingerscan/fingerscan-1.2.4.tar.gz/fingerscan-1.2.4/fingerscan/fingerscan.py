...
def finger_run(url, file_path):
    ...
    log.info("开始指纹扫描")
    result_data = []
    urlfile = open_file(file_path) if url is None else [url]
    log.info(f"需要扫描{len(urlfile)}个资产")
    with alive_bar(len(urlfile)) as bar, ThreadPoolExecutor(max_workers=max_threads) as executor:
        futures = []
        for host in urlfile:
            future = executor.submit(agent(url=host).run)
            futures.append((future, host))
        for future, host in futures:
            try:
                res = future.result(timeout=600)  # 设置单个任务最大执行时间为10分钟
                bar()  # 更新进度
                ...
                log.error(f"任务执行超时: {host}")
                continue
            except Exception as e:
                log.error(f"请求或解析异常: {e}")
                continue
            for i in res_json:
                cms = i.get("cms", "")
                title = i.get("title", "")
                ...
                isp = i.get("ip_info").get("isp")
                ip_address = i.get("ip_info").get("country") + "-" + i.get("ip_info").get("province") + "-" + i.get(
                    "ip_info").get("city")
                print(
                    f'地址:{i.get("url")}\t指纹:{cms}\ttitle:{title}\t状态码:{status_code}\t是否使用CDN:{is_cdn}\t归属地:{ip_address}\t运营商:{isp}')
                new_object = {
                    "host": host,
                    "url": i.get("url"),
                    "scheme": i.get("scheme"),
