...
from util.core import GetIconHash, GetFinger, GetTcpFinger, GetCdnInfo, GetSSLInfo, redis_client, dm_databases_client, \
    GetIcpInfo, GetIPInfo
...
class FingerAgent:
    ...
    def __init__(self, url: str, proxy: dict = None, timeout: int = 3, retries: int = 3):
        """
        初始化配置
        :param url:url地址
        :param proxy:代理信息
    ...
    def _send_tcp(self, url):
        ...
        if _res.get('length') == 0:
            ...
            """
            try:
                _res = dm_databases_client.send_dm_tcp(url)
            except Exception as e:
                log.debug(f"try error agent tcp other {e}")
                pass

        return _res

    def _send_http(self, urls):
        """
        发送http请求
        :param urls:url
        :return:json
        ...
        for _ in range(self.retries):
            try:
                ...
                if "BadStatusLine" in str(e):
                    log.debug(f"捕获到BadStatusLine异常: {e}")
                    break
                else:
                    log.debug(f"连接异常，但不是由于BadStatusLine: {e}")
                    continue
            except TimeoutError as e:
                log.debug(f"捕获请求超时异常: {e}")
                break
            except Exception as e:
    ...
    def _send_https(self, url):
        ...
        for _ in range(self.retries):
            try:
                ...
                if "BadStatusLine" in str(e):
                    log.debug(f"捕获到BadStatusLine异常: {e.args[0].line}")
                    break
                else:
                    log.debug(f"连接异常，但不是由于BadStatusLine: {e}")
                    continue
    ...
    def _verify_server(self, url, result):
        ...
        result.update(dict(scheme=scheme, host=urlparse(url).netloc, ip_info=GetIPInfo(ip=url).run()))
        return result
    ...
    async def _get_result(self):
        ...
        """
        tasks = (
            self._https_scan(url=self.url),
            self._http_scan(url=self.url)
        )
    ...
    def run(self):
        ...
        result = [self._https_scan(self.url), self._http_scan(self.url)]
        # pool = asyncio.new_event_loop()
        # asyncio.set_event_loop(pool)
        # result = pool.run_until_complete(self._get_result())
        # pool.close()
        data = list(filter(None, result))
        if not data:
            data = [dict(title=None, redirect_num=0, url=self.url, icon_hash=None, cms=None, status_code=None,
                         server=None, is_cdn=GetCdnInfo(url=self.url).run(), res_headers=None, cert=dict(), scheme=None,
