...
def update_miscdata(jd, place, miscdata):
    ...
    miscdata["sunrise"] = panchanga.sunrise_hours(jd, place)
    miscdata["sunset"] = panchanga.sunset(jd, place)
    miscdata["karana"] = karana_names[panchanga.karana(jd,place)[0] - 2]
    miscdata["ayanamsa"] = panchanga.get_ayanamsa(jd)
...
def housediffAsc(house, degree, ascdegree):
    ''' computes Bhav house based in asc degree'''
    if degree < ascdegree - 15:
        return getPreviousHouse(house)
    if degree > ascdegree + 15:
        return getNextHouse(house)
    return house


def getNextHouse(house):
    if house + 1 > 12:
        return house - 11
    return house + 1


def getPreviousHouse(house):
    if house - 1 < 1:
        return house + 11
    return house - 1
...
def compute_aspects4planet(planetname, aspectnum, division):
    ...
    if (planet != "NOT_FOUND"):
        ...
        planet["Aspects"]["signs"].append(
            signs[compute_nthsign(signno, aspectnum)-1])  # nth sign aspect
...
def update_houses(division):
    ...
    for housenum in range(1, 13):  # house 1 to 12
        ...
        mysignnum = compute_nthsign(
            signnum(division["ascendant"]["sign"]), housenum)
...
def get_distancebetweenplanets(division, fromplanet, toplanet):
    ...
    sec_fp = (((fp["house-num"]-1)*30*3600) + (fp["pos"]["deg"]
              * 3600) + (fp["pos"]["min"]*60) + (fp["pos"]["sec"]))
    sec_tp = (((tp["house-num"]-1)*30*3600) + (tp["pos"]["deg"]
              * 3600) + (tp["pos"]["min"]*60) + (tp["pos"]["sec"]))
...
def get_point2planetdistance(division, point, toplanet, should_print=False):
    ...
    sec_fp = (((point[0]-1)*30*3600) + (point[1]*3600) +
              (point[2]*60) + (point[3]))
    sec_tp = (((tp_signnum-1)*30*3600) +
              (tp["pos"]["deg"]*3600) + (tp["pos"]["min"]*60) + (tp["pos"]["sec"]))
