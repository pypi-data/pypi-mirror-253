...
def nakshatra_pada(longitude):
  ...
  return [get_nakshatra_name[quotient], 1 + pada, quotient]
...
def update_ascendant(jd, place):
  ...
  data.lagna_ascendant["nakshatra-num"] = nak_pad[2]
...
def compute_lagnaChart_custom(UserBirthData):
  ...
  lat = UserBirthData["POB"]["lat"]
  if lat > 89.9:
    lat = 89.9
  if lat < -89.9:
    lat = -89.9
  birth_place = Place( lat, #lattitude
  ...
  for planetname in data.lagna_planets:
    ...
    planet["chalit-house-num"] = gen.housediffAsc(planet["house-num"], 
                                                  planet["pos"]["dec_deg"],
                                                  data.lagna_ascendant["pos"]["dec_deg"])
