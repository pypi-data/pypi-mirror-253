...
def sunrise_hours(jd, place):
  lat, lon, tz = place
  result = swe.revjul(swe.rise_trans(jd - 1, swe.SUN, swe.CALC_RISE, [lon, lat, 0.0])[1][0])
  hours = result[3]
  while hours + tz > 24:
    hours -= 24
  return hours + tz
...
def get_ayanamsa(jd):
  return swe.get_ayanamsa_ex(jd, _rise_flags)


def sunset(jd, place):
  """Sunset when centre of disc is at horizon for given date and place"""
  lat, lon, tz = place
  result = swe.revjul(swe.rise_trans(jd - 1, swe.SUN, swe.CALC_SET, [lon, lat, 0.0])[1][0])
...
if __name__ == "__main__":
  ...
  deepabirthdate = gregorian_to_jd(Date(1997, 7, 5))
  #all_tests()
  #tithi_tests()
  #nakshatra_tests()
  #yoga_tests()
  print(tithi(shyambirthdate, honavar))
  print(tithi(deepabirthdate, honavar))
  #masa_tests()
  #ascendant_tests()
  #navamsa_tests()
  # new_moon(jd)
