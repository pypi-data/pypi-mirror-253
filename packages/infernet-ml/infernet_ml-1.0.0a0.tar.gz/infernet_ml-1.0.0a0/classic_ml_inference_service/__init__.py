"""
This module serves as the driver for classic ml inference service.
"""
import importlib
import json
import logging
...
from ml.utils.decode import decode_vector
...
def get_workflow_class(
    full_class_path: str,
) -> Optional[Type[BaseClassicInferenceWorkflow]]:
    """Return a BaseClassicInferenceWorkflow instance from a string reference

    Args:
        full_class_path (str): class path

    Returns:
        Optional[Type[BaseClassicInferenceWorkflow]]: returns None if
        error loading
    """
    ...
    try:
        ...
        try:
            ...
            if not issubclass(class_, BaseClassicInferenceWorkflow):
                logging.error(
...
def create_app(test_config: Optional[dict[str, Any]] = None) -> Quart:
    ...
    app.config.from_mapping(
        ...
        CLASSIC_WORKFLOW_CLASS="ml.workflows.inference.torch_inference_workflow.TorchInferenceWorkflow",
    ...
    CLASSIC_WORKFLOW_CLASS = app.config["CLASSIC_WORKFLOW_CLASS"]
    CLASSIC_WORKFLOW_POSITIONAL_ARGS = app.config["CLASSIC_WORKFLOW_POSITIONAL_ARGS"]
    CLASSIC_WORKFLOW_KW_ARGS = app.config["CLASSIC_WORKFLOW_KW_ARGS"]
    logging.info(
        "Loading %s %s %s",
    ...
    WORKFLOW: BaseClassicInferenceWorkflow = clazz(
    ...
    @app.route("/")
    def index() -> str:
        return f"Classic ML Inference Service : {WORKFLOW.__class__}"
    ...
    async def inference() -> Union[str, dict[str, Any]]:
        ...
        if req.method == "POST":
            ...
            if data:
                ...
                # we know base classic inference workflows
                # return str or dicts, so this cast is safe
                return cast(
                    Union[str, dict[str, Any]], WORKFLOW.inference(inference_input)
                )
            else:
