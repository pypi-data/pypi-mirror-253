...
class LcLayout:
    ...
    def __init__(self,
        ...
        # Top layout cell containing the full routing graph for visual inspection.
        self.top_cell_debug: pya.Cell = None
        ...
        self.shapes_debug: Dict[str, db.Shapes] = dict()

        self._routing_terminal_debug_layers = None

        self._placements: Iterable[Cell] = None # Eventually lazy sequence of placement candidates.
    ...
    def _03_1_init_placer(self):
        logging.info("Initialize placer")
        placements = self.placer.place(self._transistors_abstract)
        #assert isinstance(self._placements, Iterable), f"Placer returned wrong data type: {type(placements)}"
        self._placements = placements
    
    def _03_2_next_placement(self) -> Cell:
        # Place transistors
        assert self._placements is not None, "Placer is not initialized"
        logging.info('Find next transistor placement')
        
        abstract_cell = next(self._placements, None)
        if abstract_cell is None:
            logger.info("No more placements found.")
            raise Exception("Out of placement candidates.")
        else:
    ...
    def _05_draw_cell_template(self):
        ...
        for lay in tech.power_layer:
            self.shapes[lay].insert(vdd_rail).set_property('net', self.SUPPLY_VOLTAGE_NET)
            self.shapes[lay].insert(vss_rail).set_property('net', self.GND_NET)

        if len(tech.power_layer)>1:
            logger.debug("Draw power rail vias.")
            for y1 in [0, tech.unit_cell_height]:
                logger.debug('Draw power rail vias on y{}.'.format(y1))
                for i in range(0, len(tech.power_layer)-1):
        ...
        for lay in tech.power_layer:
            self._pin_shapes[self.SUPPLY_VOLTAGE_NET].append((lay, vdd_rail))
            self._pin_shapes[self.GND_NET].append((lay, vss_rail))

    def _06_route(self) -> bool:

        if self.debug_routing_graph:
            # Create full routing graph for visual inspection.
            debug_router = DefaultRouter(
                        graph_router=self.router,
                        debug_routing_graph=True,
                        tech=self.tech
                    )
            
            # Create a separate cell which holds debug information such as the full routing graph.
            self.top_cell_debug = self.top_cell.dup();
            self.top_cell_debug.name = self.top_cell.name + "_debug"
            
            self.shapes_debug = dict()
            for name, (num, purpose) in layermap.items():
                layer = self.layout.layer(num, purpose, name)
                self.shapes_debug[name] = self.top_cell_debug.shapes(layer)

            self._routing_trees_debug = debug_router.route(self.shapes_debug, io_pins=self.io_pins,
                                               transistor_layouts=self._transistor_layouts,
                                               routing_terminal_debug_layers=self._routing_terminal_debug_layers,
                                               top_cell=self.top_cell_debug)
        ...
        routing_trees = router.route(self.shapes, io_pins=self.io_pins,
                                transistor_layouts=self._transistor_layouts,
                                top_cell=self.top_cell)

        self._routing_trees = routing_trees
    
        return routing_trees is not None
    ...
    def _08_02_2_try_route_welltaps(self, gnd_net: str, vdd_net: str):
        ...
        try:
            ...
            routing_trees = router.route(self.shapes, io_pins=[],
                                         ...
                                         routing_terminal_debug_layers=None,
    ...
    def _09_post_process(self):
        ...
        for lay in tech.power_layer:
            _draw_label(self.shapes, lay + '_label', (self._cell_width // 2, 0), self.GND_NET)
            _draw_label(self.shapes, lay + '_label', (self._cell_width // 2, self._cell_height),
    ...
    def create_cell_layout(self,
            ...
            -> Tuple[pya.Cell, Dict[str, List[Tuple[str, pya.Shape]]], pya.Cell]:
        """
        ...
        :return: Layout cell, pin shapes, layout cell with debug routing graph
        """
        ...
        # Place & route loop.
        # 1) Get the next placment candidate, abort if none exists.
        # 2) Try to route the placement, if routing fails go to 1), otherwise finish the layout.
        success = False
        for i in range(self._place_max_candidates):

            logger.debug("Get next placement")
            self._03_2_next_placement()
            
            if placement_path:
                # Store placement.
                self.store_placement(placement_path)

            if self._abstract_cell is None:
                msg = "Tried all placement candidates."
                logger.error(msg)
                raise Exception(msg)
                
            self._02_setup_layout() # TODO: Swap 03 and 02 in the naming scheme.

            self._04_draw_transistors()
            self._05_draw_cell_template()
            if self._06_route():
                logger.info("Routing successful.")
                success = True
                break
            else:
                logger.info("Routing with current placement failed. Try next placement if any.")
        ...
        return self.top_cell, self._pin_shapes, self.top_cell_debug
...
def main():
    ...
    parser.add_argument('--debug-output-dir', default='.', metavar='DIR', type=str, help='output directory for debug layouts')
    ...
    parser.add_argument('--route-max-iter', default=1000, metavar='MAX_ITER', type=int, 
                        help='maximum number of iterations for routing (if the router supports it)')
    
    parser.add_argument('--place-max-candidates', default=1, metavar="MAX_NUM_PLACEMENTS", type=int,
                        help='maximum number of placement candidates to try')
    ...
    router = PathFinderGraphRouter(
        signal_router,
        max_iterations=args.route_max_iter
    )
    ...
    assert args.place_max_candidates >= 0, f"--place-max-candidates must be >= 0 but is {args.place_max_candidates}"
    layouter._place_max_candidates = args.place_max_candidates
    ...
    try:
        ...
        logger.error("failed to create layout: {}".format(e))
        
        debug_cell = layouter.top_cell_debug
        
        # Try to output the debug layout.
        if args.debug_routing_graph and debug_cell is not None:
            # Output the routing graph using defined output writers.
            from .writer.writer import Writer
            for writer in tech.output_writers:
                assert isinstance(writer, Writer)
                logger.debug("Call output writer for debug output: {}".format(type(writer).__name__))
                writer.write_layout(
                    layout=layout,
                    pin_geometries={},
                    top_cell=debug_cell,
    ...
    if debug_cell is not None:
        # Avoid writing the debug layout to the final output.
        layout.delete_cell(debug_cell.cell_index())
    ...
    # Find cell names which are not upper case in the reference netlist.
    reference_cell_names_upper_case = {n.upper(): n for n in reference_cell_names if n.upper() not in reference_cell_names}
