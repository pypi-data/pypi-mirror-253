...
class HierarchicalPlacer(TransistorPlacer):
    ...
    def place(self, transistors: Iterable[Transistor]) -> Iterable[Cell]:
        ...
        for i, (ps, ns) in enumerate(best_subcell_placements):
            ...
            cells_with_quality.append((quality, i, cell))
            
        # Sort by descending quality.
        sorted_cells = sorted(cells_with_quality, key=lambda x: x[0], reverse=True)
        
        logger.info(f"Found {len(sorted_cells)} placement candidates.")

        # TODO: The last placement candidates might be bad.
        for _, _, cell in sorted_cells:
            yield cell
