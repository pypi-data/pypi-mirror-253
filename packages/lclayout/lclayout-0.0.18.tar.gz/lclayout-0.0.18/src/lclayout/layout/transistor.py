...
# SPDX-FileCopyrightText: 2022 Philipp Gühring
...
class DefaultTransistorLayout(TransistorLayout):
    ...
    def __init__(self, abstract_transistor: Transistor, location: Tuple[int, int], tech):
        ...
        # Get gate length from tech file.
        gate_length = None
        if abstract_transistor.channel_type == ChannelType.NMOS:
            try:
                gate_length = tech.gate_length_nmos
            except:
                logger.debug("`gate_length_nmos` not defined in techfile")
        else:
            try:
                gate_length = tech.gate_length_pmos
            except:
                logger.debug("`gate_length_pmos` not defined in techfile")
            
        if gate_length is None:
            # Default to `tech.gate_length`
            try:
                gate_length = tech.gate_length
            except:
                logger.debug("`gate_length` not defined in techfile")
                pass
            
        if gate_length is None:
            # Default to gate-length from SPICE netlist.
            logger.debug("Default to gate length from SPICE netlist: {} um".format(abstract_transistor.channel_length))
            gate_length = int(abstract_transistor.channel_length / tech.db_unit) # Convert from microns to data-base units.
            
        logger.debug("gate length: {}".format(gate_length))
        ...
        # The second variant of the max function seems to be more precise, we could remove the older first variant
        w = max(tech.unit_cell_width + tech.via_size[l_diff_contact] + 2 * tech.minimum_enclosure[(l_diffusion, l_diff_contact)] , # old version
                gate_length + 2* tech.via_size[l_diff_contact] + 2 * tech.minimum_enclosure[(l_diffusion,l_diff_contact)] + 2 *tech.min_spacing[(l_poly,l_diff_contact)]) # more precise
        logger.debug(f"gate_length: {gate_length} + 2* via_size: {tech.via_size[l_diff_contact]} +2*enclosure: {tech.minimum_enclosure[(l_diffusion,l_diff_contact)]} + 2*minspacing: {tech.min_spacing[(l_poly,l_diff_contact)]}")
        ...
        logger.debug("active_box: "+str(x_eff)+" "+str(y_eff)+" "+str(x_eff+w)+" "+str(y_eff+h))
        ...
        logger.debug(f"well2active_overlap: {well2active_overlap}")
