...
class ThinkTrader(base1x.Singleton, XtQuantTraderCallback):
    """
    迅投XtQuant-miniQMT交易
    """
    __qmt_dir: str = None
    ...
    def connect(self, qmt_dir: str = '', session_id: int = 0) -> int:
    ...
    def single_available(self, num: int) -> float:
        ...
        acc_value = base1x.float_round(acc_value)
        if acc_total == 0:
            # qmt有一定概率出现总资金为0的情况
            acc_total = acc_value
        can_use_amount = base1x.float_round(can_use_amount)
        ...
        reserve_cash = base1x.float_round(acc_total * self.__config.position_ratio)
        ...
        if available > acc_cash:
            logger.warning(
                ...
                base1x.float_round(100 * (acc_value / acc_total)),
                base1x.float_round(100 * (1 - self.__config.position_ratio)))
        # 8. 计算单一标的可用资金
        single_funds_available = base1x.float_round(available / num)
        # 9. 检查可用资金的最大值和最小值
    ...
    def available_price(self, price: float) -> float:
        ...
        # 最后修订价格
        buy_price = base1x.float_round(buy_price)
        return buy_price
    ...
    def calculate_buy_fee(self, price: float, volume: int) -> float:
        ...
        _stamp_duty_fee = base1x.float_round(volume * price * self.__config.stamp_duty_rate_for_buy)
        # 2. 过户费, 按照股票数量, 双向, 0.06%
        _transfer_fee = base1x.float_round(volume * self.__config.transfer_rate)
        # 3. 券商佣金, 按照成交金额计算, 双向, 0.025%
        _commission_fee = base1x.float_round(volume * price * self.__config.commission_rate)
        if _commission_fee < self.__config.commission_min:
            _commission_fee = self.__config.commission_min
        # 4. 股票市值
        _stock_fee = base1x.float_round(volume * price)
