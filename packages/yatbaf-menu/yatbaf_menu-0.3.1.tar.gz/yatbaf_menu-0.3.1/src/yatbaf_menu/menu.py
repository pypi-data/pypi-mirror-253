...
class Menu:
    ...
    async def _build_markup(self, q: Query) -> list[list[InlineKeyboardButton]]:
        async with asyncio.TaskGroup() as tg:
            tasks = [tg.create_task(r._build(q)) for r in self._rows]

        return [  # yapf: disable
            row
            for task in tasks if (rows := task.result()) is not None
            for row in rows
        ]

    async def _get_markup(self, q: Query, /) -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup(await self._build_markup(q))
