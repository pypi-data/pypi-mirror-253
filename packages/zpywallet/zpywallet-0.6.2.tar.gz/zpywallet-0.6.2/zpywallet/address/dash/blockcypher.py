...
class BlockcypherAddress:
    ...
    Args:
        address (str): The human-readable Dash address.
    ...
    Methods:
        get_balance(): Retrieves the balance of the Dash address.
        get_transaction_history(): Retrieves the transaction history of the Dash address.
    ...
    def _get_transaction_history(self, txhash=None):
        """
        Retrieves the transaction history of the Dash address. (internal method that makes the network query)
        ...
        for address in self.addresses:
            ...
            url = f"https://api.blockcypher.com/v1/dash/main/addrs/{address}/full?limit={interval}&txlimit={txlimit}"
            for attempt in range(3, -1, -1):
                if attempt == 0:
                    raise NetworkException("Network request failure")
                try:
            ...
            while 'hasMore' in data.keys() and data['hasMore']:
                url = f"https://api.blockcypher.com/v1/dash/main/addrs/{address}/full?limit={interval}&before={block_height}&txlimit={txlimit}"
                for attempt in range(3, -1, -1):
                    if attempt == 0:
                        raise NetworkException("Network request failure")
                    try:
