...
def get_dist(image1,image2):
	# 合并图片使用
	location_list=pickle.load(open('location_list.pkl'))

	jpgfile1=io.StringIO(urllib.request.urlopen(image1).read())
	new_image1=get_merge_image(jpgfile1,location_list)

	new_image1.save('image1.jpg')

	jpgfile2=io.StringIO(urllib.request.urlopen(image2).read())
...
def format_track(track):
	track_list=[list(map(int,x.split(','))) for x in re.findall('{(.*?)}',track)]
	return track_list
...
def choice_track_list(dist):
	...
	# 计算tmp在哪个子文件中
	tmp=str(dist%10)
	t_dict=pickle.load(open('t_dict.pkl'))
...
def get_uri(page):
	links=[]
	alist=page.xpath('//a[@class="search_list_item db"]')
