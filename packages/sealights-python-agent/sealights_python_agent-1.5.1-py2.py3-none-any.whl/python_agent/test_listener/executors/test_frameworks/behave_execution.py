...
try:
    ...
    log.exception("Failed importing behave. Error: %s" % str(e))
...
class BehaveAgentExecution(AgentExecution):
    def __init__(self, config_data, labid, test_stage, cov_report, per_test, interval, test_group_id, args):
        ...
        try:
            super(BehaveAgentExecution, self).__init__(config_data, labid, test_stage, cov_report=cov_report,
                                                       per_test=per_test, interval=interval,
    ...
    def execute(self):
        if not is_behave_installed:
            log.error("Behave is not installed. Please install it using: pip install behave")
            return
    ...
    def run_before_all(self):
        try:
            ...
            log.exception("failed sending execution start form behave. error: %s" % str(e))
    ...
    def run_before_scenario(self, scenario: Scenario):
        test_name = self.get_unique_test_name(scenario)
    ...
    def run_after_scenario(self, scenario: Scenario):
        ...
        SeaLightsAPI.notify_test_end(self.execution_id, test_name, test_duration, test_status_str)
        log.debug("send test end for scenario: %s status: %s" % (test_name, test_status_str))

    def get_unique_test_name(self, scenario: Scenario):
        default_name = f"{scenario.feature.name}/{scenario.name}"
        if scenario.filename not in self.feature_file_content_cache:
            try:
                with open(scenario.filename, 'r') as file:
                    self.feature_file_content_cache[scenario.filename] = file.readlines()
            except FileNotFoundError:
                log.debug("feature file not found: %s" % scenario.filename)
                return default_name
        try:
            line_content = self.feature_file_content_cache[scenario.filename][scenario.line - 1]
            line_content = ''.join(line_content.split())
        except IndexError:
            log.debug("line number not found: %s" % scenario.line)
            return default_name
        checksum = hashlib.md5(line_content.encode()).hexdigest()
        return f"{default_name}/{checksum}"
