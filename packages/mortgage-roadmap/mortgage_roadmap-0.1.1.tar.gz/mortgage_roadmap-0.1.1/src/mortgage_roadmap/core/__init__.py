...
class Base_Mortgage(ABC):
    ...
    @argument_check(object, str, [".xls", ".xlsx", ".csv"], MethodType)
    def __create_File_Process(self, path: str, fileformat: Literal[".xls", ".xlsx", ".csv"], PROCESS: MethodType) -> bool:
        if self.__handlePandasException() :
    ...
    @argument_check(object, str)
    def __build_EXCEL(self, path) -> Literal[True]:
    ...
    @argument_check(object, str)
    def __build_CSV(self, path) -> Literal[True]:
        df = self.create_Dataframe()
        df.to_csv(path)
        return True
