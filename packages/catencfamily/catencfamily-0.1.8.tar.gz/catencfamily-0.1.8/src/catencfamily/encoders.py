...
class CatEncodersFamily(BaseEstimator, TransformerMixin):
    ...
    def _calCentralityMeasures(self,model, network):
        ...
        if (self.cMeasures[5] == 1):
            ...
            if self.k is None:
                ...
                print(f"Calculating betweenness centrality with k = {m} nodes sample")
                
            start = time.time()
            bt = nx.betweenness_centrality(network, k = m)
            end = time.time()
            tt = (end-start)/60
            print(f"Betweenness centrality calculated. Time taken: {tt} min ")
    ...
    def extractPC0sUnitvectors(self,train_trans,test_trans,n_components=2):
        """
        Parameters
        ----------
        ...
        """
        cc_tr,cc_te, _ , _ = self.pcaAndConcat(train_trans, test_trans, n_components = 2)
