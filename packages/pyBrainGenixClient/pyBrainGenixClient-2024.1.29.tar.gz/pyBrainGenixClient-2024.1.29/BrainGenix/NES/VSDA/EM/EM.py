...
class EM:
    ...
    def SaveImageStack(self, _ImageStackDirectoryPrefix:str = ""):
        ...
        for i in range(len(ImageHandles)):
            ...
            ProcessedImageHandle:str = ImageHandles[i].replace("Renders/", "").replace(f"Simulation{self.SimulationID}/", "").replace(f"Region{self.ScanRegionID}/", "")
            os.makedirs(os.path.dirname(_ImageStackDirectoryPrefix + ProcessedImageHandle), exist_ok=True)
            Filepath:str = _ImageStackDirectoryPrefix + ProcessedImageHandle
