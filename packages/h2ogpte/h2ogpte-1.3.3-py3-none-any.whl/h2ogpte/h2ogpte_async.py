...
class H2OGPTEAsync:
    ...
    async def create_chat_session(self, collection_id: str) -> str:
        """
    ...
    async def create_collection(self, name: str, description: str) -> str:
        ...
        """
        ret = await self._db("create_collection", name, description)
        return _to_id(ret)
    ...
    async def encode_for_retrieval(self, chunks: Iterable[str]) -> List[List[float]]:
        """
        ...
        """
        return await self._lang("encode_for_retrieval", chunks=chunks)
    ...
    async def ingest_website(
        ...
        """
        ret = await self._job(
            "crawl.IngestWebsiteJob", collection_id=collection_id, url=url
        )
        return await self._wait_for_completion(_to_id(ret), timeout=timeout)
    ...
    async def update_collection_prompt_settings(
        self,
        collection_id: str,
        system_prompt: str,
    ...
    def connect(self, chat_session_id: str) -> SessionAsync:
        ...
        """
        return SessionAsync(chat_session_id, self)
