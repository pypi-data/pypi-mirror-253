...
def __randstr(n=8, alphabet=string.ascii_lowercase+string.ascii_uppercase+string.digits, balance=False, blocksize=0):
    ...
    if balance and blocksize > 0:
        # NB: length of alphabets shall be greater than log2(blocksize) to ensure balancing (otherwise, it is impossible
        #      to ensure and string generation fails before completing the first block)
        from math import ceil, log
        n_bs = log(blocksize, 2)
        if len(alphabet) < n_bs:
            raise ValueError(f"Too short alphabet (shall be at least {ceil(n_bs)} long)")
    is_b = isinstance(alphabet, bytes)
    s = ["", b""][is_b]
    if is_b:
        alphabet = [alphabet[i:i+1] for i in range(len(alphabet))]
    if balance:
        t, cnts = (min(n, blocksize) if blocksize > 0 else n) / (len(alphabet) - 1 or 1), {}
        orig_alphabet = alphabet[:] if is_b else alphabet
    for i in range(n):
        if balance:
            if i > 0 and blocksize > 0 and i % blocksize == 0:
                alphabet, cnts = orig_alphabet[:], {}
            while (cnts.get(c := random.choice(alphabet), 0) + 1) >= t:
                if is_b:
                    alphabet.remove(c)
                else:
