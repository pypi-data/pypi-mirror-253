...
class GraphQLQuery:
    ...
    def __init__(self, action: str = "query"):
        """Initialize self.query as a dict

        :param action: the action to perform ("query" to fetch data, "mutation" to change data)
        """
        self.query = {"query": ""}
        self.action = action
    ...
    def process_where_conditions(self, conditions):
        condition_strings = []
    ...
    def add_query(
        ...
        :param items: a list or iterable of strings to join
        :param order: a dict of key value pairs to order on
        :param where: a dict of dicts containing a key and a { op: value}, e.g. {"eq": "value"}
        :param take: the number of objects to take (can be str, int, or float)
        :param skip: the number of objects to skip (can be str, int, or float)
        """
        entity_string = (
            f"{' ' * 4}{entity}(\n{' ' * 8}take: {take},\n{' ' * 8}skip: {skip},\n"
        )
        ...
        # construct the query
        self.query["query"] += (
            entity_string + where_string + order_string + items_string + closing_string
        )
        return self
