...
def parse_roles(component, component_data) -> list:
    """Parse responsible roles for a component.
    :param component: The component XML element
    :param component_data: The component data dictionary
    ...
    for link in links:
        if not link["href"].startswith("#"):
            if link["component_uuid"] == comp_uuid:
                ...
                resp = api_handler.post("/api/links", json=obj.dict())
...
def create_component_based_on_type(
    ...
    """
    if component_data["type"] == "interconnection":
        if response := create_interconnection(
            component_data, user_id, ssp_id, api_handler
        ):
            return response.json().get("id", 0), "interconnects"
...
def create_related_items(
    component_data, comp_id, regcomp_module, trv, api_handler, user_id
):
    """
    ...
    :param str user_id: User ID.
    """
    for item_type in ["properties", "props", "protocols", "links"]:
...
def parse_ssp_components(trv: FedrampTraversal):
    ...
    try:
        ...
        for component in components:
            ...
            comp_id, regcomp_module = create_component_based_on_type(
                component_data, user_id, ssp_id, api_handler
            )
...
def create_properties(
    ...
    for prop in properties:
        ...
        new_prop = api_handler.post("/api/properties", json=prop_obj.dict())
...
def create_component(
    ...
    resp_json = api_handler.post("/api/components", data=comp.dict())
    if resp_json:
...
def create_component_mapping(
    ...
    api_handler.post("/api/componentmapping", json=mapping)
...
def create_protocols(
    ...
    for protocol in protocols:
        ...
        response = api_handler.post("/api/portsProtocols", json=protocol_obj.dict())
...
def create_interconnection(
    ...
    response = api_handler.post("/api/interconnections", data=intercon.dict())
    return response
