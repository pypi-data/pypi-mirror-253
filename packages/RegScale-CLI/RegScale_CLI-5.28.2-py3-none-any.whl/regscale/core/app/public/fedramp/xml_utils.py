...
def extract_markup_content(element) -> str:
    """
    Extract the text content from an XML element, including text content from child elements
    :param element: The XML element to extract text content from
...
def update_SSP(ssp_updates_dict, ssp_id) -> None:
    """
    ...
    :param ssp_updates_dict: Object to update SSP with
    :param ssp_id: SSP ID to update in RegScale
    :return: None
    """
    app = Application()
    api = Api(app)
    ...
    response = api.get(
        url=urljoin(config["domain"], f"/api/securityplans/{ssp_id}"), headers=headers
    )
    if response.ok:
