...
def parse_inventory_item(inventory_item, namespaces):
    ...
    inventory_data = {
        ...
        "description": safe_first_item(
            inventory_item.xpath(
                "oscal:description/oscal:p/text()", namespaces=namespaces
            )
        ),
        "props": {},
    }
    ...
    for rp in inventory_item.xpath("oscal:responsible-party", namespaces=namespaces):
        ...
        party_uuid = safe_first_item(
            rp.xpath("oscal:party-uuid/text()", namespaces=namespaces)
        )
        inventory_data["responsible_parties"].append(
            {"role_id": role_id, "party_uuid": party_uuid}
        )

    # Parse implemented components
    inventory_data["implemented_components"] = []
    for ic in inventory_item.xpath(
        "oscal:implemented-component", namespaces=namespaces
    ):
        component_uuid = ic.get("component-uuid")
        remarks = safe_first_item(
            ic.xpath("oscal:remarks/oscal:p/text()", namespaces=namespaces)
        )
        inventory_data["implemented_components"].append(
            {"component_uuid": component_uuid, "remarks": remarks}
        )

    # Parse remarks
    inventory_data["remarks"] = safe_first_item(
        inventory_item.xpath("oscal:remarks/oscal:p/text()", namespaces=namespaces)
    )
...
def parse_inventory_item_to_asset(
    ...
    asset = Asset(
        ...
        name=parsed_inventory_item.get("description", "Asset")[
            :420
        ],  # Need to truncate name when using description
                            ...
                            trv.log_error(
                                {
                                    ...
                                    "record_type": "inventory-item",
                                    "event_msg": (
                                        f"Asset {asset['name']}" "failed to update.",
                                    ),
                                }
                            )
