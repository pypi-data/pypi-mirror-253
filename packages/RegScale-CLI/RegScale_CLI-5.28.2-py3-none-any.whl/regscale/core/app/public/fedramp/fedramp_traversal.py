...
from regscale.models.regscale_models.stakeholders import StakeHolders
...
class FedrampTraversal(BaseModel):
    ...
    def fedramp_role_id_to_regscale_system_role_id(
        self, fedramp_role_id: str
    ) -> Optional[int]:
        """
        ...
        ssp_system_roles = SystemRoles.get_all_by_ssp_id(self.api.app, self.ssp_id)

        matching_system_roles = [
            system_role
            for system_role in ssp_system_roles
            if system_role.fedrampRoleId == fedramp_role_id
        ]

        first_matching_system_role = (
            matching_system_roles[0] if matching_system_roles else None
        )

        return first_matching_system_role.id if first_matching_system_role else None

    def fedramp_party_to_regscale_stakeholder_id(
        self, party_fedramp_uuid: str
    ) -> Optional[int]:
        """
        ...
        stakeholders = StakeHolders.get_all_by_parent(
            self.api.app, "securityplans", self.ssp_id
        )
        ...
        matching_stakeholders = [
            ...
            if stakeholder.otherID == party_fedramp_uuid
        ]

        first_matching_stakeholder = (
            matching_stakeholders[0] if matching_stakeholders else None
        )
