...
class LeveragedAuthIR(XMLIR):
    ...
    def leveraged_system_identifier(
        self, v: List[etree._Element], traversal: XMLIRTraversal
    ):
    ...
    for i, elem in enumerate(leveraged_auths):
        try:
            ...
            parsed = LeveragedAuthIR().parse(
                XMLIRTraversal(
                    xpathToThis="...", root=trv.root, namespaces=trv.namespaces, el=elem
                )
            )
            ...
            api_obj = LeveragedAuthorizations(
                title=parsed.get("title"),
                fedrampId=parsed.get("uuid"),
                securityPlanId=trv.ssp_id,
                dateAuthorized=parsed.get("date_authorized"),
                securityPlanLink=security_plan_link["href"]
                if "href" in security_plan_link
                else security_plan_link,
                crmLink=crm_link["href"] if "href" in crm_link else crm_link,
                responsibilityAndInheritanceLink=responsibility_and_inheritance_link[
                    "href"
                ]
...
if __name__ == "__main__":
    ...
    def basic_test():
        ...
        parse_leveraged_auth_new(
            FedrampTraversal(api=__api, root=root, ssp_id=ARG_SSP_ID, namespaces=ns)
        )
