...
class MetadataPersonPartyIR(XMLIR):
    ...
    def member_of_organization_uuids(
        self, vlist: List[etree._Element], traversal: XMLIRTraversal
    ):
        """The user's location uuid"""
        return [v.text for v in vlist]
...
def parse_metadata(trv: FedrampTraversal):
    ...
    if len(metadatas) == 0:
                ...
                trv.log_error(
                    {
                        "model_layer": "metadata",
                        "record_type": "party",
                        "event_msg": f"Failed to create party: {party.get('shortname', '')}"
                        f" (ERROR: {str(e)})",
                    }
                )
        ...
        for responsible_party in responsible_parties:
            party_uuid = (
                ...
                if "party_uuids" in responsible_party
                and len(responsible_party["party_uuids"]) > 0
                else None
            )
            ...
            # 1. Stakeholder
            resp_party_stakeholder_id = trv.fedramp_party_to_regscale_stakeholder_id(
                party_uuid
            )
            ...
            system_role_id = trv.fedramp_role_id_to_regscale_system_role_id(
                fedramp_role_id=fedramp_role_id
            )
