...
def parse_minimum_ssp(
    ...
    :param root: lxml root Element from SSP
    :param events_list: list to be populated with parsed data
    :param new_ssp: dict to be populated with parsed data
    ...
    for additional_identifier in additional_identifiers:
        try:
            if additional_identifier.attrib["identifier-type"] != "https://fedramp.gov":
                property_ = {
                    "key": "identifier-type="
                    + additional_identifier.attrib["identifier-type"],
                    "value": additional_identifier.text,
                    "label": "Other System ID",
                    "otherAttributes": xpath_query.replace(
                        "ns1:", "{http://csrc.nist.gov/ns/oscal/1.0}"
                    ),
                }
                properties_list.append(property_)
                print("Should be logging an event here")
                events_list.append(
                    log_event(
            ...
            print("Should be capturing an error here")
            logger.error(e)
            events_list.append(
                log_error(
    ...
    #
    otherIdentifier = root.xpath(
        "/*/ns1:system-characteristics/ns1:system-name-short", namespaces=ns
    )
    if len(otherIdentifier) == 0:
        events_list.append(
            log_error(
        ...
        new_ssp["otherIdentifier"] = otherIdentifier[0].text
        events_list.append(
            log_event(
    ...
    description = root.xpath(
        "/*/ns1:system-characteristics/ns1:description", namespaces=ns
    )
    ...
    if len(authenticationLevel) == 0:
        ...
        new_ssp["authenticationLevel"] = apply_mapping(
            LEVEL_MAPPING, authenticationLevel[0].attrib["value"]
        )
        events_list.append(
            log_event(
                record_type=SYSTEM_INFO,
    ...
    if len(identityAssuranceLevel) == 0:
        ...
        new_ssp["identityAssuranceLevel"] = apply_mapping(
            LEVEL_MAPPING, identityAssuranceLevel[0]
        )
        events_list.append(
            log_event(
                record_type=SYSTEM_INFO,
    ...
    if len(authenticatorAssuranceLevel) == 0:
        ...
        new_ssp["authenticatorAssuranceLevel"] = apply_mapping(
            LEVEL_MAPPING, identityAssuranceLevel[0]
        )
        events_list.append(
            log_event(
    ...
    if len(federationAssuranceLevel) == 0:
        ...
        new_ssp["federationAssuranceLevel"] = apply_mapping(
            LEVEL_MAPPING, federationAssuranceLevel[0]
        )
        events_list.append(
            log_event(
                record_type=SYSTEM_INFO,
    ...
    #
    systemName = root.xpath(
        "/*/ns1:system-characteristics/ns1:system-name", namespaces=ns
    )
    if len(systemName) == 0:
        events_list.append(
            log_error(
        ...
        new_ssp[
            "systemName"
        ] = "Cloud Service Provider System"  # FAILSAFE W/ DUMMY DATA FOR REQUIRED VALUES
    else:
        new_ssp["systemName"] = systemName[0].text
        events_list.append(
            log_event(
    ...
    overallCategorization = root.xpath(
        "/*/ns1:system-characteristics/ns1:security-sensitivity-level", namespaces=ns
    )
    if len(overallCategorization) == 0:
        events_list.append(
            log_error(
                record_type=SYSTEM_INFO,
        ...
        new_ssp["overallCategorization"] = apply_mapping(
            CAT_MAPPING, overallCategorization[0].text
        )
        events_list.append(
            log_event(
                record_type=SYSTEM_INFO,
    ...
    if len(status) == 0:
        ...
        status_remarks = root.xpath(
            "/*/ns1:system-characteristics/ns1:status/ns1:remarks", namespaces=ns
        )
        if len(status_remarks) > 0:
            new_ssp["explanationForNonOperational"] = ""
            for p in status_remarks[0].findall("ns1:p", namespaces=ns):
                new_ssp["explanationForNonOperational"] = (
                    new_ssp["explanationForNonOperational"] + p.text + " "
                )
    #
...
def parse_system_characteristics(
    ssp_id, root: Element, events_list: Optional[list] = None, ns: Optional[dict] = None
):
    ...
    if len(authorizationBoundary) > 0:
        ...
        for p in authorizationBoundary[0].findall("ns1:p", namespaces=ns):
            ssp_updates["authorizationBoundary"] = (
                ssp_updates["authorizationBoundary"] + p.text + " "
            )
        events_list.append(
            log_event(
                model_layer=SYSTEM_CHARS,
    ...
    if net:
        networkArchitecture = net[0]
    if len(authorizationBoundary) > 0 and networkArchitecture:
        ssp_updates["networkArchitecture"] = ""
        for p in networkArchitecture[0].findall("ns1:p", namespaces=ns):
            ssp_updates["networkArchitecture"] = (
                ssp_updates["networkArchitecture"] + p.text + " "
            )
        events_list.append(
            log_event(
                model_layer=SYSTEM_CHARS,
...
def apply_mapping(mapping: List[tuple], value: str):
    """
    :param mapping: List of mapping pair tuples - (original,new)
    : param value: Thing to be mapped to a new value
    """
...
def post_SSP(api: Api, new_ssp: dict):
    ...
    try:
        ssp = SecurityPlan.from_dict(new_ssp)
    ...
    if not new_ssp_id:
        ...
        logger.warning(
            "Some required data may have failed to be imported. Trying to upload SSP with incomplete data "
        )
        import json
...
def parse_classificationType(information_types, ns, events_list, ssp_id):
    ...
    for information_type in information_types:
        ...
        if information_type.find("ns1:title", namespaces=ns) is not None:
            classificationType_dict["title"] = information_type.find(
                "ns1:title", namespaces=ns
            ).text
        if information_type.find("ns1:description", namespaces=ns) is not None:
            for p in information_type.find("ns1:description", namespaces=ns).findall(
                "ns1:p", namespaces=ns
            ):
                classificationType_dict["description"] = (
                    classificationType_dict["description"] + p.text + " "
                )
        if (
            information_type.find(
                "ns1:confidentiality-impact/ns1:selected", namespaces=ns
            )
            is not None
        ):
        ...
        if (
            information_type.find("ns1:integrity-impact/ns1:selected", namespaces=ns)
            is not None
        ):
            classificationType_dict["integrity"] = apply_mapping(
                CAT_MAPPING,
                information_type.find(
                    "ns1:integrity-impact/ns1:selected", namespaces=ns
                ).text,
            )
        if (
            information_type.find("ns1:availability-impact/ns1:selected", namespaces=ns)
            is not None
        ):
            classificationType_dict["availability"] = apply_mapping(
                CAT_MAPPING,
                information_type.find(
                    "ns1:availability-impact/ns1:selected", namespaces=ns
                ).text,
            )
        classificationTypeId = upload_classificationType(
            classificationType_dict, ssp_id
        )
...
def upload_classificationType(classificationType_dict, ssp_id):
    ...
    ):  # If successfully upload classification type, then add mapping to SSP
        classifiedRecords_entry = {}
        classifiedRecords_entry["classificationTypeId"] = json.loads(response.content)[
            "id"
        ]
...
def parse_privacy(root, events_list, ns, ssp_id):
    ...
    try:
        ...
        # raise Exception('bad') from e


def post_privacy(privacyImpactAssessment):
    app = Application()
    api = Api(app)
    headers_json = {
        "accept": "*/*",
    ...
    ssp_PIA = json.dumps(privacyImpactAssessment)
    response = api.post(
        url=app.config["domain"] + "/api/privacy", data=ssp_PIA, headers=headers_json
    )
    if response.status_code == 200:
