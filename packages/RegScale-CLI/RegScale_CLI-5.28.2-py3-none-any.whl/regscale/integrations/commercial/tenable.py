...
from regscale.core.app.utils.nessus_utils import get_cpe_file
...
def import_nessus(folder_path: click.Path, regscale_ssp_id: click.INT):
    ...
    :param folder_path: Nessus folder path
    ...
    for control in control_implementations:
        ...
        if sorted_assessments := sorted(
            ...
            if control["lastAssessmentResult"]:
                ...
                if (
                    control_obj.lastAssessmentResult == "Fail"
                    and control_obj.status != IN_REMEDIATION
                ):
                    ...
                    control_obj.stepsToImplement = (
                        "n/a"
                        if not control_obj.stepsToImplement
                        else control_obj.stepsToImplement
                    )
                elif (
                    control_obj.lastAssessmentResult == "Pass"
                    and control_obj.status != FULLY_IMPLEMENTED
                ):
