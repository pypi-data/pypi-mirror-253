...
def create_salesforce_client(config: dict) -> Salesforce:
    ...
    if any(
        ...
        error_and_exit(
            "Please update the Salesforce credentials in init.yaml or set up environment variables."
        )
    return Salesforce(
        username=username,
        password=password,
...
def sync_sf_and_regscale(
        ...
        if issues_to_update := sync_regscale_to_sf(
            ...
            with job_progress:
                ...
                updating_issues = job_progress.add_task(
                    f"[#f8b737]Updating {len(issues_to_update)} RegScale issue(s)"
                    "from Salesforce...",
    ...
    if sf_cases:
        ...
        with job_progress:
            ...
            # output the final result
            logger.info(
                "Created %i Salesforce case(s), created %i issue(s) and "
                "updated %i issue(s) in RegScale.",
                len(new_sf_cases),
...
def fetch_sf_cases(
    ...
    if sf_cases:
        ...
        logger.info(
            "Saved %i Salesforce case(s) with the provided criteria,"
            " see /artifacts/existingSalesforceCases.json",
            len(sf_cases),
        )
    return sf_cases
...
def fetch_sf_attachments(
    ...
    if attempt_one["totalSize"] > 0 and attempt_two["totalSize"] > 0:
        attachments = {
            ...
            for key in set(attempt_one).union(set(attempt_two))
        }
...
def sync_regscale_to_sf(
    ...
    :param List[File] attachments: List of files to sync from RegScale to Salesforce, defaults to None
    ...
    for issue in regscale_issues:
        ...
        if (
            sync_attachments
            and attachments
            and (new_case_details and issue.salesforceId)
        ):
            compare_files_for_dupes_and_upload(
