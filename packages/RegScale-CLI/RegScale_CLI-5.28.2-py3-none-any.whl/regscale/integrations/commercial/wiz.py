...
from regscale.models.regscale_models.calatlog import Catalog
    ...
    if asset["wizId"] in existing_wiz_ids:
        ...
        if wiz_index := next(
            ...
            if (
                ...
                and Asset.from_dict(update_asset) != Asset.from_dict(asset)
                and isinstance(asset, dict)
            ):
...
def create_or_update_inventory_report(
    ...
    :param str wiz_project_id: Wiz project ID
    :param int batch_size: Number of entities to process in each batch
    :return: List of Wiz report IDs
    :rtype: list[str]
    """
    ...
    for i in range(0, len(entities), batch_size):
        ...
        logger.info(batch_entities)
        report_name = f"{report_prefix}{i}_{i + len(batch_entities)}"
