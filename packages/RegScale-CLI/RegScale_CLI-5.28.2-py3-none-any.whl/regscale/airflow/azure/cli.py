...
def upload_dag(file_path: str, connection_string: str, container_name: str) -> None:
    ...
    if connection_string is None:
        click.echo(
            "You can set the connection string with the AZURE_STORAGE_CONNECTION_STRING environment variable."
        )
    if container_name is None:
        click.echo(
            "You can set the container name with the AZURE_STORAGE_CONTAINER_NAME environment variable."
        )
...
@cli.command()
@click.option(
    "--path", "-p", "path", default="airflow/dags/", help="Path to the DAGs folder."
)
@click.option(
    "--conn-string",
    "-c",
