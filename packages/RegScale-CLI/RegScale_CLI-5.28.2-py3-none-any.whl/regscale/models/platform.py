...
class RegScaleAuth(BaseModel):
    ...
    def authenticate(
        ...
        :param Optional[str] password: Password to log in with, defaults to None
    ...
    def refresh(self) -> None:
        """
        Refresh the token
        :param Api api: API Object for refreshing token
        :return: None
        """
        app = Application()
        api = Api(app)
        uid, auth_token = get_regscale_token(
            api=api, username=self.username, password=self.token, domain=self.domain
        )
        self.user_id = uid
