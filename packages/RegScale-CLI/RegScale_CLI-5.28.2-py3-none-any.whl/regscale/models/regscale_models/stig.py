...
class STIG_FILE:
    ...
    def __init__(
        ...
        console = Console()
        self.implementation_results = ImplementationResults(
            stig_file_name=self.file_path.name, console=console
        )
        self.update_checks()
        self._process_rules()
    ...
    @classmethod
    def _find_dict_by_key(cls, input_list: list, search_key: Any, search_value: Any):
        """
        ...
        """
        return next(
            (item for item in input_list if item.get(search_key) == search_value), None
        )
        ...
        for cci_info in controls:  # Get CCI to Control Mapping
            ...
            if control_id:
                ...
                security_control_list = [
                    ...
                    if cntrl["controlId"].upper() == control_id.upper()
                ]
                        ...
                        for _, cci in enumerate(ccis):
                            ...
                            if checklist not in {
                                ...
                                existing_checklist = {
                                    Checklist.from_dict(chk)
                                    for chk in self.existing_checklists
                                    if Checklist.from_dict(chk) == checklist
                                }.pop()
    ...
    def _update_checklists(self) -> None:
        ...
        if self.update_checklists:
            ...
            for ix, check in enumerate(self.existing_checklists):
                found_check = self._find_dict_by_key(
                    input_list=updated, search_key="id", search_value=check["id"]
                )
                if found_check:
    ...
    def _gen_notes(data: tuple[str, str, str, str, str]) -> str:
        ...
        :param tuple data: A tuple of strings
    ...
    def _get_control_implementations(self, security_control: dict) -> list[dict]:
        ...
        control_implementations = [
            imp
            for imp in self.control_implementations
            if imp["controlID"] == security_control["id"]
        ]
        return control_implementations

    def _option_lookup(self, control_id: int) -> list[ImplementationOption]:
        """
