...
class SecurityPlan(RegScaleModel):
    ...
    id: Optional[int] = 0
    uuid: Optional[str] = ""
    bDeployGov: Optional[bool] = False
    bDeployHybrid: Optional[bool] = False
    bDeployPrivate: Optional[bool] = False
    bDeployPublic: Optional[bool] = False
    bDeployOther: Optional[bool] = False
    deployOtherRemarks: Optional[str] = ""
    bModelIaaS: Optional[bool] = False
    bModelOther: Optional[bool] = False
    bModelPaaS: Optional[bool] = False
    bModelSaaS: Optional[bool] = False
    otherModelRemarks: Optional[str] = ""
    explanationForNonOperational: Optional[str] = ""
    version: Optional[str] = ""
    categorizationJustification: Optional[str] = ""
    internalUsers: Optional[int] = 0
    externalUsers: Optional[int] = 0
    externalUsersFuture: Optional[int] = 0
    internalUsersFuture: Optional[int] = 0
    ...
    def from_dict(data: dict) -> "SecurityPlan":
        ...
        :param dict data: The dictionary to create the object from.
    ...
    def create_new_ssp(
        self, api: Api, return_id: Optional[bool] = False
    ) -> Union[int, None, "SecurityPlan"]:
        """
        ...
        if ssp_response.ok:
            ...
            return SecurityPlan.from_dict(ssp_response.json())
        logger = create_logger()
        logger.error(
            f"Failed to upload SSP: {ssp_response.status_code} - {ssp_response.text}"
        )
        return None
    ...
    def update_ssp(
        ...
        logger = create_logger()
        logger.error(
            f"Failed to update SSP: {ssp_response.status_code} - {ssp_response.text}"
        )
        return None
