...
class ControlImplementation(RegScaleModel):
    ...
    _model_slug = "controlImplementation"
    _model_string = "controls"
    ...
    @classmethod
    def get_object(cls, id: int) -> Optional["ControlImplementation"]:
        """
        Get an object by ID.
        ...
        response = cls._model_api_handler.get(
            endpoint=cls.get_endpoint("get").format(id=id)
        )
    ...
    def find_by_unique(self) -> Optional["ControlImplementation"]:
        ...
        for instances in self.get_by_security_control_id(
    ...
    def get_by_security_control_id(
        ...
        if response and response.ok:
            return [cls.get_object(id=ci["id"]) for ci in response.json()]
        return []
