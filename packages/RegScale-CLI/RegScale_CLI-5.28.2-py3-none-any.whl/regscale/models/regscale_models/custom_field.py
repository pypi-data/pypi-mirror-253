...
class CustomField(RegScaleModel):
    ...
    def get_list_by_module_id(cls, module_id: int) -> List["CustomField"]:
        ...
        response = cls._model_api_handler.get(
            endpoint=cls.get_endpoint("get_module").format(
                model_slug=cls._model_slug, moduleId=module_id
            )
        )
        custom_fields = []
    ...
    @classmethod
    def get_custom_fields_by_module_id(
        cls, parent_module_id: int
    ) -> List["CustomField"]:
        """
        Get a list of custom fields by module ID
...
class CustomFieldsData(RegScaleModel):
    ...
    def get_by_module_id(
        cls, parent_id: int, module_id: int
    ) -> Optional[List["CustomFieldsData"]]:
        """
        Get a list of custom fields by module ID.
        ...
        response = cls._model_api_handler.get(
            endpoint=cls.get_endpoint("get_custom_fields_data_get").format(
                model_slug=cls._model_slug, parentId=parent_id, moduleId=module_id
            )
        )
        if response and response.ok:
    ...
    def get_by_id(cls, cid: int, module_id: int) -> Optional["CustomFieldsData"]:
        ...
        response = cls._model_api_handler.get(
            endpoint=cls.get_endpoint("get_custom_fields_data").format(
                model_slug=cls._model_slug, id=cid, moduleId=module_id
            )
        )
        if response and response.ok:
            # print(response.json())
    ...
    @classmethod
    def get_list_by_parent_id_and_module_id(
        cls, parent_id: int, module_id: int
    ) -> List["CustomFieldsData"]:
        """
        Get a list of custom fields by module ID.
    ...
    def batch_update(cls, custom_data_fields: List["CustomFieldsData"]) -> bool:
        ...
        :rtype: bool
        """
        response = cls._model_api_handler.post(
            endpoint=cls.get_endpoint("update_batch_post").format(
...
class CustomFieldsSelectItem(RegScaleModel):
    _model_slug = "customFieldsSelectItems"
