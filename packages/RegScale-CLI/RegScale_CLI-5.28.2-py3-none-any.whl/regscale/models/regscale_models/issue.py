...
class Issue(RegScaleModel):
    ...
    def bulk_insert(
        ...
        :param Optional[int] batch_size: Number of issues to insert per batch, defaults to 100
        ...
        with Progress(transient=False) as progress:
            ...
            if batch:
                ...
                for batch in batches:
                    res = api.post(url=url, json=[iss.dict() for iss in batch])
                    if not res.ok:
                        app.logger.error(
    ...
    def bulk_update(
        ...
        with Progress(transient=False) as progress:
            ...
            if batch:
                ...
                for batch in batches:
                    res = api.put(url=url, json=[iss.dict() for iss in batch])
                    if not res.ok:
                        app.logger.error(
                            "%i: %s\nError inserting batch of issues: %s",
                            res.status_code,
