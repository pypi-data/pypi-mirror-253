...
class STIG:
    ...
    def __init__(
        ...
        self.all_implementation_options: set[ImplementationOption] = set()
    ...
    def create(
        ...
        instance = cls(
            folder_path, regscale_ssp_id, cci_mapping, regscale_dod_catalog_id
        )
    ...
    @staticmethod
    def process_directory(folder_path: str) -> list[str]:
        """
        Process a directory for .ckl files, if none found it will terminate the application
        :param str folder_path: Path to directory to process for .ckl files
        ...
        return files or error_and_exit(
            f"No .ckl files found in provided directory: {folder_path}"
        )
    ...
    def final_check(self) -> None:
        ...
        for rule in self.all_rules:
            for obj in self.all_implementation_objectives:
                if rule["STIGRef"] in obj["notes"] and rule["STIGRef"] + str(
                    obj["id"]
                ) not in {
        ...
        if objs_to_update:
            ...
            ssp_imps = self.get_implementations(
    ...
    def refresh_mega_api(self) -> None:
        ...
        if mega_res.ok:
            ...
            if not self.mega_data["normalizedControls"]:
                raise ValueError(
                    "Unable to continue, please load some controls to the SSP!"
                )
    ...
    def refresh_component_implementations(self, component_ids: set[int]) -> None:
        ...
        with ssp_implementation_progress:
            ...
            for component_id in component_ids:
                ...
                if existing_component_response.ok:
                    existing_component_implementations = (
                        existing_component_response.json()
                    )
            ...
            for imp in all_existing_component_implementations:
                if imp["id"] not in {
                    imp["id"] for imp in self.all_implementations
    ...
    def get_component_ids(self) -> set[int]:
        ...
        return {
            ...
            for cmp in self.api.get(
                self.config["domain"]
                + f"/api/components/getAllByParent/{self.regscale_ssp}"
            ).json()
        }
    ...
    def get_new_implementations(
        self, cci_list: list[str], component_ids: set[int], security_control_ids: dict
    ) -> set:
        """
    ...
    def create_new_implementations(self, new_imps) -> None:
        """
        Create new control implementations in RegScale
        :param new_imps: Set of new control implementations to create
        :return: None
        """
        if new_imps:
            ...
            with ssp_implementation_progress:
                ...
                for i in range(0, len(new_imps), 1000):
                    response = self.api.post(
                        url=self.config["domain"]
                        + "/api/controlimplementation/batchcreate",
                        json=new_imps[i : i + 1000],
                    )
                    if not response.raise_for_status():
                        ssp_implementation_progress.advance(
                            creating_imps, len(new_imps[i : i + 1000])
                        )
                        for imp in new_imps:
                            if imp["id"] not in {
                                imp["id"] for imp in self.all_implementations
    ...
    def refresh_implementations(
        self, cci_list: list[str], cat_id: int
        ...
        new_imps = self.get_new_implementations(
            cci_list, component_ids, security_control_ids
        )  # Slow
        self.logger.info(
            "Creating %i new implementation(s) in RegScale...", len(new_imps)
        )
    ...
    def create_objectives(
        self,
        implementation: dict,
        options: set[ImplementationOption],
        ...
        :param set[ImplementationOption] options: Set of implementation options
        ...
        for obj in control_objectives:
            ...
            unique_objectives = {
                (obj.objectiveId, obj.implementationId)
                for obj in self.all_implementation_objectives
            }
            if (imp_obj.objectiveId, imp_obj.implementationId) not in unique_objectives:
                ...
                match_obj = [
                    obj
                    for obj in self.all_implementation_objectives
                    if obj.objectiveId == imp_obj.objectiveId
    ...
    def mass_update(self, update_objs: list[dict], insert_objs: list[dict]) -> None:
        ...
        if insert_objs:
            update(
                objs=insert_objs, progress=insert_objective_progress, method="insert"
            )
        if update_objs:
            update(
                objs=update_objs, progress=update_objective_progress, method="update"
            )
    ...
    def update_ssp_issues(self) -> None:
        ...
        for control in control_implementations:
            ...
            elif (new_issue.status != issue["status"]) or (
    ...
    def update_ssp_implementation_objectives(self) -> None:
        ...
        def process_ssp_objectives(obj) -> None:
            """
            ...
            if obj.objectiveId not in {obj.objectiveId for obj in ssp_objectives}:
                ...
                if (
                    update_obj
                    and update_obj.status != obj.status
                    and update_obj.optionId != obj.optionId
                ):
                    update_obj.status = obj.status
                    res = ImplementationObjective.update_objective(
                        app=self.app, obj=update_obj
                    )
                    if res.status_code != 200:
        ...
        updates = []
        self.logger.info(
            "Analyzing %i SSP implementation(s)...", len(ssp_control_implementations)
        )
        for _, imp in enumerate(ssp_control_implementations):
            ...
            implementation_dat: set[ImplementationObjective] = {
                dat
                for dat in self.all_implementation_objectives
            ...
            if (
                len(cntr) > 0
                and cntr[ImplementationStatus.FULLY_IMPLEMENTED.value] == cntr.total()
            ):
                ...
                with ThreadPoolExecutor(max_workers=5) as imp_executor:
                    ...
                    for imp in batch:
                        imp_executor.submit(
                            ...
                            + f"/api/controlimplementation/{imp['id']}",
                            json=imp,
                        )
                    count += len(batch)
    ...
    def update_component_implementation_objectives(self) -> None:
        ...
        with component_progess as progress:
            ...
            for component in self.existing_components:
                ...
                for imp in self.get_implementations(
                    parent_id=component["id"], parent_module="components"
                ):
                    if imp["id"] not in {
                        imp["id"] for imp in self.all_implementations
    ...
    def update_implementation(self, existing_imp: dict, status: str) -> None:
        ...
        if existing_imp["status"] != status:
            ...
            if not response.ok:
                self.self.logger.error(
                    "Unable to update Implementation: %s", existing_imp
                )
    ...
    def post_options(self, options: list[ImplementationOption]) -> None:
        """Post Implementation Option to RegScale
        :param list[ImplementationOption] options: A list of Implementation Options
        :return: None
        """
        if new_options := [
            opt.dict() for opt in options if opt not in self.all_implementation_options
        ]:
            with component_progess as progress:
                ...
                for i in range(0, len(new_options), 1000):
                    response = self.api.post(
                        url=self.config["domain"]
                        + "/api/implementationOptions/batchcreate",
    ...
    def refresh_options(self) -> None:
        ...
        for cat in self.catalog_details:
            ...
            for opt in cat["options"]:
                self.all_implementation_options.add(ImplementationOption(**opt))
    ...
    def refresh_existing_components(self) -> None:
        ...
        comps = Component.get_components_from_ssp(
    ...
    def check_components(self) -> None:
        ...
        def post_component(component: Component) -> None:
            ...
            if component.title not in {comp.title for comp in self.existing_components}:
                response = self.api.post(
                    self.config["domain"] + "/api/components", json=component.dict()
                )
        ...
        with component_progess:
            ...
            with ThreadPoolExecutor() as title_pool:
                ...
                futures = [
                    title_pool.submit(gen_component, file) for file in self.files
                ]
                for future in as_completed(futures):
                    if future.done():
            ...
            if new_comps:
                ...
                with ThreadPoolExecutor(max_workers=10) as component_pool:
                    ...
                    # Process each file
                    futures = [
                        component_pool.submit(post_component, comp)
                ...
                with ThreadPoolExecutor(max_workers=5) as mapping_pool:
                    ...
                    futures = [
                        mapping_pool.submit(post_mapping, map) for map in new_mappings
                    ]
                    for future in as_completed(futures):
                        if future.done():
    ...
    def update_options(self) -> None:
        ...
        for obj in self.all_control_objectives:
            ...
            options = [
                opt
                for opt in self.all_implementation_options
    ...
    def get_security_controls(self) -> Tuple[int, list[dict]]:
        ...
        security_controls = []
        self.logger.info(
            "Querying Additional Catalog details, this may also take a while.."
        )
        self.query_catalog_details()
    ...
    def get_control_objective(self, security_control_id: int) -> list[dict]:
        ...
        control_objectives = [
            obj
            for obj in self.mega_data
            if obj["securityControlId"] == security_control_id
        ]
    ...
    def pull_fresh_asset_list(self) -> set[Asset]:
        ...
        for component in self.existing_components:
            ...
            if (
                existing_asset_response.ok
                and existing_asset_response.status_code != 204
            ):  # Empty content is 204
                for asset in existing_asset_response.json():
                    existing_assets.update({Asset.from_dict(asset)})
    ...
    def get_category_components(
        cls, existing_components: set, asset_category: str
    ) -> list[dict]:
        """
    ...
    def get_cci_from_rule(rule: dict) -> Optional[Any]:
        ...
        for key in ["VULN_ATTRIBUTE", "ATTRIBUTE_DATA"]:
            ...
            if rule[key] is None:
                return None
        if (
            rule["VULN_ATTRIBUTE"].lower() == "cci_ref"
            and "CCI" in rule["ATTRIBUTE_DATA"]
        ):
    ...
    def get_cci_from_vuln(cls, vuln: Any) -> Optional[Any]:
        ...
        if isinstance(vuln, dict) and isinstance(vuln["STIG_DATA"], list):
            ...
            try:
                ccis = [
                    dat["ATTRIBUTE_DATA"]
                    for dat in vuln["STIG_DATA"]
                    if cls.get_cci_from_rule(dat) is not None
                ]
                if ccis:
                    cci = ccis[0]
                return cci
            except (IndexError, TypeError):
                cls.create_logger().warning(
                    "Unable to process asset from this ckl property: %s", vuln
                )
        return None
    ...
    def build_asset_from_file(
        self, file: str
    ) -> Optional[Tuple[list[str], Optional[Asset]]]:
        """
        ...
        category_components = self.get_category_components(
            self.existing_components, asset_category
        )
        stig_vulns = obj["CHECKLIST"]["STIGS"]["iSTIG"]["VULN"]
        mac = (
            obj_asset["HOST_MAC"].upper()
        ...
        if "HOST_IP" in obj_asset.keys() and obj_asset["HOST_IP"]:
            components = [
                ...
                if cat["title"].lower() == f"{title.lower()}"
            ]
            if components:
    ...
    def build_assets(self) -> Tuple[list[str], set[Asset]]:
        ...
        def insert_asset_and_mapping(new_asset):
            ...
            if res.ok:
                ...
                mapping_res = self.api.post(
                    url=self.config["domain"] + "/api/assetmapping", json=mapping
                )
            ...
            with ThreadPoolExecutor() as executor:
                ...
                if new_assets:
                    ...
                    for asset in new_assets:
                        res_futures.append(
                            executor.submit(insert_asset_and_mapping, asset)
                        )
    ...
    def process(self, regscale_ssp_id: click.INT) -> None:
        ...
        self.logger.info(
            f"Processed component implementation objectives in {comp_end - comp_start:0.4f} seconds"
        )
        ...
        self.logger.info(
            f"Processed component implementations in {comp_imp_end - comp_imp_start:0.4f} seconds"
        )
        ...
        self.logger.info(
            f"Processed ssp issues in {issue_end - issue_start:0.4f} seconds"
        )
        self.logger.info("Done!")
