...
from regscale.models.regscale_models.software_inventory import SoftwareInventory
...
class NessusReport(BaseModel):
    ...
    def process_file_asset(*args: Any) -> Tuple[dict, dict, dict]:
        ...
        if asset not in existing_assets:
            res = Asset.insert_asset(app=app, obj=asset)
        else:
            existing_asset = [
                existing_asset
            ...
            res = Asset.update_asset(app=app, obj=asset)
    ...
    def get_asset_properties(app: Application, *args: Any) -> dict:
        ...
        tag_values = {tag_map[key]: "" for key in tag_map.keys()}
        ...
        return {
            ...
            "tenable_id": tag_values["tenable_id"]
            if "tenable_id" in tag_values
            else "",
    ...
    def process_nessus_files(regscale_ssp_id: int, folder_path: Path) -> None:
        ...
        def move_files(app: Application, ssp_id: int) -> None:
            """
            Move files to processed directory

            :param Application app: The RegScale CLI application instance
        ...
        # move files
        move_files(app=app, ssp_id=regscale_ssp_id)
    ...
    def prepare_tenable_data(
        ...
        :param Iterable["NessusReport"] vulns: An iterable of NessusReport objects to prepare for download
        ...
        """
        vuln_file = tempfile.NamedTemporaryFile(delete=False)
        ...
        with open(vuln_file.name, "wb") as vuln_file_wrapper:
            ...
            pickle.dump(all_vulns, vuln_file_wrapper)
            vuln_file.close()
            del all_vulns
            gc.collect()
        ...
        return vuln_file, existing_scans
    ...
    def sync_to_regscale(
        ...
        vuln_temp_file_path, existing_scans = NessusReport.prepare_tenable_data(
            existing_assets, vulns, logger
        )
        ...
        with ThreadPoolExecutor(max_workers=threads) as executor:
            with open(vuln_temp_file_path.name, "rb") as vuln_file:
                all_vulns = pickle.load(vuln_file)
            futures = [
                executor.submit(
        ...
        # close the temporary file, and unlink
        vuln_file.close()
        try:
            Path.unlink(Path(vuln_file.name))
        except PermissionError:
            logger.warning("Unable to delete temporary file: %s", vuln_file.name)
        del all_vulns
        gc.collect()
    ...
    def process_vuln_results(
        ...
        if len(new_vulns) > 0:
            ...
            logger.info(
                "Found %i new %s with a severity > info.",
                len(new_vulns),
                "vulnerability" if len(new_vulns) == 1 else "vulnerabilities",
            )
        return new_vulns
