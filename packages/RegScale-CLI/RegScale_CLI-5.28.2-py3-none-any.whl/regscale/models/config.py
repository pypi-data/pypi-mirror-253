...
for provider_name, fields in providers.items():
    fields["provider"] = provider_name.split("Provider")[0].lower()
    globals()[provider_name] = create_model(provider_name, **fields, __base__=Provider)
...
_all_classes = {
    name: obj for name, obj in getmembers(currentframe().f_globals) if isclass(obj)
}
# iterate over the provider keys, and add an Optional dynamically created provider class
for provider in providers.keys():
    if provider in _all_classes:
        Providers.__annotations__[provider.split("Provider")[0].lower()] = Optional[
            _all_classes[provider]
        ]
