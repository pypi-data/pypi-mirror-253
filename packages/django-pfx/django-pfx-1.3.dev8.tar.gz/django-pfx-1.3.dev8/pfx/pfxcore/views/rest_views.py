...
class ListRestViewMixin(ModelResponseMixin):
    ...
    def _parse_order(self, qs, order):
        def expr(fn):
            # Validate fields and raise FieldError if not valid.
            # order_by() do that if you pass fields as string, but here
            # we generate expressions to set nulls_first/nulls_last, then
            # the fields are not validated and will crash at the query
            # execution if not valid.
            qs.query.names_to_path(fn.split(LOOKUP_SEP), self.model._meta)
            return F(fn)

        for fn in order.split(','):
            if fn.startswith('-'):
                yield expr(fn[1:]).desc(nulls_last=True)
            else:
                if fn.startswith('+'):
                    fn = fn[1:]
                yield expr(fn).asc(nulls_first=True)
    ...
    def apply_view_order(self, qs):
        ...
        if order:
            try:
                return qs.order_by(*self._parse_order(qs, order))
