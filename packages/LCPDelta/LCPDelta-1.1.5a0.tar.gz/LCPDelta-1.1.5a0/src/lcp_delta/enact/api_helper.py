...
class APIHelper(APIHelperBase):
    ...
    # Series:
    def get_series_data(self, series_id : str, date_from : datetime, date_to : datetime, country_id : str, option_id : list[str] = None, half_hourly_average : bool = False, request_time_zone_id : str = None, time_zone_id : str = None) -> pd.DataFrame:
    ...
    #Ancillary Contracts:
    def get_ancillary_contract_data(self, ancillary_contract_type : str, option_one : Union[str, int], option_two : Union[int, str] = None) -> dict[str, pd.DataFrame]:
        ...
        if contract_type in dynamic_containment_contract_types:
            for data in response['data']:
                dt_object = parser.isoparse(data['Data'][0]['DeliveryStartGmt'])
                date_str = self.convert_date_time_to_right_format(dt_object.replace(hour=0, minute=0, second=0, microsecond=0))
                df = pd.DataFrame(data['Data'])
                if not df.empty:
                    order_id_column_name = df.columns[2]
