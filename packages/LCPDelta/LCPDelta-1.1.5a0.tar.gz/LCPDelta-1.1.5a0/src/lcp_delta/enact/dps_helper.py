...
class DPSHelper:
    ...
    def __initialise(self):
        ...
        self.data_by_subscription_id : dict[str, tuple[callable, pd.DataFrame]] = {}
    ...
    def initialise_series_subscription_data(self, series_id : str, country_id : str, option_id : list[str], handle_data_method : callable):
        now = dt.now()
        day_start = dt(now.year, now.month, now.day, tzinfo=now.tzinfo)
        initial_series_data = self.api_helper.get_series_data(series_id, day_start, now, country_id, option_id)
        initial_series_data[self.last_updated_header] = now
        self.data_by_subscription_id[self.__get_subscription_id(series_id, country_id, option_id)] = (handle_data_method, initial_series_data)

    def callback_received(self, m, subscription_id : str):
        (user_callback, all_data) = self.data_by_subscription_id[subscription_id]
        adjusted_callback = lambda x: user_callback(self.handle_new_series_data(all_data, x))
        self.hub_connection.on(m['data']['pushName'], adjusted_callback)

    def handle_new_series_data(self, all_data : pd.DataFrame, data_push_holder : list) -> pd.DataFrame:
        try:
    ...
    def subscribe_to_series_updates(self, handle_data_method : callable, series_id : str, option_id : list[str] = None, country_id = 'Gb') -> None:
        '''
        ...
        (handle_data, initial_data_from_series_api) = self.data_by_subscription_id.get(subscription_id, (None, pd.DataFrame()))
        if initial_data_from_series_api.empty:
            self.initialise_series_subscription_data(series_id, country_id, option_id, handle_data_method)
        else:
