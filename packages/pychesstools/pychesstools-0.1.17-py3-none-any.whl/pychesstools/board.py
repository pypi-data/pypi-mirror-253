...
from .clock import IncrementalClock
...
class ChessBoard:
    ...
    def __init__(
        ...
        *,
        ...
        self.clocks: dict[Color, IncrementalClock] | None = (
            {color: IncrementalClock(time_control) for color in COLORS}
            if time_control
            else None
        )
    ...
    def _castle(
        ...
        *,
        ...
        rook_init_sq = cast(str, self._initial_squares["rook", color, side])
            ...
            match result.group(1):
                ...
                case _:
                    if "[TimeControl " in pgn:
                        self.clocks = {}
                        for color in COLORS:
                            self.clocks[color] = IncrementalClock.from_pgn(pgn, color)
                ...
                for move_no, seconds_remaining in clock.history.items():
                    if existing_annotation := self._move_annotations.get(move_no):
                        ...
                        self._move_annotations[move_no] = (
                            IncrementalClock.format_annotation(seconds_remaining)
    ...
    def move(
        ...
        *,
        ...
        if "O-O" in notation:
            ...
            self._castle(
                self.turn, side, game_over_checked=True, seconds_elapsed=seconds_elapsed
            )
            return (
            ...
            pt_abbr, disambiguator, final_square, promotion = match.groups()
