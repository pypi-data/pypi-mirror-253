from spb import plot_vector, plot3d_parametric_line, plot3d_parametric_surface
...
filterwarnings("ignore", module="spb", lineno=483)
filterwarnings("ignore", module="spb", lineno=490)
filterwarnings("ignore", module="spb", lineno=504)

# Rename spb plot_vector to plot_vector_field
...
class Arrow2DSeries(BaseSeries):
    ...
    def __init__(self, start, direction, label="", **kwargs):
        ...
        **kwargs : Keyword arguments to be passed on
        ...
        self.rendering_kw.setdefault("headwidth", 7)
        self.rendering_kw.setdefault("headlength", 20)
        self.rendering_kw.setdefault("headaxislength", 20)
        if label != "":
...
TWO_D_B.renderers_map.update({Arrow2DSeries: TWO_D_B.renderers_map[Vector2DSeries]})
...
def check_dims(v, s=None):
    if hasattr(v[0], '__iter__') or type(v[0]) == Matrix:
        d = len(v[0])
        if d != 2 and d != 3:
            raise TypeError(f"First vector was neither 2D or 3D")
        for vv in v:
            if len(vv) != d:
