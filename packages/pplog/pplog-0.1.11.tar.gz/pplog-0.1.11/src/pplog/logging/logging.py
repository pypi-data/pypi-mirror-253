...
def setup_logging(config: Dict[str, Any], logging_handlers: Optional[List[logging.Handler]] = None):
    ...
    project_logger.setLevel(logging_conf["level"])
    if logging_handlers:
...
def log_uncaught_exception(exception: Exception) -> None:
    ...
    Returns:
        None
    """
    project_logger_name = __name__.split(".", maxsplit=1)[
        0
    ]  # assuming __name__ is used for module level logger names
    logger = logging.getLogger(project_logger_name)

    logger.critical("An uncaught exception happened: %s", exception, exc_info=exception)
