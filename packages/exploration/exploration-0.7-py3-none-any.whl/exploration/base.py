...
def moreExplored(
    ...
    if eArgs.index(a) > eArgs.index(b):
        return a
    else:
        return b
...
def getSkillLevel(
    ...
    """
    Given a `Skill` and a `RequirementContext`, looks up that skill in
    both the common and active `FocalContext`s of the
    `RequirementContext`'s state, and adds those numbers together to get
    an effective skill level for that skill. Note that
    `SkillCombination`s can be used to set up more complex logic for
    skill combinations across different skills; if adding levels isn't
    desired between `FocalContext`s, use different skill names.
    ...
    *,
    ...
    if tCount == 0:
        raise ValueError(
            ...
            " edit, goto, bounce, or follow."
        )
    elif tCount > 1:
        raise ValueError(
            f"You may only specify one of gain, lose, set, toggle,"
            f" deactivate, edit, goto, bounce, or follow (you provided"
            f" values for {tCount} of those)."
        )
...
class InverseSkill(SkillCombination):
    ...
    def effectiveLevel(self, ctx: 'RequirementContext') -> Level:
        ...
        if isinstance(self.invert, Level):
            ...
            return -getSkillLevel(self.invert, ctx)
...
def observeChallengeOutcomes(
    ...
    >>> r = observeChallengeOutcomes(e, [c], policy='specified')
    >>> r[0]['outcome']  # re-use the new outcome
    False
...
def observedEffects(
    ...
    >>> observedEffects(
    ...     yesYes,
    ...     observeChallengeOutcomes(yesYes, [c], 'mostLikely')
    ... )
    [3, 6]
    >>> resetChallengeOutcomes([c])
    >>> observedEffects(
    ...     yesNo,
    ...     observeChallengeOutcomes(yesNo, [c], 'mostLikely')
    ... )
    [3]
    >>> resetChallengeOutcomes([c])
    >>> observedEffects(
    ...     noYes,
    ...     observeChallengeOutcomes(noYes, [c], 'mostLikely')
    ... )
    [13]
    >>> resetChallengeOutcomes([c])
    >>> observedEffects(
    ...     noNo,
    ...     observeChallengeOutcomes(noNo, [c], 'mostLikely')
    ... )
    [12, 13]
    >>> warnings.filterwarnings('default')
    """
...
ExplorationAction: 'TypeAlias' = Union[
    ...
    Tuple[
        ...
        DecisionID,
        Transition,
        Optional[DecisionName],  # new name
    ...
    Tuple[Literal['take'], ContextSpecifier, DecisionID, Transition],
    Tuple[Literal['take'], FocalPointSpecifier, Transition],
...
- The string 'start' followed by a `DecisionID` /
    ...
    position(s) specifier, a `Domain`, and then optional
    `CapabilitySet`, mechanism state dictionary, and custom state
    dictionary objects (each of which could instead be `None` for
    default). This indicates setting up starting state in a new focal
    context. It always affects the active focal context, and a
    `BadStart` error will result if that context already has any active
    decisions in the specified domain. The specified domain must already
    exist and must have the appropriate focalization depending on the
    type of position(s) specifier given; use
    `DiscreteExploration.createDomain` to create a domain first if
...
- The string 'explore' followed by:
    ...
    * A `Transition` indicating the transition taken
...
- The string 'take' followed by a `ContextSpecifier`, `DecisionID`, and
    `Transition` represents taking that transition at that decision,
    updating the specified context (i.e., common vs. active; to update a
    non-active context first swap to it). Normal
...
def describeExplorationAction(
    ...
    if aType == 'noAction':
        ...
        _, startAt, domain, capabilities, mechanisms, custom = action
        Union[DecisionID, Dict[FocalPointName, DecisionID], Set[DecisionID]]
        at: str
        if isinstance(startAt, DecisionID):
            at = f" at {graph.identityOf(startAt)}"
        elif isinstance(startAt, dict):
            at = f" with {len(startAt)} focal point(s)"
        elif isinstance(startAt, set):
            at = f" from {len(startAt)} decisions"
        else:
            raise TypeError(
                f"Invalid type for starting location: {type(startAt)}"
            )
        ...
        newName: Optional[DecisionName] = action[-3]
        nowWord = "now "
        if newName is None:
            if destID is None:
                nowWord = ""
        ...
        return (
            f"explore {transition} from decision {frDesc} to {deDesc}"
        )
        ...
        if len(action) == 4:
            ...
            assert isinstance(action[3], Transition)
            context = action[1]
            fromID = action[2]
            transition = action[3]
            destID = graph.getDestination(fromID, transition)
            if fromID == destID:  # an action
                return f"do action {transition}"
            else:  # normal transition
                frDesc = graph.identityOf(fromID)
                deDesc = graph.identityOf(destID)

                return (
                    f"take {transition} from decision {frDesc} to {deDesc}"
                )
        elif len(action) == 3:
            assert isinstance(action[1], tuple)
            assert len(action[1]) == 3
            assert isinstance(action[2], Transition)
            ...
            if frID is None:
                ...
                destID = graph.getDestination(frID, transition)

                if frID == destID:
                    return "do action {transition}"
                else:
...
def mechanismInStateOrEquivalent(
    ...
    currentState = context.state['mechanisms'].get(
        mID,
