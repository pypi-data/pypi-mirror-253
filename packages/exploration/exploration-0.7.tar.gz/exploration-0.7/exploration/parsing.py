...
class ParseFormat:
    ...
    def parseSpecificTransition(
        ...
        """
        sep = self.formatDict[Lexeme.transitionAtDecision]
        n = content.count(sep)
        if n == 0:
            raise ParseError(
 ...
 (True, None, None, 0))
    ...
    def parseEffectFromTokens(
        ...
        if fType is None:
            raise ParseError(
                f"Unrecognized effect type {firstPart}. Check the"
        ...
        if fType in ("gain", "lose"):
            ...
            assert not isinstance(groupedArgs[0], list)
            ...
            if not isinstance(transition, base.Transition):
                raise ParseError(
                    f"Invalid arguments for '{fType}' effect. Needed a"
    ...
    def parseEffect(self, effectStr: str) -> base.Effect:
        ...
        True
        """
        return self.parseEffectFromTokens(lex(effectStr, self.reverseFormat))
 ...
 {\\n  val 11;\\n  assign var $_;\\n  op + $var $var;\\n}'
    ...
    def parseDecisionSpecifier(
        ...
        try:
            ...
            tokens = lex(specString, self.reverseFormat)
