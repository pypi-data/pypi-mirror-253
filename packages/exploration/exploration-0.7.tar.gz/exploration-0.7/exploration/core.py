...
class DecisionGraph(
    ...
    def __init__(self) -> None:
        ...
        # See `Equivalences`. Determines what capabilities and/or
            ...
            if spec.name not in self.nameLookup:
                ...
                if len(filtered) == 1:
                    ...
                    if domainHint is not None:
                        filterDesc += " in domain {repr(domainHint)}"
                    if zoneHint is not None:
                        filterDesc += " in zone {repr(zoneHint)}"
                    if len(filtered) == 0:
                        raise MissingDecisionError(
    ...
    def identityOf(
        ...
        if decision is None:
            ...
            return f"{dID} ({zSpec}{self.nameFor(dID)})"
    ...
    def createZone(self, zone: base.Zone, level: int) -> base.ZoneInfo:
        """
    ...
    def addTransition(
        ...
        revName: Optional[base.Transition] = None,
        tags: Optional[Dict[base.Tag, base.TagValue]] = None,
        annotations: Optional[List[base.Annotation]] = None,
        ...
        and self.getDestination(toDecision, revName) is not None
            ...
            raise TransitionCollisionError(
                ...
                f" {revName!r}: {revName!r} is already used as an edge"
                f" name at {self.identityOf(toDecision)}."
            )
        ...
        if revName is not None:
            ...
            self.setReciprocal(fromID, name, revName)
            self.setTransitionRequirement(toDecision, revName, revRequires)
            if revConsequece is not None:
                self.setConsequence(toDecision, revName, revConsequece)
    ...
    def addUnexploredEdge(
        ...
        domain. The new decision will not be placed into any zones.
        ...
        if destinationName in self.nameLookup and WARN_OF_NAME_COLLISIONS:
            warnings.warn(
                (
                    ...
                    f" '{destinationName}': A decision with that name"
    ...
    def replaceUnexplored(
        ...
        you'll get a `MissingDecisionError`. If a `revName` is provided,
        ...
        raised if the `connectTo`'s `revName` transition does not lead
        to an unexplored decision (it's okay if this second transition
        doesn't exist). A `TransitionCollisionError` will be raised if
        the unexplored destination decision already has an outgoing
        transition with the specified `revName` which does not lead back
        to the `fromDecision`.
        ...
        if revName is not None:
            revFromUnknown = self.getDestination(oldUnexplored, revName)
            if revFromUnknown not in (None, fromID):
                raise TransitionCollisionError(
                    f"Transition '{revName}' from '{oldUnexplored}' exists"
        ...
        if placeInZone is not None:
            if placeInZone is base.DefaultZone:
                ...
                if needsZoneInfo:
                    ...
                    for parent in self.zoneParents(connectID):
        ...
        if revName is not None:
            ...
            if otherOldUnknown is None:
                if revFromUnknown is None:
                    ...
                    self.addTransition(connectID, revName, fromID)
                    otherOldReciprocal = None
                else:
                    # Reciprocal exists, as revFromUnknown
                    otherOldReciprocal = None
            else:
                otherOldReciprocal = self.getReciprocal(connectID, revName)
            ...
            self.setReciprocal(fromID, transition, revName)
        ...
        if revName is not None:
            self.tagTransition(connectID, revName, revTags)
            self.annotateTransition(connectID, revName, revAnnotations)
    ...
    def addEnding(
        ...
        applied to the added transition and/or the ending decision. The
        new transition and decision are both automatically tagged with
        'ending', and the new node is placed in the `ENDINGS_DOMAIN`
        domain. Returns the ID of the decision that was used (or created).
        ...
        if namePlus in self.destinationsFrom(fromID):
            raise TransitionCollisionError(
        ...
        try:
            endID = self.resolveDecision(namePlus)
        ...
        self.setTransitionRequirement(fromID, namePlus, requires)
        if consequence is not None:
            self.setConsequence(fromID, namePlus, consequence)

        # Tag it as unvisited
        self.tagDecision(endID, 'unvisited')

        return (namePlus, endID)
...
class DiscreteExploration:
    ...
    def getSituation(self, step: Optional[int] = None) -> base.Situation:
        """
    ...
    def movementAtStep(self, step: int = -1) -> Tuple[
        ...
        decision, transition take, and destination decision for that
        ...
        For 'start' actions, the starting position and transition will
        be `None` but the destination will be the ID of the node started
        at.
    ...
    def mechanismState(
        ...
        state at the specified step if a step index is given). Mechanism
        states are properties of a `State` but are not associated with
        focal contexts.
        """
        situation = self.getSituation(step)
    ...
    def applyTransitionEffect(
        ...
        Returns a boolean indicating whether the normal transition
        destination should be canceled, assuming this effect is applied
        as part of a transition. Normally it will return `False`; if it
        returns `True` then the transition's normal position/activation
    ...
    def applyExtraneousEffect(
        ...
        *without* accounting for charges or delay values, since the
        ...
        Returns `True` if the effect is a `bounce` or `goto` effect
        which obviates the normal position update of a transition and
        `False` otherwise.
        ...
        if typ in ("gain", "lose"):
            ...
            if isinstance(value, List):  # capabilities list
                ...
                currentState = self.mechanismState(mechanism)
                if len(states) == 1:
            ...
            if self.isTraversable(followFrom, following):  # skip if not
                ...
                skipPositionUpdate = self.applyTransitionConsequence(
                    followFrom,
                    following,
                    moveWhich,
                    challengePolicy
        ...
        return False  # default return value if we didn't return above
    ...
    def applyExtraneousConsequence(
        ...
        'bounce' effects do nothing when applied extraneously because
        there is no normal movement to cancel.
    ...
    def applyTransitionConsequence(
        ...
        """
        Applies the effects of the specified fully-observed consequence
        to the current graph and state.
        ...
        Returns a boolean indicating whether the normal transition
        position update should be applied or not, assuming these
        consequences are being applied as a result of taking a
        transition.
        ...
        dID = now.graph.resolveDecision(decision)
        searchFrom = set()
        searchFrom = now.graph.bothEnds(dID, transition)
        ...
        consequence = now.graph.getConsequence(dID, transition)
        ...
        for index in effectIndices:
            ...
            and (toIndex is None or index <= toIndex)
                ...
                if self.applyTransitionEffect(
                    (dID, transition, index),
                    moveWhich
                ):
                    cancel = True
        return cancel
    ...
    def advanceSituation(
        ...
        Although `None` can be used for pending decisions, you cannot
        call `advanceSituation` with `None` as the action.
        ...
        as the action; the previous situation will have a 'passive'
        decision type and an appropriate 'start' action.
        ...
        status (see `updatePosition` and `hasBeenVisited`).
        """
        now = self.getSituation()
        ...
        if action == ('noAction',):
         ...
         or not (2 <= aLen <= 7)
            ...
            if action[0] == "warp":
                ...
                takeTransition = cast(base.Transition, action[3])  # type:ignore [misc]  # noqa
                if (
                ...
                takeTransition = cast(base.Transition, action[2])
                targetActive = targetContext['activeDecisions']
                activeInDomain = cast(
                    Dict[base.FocalPointName, Optional[base.DecisionID]],
            ...
            if action[0] == 'explore':
                ...
                destName = cast(Optional[base.DecisionName], action[-3])
    ...
    def explore(
        self,
        transition: base.Transition,
        destination: Optional[base.DecisionName],
        ...
        traversal of the specified transition. Uses
        `deduceTransitionDetailsAtStep` to figure out from the
        ...
        decision explored to 'exploring'. Returns the decision ID for the
        destination reached.
        ...
        if now.graph.hasBeenVisited(destID):
            raise ExplorationStatusError(
                f"Cannot explore to decision"
                f" {now.graph.identityOf(destID)} because it has"
                f" already been at least partially explored. Use"
                f" returnTo instead of explore when discovering a"
                f" connection back to a previously-explored decision."
            )
        ...
        self.advanceSituation(actionTaken, decisionType, challengePolicy)
    ...
    def returnTo(
        ...
        now.graph.replaceUnexplored(fromID, transition, destID, reciprocal)
        ...
        if whichFocus is not None:
            # A move-from-specific-focal-point action
            actionTaken = ('take', whichFocus, transition)
    ...
    def takeAction(
        ...
        try:
            ...
            graph.addAction(fromID, action, requires, consequence)

        # Update the transition requirement/consequence if requested
        # (before the action is taken)
        if requires is not None:
            graph.setTransitionRequirement(fromID, action, requires)
        if consequence is not None:
            graph.setConsequence(fromID, action, consequence)
        ...
        if whichFocus is not None:
            # A move-from-specific-focal-point action
            actionTaken = ('take', whichFocus, action)
    ...
    def retrace(
        ...
        if not now.graph.hasBeenVisited(destID):
            raise ExplorationStatusError(
    ...
    def warp(
        ...
        one. Returns the ID of the decision warped to.
        ...
        if consequence is not None:
            self.applyExtraneousConsequence(
                consequence,
                where=(destID, None),
    ...
    def wait(
        ...
        if consequence is not None:
            ...
            # we're "waiting at."
            self.applyExtraneousConsequence(observed)
        ...
        *transitions: Union[
        ...
        for entry in transitions:
            if isinstance(entry, base.Transition):
                now.graph.addUnexploredEdge(fromID, entry)
            elif len(entry) == 2:
                entry = cast(
                    Tuple[base.Transition, base.AnyDecisionSpecifier],
                    entry
                ...
                try:
                    destID = now.graph.resolveDecision(entry[1])
                    now.graph.addTransition(fromID, transition, destID)
                except MissingDecisionError:
                    destName: base.DecisionName
                    if isinstance(entry[1], tuple):
                        destName = entry[1][-1]
                    elif isinstance(entry[1], base.DecisionName):
                        destName = entry[1]
                    else:
                        raise
                    now.graph.addUnexploredEdge(fromID, transition, destName)
            elif len(entry) == 3:
                entry = cast(
                    Tuple[
                        base.Transition,
    ...
    def runCommandBlock(
        ...
        ...    commands.command('empty', 'tuple'),
        ...    commands.command('append', "'left'"),
        ...    commands.command('append', "'right'"),
        ...    commands.command('assign', 'transitions'),
