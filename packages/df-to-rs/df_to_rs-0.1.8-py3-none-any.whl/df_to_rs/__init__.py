...
class df_to_rs:
    ...
    def upload_to_redshift(self, df, dest):
        ...
        #df.to_csv(csv_buffer, index=False)#, quoting=csv.QUOTE_NONNUMERIC, escapechar='\\', doublequote=False)
        
        df.to_csv(csv_buffer, index=False, quoting=csv.QUOTE_ALL, quotechar='"')
        csv_buffer.seek(0)
        print(f"Time taken: {int(time.time() - start_time)} seconds")
    ...
    def upsert_to_redshift(self, df, dest_table, upsert_columns):
        """
        ...
        staging_table = dest_table + "_stg"
        create_staging_query = f"CREATE TABLE {staging_table} AS SELECT * FROM {dest_table} WHERE 1 = 0;"
        self.redshift_c.cursor().execute(create_staging_query)
        print(f"Time taken: {int(time.time() - start_time)} seconds")
        ...
        drop_staging_query = f"DROP TABLE {staging_table};"
