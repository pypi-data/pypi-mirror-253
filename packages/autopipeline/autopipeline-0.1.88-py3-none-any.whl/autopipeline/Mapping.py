...
def get_persuasion_effect(table, column, enum, description):
    ...
    description += " "+"'"+column+"_persuasion_effect' contains the persuasion effects of the post included in the '" + column +"' column. IMPORTANT: the '"+column+"__persuasion_effect' column contains numerical values ranging from 0.0 to 1.0, where 0.0 stands for the least persuasive, and 1.0 stands for the most persuasive."
    return table, enum, description
...
def tree_path(user_query, columns, description, function_chain, function_list):
    function_tree = '''
    {
        ...
        "calculatoe" <contains functions that calculate numerical scores from text>:
            "get_persuasion_effect": "This function takes in one of the columns as input, calculates the persuasion effect score of the contents in that column, and generate an additional column to include those."
        ...
        "calculator": [
            {
                ...
                "description": 
                ''' 
                This function takes in one of the columns as input, calculates the persuasion effect score of the contents in that column, and generate an additional column to include those. Don't select this if none of the columns matches the user query.
                ''',
                "parameters": {
                    "type": "object",
                    "properties": {"column": {
                        "type": "string",
                        "description": "the column to calculate persuasion effect score, you have to select one in the 'enum' field to summarize based on the description: "+description,
                        "enum": column_description,
                    }
                    }
                },
