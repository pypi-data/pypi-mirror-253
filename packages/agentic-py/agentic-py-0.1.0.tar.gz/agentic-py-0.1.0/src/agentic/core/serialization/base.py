...
def ensure_in_proto_type_registry(msg):
    if msg.DESCRIPTOR.full_name not in _type_registry:
        _type_registry[msg.DESCRIPTOR.full_name] = type(msg)
    elif _type_registry[msg.DESCRIPTOR.full_name] != type(msg):
        raise ValueError(
            f"Type {msg.DESCRIPTOR.full_name} already registered with different message type"
        )
