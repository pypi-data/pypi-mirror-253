...
logger = Logger.create_logger(object=logger_crud_code_object)
...
class GenericCRUD:
    ...
    def insert(self, *, schema_name: str = None, table_name: str = None, data_json: dict = None,
        ...
        if ignore_duplicate:
            logger.warning("Using ignore_duplicate, is it really needed?")
        ...
        if table_name in table_definition:
            ...
            logger.warning(f"database-mysql-local-python generic_crud.py Table {table_name} not found in database-mysql-local.table_definition_table data structure, we might need to run sql2code")
        ...
        try:
            try:
                ...
                if ignore_duplicate:
                    logger.warning("Existing record found, selecting it's id")
            ...
            logger.error(self._log_error_message(
                message="Error inserting data_json", sql_statement=insert_query, schema_name=schema_name),
    ...
    def update_by_where(self, *, schema_name: str = None, table_name: str = None, where: str = None,
        ...
        try:
            ...
            logger.error(self._log_error_message(
                message="Error updating data_json", sql_statement=update_query, schema_name=schema_name),
                object={"exception": exception})
            logger.end()
            raise
        ...
        try:
            ...
            logger.error(
                self._log_error_message(message="Error while deleting",
                                        sql_statement=update_query, schema_name=schema_name),
                object={"exception": exception})
            logger.end()
            raise
    ...
    def select_multi_tuple_by_where(self, *, schema_name: str = None, view_table_name: str = None,
        ...
        try:
            ...
            logger.error(self._log_error_message(
                message="Error selecting data_json", sql_statement=select_query, schema_name=schema_name),
                object={"exception": exception})
            logger.end()
            raise
