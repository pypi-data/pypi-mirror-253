...
class CallHttp:
    ...
    def print_response(self):
        pawn.console.log(f"Response from '{self.method.upper()}' '{self.url}' 👉 {self.response}")
        if not self.response.success:
            style = "red"
        else:
            style = "rule.line"

        status_code_with_message = HTTPStatus(self.response.status_code)

        # pawn.console.rule(f"<Response {self.response.status_code}> ", align='right', style=style, characters="═")
        pawn.console.rule(f"<Response {self.response.status_code}> ", align='right', style=style, characters="═")

        if self.response.json:
            print_json(self.response.json)
        else:
            print_syntax(self.response.text, name="html", style="one-dark")
