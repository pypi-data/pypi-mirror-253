...
####+END:
    ...
    home = pathlib.Path.home()
    bposBase = home.joinpath('bpos')
    bposBase.mkdir(parents=True, exist_ok=True)
    assert bposBase.exists()
    return bposBase

####+BEGIN: b:py3:cs:method/typing :methodName "read" :deco "staticmethod"
    """ #+begin_org
**  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(outline-show-branches+toggle)][|=]] [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  Mtd-T-     [[elisp:(outline-show-subtree+toggle)][||]] /read/  deco=staticmethod  [[elisp:(org-cycle)][| ]]
    #+end_org """
    @staticmethod
    def read(
####+END:
...
#+begin_src sh :results output :session shared
  csSiteRegBox.cs -i usgAcctBposBaseCreate
#+end_src
#+RESULTS:
:
: ['22222001']
...
** [[elisp:(org-cycle)][| *CmndDesc:* | ]]
        ...
        self.captureRunStr(""" #+begin_org
#+begin_src sh :results output :session shared
  csSiteRegBox.cs -i usgAcctBposBaseRead
#+end_src
#+RESULTS:
:
: ['22222001']
