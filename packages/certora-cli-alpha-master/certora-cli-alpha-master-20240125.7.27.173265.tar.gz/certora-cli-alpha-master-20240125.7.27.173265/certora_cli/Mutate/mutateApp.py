        ...
        if self.prover_conf is not None:
            ...
            if certora_run_result:
                ...
                if local:
                    ...
                    if src.is_file():
                        try:
                            target = sources_dir / Constants.SPLIT_STATS_DATA
                            src.replace(target)
                            self.with_split_stats_data = True
                        except Exception as e:
                            logging.debug(f"did not manage to copy splitStatsdata.json to current directory\n{e}")
                    else:
                    ...
                    if download_report_file(rule_report_link, Constants.SPLIT_STATS_DATA):
                        self.with_split_stats_data = True
            if local:
            ...
            if src.is_file():
                try:
                    ...
                    logging.debug(f"did not manage to copy {Constants.SPLIT_STATS_DATA} from {src.parent} to current directory {Path.cwd()}\n{e}")
            else:
...
def download_report_file(report_url: str, filename: str) -> bool:
    ...
    try:
        ...
        logging.debug(f"request for {url} failed")
        return False
    if response.status_code == 200:
