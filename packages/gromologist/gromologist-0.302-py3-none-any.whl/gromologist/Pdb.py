...
class Pdb:
    ...
    def add_conect(self, cutoff=2.05, cutoff_h=1.3):
        """
        ...
        try:
            ...
            for n, atom in enumerate(self.atoms):
                ...
                if n not in hs:
                    selected = [self.atoms[m].serial for m, a in enumerate(dists) if (0.1 < a < cutoff and m not in hs) or (0.1 < a < cutoff_h and m in hs)]
                else:
                    selected = [self.atoms[m].serial for m, a in enumerate(dists) if 0.1 < a < cutoff_h and m in hs]
