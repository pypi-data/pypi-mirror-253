...
def geometric_draw(func=None, draw=False):
        if func is None:
            return partial(geometric_draw, draw=draw)
    # def geometric_draw(func):
        @wraps(func)
        def wrapper_draw(self, *args, **kwargs):
            ...
            # if save_path:
            #     cv2.imwrite(save_path, image, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
            return res
        return wrapper_draw
...
class topology_match():
    ...
    def build_cost(self):
        ...
        lp = list(product(self.points, self.lines))
        distance = [p.distance(l) for p,l in lp]
        distance = np.array(distance).reshape((self.point_num, self.line_num)).astype(np.uint8)
        least = lcm(self.line_num, self.point_num)
        distance = np.tile(distance, (least // self.point_num, least // self.line_num))
        # print(dis.shape)
        return distance
...
if __name__ == '__main__':
    ...
    match_map = tm.hungarian_match_with_cost(distance_threshold=60, show=True)
    # tm.build_cost()
    print(match_map)