...
def activate_jan_api(app, is_already_active: bool):
    ...
    if not is_already_active:
        ...
        while not jan_is_open:
            try:
                ...
                x_loc, y_loc, h_loc, wd_loc = pyautogui.locateOnScreen(
                    os.path.join(packdir, "settings.png")
                )
                jan_is_open = True
            except Exception:
                print("Waiting for Jan to open...", file=sys.stderr)
        ...
        while not advanced_are_open:
            try:
                x_loc, y_loc, h_loc, wd_loc = pyautogui.locateOnScreen(
                    os.path.join(packdir, "advanced.png")
                )
        ...
        time.sleep(2)
        x_loc, y_loc, h_loc, wd_loc = pyautogui.locateOnScreen(
            os.path.join(packdir, "models.png")
        )
        ...
        for match in matches:
            ...
            time.sleep(4)
            try:
                x_loc, y_loc, h_loc, wd_loc = pyautogui.locateOnWindow(
                    os.path.join(packdir, "start.png"), title="Jan"
                )
                pyautogui.click(x_loc + 2, y_loc + 2)
            except ImageNotFoundException:
                x_loc, y_loc, h_loc, wd_loc = pyautogui.locateOnWindow(
                    os.path.join(packdir, "triangle.png"), title="Jan"
                )
                pyautogui.click(x_loc + 2, y_loc + 2)
            time.sleep(5)
            is_started = False
            while not is_started:
                try:
                    ...
                    x_loc, y_loc, h_loc, wd_loc = pyautogui.locateOnScreen(
                        os.path.join(packdir, "starting.png")
                    )
                    is_started = False
                    time.sleep(1)
                except ImageNotFoundException:
            ...
            time.sleep(2)
        x_loc, y_loc, h_loc, wd_loc = pyautogui.locateOnScreen(
            os.path.join(packdir, "reduce.png")
        )
        pyautogui.click(x_loc + 2, y_loc + 2)
    else:
        pass
...
def scrape_jan_through_api(
    ...
    if auto:
        warnings.warn(
            "The automatized API opening may lead to the initialization of only one model, and it may not be the desired one; if you don't want this to happen, set the auto parameter to False and activate API for desired model manually",
            MayActivateOnlyOneModelWarning,
        )
