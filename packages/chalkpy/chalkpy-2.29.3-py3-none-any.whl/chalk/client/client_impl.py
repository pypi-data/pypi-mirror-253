...
from the command line from '{os.getcwd()}'. If you are still having trouble, please contact Chalk support.""",
    ...
    def offline_query(
        ...
        if input is None:
            ...
            assert isinstance(input, (list, tuple)) and not isinstance(
                input_times, datetime
            ), "A sequence of inputs (one per computer) must be provided if using multiple computers"
            query_input = tuple(
                _to_offline_query_input(qi, times)
                for qi, times in zip(input, input_times if input_times is not None else [None] * len(input))
            )
    ...
    def await_operation_completion(
        ...
        ).await_operation(show_progress=show_progress, must_fail_on_resolver_error=(caller_method != "resolver_replay"))
