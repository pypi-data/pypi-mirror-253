...
class ReqCheckStr_Base:
    """Base class for check exact requirement by string samples

    VARIANTS for check
    ------------------
    add attributes with bool samples (case-insensitive)
        - True - if samples is definitely acceptable - Always need at least one True!!!
    ...
    :ivar _CHECK_FULLMATCH:
        ...
        False - if partial match by finding mentioned samples in samples actual

    :ivar _GETTER: function which will get the exact samples to check
    :ivar _VALUE_ACTUAL:
    """
