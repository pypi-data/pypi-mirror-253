...
def playAudioFile2(audioFile, vlcSpeed=1.0):
    # formulate the playback command
    macVlc = "/Applications/VLC.app/Contents/MacOS/VLC"
    windowsVlc = r'C:\Program Files\VideoLAN\VLC\vlc.exe'
    if config.thisPlatform == "macOS" and os.path.isfile(macVlc):
        command = f'''{macVlc} --intf rc --play-and-exit --rate {vlcSpeed} "{audioFile}" &> /dev/null'''
    elif config.thisPlatform == "Windows" and os.path.isfile(windowsVlc):
        command = f'''"{windowsVlc}" --intf dummy --play-and-exit --rate {vlcSpeed} "{audioFile}"'''
    elif config.thisPlatform == "Windows" and HealthCheck.isPackageInstalled("vlc"):
        ...
        # > scoop install vlc
        command = f'''vlc --intf dummy --play-and-exit --rate {vlcSpeed} "{audioFile}"'''
    elif HealthCheck.isPackageInstalled("cvlc"):
        command = f'''cvlc --play-and-exit --rate {vlcSpeed} "{audioFile}" &> /dev/null'''
    elif HealthCheck.isPackageInstalled("vlc"):
        command = f'''vlc --play-and-exit --rate {vlcSpeed} "{audioFile}" &> /dev/null'''
    # execute the command
    try:
        playback_process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
        #os.system(command)
    except:
        pass
