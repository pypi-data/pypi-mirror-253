...
class ControllerArc(Controller):
    ...
    def reset_controller(self):
        ...
        try:
            ...
            if reply[0] == "ERROR":
                raise azcam.AzcamError(f"SYR reset error: {reply[1:]}")
            ...
            if e.error_code == 2:
                raise azcam.AzcamError("Could not connect to controller server", error_code=2)
            else:
                raise
    ...
    def board_command(self, Command, BoardNumber, Arg1=-1, Arg2=-1, Arg3=-1, Arg4=-1):
        ...
        # check for ERROR
        if reply[0] == "ERROR":
            raise azcam.AzcamError(reply[1:][0])
