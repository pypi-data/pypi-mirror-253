...
from .actions import BnIdReadCardAction, FindByBnIdAction
...
class SSIN(dd.Model):
    # used by lino_presto which does not use bnid
    class Meta:
        abstract = True
        unique_together = [('national_id', 'nationality'), ('birth_certificate_no', 'nationality')]
    ...
    birth_certificate_no = models.CharField(
        _("Birth certificate no"), max_length=200,
        blank=True, null=True
    )
...
class BnIdCardHolder(SSIN):
    ...
    card_type = BnIdCardTypes.field(blank=True)
    ...
    read_bnid = BnIdReadCardAction()
    find_by_bnid = FindByBnIdAction()
        ...
        if must_read:
            ...
            if dd.plugins.bnid:
                elems.append(ar.instance_action_button(
                    self.read_bnid, msg, icon_name=None))
            else:
                elems.append(msg)
    ...
    def card_number_to_image_path(cls, card_number):
        ...
        return Path(settings.STATIC_ROOT, "contacts.Person.jpg")
    ...
    def make_demo_picture(self):
        ...
        #     'media', 'bnid', self.card_number + '.jpg')
