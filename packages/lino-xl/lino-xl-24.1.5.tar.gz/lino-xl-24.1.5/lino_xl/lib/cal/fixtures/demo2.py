...
def objects():
    ...
    if len(cal_users) == 0:
        raise Exception("No user type inherits from OfficeUser")
    ...
    def make_events(ETYPES, SUMMARIES):
    ...
    if dd.get_plugin_setting('cal', 'with_demo_appointments'):
        qs = EventType.objects.filter(is_appointment=True)
        qs = qs.exclude(**dd.str2kw('name', _("Absences")))
        ETYPES = Cycler(qs)
        SUMMARIES = Cycler((
        ...
        yield make_events(ETYPES, SUMMARIES)

    if dd.get_plugin_setting('cal', 'with_demo_absences'):
    ...
    # for model in models_by_base(BirthdayGenerator):
    #     for obj in model.objects.all():
    #         rc = ses.run(obj.do_update_events)
    #         if not rc.get('success', False):
    #             raise Exception("do_update_events() on %s returned %s" %
    #                             (obj, rc))
