...
class CancelEvent(dd.ChangeStateAction):
    """Cancel this calendar entry and mark all guests cancelled."""
    button_text = EntryStates.cancelled.button_text
    label = _("Cancel")
    # show_in_workflow = True
    # show_in_toolbar = False
    # required_states = 'draft took_place cancelled'
    required_states = 'suggested draft scheduled'
    ...
    def run_from_ui(self, ar, **kw):
        ...
        def ok(ar):
            ...
            for g in obj.guest_set.all():
                g.state = GuestStates.cancelled
                g.full_clean()
                g.save()
            ar.set_response(refresh=True)

        ar.confirm(ok,
                   _("Mark this calendar entry and all guests as cancelled."),
                   _("Are you sure?"))
...
add('90', _("Cancelled"), 'cancelled', button_text="☒")
# add('10', "☐", 'invited')
...
EntryStates.cancelled.add_transition(CancelEvent, name='cancel_entry')
