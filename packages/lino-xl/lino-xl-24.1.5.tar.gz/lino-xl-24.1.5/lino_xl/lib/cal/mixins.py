    ...
    def get_wanted_auto_events(self, ar=None):
        ...
        for ee in qs:
            if date is None and ee.is_user_modified():
                next_date = rset.get_next_suggested_date(ee.start_date, ar.logger)
                date = next_date
            ...
            if ee.auto_type in existing:
                ...
                if ee.is_user_modified():
                    ...
                    if oe.is_user_modified():
                        ar.debug("Oops: duplicate auto_type {}".format(ee.auto_type))
                        return wanted, unwanted
                    unwanted.append(oe)
        ...
        if event_type is None:
            ...
            ar.warning(_("No calendar entries because no entry type is configured"))
            return nothing()
        ...
        ar.debug("Generating events between %s and %s (max. %s).",
        ...
        with translation.override(self.get_events_language()):
            ...
            while max_events is None or event_no < max_events:
                if date > until:
                    ar.debug("Reached upper date limit %s for %s",
                            until, event_no)
                    break
                event_no += 1
                # print("20221129 event_no", event_no)
                if ignore_before and date < ignore_before:
                    ar.debug("Ignore %d because it is before %s",
                ...
                date = rset.find_start_date(date)
                if date is None:
                    ar.debug("Could not find next date after %s (%s).", last_date, event_no)
                    break
...
class Component(Started, DateRangeObservable,
    ...
    summary = models.CharField(
        ...
        max_length=dd.get_plugin_setting('cal', 'summary_length', 200))
