...
class Session(UserAuthored, Started, Ended, Workable, InvoiceGenerator):
    ...
    computed_duration = dd.DurationField(_("Computed duration"), blank=True, null=True, editable=False)
    ...
    def __str__(self):
        ...
        return tpl.format(user=self.user, when=when, what="#"+str(self.ticket.id))
        # return "%s # %s" % (self._meta.verbose_name, self.pk)
