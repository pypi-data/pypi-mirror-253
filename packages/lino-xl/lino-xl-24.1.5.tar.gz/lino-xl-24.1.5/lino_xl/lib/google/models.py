    ...
    def to_html(self):
        return E.table(
            ...
            E.tbody(
                ...
                E.tr(E.td("Total modified"), E.td(str(self.calendars)), E.td(str(self.events)), E.td(str(self.contacts)))
            )
        )
...
class Synchronizer:
    ...
    def pull_events(self, resource, sub, room):
        ...
        token, created = rt.models.google.EventSyncToken.objects.get_or_create(user=sub.user, subscription=sub)
        if created:
            token.lock()

        def sync10():
            events = self.make_api_call(
                lambda: resource.events().list(
                    calendarId=sub.calendar.google_id, maxResults=10, showDeleted=True,
                    syncToken=token.sync_token, pageToken=token.page_token)
            )
    ...
    def sync_calendar(self):
        ...
        Event = rt.models.cal.Event
        token, created = rt.models.google.CalendarSyncToken.objects.get_or_create(user=self.user)
        if created:
            token.lock()
        ...
        if not settings.SITE.is_demo_site:
            ...
            calendar_qs = Calendar.get_outward_insert_update_queryset(self.user)
            ...
            try:
                ...
                if (calendar_qs := calendar_qs.exclude(pk__in=synched_cals)).count():
                    self.failed_entries.calendars.append(calendar_qs)

                if (event_qs := event_qs.exclude(pk__in=synched_events)).count():
        ...
        def sync10():
            ...
            try:
                cals = self.make_api_call(
                    lambda: gcal.calendarList().list(
                        maxResults=10, syncToken=token.sync_token, showDeleted=True,
                        showHidden=True, pageToken=token.page_token
                    )
                )
            except HttpError:
                return
            ...
            for cal in cals.get("items", []):
                ...
                try:
                    calendar, _ = Calendar.insert_or_update_google_calendar(cal, synchronizer=self)
                except Exception as e:
                    logger.exception(e)
        ...
        for sub in CalendarSubscription.objects.select_related('calendar').filter(user=self.user):
            self.pull_events(gcal, sub, sub.calendar.room_calendars.filter(
    ...
    def sync_contacts(self):
        ...
        token, created = rt.models.google.ContactSyncToken.objects.get_or_create(user=self.user)
        if created:
            token.lock()
    ...
    def __call__(self, cal_only=False, contacts_only=False):
        ...
        try:
            if not contacts_only:
                if self.has_scope_calendar():
                    try:
                        ...
                        rt.models.google.CalendarSyncToken.objects.filter(user=self.user).update(modified=timezone.now())
                        rt.models.google.EventSyncToken.objects.filter(user=self.user).update(modified=timezone.now())
                        for qs in (self.failed_entries.calendars + self.failed_entries.events):
            ...
            ss.stats = tostring(E.div(self.stats.to_html(), E.br(), E.div(*[E.p(ln) for ln in self.errors.split('\n')])))
            ss.full_clean()
            ss.save_new_instance(ss.get_default_table().request())
            for kls, item in self.failed_entries.foreign_items:
                ffi = rt.models.google.FailedForeignItem(
                    job=ss, value=item, item_class=rt.models.contenttypes.ContentType.objects.get_for_model(kls))
                ffi.full_clean()
                ffi.save_new_instance(ffi.get_default_table().request())
...
def set_delete_signal_receivers(*args, **kwargs):
    ...
    def event_will_get_deleted(sender, instance, **kw):
        if instance.google_id:
            sub = rt.models.google.CalendarSubscription.objects.filter(
                ...
                calendar=instance._get_calendar()
            ).first()
    ...
    def event_deleted(sender, instance, **kw):
        if user := DELETED_EVENTS_META.get(instance.google_id):
            entry = rt.models.google.DeletedEntry(event_id=instance.google_id, user=user, calendar=False,
