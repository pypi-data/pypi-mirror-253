...
class AccessLockable(Modified):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.pk is not None:
            if self.locked:
                raise AccessLocked(f"Access to {self} as been locked by another transaction.")
            self.lock()
    ...
    # Use the following method's for troubleshooting, never use them in any production code.
    @classmethod
    def unlock_all(cls):
        cls.objects.filter(locked=True).update(locked=False, modified=timezone.now())

    @classmethod
    def unlock_objects(cls, pk_list):
        cls.objects.filter(pk__in=pk_list).update(locked=False, modified=timezone.now())

    @classmethod
    def get_locked_objects_pk(cls):
        return list(cls.objects.filter(locked=True).values_list('pk', flat=True))
    ...
    def _force_get(cls, pk):
        ...
        stuff = cls.objects.filter(pk=pk).values(*normal_fields, *foreign_keys.keys())[0]
        self = cls(**{key: value for key, value in stuff.items() if key in normal_fields})
        for name, fld in foreign_keys.items():
            if isinstance(fld.related_model, AccessLockable):
                ...
                setattr(self, name, fld.related_model.objects.get(id=stuff[name]))
        self.id = self.pk = pk
        return self
...
class GoogleCalendarSynchronized(GoogleSynchronized):
    ...
    if dd.is_installed('google'):
        ...
        def insert_or_update_into_google(self, resource, synchronizer):
            ...
            if self.google_id:
                c = synchronizer.make_api_call(lambda: resource.get(calendarId=self.google_id))
                if c.get('timeZone') is not None:
                    body.pop('timeZone')
                synchronizer.make_api_call(lambda: resource.update(calendarId=self.google_id, body=body))
                synchronizer.stats.calendars__out_update += 1
            else:
                c = synchronizer.make_api_call(lambda: resource.insert(body=body))
        ...
        def insert_or_update_google_calendar(cls, cal: dict, synchronizer):
            ...
            room = calendar.room_calendars.filter(name=calendar.name).order_by('pk').first()
            if room is None:
                room = Room(
                    name=cal.get("summary", ""),
    ...
    if dd.is_installed('google'):
        ...
        def get_outward_insert_update_queryset(cls, user):
            ...
            return qs.filter(cal__isnull=False).annotate(sub=models.Case(
                ...
                models.When(
                    models.Q(modified__gte=rt.models.google.CalendarSubscription.objects.filter(pk=models.OuterRef("sub")).values("eventsynctoken__modified")[:1]),
                    then=models.Value(True)
                ),
                default=models.Value(False),
                output_field=models.BooleanField()
        ...
        def insert_or_update_into_google(self, resource, synchronizer):
            ...
            if self.google_id:
                synchronizer.make_api_call(
                    lambda: resource.update(calendarId=cal.google_id, eventId=self.google_id, body=body)
                )
                synchronizer.stats.events__out_update += 1
            else:
                e = synchronizer.make_api_call(
                    lambda: resource.insert(calendarId=cal.google_id, body=body)
                )
        ...
        @classmethod
        def insert_or_update_google_event(cls, event: dict, room, synchronizer):
            user = synchronizer.user
            try:
...
class GoogleContactSynchronized(GoogleSynchronized):
    ...
    if dd.is_installed('google'):
        ...
        def insert_or_update_into_google(self, resource, synchronizer):
            ...
            if self.google_id:
                synchronizer.make_api_call(lambda: resource.updateContact(resourceName=self.google_id, body=person))
                synchronizer.stats.contacts__out_update += 1
            else:
                resp = synchronizer.make_api_call(lambda: resource.createContact(body=person))
