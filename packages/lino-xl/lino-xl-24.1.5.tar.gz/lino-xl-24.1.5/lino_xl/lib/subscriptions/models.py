...
from lino_xl.lib.ledger.mixins import SequencedVoucherItem, LedgerRegistrable
...
class SubscriptionItem(SequencedVoucherItem):
    ...
    def as_paragraph(self, ar, **kwargs):
        if ar.is_obvious_field('voucher'):
            ...
            rv = "({}) ".format(ar.obj2htmls(self.voucher.get_mti_leaf(), str(self)))
        ...
        if self.unit_price:
            rv += _(" à {} {}").format(self.unit_price,
                escape(dd.plugins.ledger.currency_symbol))
        if self.remark:
...
class SubscriptionPeriod(DateRange, InvoiceGenerator):
    ...
    def __str__(self):
        ...
        return _("{} in {}").format(
            rangefmt((self.start_date, self.end_date)), self.master)
