...
class Product(BabelNamed, BabelPreviewable, Duplicable, MemoReferrable, Storable):
    ...
    category = dd.ForeignKey(
        'products.Category', verbose_name=_("Category"),
        blank=True, null=True)
    delivery_unit = DeliveryUnits.field(default='piece')
    pieces_per_unit = dd.PositiveIntegerField(_("Pieces per unit"), blank=True, null=True)
    ...
    def barcode_svg(self, ar=None):
        if dd.plugins.products.barcode_driver and self.barcode:
            return '<img src="{}" alt="{}"/>'.format(
                settings.SITE.build_media_url('barcodes', self.barcode+".svg"),
                self.barcode)
        return "No barcode"
