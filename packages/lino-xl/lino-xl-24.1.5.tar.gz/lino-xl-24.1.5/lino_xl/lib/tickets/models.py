...
#     name = models.CharField(_("Designation"), max_length=200)
    ...
    reporting_type = ReportingTypes.field(blank=True)
    deadline = models.DateField(verbose_name=_("Deadline"),blank=True, null=True)
    ...
    def unused_get_row_permission(self, ar, state, ba):
        ...
        if ((self.group and not self.group.members.filter(
            user__id=ar.get_user().id).exists())
...
class Ticket(UserAuthored, mixins.CreatedModified, TimeInvestment,
    ...
    def quick_assign_to(self, ar):
        ...
        if self.group:
            ...
            qs = settings.SITE.user_model.filter_active_users(qs, dd.today(), prefix="user__")
            for m in qs:
    ...
    def get_user_queryset(cls, user):
        ...
        if user.is_anonymous:
            ...
            qs = qs.filter(
                Q(group__members__user__id=user.pk) |
