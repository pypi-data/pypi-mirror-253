...
class Tickets(dd.Table):
    ...
    def get_queryset(self, ar, **filter):
        qs = super().get_queryset(ar, **filter)
    ...
    def get_request_queryset(self, ar):
        ...
        if pv.interesting_for:
            qs = qs.filter(
                Q(order__partner=pv.interesting_for))
...
class TicketsToTalk(Tickets):
    ...
    # order_by = ["-id"]
    column_names = "summary:50 priority #deadline waiting_for " \
                   "workflow_buttons:40 *"
...
class TicketsNeedingMyFeedback(TicketsToTalk):
    ...
    column_names = "summary:50 priority #deadline last_commenter " \
                   "workflow_buttons:40 *"
