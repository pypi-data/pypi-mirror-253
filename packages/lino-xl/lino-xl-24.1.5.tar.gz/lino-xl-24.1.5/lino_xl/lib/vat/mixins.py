...
from lino_xl.lib.ledger.mixins import Payable, PaymentRelated
...
class VatDeclaration(ledger.Declaration):
    ...
    def intracom_statement_iterator(self):
        ...
        def annotator():
            ...
            def build_case(model):
                ...
                return Case(
                    When(~Exists(model.objects.filter(
                        **get_filter()
                    ).values('pk')), then=Value(0.0)),
                    default=Subquery(
                        model.objects.filter(
                            **get_filter()
                        ).order_by().values('partner').annotate(
                            total_by_partner=Sum('total_base')
                        ).values('total_by_partner')
                    ),
            ...
            if dd.is_installed('sales'):
                kw['vpi_base'] = build_case(rt.models.sales.VatProductInvoice)
            else:
                kw['vpi_base'] = Cast(
        ...
        qs = rt.models.contacts.Partner.objects.annotate(
            **annotator()
            ...
            total_base=F('vai_base') + F('vpi_base')
        ).filter(total_base__gt=0)
    ...
    def unused_intracom_statement_iterator(self):
        ...
        if False:
            qs = qs.annotate(total_base=Sum('ledger_movement_set_by_partner__amount'))
            qs = qs.filter(
                ledger_movement_set_by_partner__journal__trade_type=TradeTypes.sales,
                # ledger_movement_set_by_partner__accounting_period=ap,
                ledger_movement_set_by_partner__vat_regime__in=intracom_regimes,
                total_base__gte=0,
                **self.get_period_filter('ledger_movement_set_by_partner__'))
            return qs
        else:
            if True:
            ...
            if dd.is_installed("sales") and True:
                qs = qs.annotate(vatproductinvoice_base=Sum(
                    ...
                    filter=Q(
                        ...
                        **self.get_period_filter('sales_vatproductinvoice_set_by_partner__')
                    )
            ...
            # print(qs.query)
            # qs = qs.order_by('vat_id').distinct()
            qs.annotate(total_base=F('vataccountinvoice_base') + F('vatproductinvoice_base')).filter(total_base__gte=0)
            return qs
            # for p in qs:
            #     # p.total_base = 0
            #     # if True:
