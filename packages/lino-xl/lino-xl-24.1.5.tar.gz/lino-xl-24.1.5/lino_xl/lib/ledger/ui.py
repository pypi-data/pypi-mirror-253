...
class Vouchers(dd.Table):
    ...
    detail_layout = "ledger.VoucherDetail"
...
class PartnerVouchers(Vouchers):
    ...
    def get_request_queryset(cls, ar, **kwargs):
        ...
        if pv.cleared == dd.YesNo.yes:
            qs = qs.exclude(ledger_movement_set_by_voucher__cleared=False)
        elif pv.cleared == dd.YesNo.no:
            qs = qs.filter(ledger_movement_set_by_voucher__cleared=False, ledger_movement_set_by_voucher__account__clearable=True)
        return qs
