...
class Journal(BabelNamed, Sequenced, Referrable, PrintableType):
    ...
    auto_check_clearings = models.BooleanField(_("Check clearing"), default=True)
    auto_fill_suggestions = models.BooleanField(_("Fill suggestions"), default=True)
    ...
    make_ledger_movements = models.BooleanField(_("Make ledger movements"), default=True)
    if dd.is_installed("storage"):
        make_storage_movements = models.BooleanField(_("Make storage movements"), default=False)
    else:
...
if has_payment_methods:
    ...
    class PaymentMethod(BabelDesignated):
        ...
        # journal = dd.ForeignKey('ledger.Journal')  # not used

        payment_account = dd.ForeignKey(
            'ledger.Account', verbose_name=_("Payment account"),
...
# class Voucher(UserAuthored, Duplicable, Registrable, UploadController, PeriodRangeObservable):
    ...
    def do_and_clear(self, func, do_clear):
        ...
        existing_ledger_mvts = self.ledger_movement_set_by_voucher.all()
        if dd.is_installed("storage"):
        ...
        if not (self.journal.auto_check_clearings
            ...
            do_clear = False
        if do_clear:
            for m in existing_ledger_mvts.filter(
        ...
        func(partners, products) # Creates wanted movements and fills the partners set.
    ...
    def create_ledger_movement(self, item, acc_tuple, project, amount, **kw):
        # dd.logger.info("20151211 ledger.create_ledger_movement()")
        ...
        # if dc == DC.debit:  # 20201219  ledger.Account.create_ledger_movement
        #     amount = - amount
        kw['amount'] = amount
...
class Movement(ProjectRelated, MovementBase):
    ...
    def get_siblings(self):
        return self.voucher.ledger_movement_set_by_voucher.all()
    ...
    def get_checkdata_problems(self, obj, fix=False):
        if obj.__class__ is rt.models.ledger.Voucher:
            if obj.get_mti_leaf() is None:
                ...
                if fix:
                    obj.ledger_movement_set_by_voucher.all().delete()
                    obj.delete()
            return
        ...
        for em in obj.ledger_movement_set_by_voucher.order_by('seqno'):
        ...
        if dd.is_installed('storage'):
            for em in obj.storage_movement_set_by_voucher.order_by('seqno'):
                ...
                for k in ('partner_id', 'product_id', 'qty', 'value_date'):
                    ...
                    if str(emv) != str(wmv):
                        diffs.append("{} (found {!r} != wanted {!r})".format(k, emv, wmv))
                if len(diffs) > 0:
                    yield (False, self.messages['storage_movement_diff'].format(
                        em, ', '.join(diffs)))
                    return
