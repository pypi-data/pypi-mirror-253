...
class Partner(Duplicable, ContactDetailsOwner, Polymorphic,
              AddressOwner, UploadController, Feasible, Printable,
              DateRangeObservable, VatSubjectable, MemoReferrable):
...
class Role(dd.Model, Addressable):
    ...
    person = dd.ForeignKey(
        "contacts.Person", related_name='rolesbyperson', blank=True, null=True)
    company = dd.ForeignKey(
        "contacts.Company", related_name='rolesbycompany', blank=True, null=True)
    ...
    def as_paragraph(self, ar):
        if ar.is_obvious_field('person'):
            ...
            txt = "{}, {}".format(ar.obj2htmls(self.person), ar.obj2htmls(self.company))
...
def show_birthdays(ar, today=None):
    ...
    qs = qs.annotate(bday=Right("birth_date", 5)).exclude(bday="00-00").order_by('bday')
    ...
    def fmt(obj):
        return "{} {}".format(obj.birth_date, ar.obj2htmls(
            obj, obj.get_full_name(salutation=False)))

    if todayqs.count():
        yield("<p>{}: ".format(_("Birthdays today")) + ", ".join([
            fmt(obj) for obj in todayqs])+"</p>")
    if recent.count():
        yield("<p>{}: ".format(_("Recent birthdays")) + ", ".join([
            fmt(obj) for obj in recent])+"</p>")
    if coming.count():
        yield("<p>{}: ".format(_("Upcoming birthdays")) + ", ".join([
            fmt(obj) for obj in coming])+"</p>")
