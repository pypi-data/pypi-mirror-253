...
from lino_xl.lib.ledger.models import LedgerRegistrable
...
class Storable(dd.Model):  # inherited by products.Product
    ...
    if dd.is_installed('storage'):
        storage_management = models.BooleanField(_("Storage management"), default=False)
    else:
    ...
    if dd.is_installed('storage'):
        ...
        def check_provisions_by_product(self, product):
            ...
            for jnl, qty in jnl_sums.items():
                ...
                for tr in rt.models.storage.TransferRule.objects.filter(journal=jnl):
            ...
            for ss, qty in state_sums.items():
                ...
                obj, created = rt.models.storage.Provision.objects.get_or_create(
                    product=product, partner=partner, storage_state=ss)
                obj.qty = qty
                obj.full_clean()
        ...
        def get_wanted_movements(self):
            ...
            if not self.journal.make_storage_movements:
                return
            # print("20230614 get_wanted_movements()")
            ...
            for i in self.items.all():
                if i.product:
                    ...
                    for ci in Component.objects.filter(parent=i.product).order_by('id'):
                        # qty = qty * ci.qty
