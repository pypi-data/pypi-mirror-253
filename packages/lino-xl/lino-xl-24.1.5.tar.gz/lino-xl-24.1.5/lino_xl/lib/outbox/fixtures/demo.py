...
def objects():
    ...
    from lino_xl.lib.outbox.models import RecipientTypes
    Person = rt.models.contacts.Person

    root = settings.SITE.user_model.objects.get(username='robin')

    mail = Instantiator('outbox.Mail').build
    recipient_to = Instantiator(
        'outbox.Recipient', type=RecipientTypes.to).build
