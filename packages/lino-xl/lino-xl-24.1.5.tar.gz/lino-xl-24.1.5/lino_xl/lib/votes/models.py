...
class Vote(UserAuthored, Created, Workable):
    ...
    state = VoteStates.field(default='watching')
    votable = dd.ForeignKey(
        dd.plugins.votes.votable_model,
        related_name="votes_by_ticket")
    priority = models.SmallIntegerField(_("Priority"), default=30)
    ...
    def on_create(self, ar):
        if dd.plugins.votes.with_candidatures and not self.state:
...
if dd.plugins.votes.with_candidatures:  # 20210908
