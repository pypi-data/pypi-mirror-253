...
class Task(Runnable):
    ...
    target_journal = dd.ForeignKey(
        'ledger.Journal', verbose_name=_("Target journal"),
    ...
    def invoice_generators(self, ar):
        if ar is None:
            return None
        return ", ".join([dd.full_model_name(m)
            for m in self.get_invoice_generators()])
    ...
    def source_journals(self, ar):
        ...
        return ", ".join([ar.obj2htmls(jnl, jnl.ref)
            for jnl in self.get_source_journals()])
...
class FollowUpRule(Sequenced):
    ...
    source_journal = dd.ForeignKey(
        'ledger.Journal', verbose_name=_("Source journal"), null=True, blank=True,
...
class Plan(UserPlan):
    ...
    provision_product = dd.ForeignKey('products.Product',
        verbose_name=_("Provision product"), blank=True, null=True,
    ...
    def __str__(self):
        ...
        return "{0} to {1} plan by {2}".format(
            self.invoicing_task, self.invoicing_task.target_journal.ref, self.user)
    ...
    def fill_plan(self, ar):
        ...
        for ig in self.get_generators_for_plan(self.partner):
            ...
            invoice_items = list(ig.get_wanted_invoice_items(info, invoice, ar))
            ...
            logger.debug("20230515 %s makes invoice %s for %s with items %s",
                ig, invoice.journal, partner, [i.product for i in invoice_items])
