...
def invoicing_task(journal_ref, **kwargs):
    ...
    try:
        ...
        raise Journal.DoesNotExist("No journal {} in {}".format(journal_ref, Journal.objects.all()))
    qs = rt.models.invoicing.Task.objects.filter(target_journal__ref=journal_ref)
    if qs.count() > 0:
        assert len(kwargs) == 0
        return qs.first()
...
def invoicing_rule(journal_ref, ig, **kwargs):
    ...
    return rt.models.invoicing.FollowUpRule(
        invoicing_task=it, invoice_generator=ct, **kwargs)
