...
class AddressOwner(AddressLocation):
    ...
    if dd.is_installed('addresses'):
        ...
        def get_primary_address(self):
            ...
            try:
                ...
                while p := self.get_address_parent():
                    if addr := p.get_primary_address():
                        return addr
            except Address.MultipleObjectsReturned:
                return
    ...
    def get_address_parent(self):
        if dd.is_installed("households"):
            if isinstance(self, dd.plugins.households.person_model):
                Member = rt.models.households.Member
                try:
                    return Member.objects.get(person=self, primary=True).household
