...
class AddressOwnerChecker(Checker):
    ...
    messages = dict(
        ...
        move_addr_to_parent=_("Primary address should be on {}."),
        remove_useless_addr=_("Same primary address as {}."),
        diff_child_addr=_("Primary address differs from that of {}."),
    )

    def get_checkdata_problems(self, obj, fix=False):
        # return  # test whether this is covered at all.
        ...
        ap = obj.get_address_parent()
        if ap != None and qs_marked_primary.count() != 0:
            mpa = qs_marked_primary[0]  # my primary address
            ppaqs = ap.addresses_by_partner.filter(primary=True) # parent's primary addresses
            if ppaqs.count() == 0:
                yield (True, self.messages['move_addr_to_parent'].format(ap))
                if fix:
                    mpa.partner = ap
                    mpa.full_clean()
                    mpa.save()
                ...
                ppa = ppaqs[0]
                diffs = ppa.get_diffs(mpa)
                if mpa.can_sync_from(diffs):
                    # print("20231225", obj, fix)
                    yield (True, self.messages['remove_useless_addr'].format(ap))
                    if fix:
                        mpa.delete()
                else:
                    yield (False, self.messages['diff_child_addr'].format(ap))
            return

        # Having no primary address is okay when the owner's address is empty.
