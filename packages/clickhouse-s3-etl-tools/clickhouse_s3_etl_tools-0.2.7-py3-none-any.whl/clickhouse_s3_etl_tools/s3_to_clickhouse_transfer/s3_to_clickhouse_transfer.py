...
def create_table_if_not_exists(
    ...
    if not conn.check_if_table_exists(
        ...
        logger.debug(f"databasemap {config.DATABASES_MAP}")
        create_table_query = update_create_table_query(
            metadata.create_table_query, config.table, config.CLUSTER_NAME, config.DATABASES_MAP
        )
...
def fetch_metadata_from_s3(
    ...
    try:
        res: List[Tuple[str, str, str, int, int]] = conn.execute_query_and_log(query)
