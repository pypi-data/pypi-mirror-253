...
from clickhouse_s3_etl_tools.configs import (
...
class ClickHouseConnector:
    ...
    def get_table_metadata(self, database: str, table_name: str) -> MetadataConfig:
        ...
        res: List[Tuple[str, str, str, int, int]] = self.execute_query_and_log(
            metadata_stat
        )

        columns: Tuple[str, str, str, int, int] = res[0]
...
def check_table_equality(ch_url_src: str,
                         ch_url_dst: str,
                         database_src: str,
                         table_name: str,
                         database_dst: Optional[str] = None) -> bool:
    """
    Check if two tables in ClickHouse are equal based on metadata and modification time.

    Args:
        ch_url_src (str): ClickHouse URL for the source database.
        ch_url_dst (str): ClickHouse URL for the destination database.
        database_src (str): Source database name.
        table_name (str): Table name to compare.
        database_dst (str, optional): Destination database name. If not provided, defaults to source database.

    Returns:
        bool: True if the tables are equal, False otherwise.

    Raises:
        ClickHouseError: If there is an error while connecting to ClickHouse.
    """
    with ClickHouseConnector(ch_url_dst) as conn_dest, ClickHouseConnector(ch_url_src) as conn_source:
        database_dst = database_dst or database_src
        conn_source.check_if_table_exists(database_src, table_name)
        if not conn_source.check_if_table_exists(database_dst, table_name, raise_error=False):
            logger.debug(f"Destination table {table_name} doesn't exist in database {database_dst}")
            return False

        metadata_src: MetadataConfig = conn_source.get_table_metadata(database_src, table_name)
        metadata_dst: MetadataConfig = conn_dest.get_table_metadata(database_dst or database_src, table_name)

        if metadata_src.total_rows != metadata_dst.total_rows:
            logger.debug("The number of rows is not equal.")
            logger.debug(
                f"Table: {table_name}, Source Total Rows: {metadata_src.total_rows}, Destination Total Rows: {metadata_dst.total_rows}")
            return False

        query_src: str = f"SELECT max(modification_time) FROM system.parts WHERE database='{database_src}' AND table = '{table_name}'"
        query_dst: str = f"SELECT max(modification_time) FROM system.parts WHERE database='{database_dst}' AND table = '{table_name}'"

        res1 = conn_dest.execute(query_src)
        res2 = conn_source.execute(query_dst)

        max_modification_time_src = res1[0][0] if res1 and res1[0] else None
        max_modification_time_dest = res2[0][0] if res2 and res2[0] else None

        if max_modification_time_dest < max_modification_time_src:
            logger.debug("The maximum modification time in source changed")
            logger.debug(
                f"Table: {table_name}, Source Max Modification Time: {max_modification_time_src}, Destination Max Modification Time: {max_modification_time_dest}")
            return False
