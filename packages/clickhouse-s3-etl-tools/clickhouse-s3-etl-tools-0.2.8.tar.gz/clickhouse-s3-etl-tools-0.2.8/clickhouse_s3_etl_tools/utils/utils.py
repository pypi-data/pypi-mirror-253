...
def check_rows_mismatch(
    ...
    if s3_row_count == clickhouse_row_count:
        ...
        mismatch_percentage = row_difference / s3_row_count * 100 if s3_row_count != 0 else 100
    else:
        mismatch_percentage = row_difference / clickhouse_row_count * 100 if clickhouse_row_count != 0 else 100

    if mismatch_percentage > max_percentage_diff:
        raise RowsMismatchError(s3_row_count, clickhouse_row_count, max_percentage_diff, mismatch_percentage)
...
def update_create_table_query(
    ...
    def _replace_databases(query: str) -> str:
        if databases_map:
            placeholders: list[Tuple] = [(f"{db.split(':')[0]}.", f"{db.split(':')[1]}.") for db in
                                         databases_map.split(',')]
    ...
    return _replace_merge_tree(create_table_query)
