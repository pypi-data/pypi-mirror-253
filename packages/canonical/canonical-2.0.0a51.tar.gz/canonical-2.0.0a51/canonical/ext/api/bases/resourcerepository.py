...
class BaseResourceRepository:
    ...
    async def get(
        ...
        *,
        ...
        if isinstance(key, str):
            ...
            key = meta.reference(name=key, namespace=namespace)
    ...
    async def import_documents(
        ...
        async with self.transaction() as tx:
            for doc in documents:
                ...
                await self.persist(new.model, new, transaction=tx, old=old)
                yield new
    ...
    async def persist(
        ...
        if old is None:
            try:
                old = await self.get(new.key, type(new))
            except self.DoesNotExist:
                old = None
        new = await self.persist_model(new, old=old, transaction=transaction)
        return new
