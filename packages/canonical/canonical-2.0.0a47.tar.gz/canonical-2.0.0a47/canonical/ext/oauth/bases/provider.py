# Copyright (C) 2023 Cochise Ruhulessin
#
# All rights reserved. No warranty, explicit or implicit, provided. In
# no event shall the author(s) be liable for any claim or damages.
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
from typing import Generic
from typing import TypeVar

import httpx
import pydantic

from canonical.ext.api import APIResourceModel
from ..builders import AuthorizationRequestBuilder
from ..models import ServerMetadata
from .providerspec import BaseProviderSpec


T = TypeVar('T', bound=BaseProviderSpec)


class BaseProvider(APIResourceModel[str], Generic[T], abstract=True):
    spec: T = pydantic.Field(
        default=...,
        description=(
            "Specification of the OAuth 2.x/OpenID Connect provider."
        )
    )

    def authorize(self, builder: AuthorizationRequestBuilder):
        assert isinstance(self.spec.metadata, ServerMetadata)
        assert self.spec.metadata.authorization_endpoint
        builder.with_authorize_url(self.spec.metadata.authorization_endpoint)

    def is_discoverable(self):
        return self.spec.is_discoverable()

    async def discover(self, client: httpx.AsyncClient):
        return await self.spec.discover(client)