...
class ShavingDynamics(_0.APIBase):
    ...
    _CastSelf = TypeVar("_CastSelf", bound="_Cast_ShavingDynamics")

    class _Cast_ShavingDynamics:
        """Special nested class for casting ShavingDynamics to subclasses."""

        def __init__(
            self: "ShavingDynamics._Cast_ShavingDynamics", parent: "ShavingDynamics"
        ):
            self._parent = parent

        @property
        def conventional_shaving_dynamics(
            self: "ShavingDynamics._Cast_ShavingDynamics",
        ):
            from mastapy.gears.manufacturing.cylindrical.axial_and_plunge_shaving_dynamics import (
                _751,
            )

            return self._parent._cast(_751.ConventionalShavingDynamics)

        @property
        def plunge_shaver_dynamics(self: "ShavingDynamics._Cast_ShavingDynamics"):
            from mastapy.gears.manufacturing.cylindrical.axial_and_plunge_shaving_dynamics import (
                _755,
            )

            return self._parent._cast(_755.PlungeShaverDynamics)
        ...
        def __getattr__(self: "ShavingDynamics._Cast_ShavingDynamics", name: str):
            try:
    ...
    def __init__(self: Self, instance_to_wrap: "ShavingDynamics.TYPE"):
    ...
    @property
    def cast_to(self: Self) -> "ShavingDynamics._Cast_ShavingDynamics":
