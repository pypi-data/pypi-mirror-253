...
import pandas as pd
from ringfit.utilities import omega
...
def data_analysis_QE(filter_query,analyses_parameters):
    measurements_iv = mr.get_measurements(filter_query=filter_query, measurement_type="iv_sweep")
    ...
    data_array_QE = []
    count = 0
    for dut in range(int(len(measurements_iv)/2)):
        alignment_error = 0
        pol_error = 0
        data_light = measurements_iv[dut*2+1]
        data_dark = measurements_iv[dut*2]
        ...
        # need tools to figure out the json files for this   
        #spectral_json = glob.glob(dut + '/spectral_iv_data' +'/*.json')
        # with open(spectral_json[0]) as f:
        #     die_dict = json.load(f)
        
        optical_power = 10**(data_light['power_before_sweep'][0]/10)*1e-3 #to be updated 
        #relative_attenuation = -np.float_(list(die_dict['splitting_ratios']['iv_relative_insertion_loss'].values()))[0] #check from the calibration file
        relative_attenuation = 26.63
...
def spectral_IV_analyses(filter_query, analyses_parameters):
    ...
    for dut in range(len(measurements)):
        # Need tools work to access this
        # spectral_json = glob.glob(dut + '/spectral_iv_data' +'/*.json') 
        # with open(spectral_json[0]) as f:
        #     die_dict = json.load(f)
        # die_dict = die_dict['splitting_ratios']['spectral_relative_insertionloss']
        
        # calibration_frequency = np.float_(np.array(list(die_dict.items()))[:,0])[4:-2]
        # calibration_insertion_loss = np.float_(list(die_dict.values()))[4:-2]
        calibration_insertion_loss  = -28 # static value is ised as a place holder
        
        spectral = measurements[dut]
        data_dark = measurements_iv[2*dut] #for now, the light and dark IVs should seperate eventually
...
def attenuation_sweep_data_analyses(filter_query):
    ...
    for dut in range(len(measurements)):
        ...
        data_dark = measurements_iv[2*dut]
        for v in voltages:
