...
class ICCreateView(generic.ObjectEditView):
    ...
    def alter_obj(self, obj, request, url_args, url_kwargs):
        ...
        if 'interface' in request.POST:
            try:
                obj.assigned_object = Interface.objects.get(pk=request.POST['interface'])
            except (ValueError, Interface.DoesNotExist):
                pass
        if 'site' in request.POST:
            try:
                obj.assigned_object =  Site.objects.get(pk=request.POST['site'])
            except (ValueError, Site.DoesNotExist):
                pass
        if 'circuit' in request.POST:
            try:
                obj.assigned_object =  Circuit.objects.get(pk=request.POST['circuit'])
        ...
        if 'application' in request.POST:
            ...
            except (ValueError, models.Application.DoesNotExist):
                pass
