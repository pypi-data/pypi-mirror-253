...
def run(
    ...
    analyses = dd.db.analysis.get_analyses_for_die_by_id(
        die_id=die_id,
    ...
    for analysis in analyses:
        device_names.append(analysis.device_data.device.name)
    ...
    if not values:
        raise ValueError(
            f"No analysis data found for die_id {die_id}, {device_attributes}, "
            f"device_analysis_function = {device_analysis_function!r}, metric = {metric!r}"
        )

    fig = plt.figure()
    plt.plot(device_names, values, "o")
    plt.xlabel("Device name")
    plt.ylabel(metric)
    plt.title(f"{metric} for die {die_id}")
