...
def plot_wafermap(
    ...
    Args:
        ...
        device_name: Name of the device to analyze.
...
def run(
    ...
    Args:
        ...
        metric: Metric to analyze.
    """
    analyses = dd.db.analysis.get_analyses_for_wafer_by_id(
        wafer_id=wafer_id,
        target_model="device_data",
        filter_clauses=[
            dd.AnalysisFunction.name == function_name,
            dd.Device.name == device_name,
        ],
    )
    ...
    for coord, analyses in analyses_per_die.items():
        max_analysis_index = np.argmax([a.id for a in analyses])
...
if __name__ == "__main__":
    ...
    d = run(
        ...
        device_name="rings_RingDouble-10-0.1-_331480_776211",
    )
    print(d["output"]["result"])
