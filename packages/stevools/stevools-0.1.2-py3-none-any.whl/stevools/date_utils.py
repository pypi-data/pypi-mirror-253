...
def split_date_evenly(start: datetime.date, end: datetime.date, n_periods: int) -> list[tuple[datetime.date, datetime.date]]:
    ...
    Examples:
        ...
        >>> print(split_date_evenly(start_date, end_date, 3))  # [(datetime.date(2021, 1, 1), datetime.date(2021, 1, 3)), (datetime.date(2021, 1, 4), datetime.date(2021, 1, 6)), (datetime.date(2021, 1, 7), datetime.date(2021, 1, 10))]
    ...
    Returns:
        (list[tuple[datetime.date, datetime.date]]) a list of tuples, each tuple contains two `datetime.date` objects
    """
    assert isinstance(start, datetime.date) and isinstance(end,datetime.date), f"start and end should be datetime.date objects, got {type(start)} and {type(end)}."
