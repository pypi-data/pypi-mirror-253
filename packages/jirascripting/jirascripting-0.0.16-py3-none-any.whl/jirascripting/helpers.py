...
import requests
from jirascripting.auth_and_headers import jira_auth_and_headers
import json
...
def get_audit_records(offset_=0, limit_=1000, filter_=None, date_time_from_=None, date_time_to_=None):
    """Returns a list of audit records. The list can be filtered.
    This function requires Administer Jira global permission and can't be used on the free version of Jira.
    Further information can be found here: https://developer.atlassian.com/cloud/jira/platform/rest/v3/api-group-audit-records/#api-rest-api-3-auditing-record-get
            Parameters
            ----------
                offset_ : int, optional
                    The number of records to skip before returning the first result. (default is 0)
                limit_ : int, optional
                    The maximum number of results to return. (default is 1000)
                filter_ : string, optional
                    The strings to match with audit field content, space separated.
                date_time_to_ : string, optional
                    The date and time on or before which returned audit results must have been created. - This format is ISO 8601: YYYY-MM-DDThh:mm:ss.sTZD
                date_time_from_ : string, optional
                    The date and time on or after which returned audit records must have been created. - This format is ISO 8601: YYYY-MM-DDThh:mm:ss.sTZD
            Returns
            ----------
                  Json Object: returns all comments in json in Jira document format
           ...
           """
    auth, headers, base_url = jira_auth_and_headers()
    url = f"{base_url}rest/api/3/auditing/record?offset={offset_}&limit={limit_}"
    if filter_ is not None:
        url = url + f"&filter={filter_}"
    if date_time_from_ is not None:
        url = url + f"&from={date_time_from_}"
    if date_time_to_ is not None:
        url = url + f"&to={date_time_to_}"
...
def get_jql(jql_in):
    """Returns a json object that contains the issue key, project key, project id, issue summary and issue description that are returned from the input JQL string
                Parameters
                ----------
                    jql_in : string
                        Example 'created >= -52w AND project = SOME_PROJECT_KEY AND labels = SOME_LABEL'

                Returns
                ----------
                      Json Object: returns all issues that satisfy the JQL query
    ...
    while url is not None:
        ...
        if len(json_res["issues"]) > 0:
            for issues in json_res["issues"]:
                issue = {
                    'issue_key': issues["key"],
                    'proj_key': issues['fields']['project']["key"],
                    'proj_id': issues['fields']['project']["id"],
                    'issue_summary': issues['fields']['summary'],
                    "description": issues['fields']['description']
                }
                issues_in_list.append(issue)

            if len(json_res["issues"]) == 50:
                url = f"{base_url}/rest/api/3/search?limit=50&startAt={count}"
                count = count + 50
            else:
                url = None
        else:
            url = None
    return issues_in_list
