# -*- coding:utf-8 -*-
'''
TODO 1: 新增seaborn繪圖風格
TODO 2: 新增風標、流線圖、風花圖
TODO 3: 檢查contourf的colors功能(非等間距色階)
'''
try:
    ...
    def initplot(ax=None, figsize=None, theme=Theme().get(), background=True):
        ...
        # 如果是cartopy.mpl.geoaxes.GeoAxes，則不做任何事
            ...
            raise TypeError(f"\033[41m[pymeili Error]\033[0m initplot() got an unexpected keyword argument: ax='{ax}'. plt.figure, plt.subplot, cartopy or Basemap are allowed.")
    ...
    def set_global(ax=None, crs=None):
        global _ax_
        if ax is None: ax = _ax_
        if type(ax) != cartopy.mpl.geoaxes.GeoAxesSubplot: 
        ...
        ax.set_global(crs=crs)
        return ax
    
    def add_shapefile(filename, ax=None, crs=ccrs.PlateCarree(), **kwargs):
        ...
        SF = ShapelyFeature(Reader(filename).geometries(), crs=crs, **kwargs)
