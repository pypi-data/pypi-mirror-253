...
def mrt_cp(sdif, sup, ldn, lup, sdir, cos_sza):
    # constants and coefficients usded for calculation of MRT
    s = 5.67e-8  # Stefan-Boltzmann constant
    fa = 0.5  # Angular factors
    ak = 0.7  # the absorption coefficient for shortwave radiation (standard value 0.7)
    ep = 0.97  # the emissivity of the human body (standard value 0.97)

    # convert cosZA to solar altitude in radians
    z = cp.arccos(cos_sza)
    h = cp.pi / 2.0 - z

    # Calculate the projected area factor (fp)
    # This formula was given by Sylvie Leroyer (2018)
    fp = 0.308 * cp.cos((cp.pi / 2.0 - z) * (1.0 - (90.0 - 180.0 / cp.pi * z) ** 2.0 / 48402.0))

    # convert vertical component of Sdir to incident solar radiation
    # h below 1 degree (0.02 radians) can result in Sdir much large than solar constant
    con1 = cp.where(h <= 0.000001, True, False)
    con2 = cp.where(h > 0.000001, True, False)
    con3 = cp.where(h <= 0.02, True, False)
    con4 = cp.where(h > 0.02, True, False)
    sdir = cp.where(con1, 0.0, sdir)
    sdir = cp.where(con2 & con3, sdir / cp.sin(0.02), sdir)
    sdir = cp.where(con4, sdir / cos_sza, sdir)

    # input the radiation parameters and calculate MRT
    mrt = ((1.0 / s) * (fa * ldn + fa * lup + ak / ep * (fa * sdif + fa * sup + fp * sdir))) ** (1.0 / 4.0) - 273.15
    return mrt


def calc_mrt(ssrd, ssr, fdir, strd, str, cos_sza, mask=None):
    """
    ...
    :param str: is surface net thermal radiation [J m-2]
    :param cos_sza: is cosine of solar zenith angle [dimentionless]
    :param mask: is mask array. If the type of the preceding parameters is numpy.ma.masked_array, mask should NOT be None

    :return: MRT [Degrees Celsius]
    """

    cos_sza_cp = cp.asarray(cos_sza)
    fdir_cp = cp.asarray(fdir)
    ssrd_cp = cp.asarray(ssrd)
    ssr_cp = cp.asarray(ssr)
    strd_cp = cp.asarray(strd)
    str_cp = cp.asarray(str)

    sdif = ssrd_cp - fdir_cp
    sup = ssrd_cp - ssr_cp
    lup = strd_cp - str_cp

    fdir_cp = fdir_cp / 3600.0
    sdif = sdif / 3600.0
    sup = sup / 3600.0
    strd_cp = strd_cp / 3600.0
    lup = lup / 3600.0

    if mask is None:
        mrt = mrt_cp(sdif, sup, strd_cp, lup, fdir_cp, cos_sza_cp)
        mrt_np = cp.asnumpy(mrt)
    else:
        mask_cp = cp.asarray(mask)
        cp.putmask(sdif, mask_cp, 1)
        cp.putmask(sup, mask_cp, 1)
        cp.putmask(strd_cp, mask_cp, 1)
        cp.putmask(lup, mask_cp, 1)
        cp.putmask(fdir_cp, mask_cp, 1)
        cp.putmask(cos_sza_cp, mask_cp, 1)

        mrt = mrt_cp(sdif, sup, strd_cp, lup, fdir_cp, cos_sza_cp)
        mrt_np = np.ma.masked_array(cp.asnumpy(mrt), mask)
...
def calc_utci(t2m_c, dt2m_c, va_ms, mrt_c, mask=None):
    """
    Calculate Human Thermal Comfort Index (UTCI) by cutci
