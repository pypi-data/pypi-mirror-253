...
class PerspectiveRayServer:
    ...
    async def update_table_rest(self, tablename: str, request: Request) -> Response:
        ...
        try:
            ...
            raise HTTPException(503, f"Exception during data ingestion: {tablename} / {format_exc()}")
...
class PerspectiveProxyRayServer:
    ...
    def __call__(self, op, tablename, data_or_schema=None):
        if op == "new":
        ...
        if op == "clear":
            if tablename:
                self._psp_handle.update.remote(tablename, None)
