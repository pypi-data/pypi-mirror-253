...
class SrElement:
    def __init__(self, dicom_data, *, warnings:bool=True) -> None:
        self._dicom_data = dicom_data
        self.issue_warning:bool = warnings
    ...
    def get(self, *concept_names: str, warnings:Optional[bool]=None) -> 'SrElement':
        '''
            Returns the first entry with the specified concept name.
            If no entry is found, a SrMissingContentException is raised.
        '''
        if len(concept_names) > 1:
            return self._get_first(concept_names[0], warnings=warnings).get(*concept_names[1:], warnings=warnings)

        if len(concept_names) == 1:
            return self._get_first(concept_names[0], warnings=warnings)

        raise ValueError('Invalid argument.')

    def _get_first(self, concept_name: str, *, warnings:Optional[bool]) -> 'SrElement':
        concept_it = self.get_all(concept_name)

        # If warnings is not explicitly specified, use default.
        if warnings is None:
            warnings = self.issue_warning

        try:
            first = next(concept_it)
        ...
        if warnings is not False:
            try:
                next(concept_it)
                warn('Warning: More than one "{0}" exists. Returning the first.'.format(concept_name))
            except StopIteration:
                pass
    ...
    def content_it(self) -> Iterator['SrElement']:
        ...
        if hasattr(self._dicom_data, 'ContentSequence'):
            return (SrElement(content, warnings=self.issue_warning) for content in self._dicom_data.ContentSequence)
