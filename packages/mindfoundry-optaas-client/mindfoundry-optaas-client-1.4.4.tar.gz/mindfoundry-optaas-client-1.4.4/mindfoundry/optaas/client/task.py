...
class Task:
    ...
    def __init__(self, json: dict, session: OPTaaSSession) -> None:
        ...
        self._targets_estimates_url = json['_links']['targets_estimates']['href']
        self._sqrt_crlb_url = json['_links']['sqrt_crlb']['href']
    ...
    def _setup_logging_level(self, logging_level: LoggingLevel) -> None:
        ...
        self._handler_id = logger.add(
            sys.stderr,
            filter=__name__.split(".", 1)[0],
            format="<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | <level>{level: <8}</level> | <cyan> Task.{function}</cyan> | <level>{message}</level>",
            level=logging_level,
            colorize=True,
        )

    def run(self, scoring_function: Callable[..., ScoringFunctionOutput], max_iterations: int,
            score_threshold: ScoreValueOrDict = None, logging_level: LoggingLevel = "INFO") -> Union[StoredResult, List[StoredResult]]:
        ...
        """
        self._setup_logging_level(logging_level)

        starting_msg = f'Running task "{self.title}" for {max_iterations} iterations'

        reached_threshold, msg = self._reached_threshold_if_defined(score_threshold)
        logger.info(f"{starting_msg} ({msg})\n")
        ...
        for i in range(max_iterations):
            ...
            if isinstance(scoring_result, tuple):
                ...
                score_msg = (
                    f"Iteration: {i} | Score: {score} | Variance: {variance}"
                )
            else:
                score = scoring_result
                variance = None  # type: ignore
                score_msg = f"Iteration: {i} | Score: {score}"

            logger.info(f"{score_msg} | Configuration: {configuration.values}")

            configuration = self.record_result(configuration=configuration, score=score, variance=variance) # type: ignore
    ...
    def _reached_threshold_if_defined(self, score_threshold: ScoreValueOrDict = None) -> Tuple[Union[Callable[[ScoreValue], bool],
                                                                                               Callable[[ScoreDict], bool]], str]:
        ...
        if score_threshold is None:
            ...
            if score_threshold is None:
                ...
                return lambda score: False, _NO_SCORE_THRESHOLD_MSG

        print(f'(or until score is {score_threshold} or better)')
        if goal == Goal.min:
            return lambda score: score <= score_threshold, _UNTIL_SCORE_THRESHOLD_MSG(
                score_threshold
            )
        return lambda score: score >= score_threshold, _UNTIL_SCORE_THRESHOLD_MSG(
                score_threshold
    ...
    def _reached_multi_objective_threshold_if_defined(objectives: List[Dict],
                                                      score_threshold: ScoreDict) -> Tuple[Callable[[ScoreDict], bool], str]:
        ...
        if score_threshold == {}:
            return lambda scores: False, _NO_SCORE_THRESHOLD_MSG
        ...
        return reached_all_thresholds, _UNTIL_SCORES_THRESHOLD_MSG(score_threshold)
    ...
    def add_user_defined_configuration(
        self,
    ...
    def record_results(self, results: List[Result], return_next_config: bool = True) -> List[Configuration]:
    ...
    def get_targets_estimates(self):
