...
from .Scaler3D import MinMaxScaler3D, StandardScaler3D, fillNaN3D, StandardScaler2D,MinMaxScaler2D
...
class DataLoader(AbstractDataLoader):
    ...
    def genEpochTrain(self, nb_batches, batch_size):
        ...
        if (CTX["ADD_AIRPORT_CONTEXT"]): x_batches_airport = np.zeros((nb_batches * batch_size, self.CTX["AIRPORT_CONTEXT_IN"]))

        NB=self.CTX["NB_TRAIN_SAMPLES"]

        toff_nb = len(x_batches)
        ...
        print("toff_nb ratio : ", toff_nb, "/", len(x_batches))
        ...
        if CTX["ADD_AIRPORT_CONTEXT"]: x_batches_airport = x_batches_airport.reshape(nb_batches, batch_size, self.CTX["AIRPORT_CONTEXT_IN"])
    ...
    def genEpochTest(self):
        ...
        if (CTX["ADD_AIRPORT_CONTEXT"]): x_batches_airport = np.zeros((NB_BATCHES, self.CTX["AIRPORT_CONTEXT_IN"]))
        ...
        for n in range(len(x_batches)):
            ...
            if (CTX["ADD_AIRPORT_CONTEXT"]):
                ...
                dists = SU.airportDistance([lat], [lon])[0]
                if (CTX["ADD_TAKE_OFF_CONTEXT"]):
                    lat, lon = SU.getAircraftPosition(CTX, x_batches_takeoff[n])
                    if (lat == 0 or lon == 0):
                        dists = np.concatenate([dists, np.zeros((len(dists),))])
                    else:
                        dists = np.concatenate([dists, SU.airportDistance([lat], [lon])[0]])
                x_batches_airport[n] = dists
    ...
    def genEval(self, path):
        ...
        if (CTX["ADD_AIRPORT_CONTEXT"]): x_batches_airport = np.zeros((len(array), self.CTX["AIRPORT_CONTEXT_IN"]))
        ...
        for t in range(0, len(array)):
            ...
            if (CTX["ADD_AIRPORT_CONTEXT"]):
                ...
                dists = SU.airportDistance([lat], [lon])[0]
                if (CTX["ADD_TAKE_OFF_CONTEXT"]):
                    lat, lon = SU.getAircraftPosition(CTX, x_batches_takeoff[t])
                    if (lat == 0 or lon == 0):
                        dists = np.concatenate([dists, np.zeros((len(dists),))])
                    else:
                        dists = np.concatenate([dists, SU.airportDistance([lat], [lon])[0]])
                x_batches_airport[t] = dists
        ...
        if (np.isnan(x_batches).any()):
            raise Exception("C ANOMALY : nan in x_batches")
