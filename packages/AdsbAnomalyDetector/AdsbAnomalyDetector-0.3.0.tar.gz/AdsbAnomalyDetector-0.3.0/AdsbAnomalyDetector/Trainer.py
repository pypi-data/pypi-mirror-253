...
from .save import write, load, formatJson
from . import Color as C
from .Color import prntC
from .plotADSB import plotADSB


from .AbstractModel import Model as _Model_
from .DataLoader import DataLoader
from .Utils import angle_diff
from .AbstractTrainer import Trainer as AbstractTrainer
...
class Trainer(AbstractTrainer):
    ...
    def train(self):
        ...
        if (CTX["ADD_AIRPORT_CONTEXT"]):
            write("./_Artifacts/"+self.model.name+".xas", self.dl.xAirportScaler.getVariables())
    ...
    def load(self):
        ...
        if (self.CTX["ADD_AIRPORT_CONTEXT"]):
            self.dl.xAirportScaler.setVariables(load("./_Artifacts/"+self.model.name+".xas"))
    ...
    def eval(self):
        ...
        for i in range(len(files)):
            ...
            for t in range(0, len(x_inputs)):
                pred = np.argmax(y_batches_[t])
                true = np.argmax(y_batches[t])

                if (y_batches_[t][pred] == 0):
                    correct_predict[t] = True
                else:
                    correct_predict[t] = pred == true
