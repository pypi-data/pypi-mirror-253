...
def feature_set(
    *,
    ...
    def decorator(function):
        ...
        if streaming_feature_set.backfill and not (
            streaming_feature_set.transformation.windows
            and streaming_feature_set.transformation.aggregations
        ):
            raise QwakException(
                "Streaming backfill is only supported for streaming aggregation feature sets at the moment. "
                "Please make sure windows and aggregations are configured properly."
            )
...
def metadata(
    *,
    ...
    def decorator(function):
        _validate_decorator_ordering(function)
        set_metadata_on_function(function, owner, description, display_name)
...
class StreamingFeatureSet(BaseFeatureSet):
    ...
    def _from_proto(cls, proto: ProtoFeatureSetSpec):
        ...
        return cls(
            ...
            transform=BaseTransformation._from_proto(streaming_def.transformation),
    ...
    def _to_proto(
        ...
        **kwargs,
        ...
        initial_tile_size: int = int()

        if not (
            not self.transformation.windows and not self.transformation.aggregations
        ):
            initial_tile_size = self._validate_streaming_aggregation()

            if self.backfill:
                self._validate_streaming_aggregation_backfill()
        ...
        if not self.transformation.windows and not self.transformation.aggregations:
    ...
    def _validate(self):
        ...
        if (
            not self.transformation.windows
            and not self.transformation.aggregations
            and self.backfill
        ):
            raise QwakException(
                "Streaming backfill is only supported for streaming aggregation feature sets at the moment"
    ...
    def _validate_streaming_aggregation(self) -> int:
        if (self.transformation.windows and not self.transformation.aggregations) or (
            not self.transformation.windows and self.transformation.aggregations
        ):
            raise QwakException(
                "At least one aggregation and one window should be defined"
            )
        ...
        if features_with_duplicates:
            ...
            for dup in features_with_duplicates:
                error_message_str += (
                    f"`{dup}` feature, appears more than once in Aggregations.\n"
                )
    ...
    def _get_streaming_aggregation_featureset_proto(
        ...
        return ProtoFeatureSetType(
            streaming_aggregation_feature_set=ProtoStreamingAggregationFeatureSet(
                ...
                backfill_spec=self.backfill._to_proto(
                    feature_registry=feature_registry,
