...
int main() {
    ...
    // Ensure msg and key are of the expected lengths
    if (msg.length() != 16 || key.length() != 16) {
        cout << "Error: Message and key should be 16 characters long." << endl;
    ...
    for (int i = 0; i < msg.length(); i++) {
     ...
     while(j!=msg.length())
     {
       cout<<msg[j]<<":"<<hexMsg[he]<<hexMsg[he+1]<<endl;
       j++;
       he+=2;
     }
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            keyGiven[j][i] = keyMsg.substr(k, 2);
            k+=2;
        }
    }
    ...
    cout << "\nInitial Transposition Matrix:(Add Round Key Operations)\n";
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            cout <<bin_to_hex(initTrans[i][j]) << " ";
        }
        cout << endl;
    }
