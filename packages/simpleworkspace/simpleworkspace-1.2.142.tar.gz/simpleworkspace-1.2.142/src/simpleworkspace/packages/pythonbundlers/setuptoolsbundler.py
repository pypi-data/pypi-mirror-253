...
class SetupToolsBundler:
    ...
    def Pipe_CleanUp(self):
        ...
        path_dist = f'{self.entryPath}/dist/'
        path_build = f'{self.entryPath}/build/'
        path_egginfo = f'{self.entryPath}/src/{self.packageName}.egg-info/'
        if(_os.path.isdir(path_dist)):
            _sw.io.directory.RemoveTree(path_dist)
            self.logger.info(f'    - Removed {path_dist}')
        if(_os.path.isdir(path_build)):
            _sw.io.directory.RemoveTree(path_build)
            self.logger.info(f'    - Removed {path_build}')
        if(_os.path.isdir(path_egginfo)):
            _sw.io.directory.RemoveTree(path_egginfo)
            self.logger.info(f'    - Removed {path_build}')
