...
class SetupToolsBundler:
    ...
    def Pipe_Init(self):
        ...
        initMessage = f'> Started Package Bundler'
        if(self.cli_Build):
            initMessage += ', Build=' + self.cli_Build
        print(initMessage, flush=True)
    ...
    def Pipe_IncrementPackageVersion(self):
        ...
        pyProject = toml.load(f"{self.entryPath}/pyproject.toml")
        currentVersion = pyProject["project"]["version"]
        newVersion = BumpMinorVersion(currentVersion)
        pyProject["project"]["version"] = newVersion
        _sw.io.file.Create(f"{self.entryPath}/pyproject.toml", toml.dumps(pyProject))
