...
class UhdTrackerJobs(TrackerJobsBase):
    ...
    def jobs_before_upload(self):
        if self.release_name.type is not utils.release.ReleaseType.movie:
            # See autodetect_automerge_group() below for an explanation.
            self.error('TV releases are currently not supported due to autogrouping issues.')
    ...
    # This should be working, but submitting a season release to an existing
    # season group didn't work. "auto_merge_group = on" was logged, but a new
    # season group was created. Until this is fixed, submission of seasons
    # should stay disabled.
    #
    # If this gets fixed, don't forget to write tests!
    async def autodetect_automerge_group(self, _):
        ...
        if self.season_number:
            ...
            for group_name in season_group_names:
                if group_name.endswith(target_group_name_suffix):
                    _log.debug('Found existing season group: %r', group_name)
                    return 'Yes'
                else:
                    _log.debug('Existing season group does not end with %r: %r',
    ...
    async def _get_season_group_names(self):
        ...
        for tag in doc.find_all('a', {'class': 'torrent_name'}):
            ...
            group_names.append(group_name.lower())
    ...
    def post_data(self):
        return {
            ...
            # Original title
            'title': (
                f'{self.release_name.title} Season {self.season_number:02d}'
                if self.season_number is not None else
                self.release_name.title
            ),
