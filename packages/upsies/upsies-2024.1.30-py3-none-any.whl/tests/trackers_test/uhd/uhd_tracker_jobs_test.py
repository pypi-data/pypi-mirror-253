...
@pytest.mark.parametrize('release_type', (utils.release.ReleaseType.season, utils.release.ReleaseType.episode))
def test_jobs_before_upload_does_not_accept_tv_release(release_type, uhd_tracker_jobs, mock_job_attributes, mocker):
    mocker.patch.object(type(uhd_tracker_jobs), 'release_name', PropertyMock(return_value=Mock(
        type=release_type,
    )))
    mocker.patch.object(uhd_tracker_jobs, 'error')
    uhd_tracker_jobs.jobs_before_upload
    assert uhd_tracker_jobs.error.call_args_list == [
        call('TV releases are currently not supported due to autogrouping issues.'),
    ]
...
def test_post_data(
    ...
    assert uhd_tracker_jobs.post_data == {
        ...
        'title': (
            f'{uhd_tracker_jobs.release_name.title} Season {season_number:02d}'
            if season_number is not None else
            uhd_tracker_jobs.release_name.title
        ),
