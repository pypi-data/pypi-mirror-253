...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        (
            ...
            ('Jake Blues, just released from prison, puts together his old band '
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        (Query('My Best Fiend', type=ReleaseType.movie, year=1999),
         ('The love-hate relationship between Werner Herzog and Klaus Kinski, '
          'the deep trust between the director and the actor, and their '
          'independently and simultaneously hatched plans to murder one another.')),
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        # Pygmalion (1983) (actor with "&apos;" in name)
        ('tt0190673', (("Peter O'Toole", 'https://www.imdb.com/name/nm0000564'),
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ...
        ('tt0192802', (('Kenneth Grahame', 'https://www.imdb.com/name/nm0334370'),
                       ('Ted Walker', 'https://www.imdb.com/name/nm0908249'))),  # Wind in the Willows (TV movie)
        ('tt0471711', (('Matt Groening', 'https://www.imdb.com/name/nm0004981'),
                       ('David X. Cohen', 'https://www.imdb.com/name/nm0169326'),
                       ('Ken Keeler', 'https://www.imdb.com/name/nm0444517'))),  # Bender's Big Score (Video)
        ('tt0097270', (('Bernard MacLaverty', 'https://www.imdb.com/name/nm0533735/'),)),  # Elephant (TV Short)
        ...
        ('tt5440238', (('Jenji Kohan', 'https://www.imdb.com/name/nm0463176'),
                       ('Anthony Natoli', 'https://www.imdb.com/name/nm6371396'),
                       ('Merritt Tierce', 'https://www.imdb.com/name/nm10019325/'))),  # Orange Is the New Black - S07E01 (episode)
        (None, ()),
    ),
    ids=lambda value: str(value),
)
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        # Blues Brothers (movie)
        ('tt0080455', 'https://m.media-amazon.com/images/M/MV5BYTdlMDExOGUtN2I3MS00MjY5LWE1NTAtYzc3MzIxN2M3OWY1XkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_.jpg'),
        # Wind in the Willows (TV movie)
        ('tt0192802', 'https://m.media-amazon.com/images/M/MV5BOGQ3NWM5OTAtOThjZS00MzMyLTkwZjMtOWIyOTdlZjFiZGI1XkEyXkFqcGdeQXVyNzMwOTY2NTI@._V1_.jpg'),
        # Bender's Big Score (Video)
        ('tt0471711', 'https://m.media-amazon.com/images/M/MV5BM2U3NmRiNDItMDQwYy00Y2Q0LWJiYzItNjAzNThkZjFlM2RiXkEyXkFqcGdeQXVyNTA4NzY1MzY@._V1_.jpg'),
        # Elephant (TV movie)
        ('tt0097270', 'https://m.media-amazon.com/images/M/MV5BMWE0ZTBiOWItY2ZkNS00MzI0LWE5Y2QtYjJmNjM1MGRkZGNmXkEyXkFqcGdeQXVyMzU0MTk1Nzc@._V1_.jpg'),
        # Kung Fury (Short)
        ('tt3472226', 'https://m.media-amazon.com/images/M/MV5BMjQwMjU2ODU5NF5BMl5BanBnXkFtZTgwNTU1NjM4NTE@._V1_.jpg'),
        # The Forest (mini series)
        ('tt6560040', 'https://m.media-amazon.com/images/M/MV5BMzY5NThkOWItN2I1OC00MzQ2LThlYjktMGExYTk2YzM1ZGRmXkEyXkFqcGdeQXVyODEyMzI2OTE@._V1_.jpg'),
        # Deadwood (series)
        ('tt0348914', 'https://m.media-amazon.com/images/M/MV5BMWE2MmU2MmEtNTVhMS00MTBhLWIyZjAtOGY2MGYxZGZlMDRhXkEyXkFqcGdeQXVyNDIzMzcwNjc@._V1_.jpg'),
        # Deadwood - S02E04 (episode)
        ('tt0556307', 'https://m.media-amazon.com/images/M/MV5BYjE2MDE3MTAtODc5Yi00NDMyLWFjNTEtNGE4MDYxNTliM2FlXkEyXkFqcGdeQXVyNTQ0NjQzNTE@._V1_.jpg'),
        # The Wingwall Auditions (no poster)
        ('tt0253248', ''),
        (None, ''),
    ),
    ids=lambda value: str(value),
...
@pytest.mark.parametrize(
    ...
    argvalues=(
        ('tt0080455', ('Jake Blues, just released from prison, puts together his old band '
                       'to save the Catholic home where he and his brother Elwood were '
                       'raised.')),  # Blues Brothers (movie)
        ...
        ('tt0200849', ('The love-hate relationship between Werner Herzog and Klaus Kinski, '
                       'the deep trust between the director and the actor, and their '
                       'independently and simultaneously hatched plans to murder one another.')),
        (None, ''),
    ),
