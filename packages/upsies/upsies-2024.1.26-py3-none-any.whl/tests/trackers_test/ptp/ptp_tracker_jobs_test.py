...
@pytest.mark.parametrize('edition, exp_edition_keys', (
    ('', []),
    ('Criterion', ['collection.criterion']),
    ('Criterion Collection', ['collection.criterion']),
))
@pytest.mark.parametrize('source, exp_source_keys', (
    ('', []),
    ('Remux', ['feature.remux']),
))
@pytest.mark.parametrize('audio_format, exp_audio_format_keys', (
    ('', []),
    ('DTS:X', ['feature.dtsx']),
    ('Atmos', ['feature.dolby_atmos']),
))
@pytest.mark.parametrize('hdr_format, exp_hdr_format_keys', (
    ('', []),
    ('DV', ['feature.dolby_vision']),
    ('HDR10', ['feature.hdr10']),
    ('HDR10+', ['feature.hdr10+']),
))
@pytest.mark.parametrize('has_dual_audio, exp_has_dual_audio_keys', (
    (False, []),
    (True, ['feature.dual_audio']),
))
@pytest.mark.parametrize('has_commentary, exp_has_commentary_keys', (
    (False, []),
    (True, ['feature.commentary']),
))
@pytest.mark.parametrize('bit_depth, exp_bit_depth_keys', (
    (None, []),
    (8, []),
    (10, ['feature.10bit']),
))
@pytest.mark.asyncio
async def test_autodetect_edition_map(
        edition, exp_edition_keys,
        source, exp_source_keys,
        audio_format, exp_audio_format_keys,
        hdr_format, exp_hdr_format_keys,
        has_dual_audio, exp_has_dual_audio_keys,
        has_commentary, exp_has_commentary_keys,
        bit_depth, exp_bit_depth_keys,
        ptp_tracker_jobs, mocker,
):
    mocker.patch.object(type(ptp_tracker_jobs), 'release_name', PropertyMock(return_value=Mock(
        edition=edition,
    ...
    exp_values_unordered = (
        exp_edition_keys
        + exp_source_keys
        + exp_audio_format_keys
        + exp_hdr_format_keys
        + exp_has_dual_audio_keys
        + exp_has_commentary_keys
        + exp_bit_depth_keys
    )
    # Order should be the same as in metadata.ptp_editions.
    exp_values_ordered = [
        value for value in ptp.metadata.ptp_editions
        if value in exp_values_unordered
    ]
    assert values == exp_values_ordered
...
def test__post_data_common_edition(ptp_tracker_jobs, mocker):
    mocker.patch.object(ptp_tracker_jobs, 'get_job_output')
    assert ptp_tracker_jobs._post_data_common_edition == {
        'remaster_title': ptp_tracker_jobs.get_job_output.return_value,
    }
    assert ptp_tracker_jobs.get_job_output.call_args_list == [
