...
def create_response(content):
    ...
    logger.info("returning success response", extra={"content": response.flaskify()[0], "status_code": 200})
    return response


def create_all_apis_failed_response(error):
...
def create_error_response(status_code, error):
    ...
    logger.info("returning failure response", extra={"content": response.flaskify()[0], "status_code": status_code})
    return response
