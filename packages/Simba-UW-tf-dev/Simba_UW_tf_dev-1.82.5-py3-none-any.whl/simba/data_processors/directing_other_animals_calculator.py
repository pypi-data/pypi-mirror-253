...
class DirectingOtherAnimalsAnalyzer(ConfigReader, FeatureExtractionMixin):
    ...
    def __init__(self,
        ...
        if not any([self.bool_tables, self.summary_tables, self.aggregate_statistics_tables, self.append_bool_tables_to_features]):
            raise InvalidInputError(msg='Please select at least ONE output format', source=self.__class__.__name__)
