...
class TradinghubClient():
	...
	async def onOrderStatusEvent(self, trade: Trade):
		if trade == self._entryTrade:
			...
			if trade.orderStatus.status == OrderStatus.Cancelled:
				...
				self._position = True
				self._establishStopLossAdjuster()
			...
			if trade.orderStatus.status == OrderStatus.Cancelled:
				...
				if self._closingLongLegs == False:
					asyncio.create_task(self._close_long_legs(self._entryTrade.orderStatus.filled)).add_done_callback(self.optraBot.handleTaskDone)
					self._onPositionClose()
	...
	async def _close_long_legs(self, positionSize: int):
	...
	async def _monitorPosition(self):
		...
		if self._stopLossAdjuster:
			if not self._stopLossAdjuster.isTriggered():
				...
				adjustedStopLossPrice = self._stopLossAdjuster.execute(currentIronFlyPrice)
				if adjustedStopLossPrice == None:
					logger.debug('No need to adjust stop loss now.')
				else:
					self._ironFlyStopPrice = adjustedStopLossPrice
					logger.debug('Performing Stop Loss Adjustment. New Stop Loss price: {}', self._ironFlyStopPrice)
			else:
	...
	def _establishStopLossAdjuster(self):
		"""
		Erstellt eine StopLoss Adjuster Instanz, sofern Stop Loss Adjustment konfiguriert ist
		und merkt sich die Instanz.
		"""
		try:
			trigger = int(self._config.get('tws.adjuststop.trigger'))
		except KeyError as keyErr:
			logger.debug('No StopLoss Adjustment configured. Nothing to do here.')
			self._stopLossAdjuster = None
			return
		
		try:
			stop = int(self._config.get('tws.adjuststop.stop'))
		except KeyError as keyErr:
			logger.debug('No StopLoss Adjustment configured. Nothing to do here.')
			self._stopLossAdjuster = None
			return
		...
		try:
			offset = float(self._config.get('tws.adjuststop.offset'))
		except KeyError as keyErr:
			logger.debug('No StopLoss Adjustment configured. Nothing to do here.')
			self._stopLossAdjuster = None
			return
		entryPrice = round(self._entryTrade.orderStatus.avgFillPrice * -1, 2)
		self._stopLossAdjuster = StopLossAdjuster(reverse=True, basePrice=entryPrice, trigger=trigger, stop=stop, offset=offset)