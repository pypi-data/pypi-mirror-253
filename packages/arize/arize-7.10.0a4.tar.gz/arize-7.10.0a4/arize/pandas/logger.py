...
from .spans.columns import (
    ROOT_LEVEL_SPAN_KIND_COL_NAME,
    SPAN_KIND_COL_NAME,
    SPAN_OPEN_INFERENCE_COLUMN_NAMES,
)
from .spans.constants import DEFAULT_DATETIME_FMT
from .spans.utils import convert_timestamps, jsonify_dictionaries
from .spans.validation import validator as spans_validator
...
class Client:
    ...
    def log_spans(
        ...
        errors = spans_validator.validate_argument_types(
            dataframe=dataframe,
            model_id=model_id,
        ...
        if validate:
            ...
            errors = spans_validator.validate_dataframe_form(dataframe)
            if errors:
                for e in errors:
                    logger.error(e)
                raise err.ValidationFailure(errors)
        ...
        df = self._remove_extraneous_columns(
            df=df, open_inference_column_list=SPAN_OPEN_INFERENCE_COLUMN_NAMES
        )
        ...
        if validate:
            ...
            errors = spans_validator.validate_dataframe_values(df)
            if errors:
                for e in errors:
                    logger.error(e)
                raise err.ValidationFailure(errors)
        ...
        if ROOT_LEVEL_SPAN_KIND_COL_NAME in df.columns and SPAN_KIND_COL_NAME not in df.columns:
            if verbose:
                logger.debug("Moving span kind to atributes")
            df.rename(columns={ROOT_LEVEL_SPAN_KIND_COL_NAME: SPAN_KIND_COL_NAME}, inplace=True)
        ...
        if validate:
            ...
            errors = spans_validator.validate_arrow_types(pa_schema)
            if errors:
                for e in errors:
                    logger.error(e)
                raise err.ValidationFailure(errors)
    ...
    def _remove_extraneous_columns(
        ...
        if open_inference_column_list is not None:
            df = df.loc[:, df.columns.intersection(set(open_inference_column_list))]
        return df
