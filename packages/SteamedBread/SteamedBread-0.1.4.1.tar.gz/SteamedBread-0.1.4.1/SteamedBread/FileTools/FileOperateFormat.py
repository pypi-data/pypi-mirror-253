...
class FileOperate:
    ...
    def read_excel(file_name, sheet_name):
        """
            读取Excel文件内容
        :param file_name: file_name: Excel文件名称
        :param sheet_name: Excel文件sheet表名
        :return: excel.to_dict(orient="list") or excel.to_dict(orient="dict")
        """
        excel = pandas.read_excel(file_name, sheet_name=sheet_name, engine='openpyxl')
        return excel

    @staticmethod
    def write_excel(data: dict, file_name, sheet_name='sheet1'):
        """
            写入Excel文件内容
        ...
        df1.to_excel(file_name, sheet_name=sheet_name, index=False, engine='openpyxl')
        print(f"✅ excel: {file_name} 写入完成")
