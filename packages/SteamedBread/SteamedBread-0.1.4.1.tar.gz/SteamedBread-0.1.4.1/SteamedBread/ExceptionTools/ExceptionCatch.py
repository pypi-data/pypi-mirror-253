...
@File: ExceptionTools.py
@Time: 2023/12/22 19:58
"""


def hook_exceptions(exception_types: (tuple, set, list) = None):
    """
    捕获异常装饰器
    若未指定捕获的非基类异常, 则默认是忽略其它异常捕获
    example: 若 exception_types = [AssertionError], 则只会捕获这个方法中抛出的AssertionError, 而会忽略类似于ValueError等其它异常
    :param exception_types: 指定的异常
    :return: None
    """
    if isinstance(exception_types, (set, list)):
        exception_types = tuple(exception_types)
    elif isinstance(exception_types, type):
        exception_types = (exception_types,)
    elif exception_types is None:
