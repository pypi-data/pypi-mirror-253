...
from waflibs import dns, error, log, utils
...
def generate_bind_file(
    ...
    for domain in domains:
        dns.bind.generate_bind_file(
            get_records(cursor, logger, domain),
            domain,
            bind_dir=bind_dir,
    ...
    for shared_name in ["ALL_PRIVATE", "ALL_PUBLIC"]:
        dns.bind.generate_bind_file(
            get_records(cursor, logger, shared_name, shared=True),
            shared_name,
            bind_dir=bind_dir,
...
def get_records(cursor, logger, domain, shared=False):
    ...
    while all_results:
        ...
        if result_type == "numeric":
            for row in results:
                ...
                hostname = utils.convert_origin(short_hostname, domain, shared)
            ...
            for row in results:
                ...
                name = utils.convert_origin(short_name, domain, shared)
