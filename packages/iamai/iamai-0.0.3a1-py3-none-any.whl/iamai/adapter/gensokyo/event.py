...
class MessageEvent(GSKEvent, BaseMessageEvent["GSKAdapter"]):
    """消息事件"""
    ...
    async def reply(
        self, message: BuildMessageType[GSKMessageSegment]
    ) -> Dict[str, Any]:
        """回复消息。
...
class PrivateMessageEvent(MessageEvent):
    ...
    async def reply(
        ...
        return await self.adapter.send_private_msg(
            user_id=self.user_id, message=GSKMessage(message)
        )
...
class GroupMessageEvent(MessageEvent):
    ...
    async def reply(
        ...
        return await self.adapter.send_group_msg(
            group_id=self.group_id, message=GSKMessage(message)
        )


class NoticeEvent(GSKEvent):
    """通知事件"""
