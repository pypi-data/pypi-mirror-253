...
__all__ = ["GSKMessage", "GSKMessageSegment", "escape"]
...
class GSKMessage(Message["GSKMessageSegment"]):
    ...
    @classmethod
    def get_segment_class(cls) -> Type["GSKMessageSegment"]:
        """获取消息字段类。

        Returns:
            消息字段类。
        """
        return GSKMessageSegment


class GSKMessageSegment(MessageSegment["GSKMessage"]):
    """GSK 消息字段。"""

    @classmethod
    def get_message_class(cls) -> Type[GSKMessage]:
        """获取消息类。
    ...
    @classmethod
    def node_custom(cls, user_id: int, nickname: str, content: "GSKMessage") -> Self:
        """合并转发自定义节点"""
        return cls(
            type="node",
            data={
