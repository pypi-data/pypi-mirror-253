...
from .event import GSKEvent, HeartbeatMetaEvent, LifecycleMetaEvent, MetaEvent
...
class GSKAdapter(WebSocketAdapter[GSKEvent, Config]):
    ...
    _api_id: int = 0
    _get_access_token_url: str = "https://bots.qq.com/app/getAppAccessToken"
    ...
    async def startup(self) -> None:
        ...
        #     self.config.access_token = await self.get_access_token()
        await super().startup()
    ...
    async def handle_websocket_msg(self, msg: aiohttp.WSMessage) -> None:
        ...
        if msg.type == aiohttp.WSMsgType.TEXT:
            ...
            if "post_type" in msg_dict:
                await self.handle_gsk_event(msg_dict)
            else:
    ...
    @classmethod
    def add_event_model(cls, event_model: Type[GSKEvent]) -> None:
    ...
    async def handle_gsk_event(self, msg: Dict[str, Any]) -> None:
        """处理 GSK 事件。
        ...
        gsk_event = event_class(adapter=self, **msg)

        if gsk_event.post_type == "meta_event":
            # meta_event 不交由插件处理
            assert isinstance(gsk_event, MetaEvent)
            if gsk_event.meta_event_type == "lifecycle":
                assert isinstance(gsk_event, LifecycleMetaEvent)
                if gsk_event.sub_type == "connect":
                    logger.info(
                        f"WebSocket connection "
                        f"from gensokyo Bot {msg.get('self_id')} accepted!"
                    )
            ...
            await self.handle_event(gsk_event)
    ...
    async def call_api(self, api: str, **params: Any) -> Any:
        ...
        logger.debug(f"api_echo is {api_echo}")
        try:
    ...
    async def send(
        self,
        message_: BuildMessageType[GSKMessageSegment],
        ...
        if message_type == "private":
            return await self.send_private_msg(
                user_id=id_, message=GSKMessage(message_)
            )
        if message_type == "group":
            return await self.send_group_msg(group_id=id_, message=GSKMessage(message_))
