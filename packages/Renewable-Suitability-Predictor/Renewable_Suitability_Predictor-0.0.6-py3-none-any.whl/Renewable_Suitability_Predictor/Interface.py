...
import numpy as np
import io
from Renewable_Suitability_Predictor.backend_methods import get_wind_model_predictions, get_solar_model_predictions 
...
def interface_multi_prediction(coordinates_array, model_type, email, api_key, random_state=-1):
    ...
    for pair in coordinates_array:
        if ( (isinstance(pair[0],float)) & (isinstance(pair[1],float)) ):
            ...
            r=requests.get(f"http://developer.nrel.gov/api/nsrdb/v2/solar/psm3-2-2-download.csv?wkt={temp_wkt}",params=pramaters) #old api is /api/nsrdb/v2/solar/psm3-download.csv
            print("Finished!")
...
def interface_single_prediction(email,api_key,random_state=-1):
    pramaters={
        "api_key":api_key,  #replace with their key and email 
        "email":email,
        "interval":"60",
        "names":"2020"
    }
    ...
    while(True):
        ...
        r=requests.get(f"http://developer.nrel.gov/api/nsrdb/v2/solar/psm3-2-2-download.csv?wkt={coordinate}",params=pramaters) #old api is /api/nsrdb/v2/solar/psm3-download.csv
