...
class CloudPodsService(ABC):
	...
	def _get_localstack_pod_version(J,pod_name,version=_A):
		B=pod_name;A=version;C=requests.get(create_platform_url(B),headers=get_platform_auth_headers())
		if not C.ok:_raise_exception_with_formatted_message(f"Unable to load pod {B}",C)
		D=json.loads(C.content);E=D[_I];F=int(D['max_version'])
		...
		G=list(filter(lambda v:v[_J]==A,E));H=G[0]if G else E[-1];I=H['localstack_version'];return I
