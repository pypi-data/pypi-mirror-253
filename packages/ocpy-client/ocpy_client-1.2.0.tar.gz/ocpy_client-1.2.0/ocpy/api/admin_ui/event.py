...
from ocpy.utils import pretty_print_POST
...
class EventApi(OpenCastBaseApiClient):
    ...
    def get_new_metadata(self, **kwargs) -> str:
        ...
        raise OcPyRequestException(
            "could not get metadata tags!", response=res)

    def get_assets(self, event_id: str):
    ...
    def get_attachments(self, event_id: str):
    ...
    def _available_event_upload_options_to_list(self, available_event_upload_options: Dict[str, Dict[str, str]]):
        workflow_id = None
        option_list = []
        for o in available_event_upload_options.keys():
            try:
                option_list.append(
                    ...
                    {**available_event_upload_options[o], "title": o, "showsAs": "uploadAsset"}
                    )
            except TypeError:
                logger.debug(o)
    ...
    def post_attachment(self, event_id, attachment_name, file,
                        publish_uploaded_assets_wf=None,
                        available_event_upload_options: Optional[Dict] = None):
        """Adds an attachment to an event. If there is already an attachment with the same name, it will be overwritten.
        ...
        Args:
            ...
            publish_uploaded_assets_wf (string, optional): Name of the workflow to run after attachment was added. Defaults to None.
            available_event_upload_options (Optional[Dict], optional): Dict of available asset upload options. Defaults to None.
        ...
        if not available_event_upload_options:
            r_api = ResourcesApi()
            available_event_upload_options, workflow_id = self._available_event_upload_options_to_list(
        ...
        event_upload_option = list(filter(
            lambda x: x["id"] == attachment_name, available_event_upload_options)).pop()
        if not event_upload_option:
            raise OcPyRequestException(
                f"Attachment {attachment_name} does not exist / is not configured!")

        data = {"assets": {
                "options": available_event_upload_options}, 
                "processing": {
                    "workflow": publish_uploaded_assets_wf,
                    "configuration": {
                        "downloadSourceflavorsExist": "true",
                        "download-source-flavors": f"{event_upload_option['flavorType']}/{event_upload_option['flavorSubType']}"
            }
        }}
        ...
        with open(os.path.expanduser(file), "rb") as f:
            ...
            files = {f"{attachment_name}.0": (file_name, f, mime_type)}  # a mime type (content type) is required!
            ...
            raise OcPyRequestException(
                "could not post attachment!", response=res)

    def post_subtitles(self, event_id, file, language_code="de", publish_uploaded_assets_wf=None):
        """Adds subtitles attachment to an event. If there is already a subtitles attachment for the language, it will be overwritten.
        ...
        possible_upload_options = list(filter(lambda x: "captions" in x["id"] and f"+{language_code}" in x["flavorSubType"], available_event_upload_options))
        if len(possible_upload_options) == 0:
            logger.error(f"Only found these upload options: {pformat(available_event_upload_options)}")
            raise OcPyRequestException("No upload option for subtitles found!")
        
        possible_upload_option = possible_upload_options[0]  # pick the first (and hopefully only) option
        if not publish_uploaded_assets_wf:
            publish_uploaded_assets_wf = workflow_id
        return self.post_attachment(event_id, possible_upload_option["id"], file, publish_uploaded_assets_wf)