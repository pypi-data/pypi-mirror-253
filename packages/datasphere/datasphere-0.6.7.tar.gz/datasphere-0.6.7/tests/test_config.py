...
def test_parse_python_env(requirements_file):
    for env_dict, expected_env, err in (
            ...
            ({'python': {'type': 'auto', 'pip': {'extra-index-urls': 'https://example.com'}}}, None, '`extra-index-urls` should be a list'),
            ...
            ({'python': {'type': 'auto', 'pip': {'extra-index-urls': ['https://pypi.yandex-team.ru/simple', 'https://pypi.ngc.nvidia.com']}, 'no-deps': 'nope'}}, PythonEnv(pip_options=PythonEnv.PipOptions(extra_index_urls=['https://pypi.yandex-team.ru/simple', 'https://pypi.ngc.nvidia.com'], no_deps=False)), None),
...
def test_working_storage():
    ...
    with raises(ValueError, match='possible working storage types are: \[SSD\]'):
