...
common_python_name_re = re.compile(r'python([23](\.\d{1,2})?([dmu])?)?')  # TODO: `Python.exe`
...
def parse_python_main_module(cmd: str) -> Optional[PythonModule]:
    ...
    if common_python_name_re.fullmatch(first_arg):
    ...
    with open(first_arg) as f:
        try:
            ...
            return None
    if first_line.startswith('#!') and os.name != 'nt':  # Shebangs are not supported on Windows
        ...
        is_python_shebang = (
            ...
            hint_path.exists() and common_python_name_re.fullmatch(hint_path.parts[-1]) or
            # User can set some custom slug for python.
...
def is_python_interpreter(path: str, raise_not_exist: bool = True) -> bool:
    ...
    try:
        stdout = subprocess.check_output([path, '--version'], timeout=5)
