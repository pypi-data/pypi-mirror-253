...
def imread_templates(folder_path):
    ...
    if os.path.exists(folder_path) and os.path.isdir(folder_path):
        ...
        for image_file in image_files:
            ...
            image = np.float32(image)  # it is the same in resize_images_tensor
            ...
            # Convert the resized image back to a PyTorch tensor
            resized_img_tensor = torch.from_numpy(resized_img).cuda().unsqueeze(0)

            image_name = image_file.split('.')[0]
            image_dict[image_name] = resized_img_tensor
