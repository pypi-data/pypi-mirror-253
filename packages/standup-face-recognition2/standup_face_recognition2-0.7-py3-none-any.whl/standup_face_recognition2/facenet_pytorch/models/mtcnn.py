...
class MTCNN(nn.Module):
    ...
    def __init__(
            self, image_size=256, margin=0, min_face_size=20,
            thresholds=[0.6, 0.7, 0.7], factor=0.709, post_process=False,
            select_largest=True, selection_method=None, keep_all=False, device=None, just_the_first_box=False
    ):
        super().__init__()
        ...
        self.just_the_first_box = just_the_first_box
    ...
    def forward(self, img, save_path=None):
        ...
        if batch_boxes[0] is not None:
            # Extract faces
            faces, box = self.extract(img, [batch_boxes[0][0].unsqueeze(0)] if self.just_the_first_box else batch_boxes,
                                      save_path)
        else:
            # Set faces and box to None if batch_boxes is None
            return [None], None, None, None, None

        # Return values
        return faces, [batch_boxes[0][0].unsqueeze(0)] if self.just_the_first_box else batch_boxes, box, batch_points, batch_probs
    ...
    def detect(self, img, landmarks=False):
        ...
        for box, point in zip(batch_boxes, batch_points):
            ...
            if len(box) == 0:
                ...
                box_order = torch.argsort(area, descending=True)
        ...
        if probs[0][0] is not None:
            # Assuming that probs is a torch tensor
            sorted_indices = torch.argsort(probs[0], descending=True)
            probs_sorted = probs[0][sorted_indices]
            boxes_sorted = boxes[0][sorted_indices]
        else:
            # Handle the case when probs is None
            probs_sorted = None
            boxes_sorted = None

        return [boxes_sorted], probs_sorted, points
    ...
    def extract(self, img, batch_boxes, save_path):
        ...
        for im, box_im, path_im in zip(img, batch_boxes, save_path):
            ...
            for i, box in enumerate(box_im):
                ...
                face, box = extract_face(im, box, self.image_size, self.margin, face_path)
            ...
            if self.keep_all:
                ...
                # faces_im = torch.cat(faces_im, dim=0)#.cpu().detach().numpy()
            else:
                faces_im = faces_im[0]
