...
def detect_face(imgs, minsize, pnet, rnet, onet, threshold, factor, device):
    ...
    #image_inds = image_inds.detach().clone()
...
def bbreg(boundingbox, reg):
    if reg.shape[1] == 1:
        reg = reg.view(reg.shape[2], reg.shape[3])
...
# allows different side length of the box
"""
...
def crop_resize(img, box, image_size):
    ...
    if isinstance(img, np.ndarray):
        ...
        # if width < height:
        #     union_side_length = height
        # else:
        #     union_side_length = width
        #
        # center_width = box[0] + width / 2
        # center_hight = box[1] + height / 2
        #
        # left = center_width - union_side_length / 2
        # right = center_width + union_side_length / 2
        #
        # bottom = center_hight - union_side_length / 2
        # upper = center_hight + union_side_length / 2
        #
        ...
        #     upper = img.shape[0]
        #
        # img = img[int(bottom):int(upper), int(left):int(right)]
        #
        out = cv2.resize(
            img,
            (height, width),
