...
class ScrapyAutomation:
    ...
    def make_request(page_save_path, url, headers, checkpoint_in_response, retries=1, request_type="GET", proxy=None,payload=None):
        ...
        if os.path.exists(page_save_path):
            ...
            return {"page_save_path": page_save_path, "response_status_code": response.status_code,
                    "response_text": response.text, "retries_count": retries_count}
...
class Wissend_Common(ScrapyAutomation):
    ...
    def feature_bullets(response):
        ...
        features = [
            response.xpath(f'//*[contains(@class, "{_}")]') for _ in common_paths
        ]

        common_paths = ['•', 'Features']
        features.extend([
            response.xpath(f'//*[contains(text(),"{_}")]') +
            response.xpath(f'//*[contains(text(), "{_}")]/../following-sibling::ul/li')
            for _ in common_paths
        ])

        return [element for element in features if element]
    ...
    def attributes(response):
        ...
        for path in common_paths:
            attribute_elements.extend(
                response.xpath(f'//*[contains(@class, "{path}") or contains(@id, "{path}")]')
            )

        return [element for element in attribute_elements if element]
    ...
    def includes(response: Union[str, Selector]) -> list:
        ...
        include_elements += [
            response.xpath(f'//*[contains(text(), "{_}")]/following-sibling::ul/li') +
            response.xpath(f'//*[contains(@id, "{_}")]') +
            response.xpath(f'//*[contains(@class, "{_}")]')
            for _ in common_paths
        ]
        return [element for sublist in include_elements for element in sublist]
    ...
    def get_description(response):
        """
        Extracts description elements from the response.

        Parameters:
            response (str or Selector): The response to extract description elements from.
        ...
        description_elements = [
            element
            for path in common_paths
            for element in response.xpath(f'//*[contains(@class, "{path}")] | '
                                          f'//*[contains(@id, "{path}")]')
        ]
