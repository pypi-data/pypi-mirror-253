...
class GeneratePyproject(object):
    ...
    def write_config_files(self):
        ...
        Before it generates the files, set_ methods are called (in order to set the required attributes):
            set_requirements_file()
            set_readme_file()
            set_license_file()
            set_init_file()
...
def main():
    ...
    version = get_release_version(ISSUE_TITLE)
    if not version:
        logging.error(f"Unable to get a valid version for {str(ISSUE_TITLE)}.")
        exit(2)

    project_name = f"ste-test-{PROJECT_NAME}"
...
if __name__ == '__main__':
    ISSUE_TITLE = os.getenv("ISSUE_TITLE", "chore(main): release 0.1.12")
    PROJECT_NAME = os.getenv("PROJECT_NAME", "hansen")
    main()
