"""vios(Virtual Input and Output System)包含以下功能模块:

1. app: 用户函数或任务定义
2. driver: 设备驱动. NOTE: 如发生改动, 需要重启server
...
3. envelope: 执行流程，见各模块说明. NOTE: 如发生改动, 需要重启server
4. systemq: 与systemq进行交互的接口
"""
...
try:
    ...
    except Exception as e:  # systemq not installed
        logger.warning(f'systemq not installed: {e}')
        SYSTEMQ = ''
        bootstrap = {}
...
def get_record_list_by_filter(task: str, start: str, stop: str):
    try:
        return sql.execute(f'select * from task where name like "%{task}%" and created between "{start}" and "{stop}" limit 100').fetchall()
    except Exception as e:
