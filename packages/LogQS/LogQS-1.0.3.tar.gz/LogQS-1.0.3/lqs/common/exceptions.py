...
class ForbiddenException(Exception):
    def __init__(self, action, resource, reason=None):
        self.message = (
            f"Action '{action}' on '{resource}' not allowed for the authenticated user."
        )
        if reason is not None:
            self.message = f"Action '{action}' on '{resource}' not allowed for the authenticated user: {reason}"
        super().__init__(self.message)
...
class InternalServerErrorException(Exception):
    def __init__(self, msg=None):
        ...
        err_msg = {
            "errorType": exception_type.__name__ if exception_type else "UnknownError",
            "errorMessage": str(exception_value),
            "stackTrace": traceback_string,
        }
        self.message = msg if msg is not None else json.dumps(err_msg)
        super().__init__(self.message)
