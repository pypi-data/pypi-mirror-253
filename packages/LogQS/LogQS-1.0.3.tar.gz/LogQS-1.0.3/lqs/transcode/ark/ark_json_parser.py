...
def handle_initialization(ark_json_schema, field, current_package):
    if "attributes" in field:
        if (
            "constant" in field["attributes"]
            and field["attributes"]["constant"] is True
        ):
            return None, field["constant_value"]
    ...
    if field["type"] in primitive_types:
        if field["type"] in numerical_types:
            ...
            return None, '"00000000-0000-0000-0000-000000000000"'
...
def convert_enum(enum_schema):
    ...
    name, object_namespace = enum_schema["name"], enum_schema["object_namespace"]
...
def converter(ark_json_schema, type_identifier, recursive_call=False):
    ...
    for field in schema["fields"]:
        ...
        if (
            ...
            class_def += (
                f"\t\tread_{field['type']}(stream, TRIM_SIZE=TRIM_SIZE)\n"
            )
            continue
        ...
        if (
            ...
            class_def += f"\t\tif read_bool(stream, TRIM_SIZE=TRIM_SIZE):\n\t\t\tself.{field['name']} = read_{field['type']}(stream, TRIM_SIZE=TRIM_SIZE)\n"
            continue
    ...
    if not recursive_call:
        ...
        import_statements = ""
        for _import in imports:
            import_statements += f"import {_import}\n"
