...
import HueEngine.components
from .__init__ import Processor
from HueEngine.__core__ import _ANY,_NULL,pgSurface

class RenderProcessor(Processor):
    def __init__(self) -> None:
        super().__init__()

    def Process(self, window:pgSurface=None, *args: _ANY, **kwargs: _ANY) -> _NULL:
        for entity, component in HueEngine.fetchAllTypes(HueEngine.TextureComponent):
            ...
            if transform and texture:
                if (window): window.blit(texture, (transform.x, transform.y))
