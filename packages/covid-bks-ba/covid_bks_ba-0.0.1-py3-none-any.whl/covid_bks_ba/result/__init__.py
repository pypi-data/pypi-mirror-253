...
def boxplot(dset, evaltype, metric):
    ...
    for target in targets:
        try:
            with open(Path("result",target,evaltype,dset,"_",f"{metric}.pickle"), 'rb') as f:
                m += [pickle.load(f)]
                l += [target]
        except:
            pass
    ...
    try:
        ...
        print(f"{dset} {evaltype} {metric}")
        quit()

    plt.ylabel(metric)
    plt.xticks(rotation=30, ha="right")
    path = Path("plot",dset,evaltype)
