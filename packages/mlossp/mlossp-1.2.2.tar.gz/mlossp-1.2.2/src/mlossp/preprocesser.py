...
class Preprocessor:
    def __init__(self, config_file: str, **kwargs):
        ...
        :param get_record_time: An optional function to retrieve the record time from a data file name
        :param get_region_level: An optional function to retrieve a region's spatial level
        :param additional_info: An optimal function to add additional info to the output pickle files
        """
        super().__init__()

        self.config_file = config_file
        self.get_record_time = kwargs.get("get_record_time", lambda x:'')
        self.get_region_level = kwargs.get("get_region_level", lambda x:'')
    ...
    def get_all_from_dir(self, path_to_dir, extension):
        for f in Path(path_to_dir).glob(extension):
            match extension:
                case "*.pkl":
                    yield self.get_record_time(f), load_from_pickle(f, self.CRS)
                case "*.json":
                    yield self.get_record_time(f), gpd.read_file(f, engine='pyogrio', use_arrow=True)
                case _:
                    raise ValueError("Unsupported File Extension")

    def permute_and_layer(self, path_to_dir, extension, regions):
        out = {}
        for t, gdf in tqdm(self.get_all_from_dir(path_to_dir, extension)):
            for r, rdf in regions.items():
                ...
                aggregate.columns = [''.join([s[1], s[0].title()]) if type(s) == tuple else s for s in
                                     aggregate.columns]
                aggregate.to_pickle(self.PATH_OF(f"{self.OUT_DIR}/{r}_{self.get_region_level(r)}_{t}.pkl"))
