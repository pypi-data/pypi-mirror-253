...
def border_reflect(imagem,filtro):
    ...
    # m, n = filtro.shape
    imagem_bordar = cv2.copyMakeBorder(imagem,  2,  2,  2,  2, cv2.BORDER_REFLECT)
...
def border_zero(imagem, filtro):
    ...
    # m, n = filtro.shape
    imagem_bordar = cv2.copyMakeBorder(imagem,  2,  2,  2,  2, cv2.BORDER_CONSTANT)
    return cv2.filter2D(imagem_bordar, -1, filtro)    
...
def border_replicate(imagem,filtro):
    ...
    # m, n = filtro.shape
    imagem_bordar =  cv2.copyMakeBorder(imagem,  2,  2,  2,  2, cv2.BORDER_REPLICATE)
    return cv2.filter2D(imagem_bordar, -1, filtro)