...
def parse_args():
	...
	parser.add_argument("--strings", nargs='+', required=True, help="List of strings that will be formed into combinations with replacement and substituted as (A, B) or (B, A) for command template wildcards")
	...
	parser.add_argument("--outAB", nargs='+', default="", help="Templates for cmdAB file outputs to read to stdout if --printOut is specified")
	parser.add_argument("--outBA", nargs='+', default="", help="Templates for cmdBA file outputs to read to stdout if --printOut is specified; assumes equal to --outAB if blank or --cmdABBA is used.")
	...
	parser.add_argument("--printCmds", type=str, help="Print a list of [{A}, {B}, command] as YAML. If a string is supplied, returns YAML of {--printCmds: commands}. Future versions may support json, etc.")
	parser.add_argument("--printOut", type=str, help="Read and print file outputs as YAML. If a string is supplied, returns YAML of {--printOut: outputs}. Future versions may support json, etc.")
	parser.add_argument("--outFn", action='store_true', default=False, help="Include filenames associated with contents")
...
def structure_commands(args, pairs):
	...
	for pair in pairs:
		...
		if A != B or args.AisB == "both": 
			commands += [[A, B, cmdAB, "AB"], [B, A, cmdBA, "BA"]]
		if A == B:
			if args.AisB == "cmdAB":
				commands.append([A, B, cmdAB, "AB"])
			elif args.AisB == "cmdBA":
				commands.append([B, A, cmdBA, "BA"])
	return commands
...
def print_outputs(args, commands):
	if args.printOut is not None:
		results = {}
		for command in commands:
			A, B, cmd, abba = command
			filenames = get_filenames(args, command)
			if args.outFn:
				contents = [{filename: open(filename).read()} for filename in filenames] if not args.dryRun else [{filename: "<<no contents: dry run>>"}]
			else:
				contents = [open(filename).read() for filename in filenames] if not args.dryRun else ["<<no contents: dry run>>"]
