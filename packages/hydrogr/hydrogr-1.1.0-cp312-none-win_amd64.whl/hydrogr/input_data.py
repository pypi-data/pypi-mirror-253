...
# TODO : Add method to create train and test set from input data (not necessary in the handler)
# TODO : Add method to get period based on the flow or the rainfall (not necessary in the handler) => Useful for specific calibration
...
class InputDataHandler(object):
    ...
    def __init__(self, Model, data):
        if not isinstance(data, pd.DataFrame):
            raise TypeError('Expecting a pandas.Dataframe for input data, received {} instead.'.format(type(data)))
    ...
    def get_sub_period(self, start_date, end_date):
        """
        ...
        if start_date < self.start_date:
            warnings.warn('The selected start date ({}) is prior to the date of the first datasample : {}'.format(start_date, self.start_date))
        if end_date > self.end_date:
            warnings.warn('The selected end date ({}) is posterior to the date of the last datasample : {}'.format(end_date, self.end_date))
    ...
    def __check_data(self):
        ...
        for prerequisite in self.Model.input_requirements:
            if prerequisite.name not in self.data:
                raise ValueError('Input data should contains "{}" data! Keyword "{}" not found.'.format(prerequisite.name, prerequisite.name))
            elif not ptypes.is_float_dtype(self.data[prerequisite.name]):
                raise ValueError('Input data "{}" should be float! Currently : {}'.format(prerequisite.name, self.data[prerequisite].dtypes))
    ...
    def __check_data_frequency(self):
        ...
        if freq.split('-')[0] in self.Model.frequency:
            ...
            raise ValueError('Incompatibility between the frequency of the data and the chosen model! \n '
                             'Data frequency : {} \n '
                             'Expected frequency for {} : {}'.format(freq, self.Model.name, self.Model.frequency))

    def __check_for_na_in_inputs(self, column_name):
        detected_na = self.data[column_name].isnull().values.any()
        if detected_na:
            warnings.warn('NA detected in {} time series! ({})'.format(InputDataHandler.data_names[column_name], column_name))

    def __check_for_negative_values_in_inputs(self, column_name):
        detected_neg = (self.data[column_name] < 0.0).any()
        if detected_neg:
            warnings.warn('Negative values detected in {} time series! ({})'.format(InputDataHandler.data_names[column_name], column_name))
