...
def main(page: ft.Page):
    ...
    adb_col = ft.Column(width=300,)
    jdk_col = ft.Column(width=300,)
    jmeter_col = ft.Column(width=300,)
    env_col = ft.Column(width=300,)
    ...
    def check_adb(e):
        ...
        version_info = re.findall('[\d\.]+', os.popen("adb version").read())[0] \
        ...
        if version_info:
            ...
            def auto_install_adb(e):
                ...
                try:
                    ...
                    try:
                        ...
                        page.update()
                    # 获取当前路径
                    current_path = os.getcwd() + database[_os_name]['pathLimiter'] + 'platform-tools' + \
                    ...
                    if permanent_env_box.value:
                        ...
                        # 设置临时环境变量
                        os.environ['PATH'] = os.environ['PATH'] + database[_os_name]['envLimiter'] + current_path
            ...
            _self_button = ft.ElevatedButton(text='仅下载ADB', on_click=slef_install_adb, icon=ft.icons.ADB,
                                             style=ft.ButtonStyle(
                                                 bgcolor=ft.colors.BLUE,
                                                 color=ft.colors.WHITE,
                                             )
                                             )
            _auto_button = ft.ElevatedButton(text='自动下载ADB并配置环境变量', on_click=auto_install_adb, icon=ft.icons.ADB,
                                             style=ft.ButtonStyle(
                                                 bgcolor=ft.colors.BLUE,
                                                 color=ft.colors.WHITE,
                                             )
    ...
    permanent_env_box = ft.Checkbox(label="永久环境变量", value=False, on_change=permanent_env)


    check_adb_button = ft.ElevatedButton(text='检查ADB环境', on_click=check_adb, icon=ft.icons.CHECK,
                                         style=ft.ButtonStyle(
                                             bgcolor=ft.colors.BLUE,
                                             color=ft.colors.WHITE,
                                         )
    ...
    check_jdk_button = ft.ElevatedButton(text='检查JDK环境', on_click=check_jdk, icon=ft.icons.CHECK,
                                         style=ft.ButtonStyle(
                                             bgcolor=ft.colors.BLUE,
                                             color=ft.colors.WHITE,
                                         )
    ...
    check_jmeter_button = ft.ElevatedButton(text='检查JMeter环境', on_click=check_jmeter, icon=ft.icons.CHECK,
                                         style=ft.ButtonStyle(
                                             bgcolor=ft.colors.BLUE,
                                             color=ft.colors.WHITE,
                                         )
