...
class AwsClientWrapper(Wrapper):
    ...
    def list_ecr_image_tags(
        ...
        paginator = self._session.client("ecr").get_paginator("describe_images")
        iterator = paginator.paginate(
            registryId=registry_id,
            repositoryName=repository_name,
            filter={"tagStatus": "TAGGED"},
        )
        jmes_expression = f"reverse(sort_by(imageDetails, &to_string(imagePushedAt)))[:{limit}].imageTags"
        filter_iterator = iterator.search(jmes_expression)
        results = list(filter_iterator)
        if not results:
            return []

        return [tag for result in results for tag in result if tag != "latest"]
