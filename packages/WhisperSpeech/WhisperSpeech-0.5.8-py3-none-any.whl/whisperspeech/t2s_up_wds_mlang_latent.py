# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/5B. Text to semantic token modeling-Copy2.ipynb.

# %% auto 0
__all__ = ['languages', 'load_datasets', 'lang_to_id', 'load_dataset', 'rand', 'Tunables', 'Encoder', 'Decoder',
           'TSARTransformer', 'make_model']

# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 1
import dataclasses
import random
import math
import torch
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 2
from pathlib import Path
import pylab as plt
import pandas as pd
import numpy as np

# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 3
import whisper
from whisperspeech.train import *
from whisperspeech.modules import *
from whisperspeech import vq_stoks, utils

# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 8
import re
...
def ar_padder(ikey, okey, length, pad_token):
    ...
    def _ar_padder(samples):
        for s in samples:
            ...
            s['in_' +okey] = F.pad(toks, (5, length - toks.shape[-1] - 1 + 4), value=pad_token)
            s['out_'+okey] = F.pad(toks, (4, length - toks.shape[-1] + 4), value=pad_token)
            yield s
    return _ar_padder
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 9
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 10
def load_datasets(
    ...
    def ds(shards, length):
        ds = wds.WebDataset(wds.ResampledShards(shards)).compose(
            ...
            wds.to_tuple('ttoks', 'language', 'cps', 'in_stoks', 'out_stoks'),
            wds.batched(64)
        )
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 11
...
def load_dataset(
    shard_spec:str,        # webdataset shards
    samples:int,           # samples per epoch
    vq_codes:int=4096,
    language:str='en',
    weight:float=1,
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 13
def rand(start, end):
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 14
class EmbeddingProjector(nn.Linear):
    pass

# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 15
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 16
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 17
class Decoder(nn.Module):
    def __init__(self, depth=6, stoks_width=384, width=384, n_head=6, prosody_length=4, length=1500, codes=1024, ffn_mult=4, pos_embs=None, tunables=Tunables()):
        ...
        self.embedding = FlexEmbeddings(codes, width, special_codes=prosody_length*2+3, frozen_width=stoks_width)
    ...
    def forward(self, Stoks, xenc, cps=None, lang=None):
...
#             Sembs = self.emb_to_hidden(Sembs)
        ...
        if lang is not None: xin[:,:self.prosody_length] += lang
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 19
class TSARTransformer(nn.Module):
    ...
    def _extract_prosody(self, stoks, xenc, cps_embs):
        prosody_stoks = stoks.clone()
        # mask out the prompt part
        prosody_stoks[:,:] = self.decoder.embedding.codes + 1
        x = torch.randint(5, stoks.shape[-1]-104, (stoks.shape[0],))
        for i,j0 in enumerate(x):
            prosody_stoks[i,self.prosody_length:self.prosody_length+100] = stoks[i,j0:j0+100]
        # setup the output tokens
        prosody_stoks[:,-self.prosody_length:] = self.decoder.embedding.codes + 3 + torch.arange(self.prosody_length)

        _, prosodyenc = self.decoder(prosody_stoks, xenc, cps=cps_embs)
        prosodyenc = prosodyenc[:,-self.prosody_length:]
        
        commit_loss, indices = 0, None
        if self.mode == "quantized":
            prosodyenc, indices, commit_loss = self.latent_quantizer(prosodyenc)
        elif self.mode == "fsq":
            half_l = (32 - 1) * (1-1e-3) / 2
            x = self.hidden_to_fsq(prosodyenc)
            q = F.tanh(x + math.tan(0.5/half_l)) * half_l - 0.5
            indices = q.round()
            q = q + (indices - q).detach()
            prosodyenc = self.fsq_to_hidden(q)
        elif self.mode == "continuous":
            prosody_stoks, indices = torch.tensor([0])
        elif self.mode == "none":
            prosodyenc = torch.zeros_like(prosodyenc)
            prosody_stoks, indices = torch.tensor([0])
        if random.random() < 0.001:
            print('prosody stoks:', prosody_stoks.shape)
            print('prosodyenc:', prosodyenc.shape)
            print('indices:', indices.shape)
            print('indices:', indices.mT)
        
        return prosodyenc, indices, commit_loss
    
    def forward(self, Ttoks, languages, cpss, in_stoks, out_stoks=None, prosodyenc=None, loss=True):
        lang_embs = self.lang_embeddings(languages).unsqueeze(1)
        with record_function("encoder"):
            xenc = self.encoder(Ttoks.to(torch.long), lang_emb=lang_embs)
        
        cps_emb = self._embed_cps(cpss)
        if prosodyenc is None: prosodyenc, _, commit_loss = self._extract_prosody(in_stoks, xenc, cps_emb)
        
        with record_function("decoder"):
            prosodyenc = prosodyenc * self.prosody_mult
            if self.training:
                in_stoks[:,-self.prosody_length:] = self.decoder.embedding.codes + 2
                in_stoks[:,:self.prosody_length] = self.decoder.embedding.codes + 3 + self.prosody_length + torch.arange(self.prosody_length)
            logits, _ = self.decoder(in_stoks, xenc, cps=cps_emb, lang=prosodyenc)
            logits = logits * self.tunables.output_mult / (self.width / self.base_width)
        if loss is not None:
            with record_function("loss"):
                loss = F.cross_entropy(logits[:,self.prosody_length:-self.prosody_length].transpose(-1,-2), out_stoks[:,self.prosody_length:-self.prosody_length]) + commit_loss
        return logits, loss
...
# %% ../nbs/5B. Text to semantic token modeling-Copy2.ipynb 20
