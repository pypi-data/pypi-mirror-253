# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb.

# %% auto 0
__all__ = []

# %% ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb 3
import os
import io
import time
import torch
...
# %% ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb 4
...
from .prepare_t2s_dataset import Transcriber
import webdataset as wds

# %% ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb 10
...
# %% ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb 21
...
# %% ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb 22
import copy

# %% ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb 23
...
# %% ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb 28
...
# %% ../nbs/2A. Whisper quantization dataset preparation-Copy1.ipynb 44
...
def process_shard(
    ...
    ds = wds_compose(vad.load_dataset(input, rename_files=rename_files),
    ...
    transcriber = Transcriber(whisper_model, lang=language)
    
    tmp = output+".tmp"
    with wds.TarWriter(tmp) as sink:
        for keys, samples in progress_bar(dl, total=n_samples):
            with torch.no_grad():
                txts = transcriber.transcribe(samples.cuda())
#                 embs = whmodel.encoder(whisper.log_mel_spectrogram().cuda())
#                 decs = whmodel.decode(embs, decoding_options)
            for key, txt in zip(keys, txts):
                sink.write({
                    "__key__": key,
                    "txt": txt,
                })
