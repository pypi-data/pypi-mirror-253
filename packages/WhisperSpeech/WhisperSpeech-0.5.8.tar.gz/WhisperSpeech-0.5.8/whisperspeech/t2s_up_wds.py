...
# %% auto 0
__all__ = ['languages', 'load_datasets', 'load_dataset', 'rand', 'Tunables', 'Encoder', 'Decoder', 'TSARTransformer',
           'make_model']
...
# %% ../nbs/5B. Text to semantic token modeling.ipynb 11
...
def load_dataset(
    ...
    language = languages.index(whisper.tokenizer.TO_LANGUAGE_CODE.get(language, language))
    
    def set_language(x):
        x['language'] = language
        return x
    
    ds = wds.WebDataset(shards, resampled=True).compose(
        wds.decode(),
        wds.select(lambda s: s['stoks.npy'].shape[-1] > 12), # select samples > .5s
        tokenizer('txt', 'ttoks', length=550),
        ar_padder('stoks.npy', 'stoks', length=750, pad_token=vq_codes-1),
...
class Decoder(nn.Module):
    ...
    def forward(self, Stoks, xenc, cps=None, lang_embs=None):
        ...
        if lang_embs is not None: xin[0] += lang_embs
...
class TSARTransformer(nn.Module):
    ...
    def forward(self, Ttoks, cpss, in_stoks, out_stoks=None, loss=True):
    ...
    def generate(self, txt, cps=15, N=None, T=0.7, top_k=None, show_progress_bar=True):
        ...
        for i in it:
            p, _ = self(ttoks, cpss, toks[:,:i], loss=None)
            last_p = p[0,-1]
            if top_k:
...
# %% ../nbs/5B. Text to semantic token modeling.ipynb 20
