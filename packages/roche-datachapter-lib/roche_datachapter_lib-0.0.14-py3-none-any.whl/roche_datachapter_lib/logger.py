"""Logger"""
from os.path import (isdir, join as path_join)
import logging
from datetime import datetime, UTC
from .__domain__ import db, app, JobExecution, JobExecutionLog
from .job_manager import JobManager


class Logger():
    """Logger that logs into a new file created at logs_dir and into DB table"""

    def __init__(self, logs_dir: str, job_name: str):
        if isdir(logs_dir):
            self.start_time = datetime.now(UTC)
            self.file_logger = logging
            self.job_name = job_name
            logs_path = path_join(
                logs_dir,
                f'process_{self.start_time.strftime("%Y%m%d_%H.%M.%S")}.log',
            )
            logging.basicConfig(
                level=logging.INFO,
                filename=logs_path,
                filemode="w",
                format="%(name)s - %(levelname)s - %(message)s",
                encoding="utf-8",
            )
            try:
                self.job_id = JobManager.get_job_id_by_job_name(job_name)
                with app.app_context():
                    execution = JobExecution(self.job_id)
                    db.session.add(execution)
                    db.session.commit()
                    self.execution_id = execution.id
            except Exception as e:
                raise ValueError(f"Error al inicializar ejecución del job '{job_name}'") from e
        else:
            raise ValueError(
                "logs_dir debe ser la ruta a un directorio/carpeta.")

    def __enter__(self):
        return self

    def log(self, level, message):
        self.file_logger.log(level=level, msg=message)
        with app.app_context():
            level_name = self.file_logger.getLevelName(level)
            log_dict = {'level': level_name, 'msg': message}
            job_execution_log = JobExecutionLog(self.execution_id, log_dict)
            db.session.add(job_execution_log)
            db.session.commit()

    def debug(self, message):
        self.file_logger.debug(message)
        self.log(logging.DEBUG, message=message)

    def info(self, message):
        self.file_logger.info(message)
        self.log(logging.INFO, message=message)

    def warning(self, message):
        self.file_logger.warning(message)
        self.log(logging.WARNING, message=message)

    def error(self, message):
        self.file_logger.error(message)
        self.log(logging.ERROR, message=message)

    def critical(self, message):
        self.file_logger.critical(message)
        self.log(logging.CRITICAL, message=message)

    def __exit__(self, exc_type, exc_value, traceback):
        with app.app_context():
            execution = db.session.query(JobExecution).filter_by(id=self.execution_id).first()
            if execution:
                if traceback:
                    execution.finish(f"Exception: {str(exc_type)}")
                else:
                    execution.finish()
            db.session.commit()
