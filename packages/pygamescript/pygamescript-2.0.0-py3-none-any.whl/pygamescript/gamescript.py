...
from minidevice import MiniDevice, Touch, ScreenCap
...
class GameScript(MiniDevice):
    def __init__(
        self,
        serial: str = None,
        capMethod: ScreenCap = None,
        touchMethod: Touch = None,
    ...
    def find(self, template: Template, isColor: bool = False, colorThreshold: int = 4):
        ...
        if isinstance(template, ImageTemplate):
            result = (
                template.match_color(screenshot, colorThreshold)
                if isColor
                else template.match(screenshot)
            )
        else:
            result = template.match(screenshot)

        if result:
            logger.debug(f"success find {template} in {result}")
        return result
    ...
    def findAndOperate(
        ...
        template: Template,
        operate,
        operateParams: Optional[dict] = None,
        isColor: bool = False,
    ...
    def rangeRandomClick(
        ...
        if duration is None:
            ...
            duration = (
                random.randint(200, 350) if randomI > 80 else random.randint(80, 120)
            )
    ...
    def curveSwipe(
        ...
        endY,
        duration,
        curveGenerateAlgo=CurveGenerate.BezierCurve,
    ):
    ...
    def findAndClick(
        self,
        template: Template,
        result: tuple | list = None,
        duration=None,
        randomPointGenerateAlgo=None,
        isColor: bool = False,
        colorThreshold: int = 4,
    ):
        clickParams = {}
        ...
        return self.findAndOperate(
            template,
            self.rangeRandomClick,
            clickParams,
            isColor=isColor,
