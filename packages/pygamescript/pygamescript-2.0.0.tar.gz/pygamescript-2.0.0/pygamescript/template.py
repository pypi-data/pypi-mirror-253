...
class ImageTemplate(Template):
    def __init__(
        ...
        self.describe = (
            describe or templatePath.split("/")[-1].split("\\")[-1].split(".")[0]
        )
        self.template = Images.read(templatePath)
    ...
    def match(self, image):
        ...
        return Images.findImage(
            image, self.template, self.threshold, self.region, self.level
        )
    ...
    def match_color(self, image, colorThreshold: int = 4):
        ...
        if result:
            color = self.template.pixel(0, 0)
            minX, minY = result[0:2]
            region = [minX, minY, minX + 10, minY + 10]
...
class MultiColorsTemplate(Template):
    ...
    def match(self, image):
        return Images.findMultiColors(
            image, self.firstColor, self.colors, self.region, self.threshold
        )
