...
class FeedbackQuerySet(models.QuerySet):
    period: Union[L['hour'], L['date'], L["month"], L["year"]] = "hour"
    ...
    def aggregate_percentage(self, *filter_args, date_arg: Union[L['hour'], L['date'], L["month"], L["year"]] = None, **filter_kwargs):
        if filter_args or filter_kwargs:
            return self.filter(*filter_args, **filter_kwargs).aggregate_percentage()
        
        filter_by = date_arg

        if filter_by is None:
            filter_by = self.period
        ...
        qs = self.values(*values_args).annotate(
        ...
        setattr(qs, "period", date_arg or self.period)
...
class AbstractFeedback(models.Model):
    ...
    def serialize(cls, instance: Self):
        return {
            ...
            "urls": {
                "list": reverse("feedback_api", kwargs={"page_pk": instance.page_id}),
                "view": reverse("feedback_api_detail", kwargs={"page_pk": instance.page_id, "pk": instance.pk}),
                "delete": reverse("feedback_api_delete", kwargs={"page_pk": instance.page_id, "pk": instance.pk}),
            },
        }
    
    @classmethod
    def get_filter_class(cls) -> Type["AbstractFeedbackFilter"]:
        filter_class = getattr(settings, "FEEDBACK_FILTER_CLASS", cls.FEEDBACK_FILTER_CLASS)
    ...
    def get_form_class(cls) -> Type["AbstractFeedbackForm"]:
        form_class = getattr(settings, "FEEDBACK_FORM_CLASS", cls.FEEDBACK_FORM_CLASS)
    ...
    def edit_handler(cls):
        ...
        for panel_attr, heading in cls.extra_panels:
            tabs.append(
                ObjectList(getattr(cls, panel_attr), heading=heading),
            )

        return TabbedInterface(tabs, permission=cls.edit_handler_permission)
