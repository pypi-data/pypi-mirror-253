...
def feedback(request, *args, **kwargs):        
    ...
    if hasattr(page, "allow_feedback_message_on_positive") or (not form.instance.positive):

        if (not form.instance.positive)\
            or (page.allow_feedback_message_on_positive() and form.instance.positive):
            form = FeedbackForm(
                request=request,
