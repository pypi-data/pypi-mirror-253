...
def renameDarwinLibraries(libname_in, idl_bin_dir):
    ...
    if not os.path.exists(os.path.join(idl_bin_dir, libpath)):
        return
...
def fixDarwinIDLLibraryNames(libname_change_in, libname_fix_in, idl_bin_dir):
    ...
    if not os.path.exists(os.path.join(idl_bin_dir, libpath_change)):
        return
...
if platform.system() == 'Darwin':
    ...
    # Older versions of IDL need to be linked to more libraries
    ...
    # For IDL 8.8 the Mesa libraries are no longer needed.
    ...
    elif os.path.exists('/opt/idl/idl/bin/'):
        idl_dir = '/opt/idl/idl/'
    else:
        raise Exception("Could not find idl installation directory.  Use the --idl-dir option.")
    
    idl_bin_dir = os.path.join(idl_dir, 'bin/bin.linux.'+machine)

    # I am not sure why I need to force idl_url.so to be linked,
    # but this gets things working on recent debian systems.
...
class CleanCommand(Command):
    ...
    def run(self):
        ...
        if self.all:
            os.system('rm -rf ./mirpyidl.so ./mirpyidl.*.so ./mirpyidl.egg-info ./dist')
