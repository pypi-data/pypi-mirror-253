...
class AnalysisSTEAM:
    ...
    def step_make_model(self, step, verbose: bool = False):
        ...
        flag_output_to_main_local_folder = step.flag_output_to_main_local_folder
    ...
    def step_modify_model(self, step, verbose: bool = False):
        ...
        for v, value in enumerate(step.variable_value):
            ...
            if len_simulation_numbers > 0:
                ...
                if 'FiQuS' in step.software:
                    BM.buildFiQuS(simulation_name=f"{step.simulation_name}_{simulation_number}", library_path=self.library_path, output_path=os.path.join(self.settings.local_FiQuS_folder, step.simulation_name))
                ...
                if 'PSPICE' in step.software:
                    BM.buildPSPICE()
                    self.setup_sim_PSPICE(simulation_name=step.simulation_name, sim_number=simulation_number, list_additional_files=BM.circuit_data.GeneralParameters.additional_files, flag_output_to_main_local_folder=step.flag_output_to_main_local_folder)
                if 'XYCE' in step.software:
                    BM.buildXYCE()
    ...
    def step_modify_model_multiple_variables(self, step, verbose: bool = False):
        ...
        for v, variable_to_change in enumerate(step.variables_to_change):
            ...
            next_step.flag_output_to_main_local_folder = step.flag_output_to_main_local_folder
    ...
    def step_setup_folder(self, step, verbose: bool = False):
        ...
        for software in list_software:
            if verbose:
                print(f'Set up folder of model {simulation_name} for {software}.')
        ...
        for event_id in self.list_viewers[viewer_name].list_events:
            ...
            for pair_var in variables_to_analyze:
                ...
                output_metric = PostprocsMetrics(metrics_to_do=metrics_to_calculate,
                                                 ...
                                                 viewer = self.list_viewers[viewer_name])
    ...
    def setup_sim_PSPICE(self, simulation_name: str, sim_number, list_additional_files: List[str], flag_output_to_main_local_folder: bool = False):
        """
        ...
        The original file is then deleted.
        flag_output_to_main_local_folder: If True, the netlist file will be placed in the local PSPICE folder and not in a structured subfolder. This option is used by ParserCOSIM()
        """
        ...
        # Make simulation folder
        if flag_output_to_main_local_folder:
            local_model_folder = os.path.join(local_PSPICE_folder)
        else:
        ...
        for file_to_copy in list_additional_files:
            try:
                if not os.path.isabs(file_to_copy):
                    file_to_copy = os.path.join(output_folder, file_to_copy)
                file_copied = os.path.join(local_model_folder, os.path.basename(file_to_copy))
                shutil.copyfile(file_to_copy, file_copied)
            except:
    ...
    def run_sim(self, software: str, simulation_name: str, sim_number: int, simFileType: str = None,
        ...
        if software == 'FiQuS':
            ...
            dFiQuS = DriverFiQuS(FiQuS_path=self.settings.FiQuS_path,
                                 path_folder_FiQuS_output=os.path.join(self.settings.local_FiQuS_folder, simulation_name),
                                 path_folder_FiQuS_input=os.path.join(self.settings.local_FiQuS_folder, simulation_name), verbose=verbose,
