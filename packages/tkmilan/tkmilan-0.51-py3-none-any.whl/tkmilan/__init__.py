    ...
    def __setstyle_checkbox_ro(self):
        ...
        if __debug__:
            ...
            if 'embossed' in root_map:
                assert ('disabled', 1) in root_map['embossed'], f'Weird Embossed Settings: {root_map["embossed"]!r}'
...
class ComboboxN(ttk.Combobox, mixin.SingleWidget, mixin.MixinValidationSingle):    # type: ignore[misc]
    ...
    def __vlabels_Combobox(vstate, why):
        ...
        if __debug__:
            ...
            assert isinstance(why.widget, Checkbox), f'Invalid Widget: {why.widget}'
        return vstate.label in why.widget.cget('values')

    ValidationLabels = model.VSettings(
        fnSimple=False, fn=__vlabels_Combobox,
    )
...
class Tooltip(SecondaryWindow):
    ...
    when the mouse touches the tooltip itself. As any `SecondaryWindow`, it can
    also be shown or hidden programatically.

    Does not contribute to the parent layout (being a `SecondaryWindow`), but
    this also doesn't contribute to the parent state.
