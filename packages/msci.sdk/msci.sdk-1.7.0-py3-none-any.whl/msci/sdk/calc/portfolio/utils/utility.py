...
def validate_portfolio_details(asset_id, not_null_cols, positions, positions_col, allowed_asset_types):
    if asset_id not in allowed_asset_types:
        logger.error(f"asset id not among {allowed_asset_types}")
        raise ValueError(f"asset_id should be one among {allowed_asset_types}")
    if asset_id not in positions_col:
        logger.error(f"asset_id: {asset_id} not present in the portfolio")
