...
class OptimizerResult:
    ...
    def _prep_portfolio_summary(self, metric_sub_type, date):
        port_summary_dict = self._get_metric_subtype_data(metric_sub_type, date)
        return port_summary_dict['optimalPortfolio'][0]

    def _get_metric_subtype_data(self, metric_sub_type, date):
        _dict = self._get_metric_values(metric_sub_type, date)
        data = _dict[metric_sub_type]
        sub_type_data_dict = json.loads(data)
        return sub_type_data_dict

    def _get_metric_values(self, metric_sub_type, date):
        _dict_n = self._search_subtype(metric_sub_type)
        _list_n = _dict_n['values']
        _list_dates = _dict_n['dataDates']
        if date not in _list_dates:
            raise ValueError("Date parameter is incorrect")
        index = _list_dates.index(date)
        _dict = _list_n[index]
        return _dict

    def _search_subtype(self, name):
    ...
    def get_portfolio_tax_summary(self, date=None) -> pd.DataFrame:
        ...
        if date:
            ...
            _dict_n_sum = self._search_subtype(metric_sub_type)
    ...
    def get_tax_by_group_category(self, date=None) -> pd.DataFrame:
        ...
        if date:
            ...
            _dict_n_group = self._search_subtype(metric_sub_type)
    ...
    def get_trade_list(self, date) -> pd.DataFrame:
        ...
        trade_list = self._get_metric_subtype_data("TRADE_LIST", date)
    ...
    def get_asset_details(self, date) -> pd.DataFrame:
        ...
        asset_details_dict = self._get_metric_subtype_data("ASSET_DETAILS", date)
    ...
    def get_tax_by_group_full_portfolio(self, date=None) -> pd.DataFrame:
        ...
        combined_data = []
        if date:
            tax_by_group_full_portfolio = self._get_metric_values(metric_sub_type, date)
            ...
            _dict_n_tax = self._search_subtype(metric_sub_type)
            _list_dates = _dict_n_tax['dataDates']
            for d in _list_dates:
                tax_by_group_full_portfolio = self._get_metric_values(metric_sub_type, d)
                if tax_by_group_full_portfolio:
    ...
    def _get_optimization_status(self, date) -> str:
        ...
        optimization_status = self._get_metric_subtype_data("OPTIMIZATION_STATUS", date)
    ...
    def get_constraint_diagnostics(self, date) -> pd.DataFrame:
        ...
        profile_diagnostics = self._get_metric_subtype_data("PROFILE_DIAGNOSTICS", date)[0]
    ...
    def get_asset_cardinality_info(self, date=None) -> pd.DataFrame:
        ...
        combined_data = []
        if date:
            profile_diagnostics = self._get_metric_subtype_data(metric_sub_type, date)[0]
            ...
            _list_dates = _dict_n['dataDates']
            for d in _list_dates:
                profile_diagnostics = self._get_metric_subtype_data(metric_sub_type, d)[0]
    ...
    def get_input_data_errors(self, date) -> str:
        ...
        input_data_errors = self._get_metric_subtype_data("INPUT_DATA_ERRORS", date)
