...
def tickets_objects():
    ...
    vt = VoucherTypes.get_for_table(rt.models.subscriptions.SubscriptionsByJournal)
    # print("20221223 create journal SLA", area, vt)

    SLA_JNL = Journal(
        ref="SLA", voucher_type=vt,
        # invoicing_area=area,
    ...
    for i, ref in enumerate(ORDER_REFS):
        ...
        sla = SLA_JNL.create_voucher(
            invoice_recipient=obj, state="draft",
    ...
    def ticket(summary, **kwargs):
        ...
        if num[0] % 2:
            prj = SITES.pop()
            kwargs.update(site=prj)
            kwargs.update(group=prj.group)
        if num[0] % 4 == 0:
    ...
    yield ticket("Bar cannot foo",
                 ...
                 [url https://luc.lino-framework.org/blog/2015/0923.html blog].</p>
                 """)
...
def working_objects():
    ...
    for u in WORKERS:
        ...
        TICKETS = Cycler(Ticket.objects.filter(site__group__in=groups))
    ...
    for i in range(12):
        ...
        if i in (2, 7):
            sub = Session(
                ticket=TICKETS.pop(), reporting_type=ReportingTypes.regular,
                session_type=TYPES.pop(), user=u)
