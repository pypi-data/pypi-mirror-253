...
class RadioDuckDialect(default.DefaultDialect):
    ...
    def get_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        list_of_maps = []
        for row in rows:
            refered_table, refered_columns = self._getReferencedDetals(row[0])
            list_of_maps.append(
                {
                    "name": row[0],
    ...
    def _getReferencedDetals(
        ...
        if (
            fk_name == None
            or fk_name.strip() == ""
            or "references" not in fk_name.lower()
        ):
            return (None, [])
        ...
        # Using regular expressions to extract content inside and outside the brackets
