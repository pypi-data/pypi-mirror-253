...
def get_list_of_matching_files(dir_path=None, dir_search_glob=None) -> list:
    ...
    if dir_path is None or dir_search_glob is None:
        ...
        count_files = len(file_list)
        etld_lib_functions.logger.info(f"Found {count_files} files from dir: {dir_path} matching glob: {dir_search_glob}")
...
def remove_null_soh_cr_and_display_utf8(field_column_type, field_name_tmp, field_data):
    ...
    if field_column_type != 'INTEGER':
        ...
        if len(unicode_character_list) > 0 and csv_distribution_display_utf8 is True:
            etld_lib_functions.logger.info(f"Unicode characters in fieldname: {field_name_tmp} - {unicode_character_list}")
    return field_data
...
def get_pcrs_user_config():
    ...
    def set_pcrs_last_scan_date():
        ...
        if qetl_manage_user_selected_datetime is not None:
            ...
            if pcrs_last_scan_date.startswith("20"):
                ...
                etld_lib_functions.logger.info(f"pcrs_last_scan_date reset to 2000-01-01T00:00:00Z.  reset from qetl_manage_user -d option")
    ...
    def set_pcrs_last_evaluation_date():
        ...
        if qetl_manage_user_selected_datetime is not None:
            ...
            if pcrs_last_evaluation_date.startswith("20"):
                ...
                etld_lib_functions.logger.info(f"pcrs_last_evaluation_date reset to 2000-01-01T00:00:00Z.  reset from qetl_manage_user -d option")
    ...
    pcrs_distribution_csv_max_field_size = \
        get_attribute_from_config_settings('pcrs_distribution_csv_max_field_size', default_distribution_csv_max_field_size)
    pcrs_last_scan_date = \
    ...
    pcrs_postureinfo_schema_normalization_flag = \
        get_attribute_from_config_settings('pcrs_postureinfo_schema_normalization_flag', True)  # See pcrs_schema_version function.
...
def setup_xmltodict_parse_using_codec_to_replace_utf8_error():
    ...
    if 'xmltodict_parse_using_codec_to_replace_utf8_error' in etld_lib_config_settings_yaml_dict:
        xmltodict_parse_using_codec_to_replace_utf8_error = etld_lib_config_settings_yaml_dict.get('xmltodict_parse_using_codec_to_replace_utf8_error')
        etld_lib_functions.logger.info(f"xmltodict_parse_using_codec_to_replace_utf8_error in etld_config_settings.yaml is set "
                                       f"to: {xmltodict_parse_using_codec_to_replace_utf8_error} ")
    else:
        xmltodict_parse_using_codec_to_replace_utf8_error = False
        etld_lib_functions.logger.warn(f"xmltodict_parse_using_codec_to_replace_utf8_error not in etld_config_settings.yaml.  defaulting to False")
    ...
    if xmltodict_parse_using_codec_to_replace_utf8_error is True or xmltodict_parse_using_codec_to_replace_utf8_error is False:
        ...
        xmltodict_parse_using_codec_to_replace_utf8_error = False
        etld_lib_functions.logger.warn(f"xmltodict_parse_using_codec_to_replace_utf8_error defaulting to False.  If this is set in etld_config_settings.yaml differently, please check syntax.")
