...
class PowerModule:
    ...
    # NOISE-TEST DEPRECATION
    # def setupForNoiseTest(self):
    #     # Import relevant QIS functions
    #     from quarchpy import qisInterface, isQisRunning, startLocalQis, GetQisModuleSelection, quarchDevice, quarchPPM, qis, requiredQuarchpyVersion
    #
    #     # Check if local QIS is running
    #     if not isQisRunning():
    #         startLocalQis(headless=True)
    #
    #     # Get connection string of module
    ...
    #     time.sleep(0.5)
    #
    #     # Setup device for QIS streaming
    #     myPpmDevice = quarchDevice(tempConString, "QIS")
    #     self.dut = myPpmDevice
    #     myQisDevice = quarchPPM(myPpmDevice)
    #     myQisDevice.openConnection()
    #
    #     return myQisDevice
    #
    # def noiseTest(self):
    #     # Skip noise test if no path is provided
    #     if calibrationResources["noisePath"] == "" or calibrationResources is None:
    #         return
    #     # Setup device for noise test
    #     oldConString = self.dut.ConString
    #     oldConType = self.dut.ConType
    #     myQisDevice = self.setupForNoiseTest()
    #
    #     # Import the required noise test class from the common test library.
    #     import sys
    #     import datetime
    #     sys.path.append(calibrationResources["noisePath"])
    #     from noise_test_common import test_main
    #
    #     calTime = datetime.datetime.now()
    #     report_path = calibrationResources["calPath"] + "\\" + self.filenameString + " [" + calTime.strftime("%d-%m-%y %H-%M") + "] " + "noise" + ".txt"
    #     # Noise Test requires 2 parameters
    #     temp = test_main(myQisDevice=myQisDevice, reportPath=report_path)
    #
    #     # Skip noise test if no limits are set
    #     if temp is None:
    #         return
    #
    ...
    #     self.dut = myPpmDevice
    #
    #     # Iterate through the rows to check noise test results (pass/fail)
    #     results = []
