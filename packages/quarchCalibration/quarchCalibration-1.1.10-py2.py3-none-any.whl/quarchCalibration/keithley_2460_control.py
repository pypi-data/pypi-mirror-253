...
class keithley2460:
    ...
    '''
    def setLoadCurrentRange(self, rangeValue):
        if rangeValue in self.currentRanges:
            # set source current range
            self.sendCommand("CURR:RANG " + str(self.currentRanges[rangeValue]))
            return
        else:
    ...
    '''
    def setLoadCurrentRangeAuto(self,state="ON"):
        # set source current range to auto
        if state in ["ON","OFF"]:
            self.sendCommand("CURR:RANG:AUTO " + state)
        else:
