...
class solarcell(solarcurve):
    ...
    def _transform(self, isc, voc, imp, vmp):
        # A transformed curve is generated by a two-element piecewise-linear interpolation
        # plus an arbitrary sigmoid smoothing curve to join the two piecewise-linear regions.

        vxyL = Polynomial.fit(x=[0, vmp], y=[0, self.xvmp], deg=1)
        iyxL = Polynomial.fit(x=[self.xisc, self.ximp], y=[isc, imp], deg=1)
        yivL = np.vectorize(lambda yv: iyxL(self.xiv(vxyL(yv))))

        vxyR = Polynomial.fit(x=[voc, vmp], y=[self.xvoc, self.xvmp], deg=1)
        iyxR = Polynomial.fit(x=[0, self.ximp], y=[0, imp], deg=1)
        yivR = np.vectorize(lambda yv: iyxR(self.xiv(vxyR(yv))))

        sigmoid = lambda yv, sign: expit(sign * (vmp - yv) * 6 / (voc - vmp))
        yiv = lambda yv: yivL(yv) * sigmoid(yv, 1) + yivR(yv) * sigmoid(yv, -1)

        # generate the final interpolation functions
        ...
        # propagate the error (roughly)
        iyx = partial(np.interp, xp=[0, self.ximp, self.xisc], fp=[0, imp, isc])
        vyx = partial(np.interp, xp=[0, self.xvmp, self.xvoc], fp=[0, vmp, voc])
