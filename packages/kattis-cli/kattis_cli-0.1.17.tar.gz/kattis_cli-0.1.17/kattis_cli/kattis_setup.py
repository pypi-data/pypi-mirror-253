...
def setup() -> None:
    ...
    if Confirm.ask("Do you have an account on Kattis?", default=True):
        while True:
            ...
            _HEADERS['X-CSRFToken'] = response.cookies['csrftoken']
            ...
            _HEADERS['User-Agent'] = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/83.0.4103.97 Chrome/83.0.4103.97 Safari/537.36'
            if response.status_code == 200:
...
https://open.kattis.com/register[/link]""")
    ...
    print("You are ready to use Kattis CLI.")
