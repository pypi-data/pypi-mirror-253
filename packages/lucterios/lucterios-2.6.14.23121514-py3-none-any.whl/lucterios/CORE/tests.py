...
class AuthentificationTest(LucteriosTest):
    ...
    def setUp(self):
        ...
        admin = LucteriosUser.objects.get(username="admin")
    ...
    def test_blank(self):
        ...
        self.assertEqual(response.content, bytes('', 'ascii'))
    ...
    def test_badconnect(self):
        ...
        self.calljson('/CORE/authentification', {'login': '', 'password': ''})
        self.assert_observer('core.auth', 'CORE', 'authentification')
        self.assert_json_equal('', '', 'BADAUTH')
        self.assertTrue('connexion' in self.response_json.keys())
        self.assertEqual(self.response_json['connexion']['REALNAME'], '')

        self.calljson('/CORE/authentification', {'login': 'aaa', 'password': 'bbb'})
        self.assert_observer('core.auth', 'CORE', 'authentification')
        self.assert_json_equal('', '', 'BADAUTH')
        self.assertTrue('connexion' in self.response_json.keys())
        self.assertEqual(self.response_json['connexion']['REALNAME'], '')
    ...
    def test_connect(self):
        ...
        self.assertEqual(self.response_json['connexion']['SUBTITLE'], str('Application générique de gestion'))
        self.assertEqual(self.response_json['connexion']['VERSION'][0:4], '2.6.')
        self.assertEqual(len(self.response_json['connexion']['INFO_SERVER']), 3 + 2)
        self.assertEqual(self.response_json['connexion']['SERVERVERSION'][0:4], '2.6.')
        self.assertEqual(self.response_json['connexion']['COPYRIGHT'][0:4], '(c) ')
        self.assertEqual(self.response_json['connexion']['LOGONAME'][0:22], 'data:image/png;base64,')
        ...
        self.assertEqual(self.response_json['connexion']['LOGIN_FIELD'], 'username')
        self.assertEqual(self.response_json['connexion']['ONLY_ADMIN'], False)

        self.calljson('/CORE/exitConnection', {})
        self.assert_observer('core.acknowledge', 'CORE', 'exitConnection')

    def test_connect_admin_only_super(self):
        Params.setvalue('CORE-OnlySuperAdmin', True)
        self.calljson('/CORE/authentification', {'login': 'admin', 'password': 'admin'})
        self.assert_observer('core.auth', 'CORE', 'authentification')
        self.assert_json_equal('', '', 'OK')
        self.assertEqual(self.response_json['connexion']['ONLY_ADMIN'], True)

        self.calljson('/CORE/exitConnection', {})
        self.assert_observer('core.acknowledge', 'CORE', 'exitConnection')

    def test_connect_empty_only_super(self):
        Params.setvalue('CORE-OnlySuperAdmin', True)
        self.calljson('/CORE/authentification', {'login': 'empty', 'password': 'empty'})
        self.assert_observer('core.auth', 'CORE', 'authentification')
        self.assert_json_equal('', '', 'ONLYADMIN')
        self.assertEqual(self.response_json['connexion']['ONLY_ADMIN'], True)

    def test_connect_email(self):
        settings.LOGIN_FIELD = 'email'
        settings.ASK_LOGIN_EMAIL = True
        self.calljson('/CORE/authentification', {'login': 'admin@localhost', 'password': 'admin'})
        self.assert_observer('core.auth', 'CORE', 'authentification')
        self.assert_json_equal('', '', 'OK')
        self.assertEqual(self.response_json['connexion']['TITLE'], 'Lucterios standard')
        self.assertEqual(self.response_json['connexion']['SUBTITLE'], str('Application générique de gestion'))
        ...
        self.assertEqual(self.response_json['connexion']['LOGIN'], 'admin')
        self.assertEqual(self.response_json['connexion']['REALNAME'], 'administrator ADMIN')
        self.assertEqual(self.response_json['connexion']['LOGIN_FIELD'], 'email')
        self.assertEqual(self.response_json['connexion']['MODE'], '0')
        self.assertEqual(self.response_json['connexion']['ONLY_ADMIN'], False)
    ...
    def test_connect_email_otherdisabled(self):
        user = add_user('other_empty')
        user.last_name = 'OTHER'
        user.email = "empty@lucterios.org"
        user.is_active = False
        user.save()

        settings.LOGIN_FIELD = 'email'
        settings.ASK_LOGIN_EMAIL = True
        self.calljson('/CORE/authentification', {'login': 'empty@lucterios.org', 'password': 'empty'})
        self.assert_observer('core.auth', 'CORE', 'authentification')
        self.assert_json_equal('', '', 'OK')
    ...
    def test_connect_email_bothemail(self):
        user = add_user('other_empty')
        user.last_name = 'OTHER'
        user.email = "empty@lucterios.org"
        user.save()
    ...
    def test_menu_connected(self):
        ...
        self.assertEqual(self.response_json['menus'][1]['text'], str('Général'))
    ...
    def test_menu_connected_with_empty(self):
        ...
        self.calljson('/CORE/configuration', {}, 'get')
    ...
    def test_connect_anonymous(self):
        ...
        self.assertEqual(self.response_json['menus'][0]['text'], str('Dummy'))
    ...
    def test_connect_free(self):
        ...
        Params.setvalue('CORE-connectmode', '2')
        ...
        self.assertEqual(len(self.response_json['menus'][1]['menus']), 10)
    ...
    def test_password(self):
        ...
        self.calljson('/CORE/changePassword', {}, 'put')
        ...
        self.assert_action_equal('POST', self.json_actions[0], ('Ok', 'images/ok.png', 'CORE', 'modifyPassword', 1, 1, 1))
    ...
    def test_changepassword(self):
        ...
        self.assert_json_equal('', 'message', str('Mot de passe actuel erroné!'))
        self.assert_json_equal('', 'code', '3')

        self.calljson('/CORE/modifyPassword', {'oldpass': 'empty', 'newpass1': '123', 'newpass2': '456'})
        self.assert_observer('core.exception', 'CORE', 'modifyPassword')
        self.assert_json_equal('', 'message', str('Les mots de passes sont différents!'))
        self.assert_json_equal('', 'code', '3')
        ...
        self.assert_json_equal('', 'text', str('Mot de passe modifié'))
        ...
        self.calljson('/CORE/authentification', {'login': 'empty', 'password': '123'})
        self.assert_observer('core.auth', 'CORE', 'authentification')
        self.assert_json_equal('', '', 'OK')
...
class ConfigTest(LucteriosTest):
    ...
    def test_config(self):
        ...
        self.assertEqual(len(self.json_actions), 2)
        self.assert_action_equal('POST', self.json_actions[0], ('Assistant', 'images/config.png', 'CORE', 'configurationWizard', 0, 1, 1))
        ...
        self.assert_json_equal('LABELFORM', "CORE-connectmode", "Connexion toujours nécessaire")
        self.assert_attrib_equal("CORE-Wizard", 'description', "Ouvrir l'assistant de configuration au démarrage.")
        self.assert_json_equal('LABELFORM', "CORE-Wizard", "Oui")
        self.assert_attrib_equal("CORE-MessageBefore", 'description', "Message avant connexion")
        self.assert_json_equal('LABELFORM', "CORE-MessageBefore", "")
        self.assert_action_equal('POST', self.json_comp["00@Général_btn"]['action'], ("Modifier", 'images/edit.png', 'CORE', 'paramEdit', 0, 1, 1, {'params': ["CORE-connectmode", "CORE-Wizard", "CORE-MessageBefore"]}))
    ...
    def test_config_edit(self):
        ...
        self.assertEqual(len(self.json_actions), 2)
        self.assert_action_equal('POST', self.json_actions[0], ('Ok', 'images/ok.png', 'CORE', 'paramSave', 1, 1, 1))
        self.assert_action_equal('POST', self.json_actions[1], ('Annuler', 'images/cancel.png'))
