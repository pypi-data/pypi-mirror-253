...
def get_package_list():
    ...
    for dist_name in get_package_listname():
        try:
            ...
            print('package %s not importable' % dist_name, file=sys.stderr)
    return package_list
...
class LucteriosGlobal(LucteriosManage):
    ...
    def set_extra_urls(self, extra_url):
        ...
        with open(join(self.instance_path, EXTRA_URL), mode='w', encoding='utf-8') as url_file:
            url_file.write("# Pypi server\n")
            for line in extra_url.split(';'):
                if line.startswith('http') and url_ok(line):
                    url_file.write(line + "\n")
    ...
    def get_pip_outdated(self):
        ...
        for env_key, env_val in environ.items():
            env_variables[env_key] = env_val
        if (("virt" in executable) or ("venv" in executable)) and ('VIRTUAL_ENV' not in env_variables):
            virtual_env = dirname(dirname(executable))
            env_variables['VIRTUAL_ENV'] = virtual_env
            env_variables['PATH'] = virtual_env + '/bin:' + env_variables['PATH']
        options = " ".join(self.get_default_args_([], with_quiet=False))
        outdated_list = check_output("python3 -m pip list %s --outdated --format json" % options, shell=True, env=env_variables)
            ...
            try:
                ...
                for migr_obj in getmembers(migrat_mod):
                    if isfunction(migr_obj[1]) and (migr_obj[0][0] != '_'):
                        getLogger('lucterios.admin').debug("run %s.%s.%s" % (target[0], target[1], migr_obj[0]))
                        migr_obj[1]()
            except Exception:
                pass
