...
def get_url_from_request(request=None):
    from django.conf import settings
    abs_url = getattr(settings, 'LUCTERIOS_ROOT_URL', '')
    if (abs_url == "") and (request is not None):
        abs_url = request.META.get('HTTP_REFERER', request.build_absolute_uri()).split("/")
        abs_url = '/'.join(abs_url[:-2])
        setattr(settings, 'LUCTERIOS_ROOT_URL', abs_url)
    return abs_url
...
class ActionsManage(object):
            ...
            if model_name in cls._ACTION_LIST.keys():
                for act_ident, act_xclass, act_title, act_icon, act_condition, act_options in cls._ACTION_LIST[model_name]:
                    try:
                        ...
                        getLogger('lucterios.framework').error('error for [%s,%s,%s,%s]', act_ident, act_xclass, act_title, act_icon)
                        raise
            if key is not None:
                acts.sort(key=key)
            for act in acts:
    ...
    @classmethod
    def affect_transition(cls, state, close=CLOSE_NO, multi_list=None, only_one=None, ignore_all=None, ignore_grid=None, **options):
        def wrapper(xclass):
            xclass.trans_list = {}
            for field in xclass.model._meta.get_fields():
                if isinstance(field, FSMFieldMixin) and (field.name == state):
                    ...
                    transitions.sort(key=lambda item: item.target if item.target is not None else -1000)
                    for transition in transitions:
                        source_labels = [smart_str(name[1]) for name in field.choices if name[0] == transition.source]
                        source_label = source_labels[0] if len(source_labels) > 0 else '---'
                        target_labels = [smart_str(name[1]) for name in field.choices if name[0] == transition.target]
                        target_label = target_labels[0] if len(target_labels) > 0 else '---'
                        ...
                        if ((only_one is None) or (transition.name in only_one)) and ((ignore_all is None) or (transition.name not in ignore_all)):
                            ...
                            if (ignore_grid is None) or (transition.name not in ignore_grid):
...
def _convert_value_N(value, format_num):
    return locale.format_string("%.*f", (int(format_num[1]), float(value)), True)
