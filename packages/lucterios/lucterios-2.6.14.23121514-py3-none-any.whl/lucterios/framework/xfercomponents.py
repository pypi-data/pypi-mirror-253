...
from lucterios.framework.filetools import md5sum, is_image_base64
...
class XferComponent(object):
    ...
    def get_json(self):
        ...
        if isinstance(self.row, int):
            compjson['y'] = self.row
        if isinstance(self.colspan, int):
            compjson['colspan'] = self.colspan
        if isinstance(self.rowspan, int):
            compjson['rowspan'] = self.rowspan
        if isinstance(self.vmin, int):
            compjson['VMin'] = self.vmin
        if isinstance(self.hmin, int):
...
class XferCompLabelForm(XferComponent):
    ...
    def get_format(self):
        return self._formatnum, self._formatstr
    ...
    def _convert_select_to_label(cls, old_obj):
        ...
        if isinstance(old_obj.select_list, list):
            for key, sel_val in old_obj.select_list:
                format_string[key] = str(sel_val) if sel_val is not None else '---'
    ...
    def convert_to_label(cls, old_obj):
        ...
        try:
            if value is None:
                ...
                format_string = 'D'
                value = date(*[int(subvalue) for subvalue in re.split(r'[^\d]', str(value))[:3]]) if not isinstance(value, date) else value
            elif isinstance(old_obj, XferCompDateTime):
                format_string = 'H'
                value = datetime(*[int(subvalue) for subvalue in re.split(r'[^\d]', str(value))]) if not isinstance(value, datetime) else value
            elif isinstance(old_obj, XferCompTime):
                format_string = 'T'
                value = time(*[int(subvalue) for subvalue in re.split(r'[^\d]', str(value))[:2]]) if not isinstance(value, time) else value
...
class XferCompSelect(XferCompButton):
    ...
    def set_select_query(self, query):
        ...
        for item in query:
            try:
                ...
                self.select_list.append((item.id, str(item)))
        self._check_case()
    ...
    def remove_select(self, value_to_remove):
        self.select_list = list(self.select_list)
        removes = [item for item in self.select_list if item[0] == value_to_remove]
        for remove in removes:
            self.select_list.remove(remove)
    ...
    def get_value_text(self):
        if isinstance(self.select_list, dict):
            ...
            for select_item in self.select_list:
                if str(select_item[0]) == str(self.value):
...
class XferCompDownLoad(XferCompButton):
    ...
    def set_download(self, filename):
        ...
        self.set_filename("CORE/download?filename=%s&sign=%s" % (filename, sign_value))
...
class XferCompGrid(XferComponent):
    ...
    def move_action(self, action_url, pos_act=-1):
        search_action = None
        for action in self.actions:
            if action[0].url_text == action_url:
                search_action = action
                self.actions.remove(search_action)
                break
        if search_action is not None:
            if pos_act != -1:
                self.actions.insert(pos_act, search_action)
            else:
    ...
    def _new_record(self, compid):
        if compid not in self.records.keys():
            ...
            for header in self.headers:
                ...
                elif isinstance(header.htype, str) and header.htype.startswith('N'):
    ...
    def get_json_value(self):
        ...
        for key in self.record_ids:
            ...
            for rec_name in record.keys():
                if isinstance(rec_name, str) and rec_name.startswith('__') and (rec_name not in self.headers):
            ...
            if isinstance(fieldname, tuple):
                ...
                if ('.' not in fieldname) and len(query_set) > 0:
                    try:
                        ...
                        if isinstance(first_value, str) and is_image_base64(first_value):
                            hfield = 'icon'
                    except Exception:
                        pass
            ...
            for fieldname in fieldnames:
                ...
                if fieldname[-4:] == '_set':  # field is one-to-many relation
                    ...
                    for sub_items_value in sub_items:
                        resvalue.append(str(sub_items_value))
                else:
                    resvalue = child
