...
from lucterios.framework.tools import toHtml, clean_xml, format_to_string
from lucterios.framework.filetools import BASE64_PREFIX, get_image_absolutepath, get_image_size, is_image_base64
from lucterios.framework.xferbasic import XferContainerAbstract
from lucterios.framework.xferprinting import PRINT_PDF_FILE, PRINT_EXT_FILE
...
def convert_to_html(tagname, text, font_family="sans-serif", font_size=9, line_height=10, text_align='left'):
    try:
        html_text = toHtml(text)
        parser = etree.XMLParser(remove_blank_text=True, resolve_entities=False)
        xml_text = etree.XML("<%(tagname)s>%(text)s</%(tagname)s>" % {'tagname': tagname, 'text': html_text}, parser=parser)
...
def add_formating_xml(xml_text, value, formatnum, formatstr):
    def jsonconverter(data):
        from datetime import datetime, date, time
        from decimal import Decimal
        if isinstance(data, Decimal):
            return float(data)
        elif isinstance(data, LucteriosModel) or isinstance(data, datetime) or isinstance(data, date) or isinstance(data, time) or (type(data).__name__ == '__proxy__'):
            return str(data)
        else:
            return data
    if (formatnum is not None) or (formatstr is not None):
        xml_text.attrib['jsonvalue'] = dumps(value, default=jsonconverter)
        if formatnum is not None:
            xml_text.attrib['formatnum'] = dumps(formatnum)
        if formatstr is not None:
            xml_text.attrib['formatstr'] = formatstr
        clean_xml(xml_text)
        return True
    else:
        return False
...
class PrintItem(object):
    ...
    def calcul_text_size(self, text, font_size=9, line_height=10, text_align='left', is_cell=False):
        text = str(text)
        ...
        if text == '\n':
            text = 'A'
        width, height = self.owner.get_text_size(remove_format(text), font_size, line_height, text_align, is_cell)
...
class PrintImage(PrintItem):
    ...
    def __init__(self, comp, owner):
        ...
        if self.comp.type != '':
            if is_image_base64(self.comp.value):
...
class PrintTable(PrintItem):
    ...
    def compute_table_size(self):
        for header in self.comp.headers:
            size_cx, size_cy = self.calcul_text_size(header.descript, 9, 10, "center", True)
        ...
        for recordid in self.comp.record_ids:
            ...
            for header in self.comp.headers:
                ...
                if header.htype == 'icon':
                    ...
                    size_x, size_y = self.calcul_text_size(format_to_string(value, header.htype, None), 9, 10, "start", True)
                self.rows[row_idx].append((value, header.htype, header.formatstr))
                if (isinstance(header.htype, str) and (header.htype.startswith('C') or header.htype.startswith('N'))):
                    self.columns[hd_idx][1] = max(self.columns[hd_idx][1], size_x * self.RATIO_NUM)
                else:
    ...
    def get_table_sizes(self):
        ...
        if len(self.size_rows) == 1:
            _cx, size_cy = self.calcul_text_size("A", 9, 10, "center", True)
            size_y += size_cy
    ...
    def write_columns(self, xml_table):
        for column in self.columns:
            ...
            xml_text = convert_to_html('cell', "{[i]}%s{[/i]}" % column[0], "sans-serif", self.size_ratio(9), self.size_ratio(10), "center")
            new_col.append(xml_text)

    def write_rows(self, xml_table):
        for row in self.rows:
            new_row = etree.SubElement(xml_table, "rows")
            if hasattr(row, 'get_color_ref'):
                color_ref = row.get_color_ref()
                if color_ref is not None:
                    new_row.attrib['color_ref'] = str(color_ref)
            for value, formatnum, formatstr in row:
                if formatnum == 'icon':
                    xml_text = convert_to_html('cell', value, "sans-serif", self.size_ratio(9), self.size_ratio(10), "start")
                    xml_text.attrib['image'] = "1"
                else:
                    xml_text = convert_to_html('cell', '', "sans-serif", self.size_ratio(9), self.size_ratio(10), "start")
                    add_formating_xml(xml_text, value, formatnum, formatstr)
    ...
    def get_xml(self):
        if len(self.rows) == 0:
            ...
            self.size_rows.append(self.calcul_text_size('')[1])
        xml_table = etree.Element('table')
        self.width_ratio = self.width / self.size_x
        self.letter_ratio = self.owner.get_letter_ratio(self.width_ratio)
...
class PrintTab(PrintItem):
    ...
    def __init__(self, comp, owner):
        PrintItem.__init__(self, comp, owner)
        self.size_x, size_y = self.calcul_text_size(self.comp.value)
...
class PrintLabel(PrintItem):
    ...
    def __init__(self, comp, owner):
        ...
        try:
            ...
            self.value = str(self.comp.value)
        self.init_label()
        self.with_formating = True

    def init_label(self):
        self.size_x, self.height = self.calcul_text_size(self.value)
        self.height = self.height * 1.1

    def get_xml(self):
        xml_text = convert_to_html('text', self.value)
        self.fill_attrib(xml_text)
        if self.with_formating:
            formatnum, formatstr = self.comp.get_format()
            add_formating_xml(xml_text, self.comp.value, formatnum, formatstr)
        return xml_text
...
class ReportGenerator(object):
    ...
    def generate(self, request):
        def get_text_size(para_text, font_size=9, line_height=10, text_align='left', is_cell=False):
            return 1, 1
        if self.get_text_size is None:
            self.get_text_size = get_text_size
        if self.get_letter_ratio is None:
            self.get_letter_ratio = lambda _ratio: 1
        self.modelxml = etree.Element('model')
        self.add_page()
    ...
    def change_format(self, report_format):
        try:
            reporting_modulename = "lucterios.framework.reporting_%s" % report_format
            reporting_module = import_module(reporting_modulename)
            self.get_text_size = getattr(reporting_module, "get_text_size", None)
            self.get_letter_ratio = getattr(reporting_module, "get_letter_ratio", None)
            build_from_xml = getattr(reporting_module, "build_from_xml")
        except Exception:
            getLogger('lucterios.core.print').exception("import of %s", reporting_modulename)
            raise LucteriosException(IMPORTANT, _('Report format "%s" not supported !') % report_format)
        return build_from_xml

    def generate_report(self, request, report_format):
        build_from_xml = self.change_format(report_format)
        report_content = self.generate(request)
        try:
            content = build_from_xml(report_content, self.watermark)
            if len(content) > 0:
                return content
            else:
                return b""
        except ValueError:
            getLogger("lucterios.core.print").exception('build_from_xml')
...
class ActionGenerator(ReportGenerator):
    ...
    def createpdf_from_action(cls, class_action, params, title, page_width=210, page_height=297):
        ...
        return generator.generate_report(pdf_request, PRINT_EXT_FILE[PRINT_PDF_FILE])
...
class ReportModelGenerator(ReportGenerator):
    ...
    def _get_sub_values(item_xml, current_item):
        ...
        if hasattr(current_item, str(data)):
    ...
    @staticmethod
    def _create_new_xml(sub_value, itemtext, tagname):
        value, formatnum, formatstr = sub_value.evaluate_with_formating(itemtext.strip())
        if (formatnum is None) and (formatstr is None):
            new_item = convert_to_html(tagname, sub_value.evaluate(itemtext))
        else:
            new_item = convert_to_html(tagname, '')
            add_formating_xml(new_item, value, formatnum, formatstr)
        return new_item
    ...
    def _append_text(item, current_item, node, xfer):
        ...
        for sub_value in sub_values:
            ...
            new_item = ReportModelGenerator._create_new_xml(sub_value, item.text, 'text')
    ...
    def _append_table(item, current_item, node, xfer):
        ...
        for column in item.xpath('columns'):
            ...
            new_cell = ReportModelGenerator._create_new_xml(current_item, column.text, 'cell')
        ...
        for row in item.xpath('rows'):
            ...
            for sub_value in sub_values:
                ...
                color_ref = sub_value.get_color_ref()
                if color_ref is not None:
                    new_row.attrib['color_ref'] = str(color_ref)
                for cell in row.xpath('cell'):
                    new_cell = ReportModelGenerator._create_new_xml(sub_value, cell.text, 'cell')
...
class LabelGenerator(ReportModelGenerator):
    ...
    def fill_content(self, request):
        ...
        for item in self.get_items_filtered():
            item.set_context(self.xfer)
            label_values.append(item.evaluate(self.label_text).replace('&', '&amp;'))
        index = 0
        for labelval in label_values:
            ...
            if self.mode == 0:
                ...
                try:
                    ...
                    getLogger('lucterios.core.print').error('XMLSyntaxError : %s', labelval)
                    raise
