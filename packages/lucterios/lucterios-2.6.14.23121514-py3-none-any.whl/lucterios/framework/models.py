...
class LucteriosQuerySet(QuerySet):

    def __init__(self, model, *args, initial=[], **kwargs):
        super(LucteriosQuerySet, self).__init__(model=model, *args, **kwargs)
        self._result_cache = initial

    def order_by(self, *field_names):
        new_query = LucteriosQuerySet(model=self.model, initial=self._result_cache[:])
        fieldnames = list(field_names)
        fieldnames.reverse()
        for fieldname in fieldnames:
            if fieldname[0] == '-':
                new_query._result_cache.sort(key=lambda item: getattr(item, fieldname[1:], None), reverse=True)
            else:
                new_query._result_cache.sort(key=lambda item: getattr(item, fieldname, None), reverse=False)
        return new_query
...
class LucteriosModel(models.Model):
    ...
    def convert_field_for_search(cls, fieldname, fieldsearched, add_verbose=True):
        def convertQ(query):
            if isinstance(query, Q):
                ...
                return None
        if isinstance(fieldsearched, str):
    ...
    def get_select_contact_type(cls, with_current=True):
        ...
        for sub_class in cls.__subclasses__():
            if not sub_class._meta.proxy:
                select_list.extend(sub_class.get_select_contact_type(with_current=with_current or not sub_class._meta.abstract))
        return select_list

    @classmethod
    def create_print_template(cls):
        from lucterios.CORE.models import PrintModel
        fullmodel, _create = PrintModel.objects.get_or_create(name=_('full list'),
                                                              kind=PrintModel.KIND_LISTING,
                                                              modelname=cls.get_long_name(),
                                                              mode=PrintModel.MODE_SIMPLE)
        fullmodel.value = """297
210
"""
        for field in cls.get_all_print_fields(with_plugin=False):
            if isinstance(field, tuple) and (len(field) == 2) and ((field[1].count('.') <= 1) or ((field[1].count('.') == 2) and field[1].endswith('.str'))):
                fullmodel.value += "1//%s//#%s\n" % field
        fullmodel.save()
        return fullmodel.name
    ...
    def _convert_field_booleanfield(cls, fieldvalue, **_args):
        tmp_value = str(fieldvalue).lower().strip()
        field_value = (tmp_value == 'true') or (tmp_value[0] == 'y') or (tmp_value[0] == 'o') or (tmp_value == '1')
        return field_value
    ...
    def _convert_field_foreignkey(cls, fieldvalue, dep_field, fieldname, **_args):
        if not fieldname.endswith('_id'):
            ...
            for sub_item in dep_field.remote_field.model.objects.all():
                if str(sub_item.get_final_child()) == str(sub_value):
                    fieldvalue = sub_item
                    break
            if fieldvalue is None:
                base_dep_field = cls.get_field_by_name(fieldname.split('.')[0])
                if not base_dep_field.null:
                    raise LucteriosException(GRAVE, _("%(name)s '%(value)s' unknown !") % {'name': base_dep_field.verbose_name, 'value': sub_value})
        return fieldvalue
    ...
    def merge_objects(self, alias_objects=[]):
        ...
        for alias_object in alias_objects:
            if not isinstance(alias_object, primary_class):
                raise LucteriosException(IMPORTANT, _('Only record of same type can be merged.{[br/]}You must to be promote it.'))

        Signal.call_signal("pre_merge", self, alias_objects)
        generic_fields = []
    ...
    def evaluate_with_formating(self, text):
        value, formatnum, formatstr = None, None, None
        matchfield = self.TO_EVAL_FIELD.match(text)
        if (matchfield is not None) and (matchfield.span() == (0, len(text))):
            field_list = text[1:].split('.')
            if hasattr(self, field_list[0]):
                field_value = getattr(self, field_list[0])
                dep_field = self.get_field_by_name(field_list[0])
                if (dep_field is None or is_simple_field(dep_field)) and not isinstance(field_value, LucteriosModel):
                    value = adapt_value(field_value)
                    formatnum, formatstr = extract_format(get_format_from_field(dep_field))
                elif isinstance(field_value, LucteriosModel) and not hasattr(field_value, "all"):
                    value, formatnum, formatstr = field_value.evaluate_with_formating("#" + ".".join(field_list[1:]))
        return value, formatnum, formatstr
    ...
    def evaluate(self, text):
        ...
        if text is not None:
            for field in self.TO_EVAL_FIELD.findall(text):
                ...
                value = value.replace(field, str(field_val))
        return value
    ...
    def get_field_for_print(cls, with_plugin, field_name):
        ...
        if PrintFieldsPlugIn.is_plugin(field_name):
            ...
            for (sub_title, new_field) in child.model.get_field_for_print(with_plugin, last_field_name):
                if field_name.endswith('.str'):
                    extend_field = field_name
                else:
                    extend_field = ".".join([field_name] + new_field.split('.')[1:])
                fields.append(("%s > %s" % (field_title, sub_title), extend_field))
        else:
    ...
    def delete(self, using=None):
        try:
            ...
            logging.getLogger('lucterios.framwork').exception("delete %s" % self)
    ...
    def editor(self):
        try:
            root_modules = self.__module__.split('.')[:-1]
            if root_modules[-1] == 'models':
                root_modules = root_modules[:-1]
            root_module_name = ".".join(root_modules)
...
class LogEntryManager(models.Manager):
    ...
    def get_for_object(self, instance):
        ...
        if isinstance(pk, int):
            ...
            return self.filter(modelname=instance.__class__.get_long_name(), object_pk=smart_str(pk))
...
class LucteriosLogEntry(LucteriosModel):
    ...
    class Action:
        ...
        def get_action_title(cls, action_id):
            for choices_id, choices_val in cls.choices:
                if str(choices_id) == str(action_id):
                    return str(choices_val)
    ...
    def get_typeselection(cls):
        ...
        for modelname in cls.objects.values_list('modelname', flat=True).distinct():
            ...
            res.append((modelname, str(model_item._meta.verbose_name)))
    ...
    def change_additional_data(self, sender_ident, log_action, addon_data):
        ...
        self.additional_data = json.dumps(additional_data, default=str)
        self.save()
...
def correct_db_field(fields):
    ...
    if environ.get('FORCE_REFRESH', '1') != '1':
        return
    ...
    if ret > 0:
        logging.getLogger('lucterios.framwork').info("%s DB corrections" % ret)
