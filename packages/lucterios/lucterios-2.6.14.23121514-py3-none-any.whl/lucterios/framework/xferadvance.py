...
class XferAddEditor(XferContainerCustom):
    ...
    def _initialize(self, request, *_, **kwargs):
        ...
        if isinstance(self.redirect_to_show, str):
    ...
    def run_save(self, request, *args, **kwargs):
        ...
        return save.post(request, *args, **kwargs)
...
class XferSave(XferContainerAcknowledge):
    ...
    def fillresponse(self):
        ...
        for param_name in list(self.params.keys()):
            if isinstance(self.params[param_name], str) and len(self.params[param_name]) > 100:
        ...
        elif isinstance(self.redirect_to_show, str):
...
class XferTransition(XferContainerAcknowledge):
    ...
    def fill_simple_fields(self):
        if self.field_of_transition in self.params:
            del self.params[self.field_of_transition]
        XferContainerAcknowledge.fill_simple_fields(self)
    ...
    def _confirmed(self, transition):
        try:
            ...
            if hasattr(item, '_error_transaction'):
                getLogger("lucterios.core.transition").error(item._error_transaction(transition) + str(err))
