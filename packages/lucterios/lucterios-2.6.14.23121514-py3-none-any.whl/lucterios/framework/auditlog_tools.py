    ...
    if len(diff) == 0:
        ...
        for field in fields:
            if (field.name in model_fields['mapping_fields']) and (field.name not in diff.keys()):
                diff[field.name] = smart_str(get_field_value(old, field)), smart_str(get_field_value(new, field))
    return diff
