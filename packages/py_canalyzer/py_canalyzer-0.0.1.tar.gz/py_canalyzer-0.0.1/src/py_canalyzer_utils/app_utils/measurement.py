...
class CanalyzerMeasurementEvents:
    """Handler for CANalyzer Measurement events"""
    app_com_obj = object
    ...
    def OnInit():
        ...
        """
        app_com_obj_loc = CanalyzerMeasurementEvents.app_com_obj
        for fun in CanalyzerMeasurementEvents.user_capl_function_names:
            CanalyzerMeasurementEvents.user_capl_function_obj_dict[fun] = app_com_obj_loc.CAPL.GetFunction(fun)
...
class Measurement:
    """The Measurement object represents measurement functions of CANalyzer.
    """
    STARTED = False
    STOPPED = False
    ...
    def __init__(self, app_com_obj, user_capl_function_names=tuple(), enable_meas_events=True):
        ...
        CanalyzerMeasurementEvents.app_com_obj = app_com_obj
        CanalyzerMeasurementEvents.user_capl_function_names = user_capl_function_names
        ...
        self.wait_for_canalyzer_meas_to_start = lambda: DoEventsUntil(lambda: Measurement.STARTED, lambda: self.meas_start_stop_timeout)
        self.wait_for_canalyzer_meas_to_stop = lambda: DoEventsUntil(lambda: Measurement.STOPPED, lambda: self.meas_start_stop_timeout)
        if enable_meas_events:
            win32com.client.WithEvents(self.com_obj, CanalyzerMeasurementEvents)
    ...
    def user_capl_function_obj_dict(self):
        return CanalyzerMeasurementEvents.user_capl_function_obj_dict
    ...
    def start(self) -> bool:
        ...
        if not self.running:
            ...
            if not self.running:
                ...
                self.wait_for_canalyzer_meas_to_start()
            self.__log.info(f'CANalyzer Measurement {meas_run_sts[self.running]}.')
        else:
            self.__log.info(f'CANalyzer Measurement Already {meas_run_sts[self.running]}.')
        return self.running
    ...
    def stop_ex(self) -> bool:
        ...
        if self.running:
            ...
            if self.running:
                ...
                self.wait_for_canalyzer_meas_to_stop()
            self.__log.info(f'CANalyzer Measurement {meas_run_sts[self.running]}.')
        else:
            self.__log.info(f'CANalyzer Measurement Already {meas_run_sts[self.running]}.')
