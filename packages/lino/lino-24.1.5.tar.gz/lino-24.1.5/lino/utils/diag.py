...
class Analyzer(object):
    ...
    def show_db_overview(self):
        ...
        for model in models_list:
            if True:  # model._meta.managed:
                ...
                for base in model.__mro__:
                    if not issubclass(base, (models.Model, dd.Model)) or base in [models.Model, dd.Model] or base.__name__ == model.__name__:
                        continue
                    ...
                    if len(subbases) > 1 and len(base._meta.concrete_fields):
                        trees = sorted(list({item for item in trees if item.endswith(model.__name__)}), key=lambda v: len(v))
        ...
        if errors:
            s += ('\nFound ' + str(len(errors)) + ' model with duplicate bases\n' + '~'*70 + (('\n' + '~'*70)*2).join(errors))
        return s
