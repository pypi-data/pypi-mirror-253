...
''')
        ...
        appnames = {p.app_name.split('.')[0] for p in settings.SITE.installed_plugins if (
            _ := lambda x: [1 for p in settings.SITE.migratable_plugin_prefixes + ['lino']
                            if x.app_name.startswith(p)]
        )(p)}
        os_dict = {appname: import_string(f"{appname}.__version__") for appname in appnames}
