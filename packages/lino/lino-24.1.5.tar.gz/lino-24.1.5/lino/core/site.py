...
class Site(object):
    ...
    *within* the framework.
    ...
    def setup_logging(self):
        ...
        file_level = os.environ.get('LINO_FILE_LOGLEVEL', 'INFO').upper()
...
site. :manage:`diag` is a command-line shortcut to this.
    ...
    def get_quicklinks_html(self, ar, user):
        ...
        if getattr(ar.renderer.front_end, 'autorefresh_seconds', 0) > 0:
            qll.append('<a href="javascript:Lino.autorefresh();">autorefresh</a>')
        if False:
            qll.append('<a href="javascript:{}" style="text-decoration:none">{}</a>'.format(
                self.kernel.default_renderer.reload_js(), _("Refresh")))
...
#                     raise ChangedAPI(msg)
    ...
    def get_middleware_classes(self):
        ...
        if has_socialauth and self.get_plugin_setting('users', 'third_party_authentication', False):
