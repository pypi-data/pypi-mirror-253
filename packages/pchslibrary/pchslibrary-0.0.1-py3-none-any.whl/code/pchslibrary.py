import cmath, collections, math, re, sys, time, numpy, roman
...
def so_Fibonacci(n, memo={0: 0, 1: 1}):
    if n not in memo:
        memo[n] = so_Fibonacci(n - 1, memo) + so_Fibonacci(n - 2, memo)
    return memo[n]


# Hàm tạo danh sách các số Fibonacci đến n
def tao_danh_sach_so_Fibonacci(n):
    return [so_Fibonacci(i) for i in range(n)]


# # Các hàm số hoàn thiện, số tự mãn, hứa hôn, hoàn hảo, thân thiết:
# Hàm tính tổng các ước số của n không tính n à không âm
def tong_uoc_so(n):
    tong = 0
    for i in range(1, n):
        if n % i == 0:
...
# Hàm giải phương trình bậc 1, 2 một ẩn và vài lưu ý
def giai_pt_bac_1v2_nc(phuong_trinh):
...
# Hàm giải phương trình bậc 1 - 5 bằng cách nhập hệ số
...
def an_ki_tu(s):
    ...
    if len(danh_sach) > 0:
        if "/" not in danh_sach:
            ...
            an = ["•" for i in ("".join([i for i in danh_sach if i != "/"]))]
...
def tinh_toan_tien_dien(a, b):
    a = trich_xuat_cac_so_tu_so(a)
    b = trich_xuat_cac_so_tu_so(b)
