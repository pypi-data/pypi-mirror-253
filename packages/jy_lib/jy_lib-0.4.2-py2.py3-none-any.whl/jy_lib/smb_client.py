    ...
    def upload_file(self, path: AnyStr, data: AnyStr | SupportsBytes | IO):
        ...
        while True:
            try:
                ...
                logger.success(
                    f"文件上传成功 PID={os.getpid()} " f"TIME={elapse:3d}ms SIZE={humanize.naturalsize(size)} PATH={path}"
                )
                return
            except (NotConnectedError, BrokenPipeError, ConnectionResetError):
                self.reload()
                ...
                logger.error(f"[{e.__class__.__name__}]{e}")
                logger.error(
                    f"文件上传失败 PID={os.getpid()} " f"TIME={elapse:3d}ms SIZE={humanize.naturalsize(size)} PATH={path}"
                )
                raise e
            finally:
    ...
    def download_file(self, path: AnyStr, target: AnyStr | io.IOBase | None = None) -> io.BytesIO | None:
        ...
        while True:
            try:
                ...
                size: int = bio.tell()
                logger.success(
                    f"文件下载成功 PID={os.getpid()} TIME={elapse:3d}ms SIZE={humanize.naturalsize(size)} PATH={path}"
                )
                if target is None:
                    return bio
                else:
