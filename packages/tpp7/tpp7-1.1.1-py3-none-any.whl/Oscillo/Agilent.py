...
class Agilent(_Oscillo):
    __version__ = 'Agilent-1.0.1'
    ...
    def isActiveChannel(self, ch):
        return self.instr.ask(f":{ch}:DISPlay?")=="1"
    ...
    def getChannelData(self, channel='CHAN1'): 
        self.instr.write(f":WAVeform:SOURce {channel}")
        ...
        dt = dt.newbyteorder('>')
        V = (yref - np.frombuffer(raw, dtype=dt))*yincr-yorg
